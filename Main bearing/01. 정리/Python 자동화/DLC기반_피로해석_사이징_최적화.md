...
|      S3-c      | 본 최적화 — 224×150 · 33,600 평가                                 | 프론트 CSV·그림            |            **약 2.6 h** |
...
**예산 재산정** — 재매개화의 MASTA 호출 비율은 50세대 평균 92.8% 이고 수렴할수록 캐시 적중이 늘어 더 내려간다. `33,600 × 0.88 ≈ 29,600회 × 0.316 s ≈ **2.6시간**`으로 잡는다.
...
| 5 | 0.1 mm 분해능이 프론트를 과소분해하는지 — **S2 로는 검증 불가**(격자 증분이 30·75 mm). 재매개화에서는 `u` 10,001 단계가 밴드 폭 1,100~1,200 스텝을 덮어 **분해능은 오히려 과잉**이다(§6-11.3 ②) — 프론트의 계단 구조로 확인한다 | **S3**             |
| 6 | ~~세장비 수리가 경계에 개체를 몰아 프론트를 왜곡하는지~~ → **확인: 51.6 ~ 84.2% 가 경계에 붙는다**(§6-11.2). **수리 폐기 · 밴드 재매개화 채택**(§6-11.3). 재매개화의 잔여 경계 개체(대용 26.1%)는 선택압의 결과이므로 S3 프론트에서 **서술 대상**으로 남긴다                                                                                   | ✅ 종결 |
| 7 | ~~예산 대부분이 실제 MASTA 호출이 되는지~~ → **확인: 재매개화 기준 탈락 0.7% · 호출 비율 92.8%(50세대 평균).** 소요 **약 2.6시간** 재확정 (§6-11.3) | ✅ 종결                |
