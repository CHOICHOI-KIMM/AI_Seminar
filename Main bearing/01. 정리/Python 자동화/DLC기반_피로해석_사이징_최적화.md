...
| **샤프트 질량** | **MASTA `shaft.mass_of_shaft_body`** | 43,225.8 kg (실측) — §9-10 참조 |
...
| 3 | ~~z1·z2 개별 변경 자동화~~ | ✅ delete → try_mount_on (§8-3.6) |
| 4 | ~~샤프트 길이 연장 자동화~~ | ✅ 단면 재구성 (§8-3.6) |
...
S1(핀지지 정역학 해석식)은 설계 스크리너가 아니라 **dt=20 빈 압축용 중앙타겟 k 를 산출**하는 도구다(§8-3.2). 부록 3(DLC별해석)에서 함대 수명 참값 대비 +1.48%(보수)로 유효성이 입증되었으나, 그 검증은 **단일 설계·고정 α** 조건이었다.
...
1. MASTA 에서 `contact_angle = 30°` 설정 후 `cone_angle`·`e`·`Y1`·`C`·`C_u`·`a` 가 **자동 갱신되는지 실측** (이론식 `e = 1.5tanα = 0.866`, `Y1 = 0.4cotα = 0.693` 과 대조)
2. 실측값으로 S1 상수를 동기화하고 두 기하 변형의 k 를 각각 산출
   - A 현행기하: L=2.500 / A=−0.500 / B=3.000 — **α 무관 고정**
   - B L_eff: 실측 a 로부터 산출 — **α 반영**
...
| 결과 | 해석 | 후속 |
|---|---|---|
| 차이 < 2% | α 변화에도 S1 기하 모델 선택이 결과를 좌우하지 않음 | 참값 불필요, 연속성 우선으로 확정 |
| 차이 ≥ 2% | 어느 쪽이 옳은지 판정 불가 | **준참값(dt=1, 약 2.3시간) 투입** |

### 3-5.4 구조적 고려 — 탐색용으로는 L_eff 가 필수

Stage 0·1 의 결론과 별개로, **현행 기하 모델은 α 를 구조적으로 반영하지 못한다**. `L_, A_, B_` 가 하드코딩이라 α 가 12°든 30°든 같은 기하를 쓴다. 반면 L_eff 모델은 `L_eff = (z2−z1) + 2(a − T/2)` 를 통해 α·D_pw·L_we 를 자동 반영한다.

즉 Stage 0 의 "차이 없음"은 **α 고정일 때만 성립하는 결론**이며, 설계공간 탐색에서 α 를 여는 순간 현행 모델은 쓸 수 없다. Stage 1 은 이 전환의 **안전성을 확인하는 절차**로 보는 것이 정확하다.

## 3-6. 결론 (Stage 1 완료 후 확정)

- **확정**: S1 기하 모델의 물리적 정정식이 부록 1 회귀값과 일치함을 확인 (§3-3)
- **확정**: α=19° 에서 두 기하 변형의 함대 수명 차이는 **0.83%** (§3-4)
- **대기**: α=30° 검증 결과 (§3-5)
- **대기**: S1 기하 모델 최종 채택 (Stage 1 결과 보고 확정)
