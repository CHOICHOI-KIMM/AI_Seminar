...
| **수리** | **`L_we ← clip(L_we, 1.5·D_we, 2.5·D_we)`** | 세장비가 해석식 탈락의 60% (§6-8.1.4 ⒜) |
| **세대수** | **150 (상한)** | 총 33,600 평가 · **MASTA 는 중복 제거로 약 30,000회 ≈ 2.6시간** |
...
`고유평가` 가 **실제 MASTA 호출 횟수**다. `224 × 150` 은 예산 33,600 중 서로 다른 설계가 5,018개뿐 — 중복 제거가 **6.7배**를 절감한다.
...
**S3 예산 재확인** — 예상 MASTA 호출 약 30,000회 × 0.316 s = **2.6시간**. S2 에서는 중복 제거가 6.7배를 줄였으나, S3 는 `1.21 × 10¹²` 조합이라 중복이 거의 없어 절감을 기대하기 어렵다 — 예산을 보수적으로 잡는다.
