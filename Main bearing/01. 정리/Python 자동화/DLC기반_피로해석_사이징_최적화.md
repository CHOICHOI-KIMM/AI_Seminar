...
> 전체 개정 경위는 문서 최하단 **[개정 이력](#개정-이력)** 참조 — 번복·무효화 26건을 R0~R14 로 정리했다.
...
| **부록 6 S3-a** — 본최적화 스크립트 | ✅ **완료** | `nsga_s3_run.py` · 배선 시험 4항목 통과 · 예산 재산정 **약 2.9 h** (§6-10.6) |
| 부록 6 S3-b~S5 | ⬜ 미착수 | 드라이런(6분) → 본최적화(2.9 h) → 프론트 P2 검증 → 엑셀 |
| P3 — 최종 확정 | ⬜ 미착수 | |
...
| # | D_pw [mm] | d [mm] | D [mm] | α [°] | D_we [mm] | L_we [mm] | 세장비 | z1 [m] | z2 [m] | Z [개] | L_eff [m] | **베어링** [t] | 샤프트 [t] | **합계** [t] | σ_max [MPa] |
|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|
| 1 | 4,500 | 4,134 | 4,857 | 19 | 140 | 325 | 2.321 | 1.0 | 6.0 | 92 | 6.523 | **13.76** | 147.0 | **174.5** | 2,086.4 |
| 2 | 4,500 | 4,123 | 4,867 | 15 | 170 | 325 | 1.912 | 1.0 | 6.0 | 76 | 6.180 | **13.79** | 146.2 | **173.8** | 2,097.9 |
| 3 | 4,500 | 4,094 | 4,896 | 15 | 200 | 325 | 1.625 | 1.0 | 6.0 | 65 | 6.180 | **14.65** | 144.2 | **173.5** | 2,095.8 |
| 4 | 4,500 | 4,059 | 4,932 | 31 | 170 | 325 | 1.912 | 1.0 | 5.5 | 76 | 7.180 | **16.42** | 130.8 | **163.7** | 2,096.7 |
| 5 | 4,500 | 4,033 | 4,958 | 31 | 200 | 325 | 1.625 | 1.0 | 5.5 | 65 | 7.181 | **17.07** | 129.1 | **163.3** | 2,093.9 |
| 6 | 4,200 | 3,774 | 4,614 | 15 | 170 | 400 | 2.353 | 1.0 | 5.5 | 71 | 5.602 | **18.23** | 113.1 | **149.5** | 2,094.8 |
| 7 | 4,200 | 3,709 | 4,679 | 27 | 170 | 400 | 2.353 | 1.0 | 5.0 | 71 | 6.119 | **21.42** | 100.1 | **143.0** | 2,092.8 |
| 8 | 4,500 | 4,009 | 4,979 | 27 | 170 | 400 | 2.353 | 1.0 | 4.0 | 76 | 5.271 | **22.95** | 95.7 | **141.6** | 2,097.4 |
| 9 | 3,900 | 3,396 | 4,390 | 15 | 200 | 475 | 2.375 | 1.0 | 5.5 | 56 | 5.526 | **23.77** | 91.6 | **139.1** | 2,098.3 |
| 10 | 3,900 | 3,315 | 4,471 | 23 | 230 | 475 | 2.065 | 1.0 | 5.0 | 49 | 5.639 | **27.65** | 80.0 | **135.3** | 2,095.5 |
...
| # | D_pw [mm] | d [mm] | D [mm] | α [°] | D_we [mm] | L_we [mm] | 세장비 | z1 [m] | z2 [m] | Z [개] | L_eff [m] | **베어링** [t] | 샤프트 [t] | **합계** [t] | σ_max [MPa] |
|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|
| 1 | 4,200 | 3,751 | 4,637 | 19 | 170 | 400 | 2.353 | 1.5 | 6.0 | 71 | 5.924 | **19.38** | 121.0 | **159.8** | 2,086.3 |
| 2 | 4,200 | 3,723 | 4,666 | 19 | 200 | 400 | 2.000 | 1.5 | 6.0 | 60 | 5.925 | **20.26** | 119.2 | **159.7** | 2,086.2 |
| 3 | 4,200 | 3,695 | 4,694 | 19 | 230 | 400 | 1.739 | 1.5 | 6.0 | 52 | 5.926 | **21.11** | 117.4 | **159.7** | 2,087.5 |
| 4 | 4,200 | 3,690 | 4,698 | 31 | 170 | 400 | 2.353 | 1.5 | 5.5 | 71 | 6.503 | **22.30** | 108.1 | **152.7** | 2,096.2 |
| 5 | 4,200 | 3,665 | 4,724 | 31 | 200 | 400 | 2.000 | 1.5 | 5.5 | 60 | 6.504 | **22.96** | 106.7 | **152.6** | 2,096.6 |
| 6 | 4,200 | 3,639 | 4,750 | 31 | 230 | 400 | 1.739 | 1.5 | 5.5 | 52 | 6.506 | **23.64** | 105.1 | **152.4** | 2,098.7 |
| 7 | 3,900 | 3,368 | 4,419 | 15 | 230 | 475 | 2.065 | 1.5 | 6.0 | 49 | 5.527 | **24.83** | 97.6 | **147.2** | 2,093.9 |
| 8 | 3,900 | 3,292 | 4,494 | 27 | 230 | 475 | 2.065 | 1.5 | 5.5 | 49 | 5.972 | **28.84** | 86.0 | **143.7** | 2,095.4 |
...
| **수리** | **`L_we ← clip(L_we, ⌈1.5·D_we⌉₀.₁, ⌊2.5·D_we⌋₀.₁)`** | 세장비가 해석식 탈락의 60% · 경계는 0.1 mm 격자로 정렬 (§6-8.1.4 ⒜) |
| **세대수** | **150 (고정)** | 총 33,600 평가 · **MASTA 약 30,000회 ≈ 2.6시간** |
...
>
> 초안은 `eliminate_duplicates=True` 였으나 **S1·S2 구현은 `None`(끔)** 이다. 두 가지 때문이다. ① pymoo 의 중복 판정은 **설계변수 `x` 공간**에서 이뤄지는데, 세장비 수리를 켜면 서로 다른 `x` 가 클립 후 같은 설계가 되므로 `x` 공간에서는 보이지 않는다 ② 중복 회피의 실익은 **MASTA 호출을 아끼는 것**이고, 그 역할은 `eval_cache` 가 수리 **이후**의 설계 키로 이미 수행한다.
>
...
**⒜ 세장비 수리(repair) — 효과가 가장 크다**

세장비 하나가 해석식 탈락의 **60%** 를 차지한다. 이는 페널티로 다룰 것이 아니라 **수리**할 수 있다.

```
lo = ceil (1.5 · D_we / 0.1) · 0.1        ← 안쪽으로 올림
hi = floor(2.5 · D_we / 0.1) · 0.1        ← 안쪽으로 내림
L_we ← clip(L_we, lo, hi)
```

`D_we` 를 뽑은 뒤 `L_we` 를 그 범위로 밀어 넣으면 세장비 위반이 **구조적으로 사라진다.** 해석식 통과율이 33% → 90%대로 오르고, 초기 가능해 기대치가 **약 3배**가 된다. 평가 비용은 0 이다.

> **경계를 0.1 mm 격자에 정렬한다 (260804 추가).** 소박한 `clip(L_we, 1.5·D_we, 2.5·D_we)` 는 `D_we` 가 0.1 mm 단위일 때 경계를 **0.05 mm 단위**로 만든다 — `D_we` 110.1 mm 면 하한 165.15 · 상한 275.25 mm 다. 그러면 §6-3.1 에서 `Real` 을 배제한 근거("값이 항상 정확히 0.1 mm 배수이고 결과가 곧 도면 치수")가 수리 경로로 깨지고, 보고된 `x` 와 실제 평가된 설계가 어긋나는 문제도 같은 경로로 되살아난다.
>
> 따라서 **하한은 올림·상한은 내림**으로 격자에 맞춘다. 두 방향 모두 **구간 안쪽**이므로 세장비 제약을 새로 위반시키지 않는다. 폭이 0 이 되는 경우(`hi < lo`)는 `D_we` 최대 230 mm 에서도 구간 폭이 230 mm 라 발생하지 않는다.
>
> 아울러 프론트 CSV 에는 **수리 후 `L_we`** 를 기록한다 — pymoo 가 반환하는 `res.X` 는 수리 전 값이라 그대로 적으면 실제 평가된 설계와 다르다.
...
| 개선안 | 채택 | 반영 |
|---|:-:|---|
| ⒜ 세장비 수리 | **채택** | 해석식 통과율 33% → 90%대 · 초기 가능해 5.9 → 약 18개 · **경계는 0.1 mm 격자 정렬** |
...
⒜⒝ 로 초기 가능해가 **5.9 → 약 18개**로 늘고 범주 커버리지가 완전해진다. 총 평가는 `224 × 150 = 33,600` ≈ **2.8시간**이다.

**남은 위험은 ⒜ 의 부작용이다.** `L_we` 를 클립하면 세장비 경계(1.5·`D_we`, 2.5·`D_we`)에 개체가 몰린다. §8-4.4.4 의 전수 파레토 18건 중 세장비가 경계에 붙은 것은 없으므로(1.625 ~ 2.375) 프론트 자체를 왜곡할 가능성은 낮지만, **S2 재현시험에서 경계 집중 여부를 확인**한다.
...
| **S3-a** | 본최적화 스크립트 + **배선 시험**(대용 평가기 · MASTA 0회) | `nsga_s3_run.py` | ✅ **완료** (§6-10.6) |
| **S3-b** | **드라이런 224×5** — 실 MASTA 로 격자·시딩·체크포인트 확인 | 검증 로그 | **6분** |
| S3-c | 본 최적화 — 224×150 · 33,600 평가 | 프론트 CSV·그림 | **약 2.9 h** |
...
**S3-b 드라이런을 끼운다 (260804 추가).** S2 는 6변수를 전부 `Choice`(격자 수준)로 선언한 구성이라, S3 의 스케일 `Integer`·범주 전수 시딩·수리·정수화는 **한 번도 함께 돌아본 적이 없다.** 2.6시간을 태운 뒤에 `L_we` 가 격자 밖이었음을 알게 되는 것은 §6-10.4 에서 이미 한 번 겪은 실패(수리가 캐시를 무력화해 1,400점을 헛돌린 건)의 반복이다. 5세대만 돌려 ① 시딩 커버리지 224/224 ② 수리 후 `L_we` 가 전부 0.1 mm 배수 ③ 프론트 CSV 가 수리 **후** 값을 기록 ④ 체크포인트에서 재시작 을 확인하고 본런에 들어간다.
...
### 6-10.6 S3-a — 배선 시험 (MASTA 0회 · 260804)

`nsga_s3_run.py` 는 S2 와 네 곳이 다르다(스케일 `Integer` · 정수화 · 수리 · 시딩). **MASTA 를 대용식으로 갈아끼우고 `224 × 4` 를 돌려** 배선만 먼저 확인했다. 기하·제약·수리·정수화는 실제 코드 그대로이고 σ·질량만 가짜다.
...
**예산이 예상보다 크다 (미결 #7 해소).** 896 평가 중 **해석식 탈락은 24건(2.7%)** 뿐이었다 — Phase 3 전수의 67% 에서 급감한 것으로, 세장비 수리의 의도된 효과다. 캐시 적중도 3건(0.3%)에 그쳤다. 즉 **예산의 약 97% 가 실제 MASTA 호출**이 된다.

```
33,600 × 0.97 × 0.316 s ≈ 2.9 시간
```

§6-8 의 "중복 제거로 약 30,000회" 라는 표현은 낙관이었다. **S3 소요는 2.6 → 약 2.9시간**으로 잡는다. 탈락률·적중률은 대용 평가기에서 잰 값이지만 둘 다 **MASTA 와 무관한 경로**(해석식 제약·캐시 키)라 실측과 같다.
...
| 4 | UW/DW 제원 분리(§4-2 2차 탐색, 변수 10개) 적용 여부 | S3 이후 |
| 5 | 0.1 mm 분해능이 프론트를 과소분해하는지 — **S2 로는 검증 불가**(격자 증분이 30·75 mm 라 0.1 mm 를 시험할 수 없다) | **S3** |
| 6 | 세장비 수리가 경계에 개체를 몰아 프론트를 왜곡하는지 — **S2 로는 검증 불가**(§6-10.4) | **S3** |
| 7 | ~~수리로 해석식 탈락이 줄면 예산 대부분이 실제 호출이 되는지~~ → **확인: 탈락 2.7% · 적중 0.3% → 예산의 97% 가 실호출.** 소요 2.6 → **약 2.9시간** (§6-10.6) | ✅ 종결 |
...
S0 의 §6-8 파라미터는 S1·S2 를 수행하기 전에 관례로 정한 초안이었다. 실제 구현·수행 결과와 대조해 **3건을 번복**한다. 어느 것도 S2 재현율 100% 라는 결론을 바꾸지 않는다.
...
| 26 | **§6-8.1.4 ⒜ 수리 경계** | `clip(L_we, 1.5·D_we, 2.5·D_we)` → **경계를 0.1 mm 격자로 정렬**(하한 올림·상한 내림) | `D_we` 가 0.1 mm 단위면 경계가 0.05 mm 단위가 되어(예 `D_we` 110.1 → 165.15 / 275.25), §6-3.1 이 `Real` 을 배제한 근거인 "항상 0.1 mm 배수"가 수리 경로로 깨진다 | 수리를 켜는 S3 부터 적용. 두 방향 모두 구간 안쪽이라 제약을 새로 위반시키지 않는다. 프론트 CSV 는 **수리 후** `L_we` 를 기록 |
...
**S3-b 드라이런을 신설한다.** S2 는 6변수를 전부 `Choice` 로 선언했으므로 스케일 `Integer`·시딩·수리·정수화가 함께 돌아본 적이 없다. 224×5 (약 6분)로 격자 정합·시딩 커버리지·체크포인트를 확인한 뒤 본런에 들어간다(§6-9).
...
| §6-8.1.4 ⒜ 수리 | 개정 | **R14** | 클립 경계를 0.1 mm 격자로 정렬 |
| §6-11 미결 #5 | 이관 | **R14** | 결정 시점 S2 → **S3** (격자 증분이 30·75 mm) |
