...
| 파일 | 내용 |
|---|---|
| `p1_grid.csv` | **설계점별 전량 (5,200행)** — 자유변수 6 + 종속변수(d·D·T·B·C·Z·질량) + **L_eff 실측·근사 2열** + 3 LC × 2 베어링 응력 6개 + σ_max + 지배케이스 + warn + 소요시간. **매 점 append** (중단·재개 대응) |
...
`L_eff ≈ (z2−z1) + D_pw·tan α` 는 MASTA 실측 대비 **+0.0276 m (약 +0.4%) 의 일정 오프셋**을 보인다. 순위가 보존되므로 정렬용으로는 충분하나, 기록은 실측값을 쓴다(P1 CSV 의 `L_eff_m` 열).
