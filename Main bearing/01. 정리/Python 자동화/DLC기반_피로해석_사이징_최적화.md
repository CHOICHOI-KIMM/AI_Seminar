...
### 9-10. SSOT 규약 이원화 — `sizing_geom` 과 부록 8~10 이 다르다 (260831 등록)

**`Results/사이징최적화/sizing_geom.py`(R6판)와 부록 8~10 의 종속변수 산출 규약이 서로 다르다.** 같은 설계변수를 넣어도 보어·외경·`T`·`B`·`C` 가 다르게 나온다.

| 항목 | `sizing_geom.bearing()` (R6 · 부록 6 기준) | 부록 8~10 (§10-4) |
| --- | --- | --- |
| 폭 비율의 기준 길이 | **`L_we`**(유효길이) 입력 | **`L_w`**(롤러 전장) — `L_we` = `L_w` − 8.6 mm |
| `T`·`B`·`C` | 실수 그대로 | **1 mm 반올림** |
| 보어·외경 | 비율식(`t_i`·`t_o` 를 거쳐 산출) | `D_pw ∓ D_we·cosα ∓ L_w·sinα ∓ 2t` · **1 mm 반올림** |

**실측 확인 (260831)** — 부록 10 프론트 `#210`(`D_pw` 4,596 · α 15° · `D_we` 170.0 · `L_we` 347.6)에 §10-4 식을 적용하면 보어 4,199.04 → **4,199**, 외경 4,982.90 → **4,983** 으로 `a10_pareto.csv` 와 정확히 일치한다. 같은 설계변수를 `sizing_geom.bearing(D_pw, α, D_we, L_we)` 에 넣으면 **4,204.7 / 4,977.4** 이 나온다(각각 +5.7 / −5.6 mm 어긋남) — `L_we` 를 `L_w` 자리에 넣은 것과 반올림 부재가 겹친 결과다.

**드러난 경위** — 260831 에 [`DLC기반_피로해석_소재,열처리,거칠기,공차.md`](DLC기반_피로해석_소재,열처리,거칠기,공차.md) §0 을 작성하며 `sizing_geom` 으로 `#1`·`#103`·`#210` 의 제원을 재현하려다 §10-11.2.2 표와 값이 어긋나 발견했다. 해당 문서는 **원자료 `a10_pareto.csv` 직접 인용으로 정정**했다(R3).

**리스크** — 부록 8~10 이후 새로 작성되는 문서·스크립트가 `sizing_geom` 을 「SSOT」로 믿고 호출하면 같은 오차가 되풀이된다. 부록 6 산출물은 R6 규약으로 만들어졌으므로 **소급 수정은 하지 않는다.**

**미결** — ⓐ `sizing_geom` 에 부록 8~10 규약 모드를 추가할지, ⓑ 함수 독스트링·문서에 「부록 6 전용」 경고만 달지, ⓒ 부록 8~10 규약을 별도 모듈로 분리할지. **사용자 결정 대기.**
...
> 베어링 **80 °C** · 샤프트 72 °C 인데 `Load Case 1` 은 `use_default_temperatures = False` 라 자기 온도를 쓴다.
>
...
> 점도와 무관하고, **열팽창도 꺼져 있어 내부 틈새로도 들어가지 않는다** (§8-7.4). 남는 것은 「피로는 50/70 °C · 응력은 80 °C」라는 표기상의 불일치뿐이다 — 미결 #11 은 그만큼 좁아진다.
...
요청이 "베어링 온도"였으므로 그대로 따른다.

> **온도차 30 °C 는 결과에 들어가지 않는다 (260806 확인 · 미결 #12 종결).**
> 모델의 하중조건은 `include_thermal_expansion_effects = False` 다 — 열팽창을
> 계산에 넣지 않으므로 샤프트·하우징 온도는 **경로 자체가 없다.** 실측으로
> 확인했다(§8-7.4): 같은 설계·같은 DLC 를 「베어링 70 / 샤프트 40 / 하우징 40」
> 과 「전부 70」으로 돌린 손상이 **유효숫자 전부 일치**한다(비 1.000000000).
> 따라서 이 절의 손상 증가 2.84배는 **전부 점도 효과**다.
...
> **한계** — ⑴ ~~베어링만 승온해 열팽창 효과가 섞여 있다~~ → **아니다. 열팽창이 꺼져 있어 2.84배는 전부 점도 효과다**(260806 실측 · §8-7.4). ⑵ 프론트 양 끝 20건씩만 검토했으므로 **중간 구간의 경계 위치는 보간 추정**이다 — 손상비가 좁게 모이므로 50 °C 값에 2.84를 곱해 판정할 수 있으나, 실제 확인은 하지 않았다. ⑶ 설계 운전 온도를 50 °C 로 볼지 70 °C 로 볼지가 **프론트의 사용 가능 구간을 절반으로 가르므로**, 이 선택이 P3 최종 확정의 선결 조건이다.
...
| 11 | **극한 응력 LC 가 80 °C 조건이었다**(§6-11.7) — 하중조건 17개 중 자체 온도를 쓰는 것은 `Load Case 1`(베어링 50 °C) 하나뿐이고, `Myz_max` 를 포함한 16개는 설계 기본값(베어링 **80 °C** · 샤프트 72 °C)을 쓴다. **P1·S3 의 σ 는 80 °C 에서 나온 값**이다. 응력은 점도와 무관하나 열팽창을 통한 내부 틈새에는 영향이 있을 수 있어, 피로(50/70 °C)와 응력(80 °C)이 **서로 다른 온도**에서 평가된 상태다. 다만 **열팽창이 꺼져 있어 내부 틈새 경로는 없다**(§8-7.4) — 남는 것은 표기상의 불일치뿐이다 | ⚠**축소됨**         |
| 12 | ~~**70 °C 조건에 열팽창 효과가 섞여 있다**~~ → **종결(260806).** 모델이 `include_thermal_expansion_effects = False` 라 샤프트·하우징 온도는 결과에 들어가지 않는다. 「베어링 70/샤프트 40/하우징 40」과 「전부 70」의 손상이 **유효숫자 전부 일치**한다(§8-7.4). 손상 증가 2.84배·2.73배는 **전부 점도 효과**다                                                                                                                                                                                                   | ✅**종결**           |
...
### 7-6.6 111 DLC 전수 (`a62`)

`a01` 은 **베어링 최경량**(프론트 #1)이라 보어가 커서 샤프트도 굵다. 반대쪽인
**총질량 최경량군**에서 `a62`(프론트 #62)를 골라 같은 검토를 반복한다 —
샤프트 질량이 **142.6 → 75.3 t** 로 절반이고 외경도 4,069 → 3,371 mm 다.

비교 기준은 **`a62` 자신의 극한 LC 값 2.418**(§7-3)이다. 극한 16 LC 는 다시
돌리지 않았다 — §7-6.1 에서 `Myz_max` 가 지배임이 확인됐고 `a62` 의 그 값은
이미 `din743.csv` 에 있다.
...
| 계열         | DLC 수 |    빈 | 베어링 손상 비중 | **피로 안전율**  | 영구변형    | 극한 LC 대비  |
| ------------ | -----: | ----: | ---------------: | ---------------------- | ----------- | ------------- |
| `DLC1.2`   |     66 | 1,980 |            96.3% | **5.42 ~ 12.92** | 16.0 ~ 35.5 | 2.24 ~ 5.34배 |
| `DLC2.4.1` |     12 |    36 |             0.0% | **7.38 ~ 11.12** | 18.3 ~ 27.2 | 3.05 ~ 4.60배 |
| `DLC2.4.2` |     12 |   180 |             0.0% | **7.39 ~ 9.77**  | 22.6 ~ 26.6 | 3.06 ~ 4.04배 |
| `DLC4.1`   |      9 |    90 |             0.9% | **8.79 ~ 13.23** | 25.5 ~ 32.6 | 3.63 ~ 5.47배 |
| `DLC6.4`   |     12 |   360 |             2.7% | **9.51 ~ 14.89** | 23.7 ~ 37.0 | 3.93 ~ 6.16배 |

**샤프트 안전율 최악 10건**

|  # | DLC             | 베어링 ΣD30_UW | 비중 | `k` | 빈 | **무한수명 피로** | 영구변형 | 극한 LC 대비 |
| -: | --------------- | --------------: | ---: | ----: | -: | ----------------------: | -------: | -----------: |
|  1 | `DLC1.2-k-s5` |         0.00001 | 0.0% |  0.52 | 30 |         **5.420** |   16.001 |       2.24배 |
|  2 | `DLC1.2-k-s4` |         0.00001 | 0.0% |  0.64 | 30 |         **6.021** |   17.896 |       2.49배 |
|  3 | `DLC1.2-h-s1` |         0.00021 | 0.3% |  0.44 | 30 |         **6.226** |   18.477 |       2.57배 |
|  4 | `DLC1.2-i-s3` |         0.00008 | 0.1% |  0.46 | 30 |         **6.282** |   18.588 |       2.60배 |
|  5 | `DLC1.2-i-s5` |         0.00007 | 0.1% |  0.54 | 30 |         **6.288** |   18.694 |       2.60배 |
|  6 | `DLC1.2-f-s4` |         0.00071 | 1.1% |  0.35 | 30 |         **6.326** |   18.961 |       2.62배 |
|  7 | `DLC1.2-f-s5` |         0.00072 | 1.1% |  0.34 | 30 |         **6.517** |   19.467 |       2.70배 |
|  8 | `DLC1.2-f-s3` |         0.00077 | 1.2% |  0.36 | 30 |         **6.663** |   19.843 |       2.76배 |
|  9 | `DLC1.2-k-s6` |         0.00001 | 0.0% |  0.62 | 30 |         **6.740** |   19.832 |       2.79배 |
| 10 | `DLC1.2-e-s1` |         0.00127 | 2.0% |  0.31 | 30 |         **6.756** |   20.426 |       2.79배 |

*전 111건은 `부록7_샤프트/all111_dlc.csv`.*

<!-- /A7:ALL111_a62 -->

**111 DLC 전수 · 249초.** 피로 안전율 **5.42 ~ 14.89**, 자기 극한 LC(2.418)
대비 **2.24 ~ 6.16배**다.

#### `a01` 과의 대조 — 설계 차이는 **하나의 배율**로 흡수된다

| 지표                          |       `a01` |       `a62` |              비 |
| ----------------------------- | ------------: | ------------: | --------------: |
| 샤프트 질량                   |       142.6 t |        75.3 t |           0.528 |
| 샤프트 외경                   |      4,069 mm |      3,371 mm |           0.828 |
| **극한 LC 피로 안전율** |         4.253 |         2.418 | **0.569** |
| **111 DLC 최악**        |         9.566 |         5.420 | **0.567** |
| 111 DLC 최선                  |        26.248 |        14.886 |           0.567 |
| 극한 LC 대비 범위             | 2.25 ~ 6.17배 | 2.24 ~ 6.16배 |              — |

**① 두 설계의 안전율 비가 111 DLC 전부에서 거의 일정하다.** DLC별로 `a62/a01`
비를 내면 **0.556 ~ 0.585**(평균 **0.565**)에 몰리고, 이는 극한 LC 에서의 비
**0.569** 와 사실상 같다.

**따라서 극한 LC 안전율 하나로 운전 하중에서의 여유를 추정할 수 있다.**
설계를 바꿔도 하중 스펙트럼의 상대적 가혹도 순서는 그대로이고, 설계 차이는
**단일 스케일 인자**로 흡수된다. 40건 전부를 111 DLC 로 돌릴 필요가 없다는
뜻이며, §7-3 의 극한 LC 표에 약 **2.25배**(최악 DLC 기준)를 곱하면 운전 하중
기준 여유가 된다.

**② 최악 DLC 가 `DLC1.2-k-s5` 로 동일하다.** 계열별 순서도 같다 —
`DLC1.2` 가 가장 가혹하고 `DLC6.4` 가 가장 여유롭다. **어떤 하중이 샤프트를
지배하는지는 설계에 의존하지 않는다.**

**③ 그럼에도 `a62` 는 여전히 안전하다.** 최악 5.420 이 요구치 1.0 의 **5.4배**
다. 샤프트 질량을 절반으로 줄여도(142.6 → 75.3 t) 구속 조건이 되지 않는다 —
§7-3 의 결론이 프론트의 반대쪽 끝에서도 유지된다.

### 7-6.7 두께 비율 검토 (`a01`)

샤프트 여유가 요구치의 9.6배(`a01`)나 되므로 **벽을 얼마나 얇게 할 수 있는지**
를 본다. `sizing_geom` 의 내경 규칙 `ID = floor(r · OD)` 에서 `r` 을 현행
**0.88543** 보다 키우면 벽이 얇아지고 질량이 준다.

#### 7-6.7.1 최악 빈과 그 대표하중

§7-6.5 의 최악 DLC(`DLC1.2-k-s5`) 안에서 **어느 빈이 최저 안전율을 내는지**와
그 빈의 6자유도 대표하중이다. 스윙은 이 빈 하나로 한다 — 듀티사이클 집계가
최악 LC 기준이라(§7-6.3) DLC 전체를 돌린 것과 값이 같고 30배 빠르다.
...
#### 7-6.7.2 비율 스윙 — 피로 5.0 을 만족하는 한계

`r` 을 0.885 ~ 0.97 로 훑고 **피로 안전율 5.0** 교차 구간을 0.001 로 좁혔다.
베어링 제원은 그대로이고 **샤프트 내경만** 바뀐다.

<!-- A7:THICK -->

|               비율`r` | 내경 [mm] | 두께 [mm] | `OD/t` | 샤프트 [t] |  **피로** | 영구변형 | 판정 |
| ----------------------: | --------: | --------: | -------: | ---------: | --------------: | -------: | :--: |
|                  0.8850 |     3,601 |     234.0 |    17.39 |      142.9 | **9.583** |   28.377 |  ✅  |
| 0.8854 ←**현행** |     3,602 |     233.5 |    17.43 |      142.6 | **9.566** |   28.327 |  ✅  |
|                  0.8900 |     3,621 |     224.0 |    18.17 |      137.2 | **9.243** |   27.371 |  ✅  |
|                  0.8950 |     3,641 |     214.0 |    19.01 |      131.4 | **8.896** |   26.348 |  ✅  |
|                  0.9000 |     3,662 |     203.5 |    20.00 |      125.3 | **8.526** |   25.255 |  ✅  |
|                  0.9050 |     3,682 |     193.5 |    21.03 |      119.4 | **8.168** |   24.196 |  ✅  |
|                  0.9100 |     3,702 |     183.5 |    22.17 |      113.6 | **7.804** |   23.120 |  ✅  |
|                  0.9150 |     3,723 |     173.0 |    23.52 |      107.4 | **7.416** |   21.970 |  ✅  |
|                  0.9200 |     3,743 |     163.0 |    24.96 |      101.4 | **7.039** |   20.857 |  ✅  |
|                  0.9250 |     3,763 |     153.0 |    26.59 |       95.4 | **6.657** |   19.726 |  ✅  |
|                  0.9300 |     3,784 |     142.5 |    28.55 |       89.1 | **6.249** |   18.518 |  ✅  |
|                  0.9350 |     3,804 |     132.5 |    30.71 |       83.1 | **5.854** |   17.349 |  ✅  |
|                  0.9400 |     3,824 |     122.5 |    33.22 |       77.0 | **5.452** |   16.161 |  ✅  |
| 0.9450 ←**한계** |     3,845 |     112.0 |    36.33 |       70.6 | **5.024** |   14.893 |  ✅  |
|                  0.9460 |     3,849 |     110.0 |    36.99 |       69.4 | **4.942** |   14.649 |  ❌  |
|                  0.9470 |     3,853 |     108.0 |    37.68 |       68.1 | **4.859** |   14.404 |  ❌  |
|                  0.9480 |     3,857 |     106.0 |    38.39 |       66.9 | **4.776** |   14.159 |  ❌  |
|                  0.9490 |     3,861 |     104.0 |    39.13 |       65.7 | **4.693** |   13.913 |  ❌  |
|                  0.9500 |     3,865 |     102.0 |    39.89 |       64.4 | **4.610** |   13.666 |  ❌  |
|                  0.9550 |     3,885 |      92.0 |    44.23 |       58.3 | **4.189** |   12.419 |  ❌  |
|                  0.9600 |     3,906 |      81.5 |    49.93 |       51.8 | **3.740** |   11.089 |  ❌  |
|                  0.9650 |     3,926 |      71.5 |    56.91 |       45.5 | **3.305** |    9.801 |  ❌  |
|                  0.9700 |     3,946 |      61.5 |    66.16 |       39.3 | **2.864** |    8.494 |  ❌  |

<!-- /A7:THICK -->

**한계는 `r` = 0.945 다.** 피로 안전율 5.024 로 요구치를 겨우 넘고, 0.946 에서
4.942 로 떨어진다.

|                       | 현행`r` = 0.88543 | **한계 `r` = 0.945** |                         차 |
| --------------------- | ------------------: | ---------------------------: | -------------------------: |
| 내경                  |            3,602 mm |                     3,845 mm |                       +243 |
| **두께**        |            233.5 mm |           **112.0 mm** |  **−121.5 (−52%)** |
| `OD/t`              |                17.4 |                         36.3 |                      ×2.1 |
| **샤프트 질량** |             142.6 t |             **70.6 t** | **−72.0 t (−50%)** |
| 피로 안전율           |               9.566 |                        5.024 |                      −47% |
| 영구변형              |              28.327 |                       14.893 |                      −47% |

**① 샤프트 질량을 절반으로 줄일 수 있다.** 피로 5.0 을 지키면서 **−72 t** 다.
`a01` 의 총질량은 169.5 t(= 2 × 13.42 + 142.6)이므로 **97.5 t 로 42% 감소**한다 —
프론트 최경량(133.5 t)보다도 가볍다. **샤프트 두께 규칙이 총질량의 지배 인자**
라는 뜻이다.

**② 안전율은 두께에 거의 비례한다.** 두께 233.5 → 112.0 mm(×0.48)에 피로
9.566 → 5.024(×0.53)로, 단면계수가 아니라 **두께에 준하는 선형 거동**이다.
`OD` 가 고정된 얇은 링에서는 단면계수 `≈ π·OD²·t/4` 라 `t` 에 비례한다.

**③ 정적 안전율도 같은 비율로 준다**(28.33 → 14.89 · −47%). 두 지표가 같은
단면 특성에 걸려 있어 **피로가 먼저 한계에 닿는다** — 정적은 한계점에서도
14.9배로 여유롭다.

> **이 검토는 `sizing_geom` 의 규칙을 건드린다.** `ID_OVER_OD = 0.88543` 은
> v1.3 실측에서 역산한 값이고 §4-3 의 종속 사슬 전체가 여기 걸려 있다. 비율을
> 바꾸면 **P1 극한 응력·S3 프론트·부록 6 전체를 다시 계산**해야 하므로, 여기서는
> **여유가 얼마나 남아 있는지만** 본 것이다. 채택하려면 별도 결정이 필요하다.
>
> **한계 — 검토하지 않은 것 셋.** ⑴ 얇은 벽의 **국부 좌굴·타원화**는 DIN 743
> 범위 밖이다(`OD/t` 36 은 얇은 축에 속한다) ⑵ 베어링 **끼워맞춤 압력**에 의한
> 링 변형과 접촉응력 변화 ⑶ 볼트 체결부·플랜지 등 **국부 구조**. 실제 채택
> 전에는 이 셋을 별도로 봐야 한다.

#### 7-6.7.3 OD별 한계 두께 — 종속변수 규칙 후보

§7-6.7.2 는 `a01` 한 설계였다. **40건 전부**에 대해 피로 5.0 을 만족하는 최대
비율 `r*` 과 그때의 두께 `t*` 를 찾아, `OD` 와의 관계를 규칙으로 쓸 수 있는지
본다.

하중은 설계마다 **자기 `k`** 로 `DLC1.2-k-s5` 를 30빈으로 변환해 쓰고, 먼저
30빈에서 그 설계의 최악 빈을 찾은 뒤 비율 탐색은 그 빈 하나로 한다. `r*` 은
이분탐색 12회 후 **0.001 단위로 내림**(안전측)하고, 마지막에 `r*` 에서 30빈을
다시 돌려 **최악 빈이 바뀌지 않았는지** 확인한다.

<!-- A7:ODTHICK -->

| 태그    | OD [mm] | z2 [m] | 현행 SF | **`r*`** | 내경* [mm] | **`t*` [mm]** | `OD/t*` |  SF* |      질량 [t] |
| ------- | ------: | -----: | ------: | ---------------: | ---------: | --------------------: | --------: | ---: | ------------: |
| `b01` |   4,086 |    6.0 |    9.63 |  **0.945** |      3,861 |       **112.5** |      36.3 | 5.06 | 143.9 → 71.2 |
| `a01` |   4,069 |    6.0 |    9.57 |  **0.945** |      3,845 |       **112.0** |      36.3 | 5.02 | 142.6 → 70.6 |
| `a02` |   4,060 |    6.0 |    9.50 |  **0.945** |      3,836 |       **112.0** |      36.3 | 5.00 | 142.0 → 70.4 |
| `a03` |   4,047 |    6.0 |    9.39 |  **0.944** |      3,820 |       **113.5** |      35.7 | 5.02 | 141.0 → 71.1 |
| `b02` |   4,046 |    6.0 |    9.35 |  **0.943** |      3,815 |       **115.5** |      35.0 | 5.08 | 140.9 → 72.3 |
| `a04` |   4,041 |    6.0 |    9.36 |  **0.944** |      3,814 |       **113.5** |      35.6 | 5.01 | 140.5 → 71.0 |
| `b03` |   4,023 |    6.0 |    9.20 |  **0.943** |      3,793 |       **115.0** |      35.0 | 5.01 | 139.2 → 71.6 |
| `b04` |   4,016 |    6.0 |    9.16 |  **0.942** |      3,783 |       **116.5** |      34.5 | 5.05 | 139.0 → 72.4 |
| `a05` |   4,009 |    6.0 |    9.13 |  **0.942** |      3,776 |       **116.5** |      34.4 | 5.04 | 138.4 → 72.2 |
| `a06` |   3,982 |    6.0 |    8.97 |  **0.941** |      3,747 |       **117.5** |      33.9 | 5.02 | 136.6 → 72.3 |
| `a07` |   3,978 |    6.0 |    8.93 |  **0.941** |      3,743 |       **117.5** |      33.9 | 5.01 | 136.2 → 72.3 |
| `b05` |   3,976 |    6.0 |    8.89 |  **0.940** |      3,737 |       **119.5** |      33.3 | 5.06 | 136.1 → 73.4 |
| `a08` |   3,968 |    6.0 |    8.85 |  **0.940** |      3,729 |       **119.5** |      33.2 | 5.05 | 135.5 → 73.3 |
| `a09` |   3,962 |    6.0 |    8.80 |  **0.940** |      3,724 |       **119.0** |      33.3 | 5.01 | 135.0 → 72.8 |
| `b06` |   3,944 |    6.0 |    8.67 |  **0.939** |      3,703 |       **120.5** |      32.7 | 5.01 | 133.8 → 73.4 |
| `b07` |   3,936 |    6.0 |    8.60 |  **0.938** |      3,691 |       **122.5** |      32.1 | 5.06 | 133.3 → 74.4 |
| `a10` |   3,935 |    6.0 |    8.63 |  **0.938** |      3,691 |       **122.0** |      32.3 | 5.05 | 133.2 → 74.1 |
| `b08` |   3,917 |    6.0 |    8.48 |  **0.937** |      3,670 |       **123.5** |      31.7 | 5.04 | 132.0 → 74.6 |
| `a63` |   3,915 |    3.0 |    8.50 |  **0.937** |      3,668 |       **123.5** |      31.7 | 5.06 |  71.1 → 40.2 |
| `b09` |   3,909 |    6.0 |    8.42 |  **0.937** |      3,662 |       **123.5** |      31.7 | 5.02 | 131.5 → 74.5 |
| `b10` |   3,861 |    6.0 |    8.14 |  **0.934** |      3,606 |       **127.5** |      30.3 | 5.05 | 128.4 → 75.8 |
| `a64` |   3,814 |    3.0 |    7.85 |  **0.932** |      3,554 |       **130.0** |      29.3 | 5.01 |  67.4 → 41.1 |
| `a59` |   3,721 |    4.0 |    7.29 |  **0.926** |      3,445 |       **138.0** |      27.0 | 5.01 |  82.6 → 54.5 |
| `a60` |   3,679 |    4.0 |    7.04 |  **0.923** |      3,395 |       **142.0** |      25.9 | 5.02 |  80.7 → 55.4 |
| `a55` |   3,652 |    4.5 |    6.89 |  **0.921** |      3,363 |       **144.5** |      25.3 | 5.02 |  88.4 → 62.1 |
| `a56` |   3,640 |    4.5 |    6.84 |  **0.920** |      3,348 |       **146.0** |      24.9 | 5.04 |  87.9 → 62.5 |
| `a57` |   3,628 |    4.5 |    6.76 |  **0.919** |      3,334 |       **147.0** |      24.7 | 5.03 |  87.2 → 62.7 |
| `a58` |   3,581 |    4.5 |    6.50 |  **0.915** |      3,276 |       **152.5** |      23.5 | 5.04 |  85.0 → 64.1 |
| `b37` |   3,486 |    5.0 |    5.97 |  **0.907** |      3,161 |       **162.5** |      21.5 | 5.01 |  88.6 → 72.8 |
| `b40` |   3,418 |    5.0 |    5.62 |  **0.900** |      3,076 |       **171.0** |      20.0 | 5.02 |  85.1 → 74.8 |
| `b35` |   3,412 |    5.5 |    5.59 |  **0.899** |      3,067 |       **172.5** |      19.8 | 5.04 |  92.5 → 82.2 |
| `a61` |   3,377 |    4.5 |    5.44 |  **0.896** |      3,025 |       **176.0** |      19.2 | 5.03 |  75.5 → 69.0 |
| `a62` |   3,371 |    4.5 |    5.42 |  **0.895** |      3,017 |       **177.0** |      19.0 | 5.03 |  75.3 → 69.3 |
| `b36` |   3,364 |    5.5 |    5.36 |  **0.894** |      3,007 |       **178.5** |      18.8 | 5.03 |  90.0 → 83.6 |
| `b32` |   3,333 |    6.0 |    5.21 |  **0.891** |      2,969 |       **182.0** |      18.3 | 5.01 |  95.6 → 91.3 |
| `b38` |   3,333 |    5.5 |    5.21 |  **0.891** |      2,969 |       **182.0** |      18.3 | 5.01 |  88.2 → 84.3 |
| `b41` |   3,333 |    5.0 |    5.21 |  **0.891** |      2,969 |       **182.0** |      18.3 | 5.01 |  80.9 → 77.3 |
| `b33` |   3,312 |    6.0 |    5.13 |  **0.888** |      2,941 |       **185.5** |      17.9 | 5.03 |  94.5 → 92.4 |
| `b39` |   3,303 |    5.5 |    5.08 |  **0.887** |      2,929 |       **187.0** |      17.7 | 5.02 |  86.7 → 85.7 |
| `b34` |   3,287 |    6.0 |    5.00 |  **0.885** |      2,908 |       **189.5** |      17.3 | 5.02 |  93.0 → 93.5 |

<!-- /A7:ODTHICK -->

**회귀** — 얇은 링의 단면계수가 `≈ π·OD²·t/4` 이므로 하중이 같다면 `OD²·t` 가
일정, 즉 **`t* ∝ OD⁻²`** 가 예상된다. 실제로 그런지, 아니면 스팬(`z2`)이 더
지배적인지를 가른다.
...
| 모형          | 식                                               |   R² |
| ------------- | ------------------------------------------------ | ----: |
| OD 단독       | `t* = 6.32e+10 · OD^-2.42`                    | 0.999 |
| OD + 스팬     | `t* = 6.61e+10 · OD^-2.43 · z2^+0.01`        | 0.999 |
| *물리 예상* | `t* ∝ OD^−2` (단면계수 `≈ π·OD²·t/4`) |    — |

<!-- /A7:ODFIT -->

#### 7-6.7.4 결과 요약 — 필요 **단면계수**가 일정하다

**40건 · 392초 · 빈 이동 0건.** `t*` 112.0 ~ 189.5 mm · `r*` 0.885 ~ 0.945 ·
현행 안전율 5.00 ~ 9.63.

**① 불변량은 두께비가 아니라 단면계수다.** 네 후보의 흩어짐을 재면 정확
단면계수가 압도적으로 일정하다.

| 불변량 후보                                             |           편차 |       변동계수 |
| ------------------------------------------------------- | -------------: | -------------: |
| **정확 단면계수 `W = π(OD⁴−ID⁴)/(32·OD)`** | **2.1%** | **0.5%** |
| `OD^2.42 · t*` (회귀 지수)                           |           2.5% |           0.6% |
| `OD² · t*` (얇은 링 근사)                           |          10.9% |           3.3% |
| `OD³ · t*`                                          |          14.0% |           4.4% |

**`W = 1.347 × 10⁹ mm³` 로 놓고 `t` 를 역산하면 40건 오차가 −1.2% ~ +1.0%**
(평균절대 0.4%)다. `r*` 을 0.001 로 이산화한 잡음(약 1.5%)보다 작다.

**`W` 가 무엇인지 — 안전율이 `W` 에 정확히 비례한다.** `a01` 스윕 23점에서

```
SF = 3.7478 · (W / 10⁹)      R² = 1.000000 · 절편 +0.0001
정적 SF = 11.11 · (W / 10⁹)   편차 0.15%
```

DIN 743 무한수명 안전율은 `SF = σ_ADK / σ_a` 이고 굽힘 진폭이 `σ_a = M/W` 이므로
`SF = (σ_ADK/M)·W` 다. 측정된 기울기가 곧 `σ_ADK/M = 3.7478×10⁻⁹ mm⁻³`,
뒤집으면 **`M/σ_ADK = 2.668×10⁸ mm³`** 다. 따라서

$$
W_{req} = SF_{target} \cdot \frac{M}{\sigma_{ADK}}
$$

**`W` 는 「지배 하중의 굽힘모멘트를 DIN 743 허용 응력진폭의 1/SF 이하로 눌러
두는 데 필요한 단면계수」** 이고, 40건에서 이 값이 일정하다는 것은 **지배 단면의
모멘트 `M` 이 설계에 무관하게 같다**는 뜻이다 — `z1`·`z2` 가 무관했던 이유가
여기 있다.

> **산포 0.8% 가 진짜 값이다.** 위의 2.1% 는 `r*` 을 0.001 단위로 **내림**한
> 탓에 실제 안전율이 5.001 ~ 5.078(평균 5.029)로 흩어진 것이 섞인 값이다.
> 안전율을 정확히 5.0 으로 환산하면 `W` 는 **1.3341 ~ 1.3442 × 10⁹**(평균
> **1.3395**, 편차 **0.8%**)로 좁아진다.

**② 얇은 링 근사는 이 범위에서 못 쓴다.** `t/OD` 가 0.028 ~ 0.058 이라
`1−r⁴ ≈ 4(1−r)` 가 최대 11% 어긋난다. 전개하면

```
W ∝ OD² · t · (1 − 3·t/OD)
```

이고 이 보정항이 `OD²·t*` 의 편차 10.9% 를 그대로 설명한다. **회귀식
`t* ∝ OD^−2.42`(R² 0.999)는 물리 지수가 아니라 이 정확식을 표본 구간에서
근사한 값**이다 — OD 범위를 벗어나면 어긋난다.

> 표본이 좁으면 오해하기 쉽다. `a01`~`a10`(`z2` 6.0 동일 · OD 범위 3.4%)만 보면
> `OD²·t*` 가 편차 2.3% 로 일정해 보여 `t* ∝ OD⁻²` 로 읽힌다. **OD 범위를 24% 로
> 넓히고서야 근사가 깨지는 것이 보였다.**

**③ 스팬은 무관하다.** `z2` 를 회귀에 넣어도 지수가 **+0.014**, R² 는 0.9990 →
0.9992 로 사실상 개선이 없다. 그룹별로 봐도 같다.

| 그룹                                          | `W` [10⁹ mm³]                                  |
| --------------------------------------------- | -------------------------------------------------- |
| `z1` 1.0 (20건) / 1.5 (20건)                | 1.3437 / 1.3505                                    |
| `z2` 3.0 · 4.0 · 4.5 · 5.0 · 5.5 · 6.0 | 1.346 · 1.343 · 1.347 · 1.347 · 1.350 · 1.347 |

배치를 `z2` 3.0 ~ 6.0 m 로 바꿔도 **샤프트가 요구하는 단면계수는 같다.** 지배
단면의 모멘트가 스팬이 아니라 **로터 오버행(`z1` 쪽)** 에서 결정되기 때문으로
보인다 — §7-6.6 이 「어떤 하중이 샤프트를 지배하는지는 설계에 의존하지 않는다」
고 한 것과 같은 방향이다.

**④ 질량 효과.** 40건 합계 **4,435 → 2,872 t**, 설계당 평균 **35% 감소**다.

#### 7-6.7.5 종속변수 반영 — 목표 안전율 **5.2** 기준

`SF ∝ W` 이므로 목표 안전율은 `W` 에 그대로 곱해진다. **5.2 를 채택한다** —
여유 4% 로 ⑴ `W` 산포 0.8% ⑵ `r` 을 1 mm 격자로 내릴 때의 1 ~ 2% 를 덮는다.
대가는 두께 +3.9%(질량 +107 t / 40건 합계)로, §7-6.7.2 에서 얻은 이득(−1,544 t)에
비하면 작다.

|                  목표 SF | `W` [10⁹ mm³] |       `a01` 두께 |      40건 질량 합 |
| -----------------------: | ----------------: | -----------------: | ----------------: |
|               5.0 (한계) |            1.3395 |           111.9 mm |           2,891 t |
|     **5.2 (채택)** |  **1.3930** | **116.8 mm** | **2,998 t** |
|                      5.5 |            1.4734 |           124.2 mm |           3,160 t |
| *현행 `r` = 0.88543* |    2.55 (`a01`) |           233.5 mm |           4,435 t |

**규칙 두 형태.** 정확형을 권한다 — 근사형은 표본 구간 밖에서 보장이 없다.

| 형태             | 식                                                                       |                                       오차 |
| ---------------- | ------------------------------------------------------------------------ | -----------------------------------------: |
| **정확형** | `ID = (OD⁴ − 32·W·OD/π)^¼` · **`W = 1.393×10⁹ mm³`** |     적용 후 실제 SF**5.182 ~ 5.221** |
| 근사형           | `t = 7.093×10¹⁰ · OD^−2.434`                                      | −0.6% ~ +0.5% (OD 3,287 ~ 4,086 안에서만) |

정확형을 적용한 결과다.

| `OD` [mm] | `ID` [mm] | 두께 [mm] | `r` = ID/OD |
| ----------: | ----------: | --------: | ------------: |
|       3,300 |       2,911 |     194.7 |        0.8820 |
|       3,600 |       3,288 |     156.0 |        0.9133 |
|       3,900 |       3,642 |     128.8 |        0.9339 |
|       4,100 |       3,870 |     114.8 |        0.9440 |

**`r` 이 0.88 ~ 0.94 로 변한다** — 현행처럼 상수 하나로 둘 수 없다는 것이 이
검토의 결론이다. 작은 축일수록 두꺼워야 하고, 그 관계는 `W` 일정이 지배한다.

**정수화는 `floor` 다.** 정확형은 실수를 주므로 1 mm 로 끊어야 하는데, §6-4.2 와
같은 이유로 **내림**을 쓴다 — 내경을 깎아 내려야 살두께가 두꺼워져 안전측이다.

| 정수화              | 환산 SF                 | **목표 5.2 미달** | 프론트 64건 질량 합 |
| ------------------- | ----------------------- | ----------------------: | ------------------: |
| **`floor`** | **5.200 ~ 5.218** |           **0건** |             5,039 t |
| `round`           | 5.190 ~ 5.210           |          **32건** |             5,031 t |

`round` 는 절반이 목표를 밑돈다(미달 폭 최대 0.19%). `floor` 의 대가는 질량
**+8.1 t (+0.16%)** 뿐이다. 소수부가 0.007 ~ 0.987 로 고르게 퍼져 있어 설계마다
0 ~ 1 mm 의 보너스가 붙고, 그래서 실제 SF 가 5.200 ~ 5.218 로 흩어진다.

#### 7-6.7.6 파레토 프론트 재검토

§6-11.5a 의 프론트 **64건**에 위 규칙을 적용해 내경을 다시 잡고 **MASTA 로 다시
돌린다.**
...
**질량만 해석식으로 고치지 않는 이유** — 두께가 바뀌면 샤프트 강성이 달라지고,
유연체 FE 모델에서는 처짐이 베어링 하중 분포를 바꿔 **`σ_max` 도 달라진다.**
σ ≤ 2,100 제약이 유지되는지 확인해야 한다. 또 총질량이 설계마다 다른 폭으로
줄므로 **지배 관계가 깨질 수 있어** 64건을 다시 비지배 판정한다.

<!-- A7:FRONT52 -->

σ 통과 **0 / 64건** · 한도 2,100 MPa

|  # | OD [mm] |               내경 [mm] | 두께 [mm] | **베어링** [t] |    샤프트 [t] | **총질량** [t] | Δ총질량 |                σ [MPa] | UW 미스얼라인먼트 [mrad] | 판정 |
| -: | ------: | ----------------------: | --------: | -------------------: | ------------: | -------------------: | -------: | ----------------------: | -----------------------: | :--: |
|  1 |   4,069 | 3,602 →**3,835** |     117.0 |      **13.42** | 142.6 → 73.6 |      **100.5** |    -69.0 | 2,098 →**2,194** |  0.638 →**1.075** |  ❌  |
|  2 |   4,060 | 3,594 →**3,825** |     117.5 |      **13.58** | 142.0 → 73.8 |      **101.0** |    -68.2 | 2,098 →**2,187** |  0.647 →**1.081** |  ❌  |
|  3 |   4,047 | 3,583 →**3,810** |     118.5 |      **13.63** | 141.0 → 74.1 |      **101.4** |    -66.8 | 2,099 →**2,193** |  0.619 →**1.028** |  ❌  |
|  4 |   4,041 | 3,578 →**3,803** |     119.0 |      **13.68** | 140.5 → 74.3 |      **101.7** |    -66.1 | 2,098 →**2,193** |  0.650 →**1.074** |  ❌  |
|  5 |   4,009 | 3,549 →**3,767** |     121.0 |      **13.83** | 138.4 → 74.9 |      **102.6** |    -63.5 | 2,100 →**2,191** |  0.645 →**1.047** |  ❌  |
|  6 |   3,982 | 3,525 →**3,736** |     123.0 |      **13.97** | 136.6 → 75.6 |      **103.5** |    -61.0 | 2,100 →**2,186** |  0.681 →**1.083** |  ❌  |
|  7 |   3,978 | 3,522 →**3,731** |     123.5 |      **14.17** | 136.2 → 75.8 |      **104.2** |    -60.4 | 2,095 →**2,182** |  0.682 →**1.080** |  ❌  |
|  8 |   3,968 | 3,513 →**3,720** |     124.0 |      **14.20** | 135.5 → 75.9 |      **104.3** |    -59.6 | 2,100 →**2,186** |  0.647 →**1.021** |  ❌  |
|  9 |   3,962 | 3,508 →**3,713** |     124.5 |      **14.39** | 135.0 → 76.1 |      **104.9** |    -58.9 | 2,099 →**2,184** |  0.680 →**1.067** |  ❌  |
| 10 |   3,935 | 3,484 →**3,682** |     126.5 |      **14.41** | 133.2 → 76.7 |      **105.6** |    -56.5 | 2,100 →**2,185** |  0.701 →**1.081** |  ❌  |
| 11 |   3,911 | 3,462 →**3,655** |     128.0 |      **14.65** | 131.8 → 77.1 |      **106.4** |    -54.7 | 2,096 →**2,180** |  0.712 →**1.085** |  ❌  |
| 12 |   3,902 | 3,454 →**3,644** |     129.0 |      **14.74** | 131.2 → 77.5 |      **107.0** |    -53.7 | 2,099 →**2,184** |  0.715 →**1.081** |  ❌  |
| 13 |   3,892 | 3,446 →**3,633** |     129.5 |      **14.97** | 130.3 → 77.6 |      **107.5** |    -52.7 | 2,099 →**2,176** |  0.678 →**1.018** |  ❌  |
| 14 |   3,891 | 3,445 →**3,631** |     130.0 |      **14.98** | 130.3 → 77.9 |      **107.8** |    -52.4 | 2,100 →**2,179** |  0.694 →**1.039** |  ❌  |
| 15 |   3,875 | 3,431 →**3,613** |     131.0 |      **14.99** | 129.2 → 78.1 |      **108.1** |    -51.0 | 2,099 →**2,178** |  0.723 →**1.071** |  ❌  |
| 16 |   3,855 | 3,413 →**3,590** |     132.5 |      **15.13** | 127.9 → 78.6 |      **108.8** |    -49.4 | 2,099 →**2,169** |  0.730 →**1.067** |  ❌  |
| 17 |   3,844 | 3,403 →**3,577** |     133.5 |      **15.28** | 127.3 → 78.9 |      **109.5** |    -48.4 | 2,096 →**2,168** |  0.755 →**1.095** |  ❌  |
| 18 |   3,821 | 3,383 →**3,550** |     135.5 |      **15.48** | 125.6 → 79.5 |      **110.5** |    -46.1 | 2,095 →**2,166** |  0.766 →**1.093** |  ❌  |
| 19 |   3,810 | 3,373 →**3,537** |     136.5 |      **15.67** | 125.0 → 79.9 |      **111.2** |    -45.1 | 2,098 →**2,166** |  0.741 →**1.050** |  ❌  |
| 20 |   3,788 | 3,354 →**3,512** |     138.0 |      **15.68** | 123.4 → 80.2 |      **111.6** |    -43.2 | 2,100 →**2,172** |  0.780 →**1.092** |  ❌  |
| 21 |   3,777 | 3,344 →**3,499** |     139.0 |      **15.88** | 122.8 → 80.5 |      **112.3** |    -42.2 | 2,095 →**2,166** |  0.785 →**1.091** |  ❌  |
| 22 |   3,765 | 3,333 →**3,485** |     140.0 |      **15.92** | 122.1 → 80.8 |      **112.7** |    -41.3 | 2,100 →**2,165** |  0.765 →**1.055** |  ❌  |
| 23 |   3,759 | 3,328 →**3,477** |     141.0 |      **15.98** | 121.6 → 81.3 |      **113.2** |    -40.4 | 2,100 →**2,165** |  0.788 →**1.079** |  ❌  |
| 24 |   3,730 | 3,302 →**3,443** |     143.5 |      **16.25** | 119.8 → 82.0 |      **114.5** |    -37.9 | 2,099 →**2,164** |  0.812 →**1.093** |  ❌  |
| 25 |   3,725 | 3,298 →**3,437** |     144.0 |      **16.37** | 119.4 → 82.1 |      **114.9** |    -37.3 | 2,100 →**2,158** |  0.802 →**1.072** |  ❌  |
| 26 |   3,715 | 3,289 →**3,425** |     145.0 |      **16.51** | 118.8 → 82.5 |      **115.5** |    -36.4 | 2,098 →**2,162** |  0.810 →**1.077** |  ❌  |
| 27 |   3,689 | 3,266 →**3,395** |     147.0 |      **17.01** | 117.1 → 82.9 |      **116.9** |    -34.2 | 2,093 →**2,154** |  0.804 →**1.053** |  ❌  |
| 28 |   3,665 | 3,245 →**3,366** |     149.5 |      **17.12** | 115.6 → 83.7 |      **118.0** |    -31.9 | 2,099 →**2,146** |  0.839 →**1.074** |  ❌  |
| 29 |   3,657 | 3,238 →**3,356** |     150.5 |      **17.27** | 115.0 → 84.1 |      **118.6** |    -31.0 | 2,100 →**2,146** |  0.814 →**1.036** |  ❌  |
| 30 |   3,642 | 3,224 →**3,338** |     152.0 |      **17.40** | 114.3 → 84.5 |      **119.3** |    -29.8 | 2,097 →**2,152** |  0.832 →**1.053** |  ❌  |
| 31 |   3,638 | 3,221 →**3,333** |     152.5 |      **17.50** | 113.9 → 84.7 |      **119.7** |    -29.2 | 2,097 →**2,151** |  0.826 →**1.041** |  ❌  |
| 32 |   3,598 | 3,185 →**3,285** |     156.5 |      **17.88** | 111.5 → 85.8 |      **121.5** |    -25.8 | 2,100 →**2,140** |  0.919 →**1.123** |  ❌  |
| 33 |   3,555 | 3,147 →**3,233** |     161.0 |      **18.54** | 108.9 → 87.0 |      **124.1** |    -21.8 | 2,097 →**2,132** |  0.924 →**1.096** |  ❌  |
| 34 |   3,542 | 3,136 →**3,217** |     162.5 |      **18.61** | 108.0 → 87.5 |      **124.7** |    -20.5 | 2,099 →**2,133** |  0.960 →**1.127** |  ❌  |
| 35 |   3,533 | 3,128 →**3,206** |     163.5 |      **18.62** | 107.4 → 87.7 |      **125.0** |    -19.7 | 2,100 →**2,132** |  0.967 →**1.128** |  ❌  |
| 36 |   3,523 | 3,119 →**3,193** |     165.0 |      **18.90** | 106.9 → 88.3 |      **126.1** |    -18.6 | 2,098 →**2,128** |  0.951 →**1.100** |  ❌  |
| 37 |   3,506 | 3,104 →**3,172** |     167.0 |      **18.97** | 105.8 → 88.8 |      **126.7** |    -17.0 | 2,099 →**2,128** |  0.988 →**1.128** |  ❌  |
| 38 |   3,463 | 3,066 →**3,119** |     172.0 |      **19.64** | 103.2 → 90.2 |      **129.4** |    -13.1 | 2,098 →**2,122** |  1.016 →**1.126** |  ❌  |
| 39 |   3,442 | 3,047 →**3,093** |     174.5 |      **20.05** | 102.1 → 90.8 |      **130.9** |    -11.2 | 2,099 →**2,120** |  1.021 →**1.116** |  ❌  |
| 40 |   3,437 | 3,043 →**3,086** |     175.5 |      **20.10** | 101.7 → 91.2 |      **131.4** |    -10.5 | 2,100 →**2,119** |  1.015 →**1.103** |  ❌  |
| 41 |   3,420 | 3,028 →**3,065** |     177.5 |      **20.54** | 100.6 → 91.7 |      **132.8** |     -9.0 | 2,100 →**2,119** |  1.024 →**1.101** |  ❌  |
| 42 |   3,399 | 3,009 →**3,038** |     180.5 |      **21.08** |  99.5 → 92.5 |      **134.7** |     -7.0 | 2,098 →**2,111** |  1.032 →**1.091** |  ❌  |
| 43 |   3,676 | 3,254 →**3,379** |     148.5 |      **21.51** |  98.5 → 70.6 |      **113.6** |    -27.9 | 2,098 →**2,140** |  0.695 →**0.882** |  ❌  |
| 44 |   3,497 | 3,096 →**3,161** |     168.0 |      **21.58** |  97.2 → 82.2 |      **125.4** |    -14.9 | 2,099 →**2,126** |  0.945 →**1.069** |  ❌  |
| 45 |   3,474 | 3,075 →**3,133** |     170.5 |      **21.92** |  96.0 → 82.8 |      **126.7** |    -13.2 | 2,098 →**2,123** |  0.957 →**1.068** |  ❌  |
| 46 |   3,614 | 3,199 →**3,304** |     155.0 |      **22.31** |  95.3 → 72.3 |      **116.9** |    -23.0 | 2,099 →**2,134** |  0.733 →**0.892** |  ❌  |
| 47 |   3,610 | 3,196 →**3,300** |     155.0 |      **22.45** |  94.9 → 72.2 |      **117.1** |    -22.8 | 2,098 →**2,133** |  0.727 →**0.882** |  ❌  |
| 48 |   3,444 | 3,049 →**3,095** |     174.5 |      **22.59** |  94.3 → 83.9 |      **129.1** |    -10.4 | 2,098 →**2,117** |  0.967 →**1.054** |  ❌  |
| 49 |   3,438 | 3,044 →**3,087** |     175.5 |      **22.62** |  93.9 → 84.2 |      **129.4** |     -9.7 | 2,100 →**2,116** |  0.890 →**0.964** |  ❌  |
| 50 |   3,424 | 3,031 →**3,070** |     177.0 |      **22.79** |  93.2 → 84.5 |      **130.1** |     -8.7 | 2,097 →**2,113** |  0.948 →**1.020** |  ❌  |
| 51 |   3,417 | 3,025 →**3,061** |     178.0 |      **22.83** |  92.8 → 84.8 |      **130.4** |     -8.1 | 2,099 →**2,113** |  1.002 →**1.070** |  ❌  |
| 52 |   3,398 | 3,008 →**3,037** |     180.5 |      **23.11** |  91.8 → 85.4 |      **131.6** |     -6.4 | 2,099 →**2,111** |  1.002 →**1.057** |  ❌  |
| 53 |   3,376 | 2,989 →**3,009** |     183.5 |      **23.72** |  90.5 → 86.1 |      **133.6** |     -4.4 | 2,098 →**2,106** |  1.032 →**1.070** |  ❌  |
| 54 |   3,359 | 2,974 →**2,987** |     186.0 |      **24.12** |  89.6 → 86.8 |      **135.0** |     -2.8 | 2,095 →**2,101** |  1.002 →**1.026** |  ❌  |
| 55 |   3,652 | 3,233 →**3,350** |     151.0 |      **24.53** |  88.4 → 64.8 |      **113.8** |    -23.6 | 2,096 →**2,136** |  0.689 →**0.853** |  ❌  |
| 56 |   3,640 | 3,222 →**3,336** |     152.0 |      **24.76** |  87.9 → 65.0 |      **114.5** |    -22.9 | 2,100 →**2,139** |  0.719 →**0.884** |  ❌  |
| 57 |   3,628 | 3,212 →**3,321** |     153.5 |      **24.77** |  87.2 → 65.3 |      **114.9** |    -21.8 | 2,099 →**2,137** |  0.726 →**0.883** |  ❌  |
| 58 |   3,581 | 3,170 →**3,264** |     158.5 |      **25.85** |  85.0 → 66.5 |      **118.2** |    -18.5 | 2,096 →**2,129** |  0.748 →**0.885** |  ❌  |
| 59 |   3,721 | 3,294 →**3,433** |     144.0 |      **25.99** |  82.6 → 56.8 |      **108.8** |    -25.8 | 2,100 →**2,139** |  0.618 →**0.784** |  ❌  |
| 60 |   3,679 | 3,257 →**3,383** |     148.0 |      **26.91** |  80.7 → 57.6 |      **111.4** |    -23.1 | 2,099 →**2,136** |  0.642 →**0.796** |  ❌  |
| 61 |   3,377 | 2,990 →**3,010** |     183.5 |      **29.26** |  75.5 → 71.8 |      **130.3** |     -3.7 | 2,100 →**2,106** |  0.842 →**0.870** |  ❌  |
| 62 |   3,371 | 2,984 →**3,002** |     184.5 |      **29.31** |  75.3 → 72.0 |      **130.6** |     -3.3 | 2,098 →**2,104** |  0.845 →**0.869** |  ❌  |
| 63 |   3,915 | 3,466 →**3,659** |     128.0 |      **31.23** |  71.1 → 41.6 |      **104.0** |    -29.5 | 2,100 →**2,134** |  0.544 →**0.707** |  ❌  |
| 64 |   3,814 | 3,377 →**3,542** |     136.0 |      **33.05** |  67.4 → 42.9 |      **109.0** |    -24.5 | 2,093 →**2,124** |  0.564 →**0.704** |  ❌  |

<!-- /A7:FRONT52 -->

![두께 규칙 적용 전후 파레토 프론트](Results/사이징최적화/figures/pareto_w52.png)

##### 7-6.7.6.1 결과 — **σ 제약을 64건 전부 위반한다**

| 지표             | 현행 규칙         | 두께 규칙 적용              |
| ---------------- | ----------------- | --------------------------- |
| 최소 총질량      | 133.5 t           | **100.5 t (−25%)**   |
| σ_max           | 2,093 ~ 2,100 MPa | **2,101 ~ 2,194 MPa** |
| σ 증가          | —                | **+5 ~ +96 MPa**      |
| σ ≤ 2,100 통과 | 64 / 64           | **0 / 64**            |

**질량은 크게 줄지만 쓸 수 없다.** 샤프트를 얇게 하면 굽힘 강성이 떨어져 처짐이
커지고, 유연체 FE 모델에서 그 처짐이 롤러 하중 분포를 기울여 **베어링 접촉응력을
올린다.** 프론트는 정의상 **σ 제약 경계(2,093 ~ 2,100 MPa)에 붙어 있으므로**
강성이 조금만 줄어도 곧바로 한도를 넘는다.

**가장 얇게 깎인 설계일수록 σ 증가가 크다** — `+96 MPa` 는 보어가 큰(=벽이 가장
얇아진) 설계에서 나온다. 반대로 보어가 작은 설계는 `+5 MPa` 로 겨우 넘는다.

**메커니즘이 미스얼라인먼트로 직접 확인된다.** UW 베어링의 상대
미스얼라인먼트가 **0.544 ~ 1.032 → 0.704 ~ 1.128 mrad**(+0.024 ~ +0.436 ·
평균 **1.30배**)로 커졌고, **σ 증가와의 상관이 +0.995** 다.

```
얇은 벽 → 굽힘 강성 ↓ → 샤프트 처짐 ↑ → 베어링 미스얼라인먼트 ↑
      → 롤러 하중 분포 편향 ↑ → 접촉응력 ↑
```

즉 σ 상승은 재료가 줄어서가 아니라 **정렬이 틀어져서** 생긴다. 롤러 프로파일
(`DIN_LUNDBERG`)이 흡수할 수 있는 범위를 넘어서면 끝단 하중집중이 커지기
때문이다.

> **재현성 확인** — 현행 내경으로 다시 돌린 σ 가 `s3_pareto.csv` 기록값과
> **0.0 MPa** 차이였다. 두 값의 차이는 전적으로 내경 변경에서 온 것이다.

##### 7-6.7.6.2 두 목적의 상충이 사라진다

그림에서 붉은 곡선이 **우상향**이다 — 현행(회색)이 우하향인 것과 정반대다.

**두께 규칙 아래에서는 보어가 클수록 샤프트도 가벼워진다.** `W` 를 일정하게
두면 재료가 중립축에서 멀수록 유리하므로, 같은 단면계수를 더 적은 단면적으로
얻는다(`OD` 3,300 → 1.90 m² · `OD` 4,100 → 1.44 m²). 그런데 §6-11.8 에서 본
대로 **프론트에서 보어가 큰 쪽은 베어링이 가벼운 쪽**이다. 따라서

```
베어링 경량  →  보어 큼  →  샤프트도 경량  →  총질량도 최소
```

가 되어 **두 목적이 같은 방향을 가리키고 파레토 프론트가 한 점으로 붕괴**한다
(재계산 후 비지배 판정 시 1건). 최소 총질량 100.5 t 는 베어링 최경량(13.42 t)
설계에서 나온다.

##### 7-6.7.6.3 결론 — 사후 적용이 아니라 **최적화 루프 안**으로

**두께 규칙은 완성된 프론트에 나중에 얹을 수 없다.** 프론트가 σ 경계 위에서
정의됐기 때문에, 강성을 낮추는 어떤 변경도 그 경계를 깬다.

**쓰려면 S3 를 다시 돌려야 한다.** 두께 규칙을 종속변수 사슬에 넣고 최적화하면
σ ≤ 2,100 을 만족하는 새로운 조합(더 큰 `D_pw`·`D_we` 로 응력을 낮추고 샤프트는
얇게)을 알고리즘이 찾을 것이다. 비용은 §6-11.3 기준 **약 2.9시간**이고,
그 전에 §7-6.7.2 의 미검토 셋(국부 좌굴·끼워맞춤 압력·국부 구조)을 정리해야 한다.

> **부수 확인** — 이번 재해석은 §7-6.7.5 의 규칙이 **피로 관점에서는** 정확히
> 작동함을 보였다(내경이 의도대로 나오고 질량이 예측대로 줄었다). 실패한 것은
> 규칙이 아니라 **사후 적용이라는 방식**이다.

# 부록 8. 추가 최적화 — 샤프트 두께 규칙 반영 (NSGA-II)

> 절 번호 `8-x` 는 부록 8 안에서만 쓴다. 본문 §8(P1·P2 수행)과는 별개다.

## 8-1. 목적

§7-6.7.5 는 샤프트 두께 규칙을 얻었고, §7-6.7.6 은 그것을 **완성된 프론트에
사후 적용할 수 없음**을 보였다 — 프론트가 σ 제약 경계 위에 정의돼 있어 강성을
낮추는 어떤 변경도 그 경계를 깬다(64/64 위반 · 미스얼라인먼트 1.30배).

**그래서 규칙을 종속변수 사슬에 넣고 최적화를 다시 돌린다.** 알고리즘이
σ ≤ 2,100 을 만족하는 새 조합을 스스로 찾게 하는 것이 부록 8 의 목적이다.
아울러 그동안 빠져 있던 **롤러 코너 반경**을 함께 반영한다(§8-3 ③).

## 8-2. 부록 6 에서 그대로 가져오는 것

**검증된 방법론을 재작성하지 않는다.** 아래는 전부 부록 6 의 것을 코드째
재사용한다(`a8_s3_run.py` 가 `nsga_s3_run.py` 를 불러 쓴다).

| 항목         | 내용                                                                                                  | 근거              |
| ------------ | ----------------------------------------------------------------------------------------------------- | ----------------- |
| 설계변수 6개 | `z1`·`z2`·α `Choice` / `D_pw`·`D_we` 스케일 `Integer` / **`u` 밴드 재매개화** | §6-3 · §6-11.3 |
| 제약         | 기하 8개(해석식) + σ ≤ 2,100 MPa · 피로 루프 제외                                                  | §6-5 · §6-7    |
| 목적함수     | `f1` 베어링 1개 질량 ↓ · `f2` 총질량 ↓ (MASTA 실측)                                            | §6-6             |
| 비용 모델    | σ·질량을 MASTA 한 번으로 함께 회수                                                                  | §6-6             |
| 파라미터     | **224 × 150** · 시드 1 · 범주 전수 시딩 · 정수화 켬 · 중복제거 끔 · 세대수 고정           | §6-11.5 as-run   |

**`L_w` 는 밴드 재매개화를 쓴다** — 수리(clip)는 개체의 절반 이상을 세장비
경계에 박아 넣으므로 폐기된 방식이다(§6-11.2).

### 8-2.1. 명명 정정 — `L_w` 와 `L_we`

부록 1 ~ 7 은 롤러 길이를 `L_we` 로 불러 왔으나, 실제로 MASTA 에 넣은 것은
`roller_length`, 즉 **롤러 전장**이었다. 코너 반경이 0 이라 전장과 유효 길이가
같았기 때문에 구분할 필요가 없었을 뿐이다. 부록 8 은 둘을 분리한다.

| 기호               | 지위               | 내용                                                                                                                                        |
| ------------------ | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| **`L_w`**  | **설계변수** | 롤러 전장.`u` 밴드 재매개화가 정하고 MASTA `roller_length` 로 주입. **세장비 제약 1.5 ≤ L_w/D_we ≤ 2.5 와 T·B·C 비율의 기준** |
| **`L_we`** | **종속변수** | 유효 롤러 길이. MASTA`effective_roller_length` 를 읽는다                                                                                  |

**세장비 기준은 `L_w` 로 둔다.** 부록 6 의 제약을 문자 그대로 계승하기 위함이며,
T·B·C 비율(1.30226 / 1.26025 / 1.06281)과 d·D 기하식이 모두 전장 기준이고,
하한 1.5 가 보장하던 살두께 t/D_we ≥ 0.274 도 전장에서 나오기 때문이다(§6-5).
따라서 표에 찍히는 `L_we` 세장비는 D_we 에 따라 1.43 ~ 2.50 으로 하한을 살짝
밑돈다 — **표의 세장비 열은 `L_w` 기준으로 표기**한다.

개명은 **부록 8 계층에서만** 한다. 부록 1 ~ 7 의 as-run 스크립트 40여 개와 그
CSV 열 이름은 기록이므로 건드리지 않는다(`a8_eval.finish()` 가 기반 평가기가
넘긴 `L_we_mm` 을 `L_w_mm` 으로 바꾸고, 새로 읽은 유효 길이를 `L_we_mm` 에 넣는다).

## 8-3. 바뀌는 것 — 종속변수 세 곳

|  # | 항목              | 부록 6                   | **부록 8**                                                           |
| -: | ----------------- | ------------------------ | -------------------------------------------------------------------------- |
| ① | 샤프트 내경       | `floor(0.88543 · OD)` | **`floor((OD⁴ − 32·W·OD/π)^¼)`** · `W` = 1.393×10⁹ mm³ |
| ② | 롤러 코너 반경    | 0 (미설정)               | **좌·우 각 4.3 mm** 주입 → `L_we` 를 읽어 기록                   |
| ③ | 롤러 테이퍼각 2β | MASTA 자동 (읽지 않음)   | MASTA 자동 —**읽어서 기록** (`element_taper_angle`)               |

③은 해석 결과를 바꾸지 않는다. 주입하지 않고 읽기만 하므로 설계별로 얼마가
나왔는지가 캐시와 프론트 CSV 에 남을 뿐이다. `cone_angle` 도 함께 뺀다.

②의 관계는 실측으로 **`L_we` = `L_w` − 8.6 mm 로 전 구간 일정**함을 확인했다
(L_w 180 · 238 · 300 mm 및 D_we 60 mm 소형까지 동일). 유효 접촉 길이가 8.6 mm
줄므로 **σ 는 올라간다.**

> **①은 방향이 한쪽이 아니다.** 두 규칙은 **보어 ≈ 3,328 mm** 에서 교차한다 —
> 그보다 작으면 새 규칙이 **더 두껍고**, 크면 더 얇다. 반대로 §6-11.5a
> 프론트(보어 3,359 ~ 4,069)는 전부 얇아지는 쪽이었다(§7-6.7.6).
>
> 따라서 부록 8 의 최적화는 **작은 보어에 질량 벌점, 큰 보어에 응력 벌점**이
> 걸린 새 지형에서 균형점을 찾는 문제가 된다.

### 8-3.1. v1.3 기준선에서 본 세 변경의 효과

정수화를 끄고 v1.3 기준선 1점을 실제로 돌려 변경을 분해했다.

| 구성           |        샤프트 내경 |      샤프트 질량 |            `L_we` |               **σ** |
| -------------- | -----------------: | ---------------: | ------------------: | -------------------------: |
| 부록 6 (현행)  |           2,704 mm |           43.2 t |           238.05 mm |                3,424.2 MPa |
| + ① 두께 규칙 | **2,571 mm** | **58.4 t** |           238.05 mm | **3,360.7** (−63.5) |
| + ② 코너 반경 |           2,571 mm |           58.4 t | **229.45 mm** |  **3,407.9** (+47.2) |

기준선(보어 3,055)은 교차점보다 작아 샤프트가 두꺼워지고 σ 가 내려가지만,
코너 반경이 그 이득의 3/4 을 되돌린다. **프론트 영역(보어 3,359 ~ 4,069)에서는
두 변경이 모두 σ 를 올리는 방향**이므로 부록 6 프론트보다 상당히 밀릴 것으로
본다. (기준선은 σ 3,424 로 애초에 제약을 크게 초과하는 참조점일 뿐이다.)

## 8-4. S3-a 배선 시험 (260805)

부록 6 과 같은 방식으로 **MASTA 를 대용식으로 갈아끼우고 `224 × 4`** 를 돌려
배선을 먼저 확인하고(`a8_s3_wiretest.py` · MASTA 0회), 실 MASTA 1점으로 주입을
검증했다(`a8_eval.py` 자기검증).

|  # | 점검                                                     | 결과                                                                                             |
| -: | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| ① | 두께 규칙이 전 격자에서 성립                             | 표본 위반**0건** · 보어 2,701 ~ 4,247 → 두께 **107 ~ 369 mm**(현행 규칙 155 ~ 243) |
| ② | 범주 시딩 커버리지                                       | **224 / 224**                                                                              |
| ③ | `L_w` 0.1 mm 격자 · 세장비 밴드                       | 이탈**0** · 1.5016 ~ 2.4983                                                               |
| ④ | 프론트 CSV 열`L_w_mm`·`L_we_mm`·`2β`            | **모두 포함** · 프론트 6건                                                                |
| ⑤ | pymoo 배선                                               | 4세대 완주 · 예산 896 · 세대 로그·체크포인트 정상                                             |
| ⑥ | **실 MASTA 1점** — 코너 반경 주입·유효 길이 회수 | 코너 4.3/4.3 ·**`L_we` 229.45 = 238.05 − 8.6** · 2β 1.2° · cone 17.8°             |

**판정 통과.** 두께 범위가 현행(155 ~ 243 mm)보다 훨씬 넓어진 것(107 ~ 369)이
이번 최적화의 성격을 보여준다 — 보어가 설계변수처럼 작동하는 폭이 커졌다.

> **대용 평가기의 한계** — σ 는 「얇을수록 오른다」는 방향만 흉내 냈다. 크기는
> 실측이 아니므로 **프론트 위치나 가능해 비율을 예측하지 못한다.** 그 판단은
> S3-b 드라이런(실 MASTA)에서 한다.

## 8-5. S3-b 드라이런 (260805)

`224 × 5 = 1,120` 평가 · 실 MASTA · **5.9분 · 0.320 s/MASTA**.

### 8-5.1. 사전 게이트 — 기댓값을 새 값으로 다시 못박음

첫 시도는 사전 게이트에서 정지했다(exit 4). 부록 6 의 v1.3 기댓값을 그대로 썼기
때문인데, **게이트가 제 역할을 한 것이다** — §8-3 의 변경이 이 기준선에서 바꾸는
값이 정확히 둘이고, 걸린 것도 정확히 그 둘이었다.

| 항목                                       |        실측 | 부록 6 기댓값 |   판정   |
| ------------------------------------------ | ----------: | ------------: | :-------: |
| σ                                         |     3,407.9 |       3,424.2 | ✗ 0.48% |
| 샤프트 질량                                | 58,384.5 kg |   43,225.8 kg | ✗ 35.07% |
| bore · D · T · Z · 베어링질량 · L_eff |          — |            — |  ✓ 6/6  |

**게이트를 푸는 대신 두 항목만 새 값으로 다시 못박았다**(`a8_s3_run.py` 의
`BASE_EXP` 갱신). 나머지 여섯은 부록 6 값 그대로이므로 모델 파일이나 MASTA
설정이 어긋나면 여전히 여기서 걸린다. 재실행에서 8/8 일치.
...
| -: | ------------------- | ------------------------------------------------- |
| ① | 시딩 커버리지       | **224 / 224**                               |
| ② | `L_w` 0.1 mm 격자 | 1,111점 중 이탈**0**                        |
| ③ | 세장비 밴드         | 1.5012 ~ 2.5000 ·**경계집중 0.2%**         |
| ④ | 프론트 환산값       | 3건 · 격자·밴드 이탈 0                          |
| ⑤ | 체크포인트          | 생성 · 재시작 시 캐시 적중으로 MASTA 재호출 없음 |

밴드 재매개화가 의도대로 작동한다 — 경계집중 **0.2%** 로, 폐기한 clip 방식의
51.6 ~ 84.2%(§6-11.2)와 비교가 되지 않는다.

### 8-5.3. 부록 6 드라이런과의 대조 — **핵심**

같은 224 × 5 · 같은 시드로 부록 6 드라이런과 나란히 놓았다.

| 지표                        |            부록 6 |            **부록 8** | 차이                          |
| --------------------------- | ----------------: | --------------------------: | ----------------------------- |
| 가능해 비율                 | 19.9% (222/1,115) | **15.4%** (171/1,112) | −4.5%p                       |
| σ 중앙값                   |         2,314 MPa |             **2,340** | +26                           |
| σ 범위                     |     1,785 ~ 4,126 |               1,830 ~ 4,251 | 상향                          |
| 가능해**베어링** 질량 |   16.35 ~ 39.61 t |   **19.93** ~ 42.93 t | **+3.6 t** (무거워짐)   |
| 가능해**총**질량      | 135.91 ~ 202.45 t | **109.67** ~ 178.29 t | **−26.2 t** (가벼워짐) |

**예측대로 σ 가 올라 가능해 비율이 떨어졌지만(19.9 → 15.4%), 탐색을 막을 수준은
아니다.** 세대수는 150 을 유지한다.

두 질량이 **반대로 움직인 것**이 이번 변경의 성격을 그대로 보여준다.

- **베어링은 무거워진다** — σ 여유가 줄어드니 가능해로 남으려면 전동체를 키워야
  한다. 부록 6 이 16.35 t 짜리 경량안을 허용하던 자리가 사라졌다.
- **총질량은 가벼워진다** — 두께 규칙이 큰 보어에서 벽을 얇게 만들고(교차점
  3,328 mm 위), 가능해는 대부분 그쪽에 몰려 있다. 샤프트에서 얻는 것이 베어링에서
  잃는 것보다 훨씬 크다.

즉 부록 8 의 프론트는 부록 6 프론트를 단순히 「밀어낸」 것이 아니라 **두 목적함수
사이의 교환비 자체가 달라진** 것이다. `f1`(베어링) 쪽 끝은 후퇴하고 `f2`(총질량)
쪽 끝은 크게 전진한다. 드라이런 최경량 총질량이 이미 **116.07 t** 로 나왔다.

> 5세대짜리 드라이런이므로 이 수치는 **프론트가 아니라 출발점**이다. 프론트 3건 ·
> HV 2,814 는 부록 6 본런의 성숙한 프론트와 비교할 대상이 아니다.

## 8-6. S3-c 본 최적화 결과

> **아직 돌리지 않았다.** 이 절은 표 양식을 미리 굳혀 둔 것이고, 표 본문은
> S3-c 종료 후 마커(`<!-- A8:… -->`) 사이에 자동 기입된다.

### 8-6.1. 적용 파라미터 (as-run)

**§6-11.5 의 as-run 표를 그대로 계승한다.** 아래는 부록 8 에서 달라지는 항목과,
실행 후에야 확정되는 항목만 적는다.

| 구분 | 항목                       | 적용값                                                                          | 근거                  |
| ---- | -------------------------- | ------------------------------------------------------------------------------- | --------------------- |
| 문제 | 설계변수`L_w`            | **롤러 전장** · `u` 밴드 재매개화 `L_w = lo + round(u(hi−lo)/10⁴)` | §8-2.1               |
| 문제 | 세장비                     | **1.5 ≤ `L_w`/`D_we` ≤ 2.5** — 전장 기준                           | §8-2.1               |
| 문제 | 종속변수 — 샤프트 내경    | **`floor((OD⁴ − 32·W·OD/π)^¼)`** · `W` = 1.393×10⁹ mm³      | §7-6.7.5 · §8-3 ① |
| 문제 | 종속변수 — 롤러 코너 반경 | **좌·우 각 4.3 mm** → `L_we` = `L_w` − 8.6 mm                      | §8-3 ②              |
| 문제 | 종속변수 — 2β            | MASTA`element_taper_angle` **읽어서 기록**                              | §8-3 ③              |
| 실행 | 사전 게이트                | v1.3 기준선 8/8 일치 (σ 3,407.9 · 샤프트 58,384.5 kg 로 재고정)               | §8-5.1               |
| 실행 | 실측                       | *S3-c 종료 후 기입*                                                           | —                    |

그 밖의 모든 항목 — 개체수 224 · 세대수 150 고정 · 시드 1 · 범주 224 전수 시딩 ·
`RandomSelection` · SBX/UX · PM/`ChoiceRandomMutation` · `RankAndCrowdingSurvival` ·
중복제거 끔 · 지배 LC `Myz_max` 22,673 kNm 듀티사이클 래핑 · v1.4 모델 50 °C ·
`DIN_LUNDBERG` · 무프리로드 — 은 §6-11.5 와 **완전히 같다.**

### 8-6.2. 표에 새로 붙는 네 열

| 열                           | 출처                                                | 비고                                                                 |
| ---------------------------- | --------------------------------------------------- | -------------------------------------------------------------------- |
| **`t`** (축 두께)    | `(d − ID)/2` · 해석식                           | `d` 는 베어링 보어 = 샤프트 외경. 두께 규칙이 실제로 만든 벽두께다 |
| **`2β`** (테이퍼각) | MASTA`element_taper_angle`                        | 본런 중 전량 기록 (§8-3 ③)                                         |
| **UW 미스얼라인먼트**  | MASTA system deflection ·`Relative Misalignment` | **프론트만 후처리**                                            |
| **축 피로 SF**         | DIN 743 무한수명 안전율                             | **프론트만 후처리** · 하중은 **DLC1.2-k-s5 최약 빈**    |

**축 피로 SF 의 하중 기준을 DLC1.2-k-s5 최약 빈으로 잡은 이유** — 두께 규칙
`W` = 1.393×10⁹ mm³ 를 유도한 바로 그 하중이기 때문이다(§7-6.7.5). 목표 5.2 는
그 하중·그 스팬에서 세운 값이므로, 스팬(`z1`·`z2`)과 하중 전달이 달라지는 프론트
설계들에서 **실제로 5.2 가 나오는지, 얼마나 벗어나는지**가 이 열의 핵심 정보다.
극한 LC `Myz_max` 는 실운전 대비 과대하고(§7-6) 두께 규칙의 근거 하중도 아니라
쓰지 않는다.

**두 열을 본런 중에 뽑지 않는 이유** — 미스얼라인먼트는 이미 돌아가는 해석에
들어 있어 공짜지만, 축 SF 는 별도 하중을 걸어야 한다. 3시간 무인 런에 해석을
추가하면 실패 위험만 커지고, 표에 오르는 것은 프론트 20 ~ 40점뿐이다. **부록 7 의
검증된 경로를 그대로 재사용해 사후에 뽑는다**(추가 5 ~ 10분).

### 8-6.3. 결과

> 세장비 열은 **`L_w`/`D_we`** 기준이다(§8-2.1). 표에 찍힌 `L_we` 에서 전장은
> `L_w` = `L_we` + 8.6 mm 로 복원된다.

#### 8-6.3.1 실행 요약

<!-- A8:RUNSUM -->

`224 × 150` **완주**. **33,600 평가 · MASTA 32,935회(98.0%) · 캐시적중
666 · 해석식 탈락 0 · 밴드경계 0 · 167.9분(2.80시간) · 0.306 s/점.** 예산
추정(2.96시간)과 5.4% 차이다.

**가능해 32.5%**(10,710 / 32,935) — 부록 6 의 33.9%(11,253 / 33,162)와 거의
같다. §8-5.3 드라이런에서 본 15.4% 는 초기 집단의 값이었고, 세대가 진행되며
탐색이 가능영역으로 이동해 최종적으로는 차이가 사라졌다.

**프론트는 평가 전량에서 다시 계산했다** — NSGA 최종 집단 기준 12행이지만,
32,935점 전체를 대상으로 하면 `z1 ≥ 1.0` **8건** · `z1 ≥ 1.5` **6건**이다.
최종 집단에 남은 점이 과거 세대의 설계에 지배되기 때문이며(§6-11.5 와 같은
현상), `a8_analyze.py` 가 접는다.

<!-- /A8:RUNSUM -->

**세대별 진행**은 다음과 같다. §6-11.5 와 같은 양식으로, 10세대 간격에 **두
목적의 최솟값이 갱신된 세대(★)** 를 더해 싣는다 — 균등 간격만 실으면 정작
`G` = 150 을 정당화한 사건이 행 사이에 가려진다.

<!-- A8:GENLOG -->

|   세대 | 프론트 |      HV |   ΔHV |  최소 베어링 [t] |   최소 총질량 [t] |
| -----: | -----: | ------: | -----: | ---------------: | ----------------: |
|   1 ★ |      1 | 2,689.3 |     — | **20.121** | **114.712** |
|   2 ★ |      2 | 2,788.2 |  +98.9 |           20.121 | **109.674** |
|   5 ★ |      3 | 2,814.3 |  +26.1 | **19.926** |           109.674 |
|   6 ★ |      4 | 2,850.6 |  +36.3 |           19.926 | **107.751** |
|   7 ★ |      5 | 2,951.0 | +100.4 | **19.168** |           107.751 |
|   8 ★ |      4 | 3,058.2 | +107.3 | **18.441** |           107.751 |
|   9 ★ |      5 | 3,062.3 |   +4.0 |           18.441 | **107.493** |
|     10 |      5 | 3,062.3 |   +0.0 |           18.441 |           107.493 |
|  11 ★ |      4 | 3,176.6 | +114.3 | **17.650** |           107.493 |
|  13 ★ |      4 | 3,214.7 |  +38.1 | **17.403** |           107.493 |
|  18 ★ |      4 | 3,224.6 |   +9.8 |           17.403 | **107.029** |
|     20 |      5 | 3,225.2 |   +0.6 |           17.403 |           107.029 |
|  24 ★ |      5 | 3,232.7 |   +7.5 | **17.351** |           107.029 |
|  26 ★ |      6 | 3,246.1 |  +13.4 | **17.264** |           107.029 |
|  30 ★ |      6 | 3,253.7 |   +7.6 |           17.264 | **106.595** |
|  32 ★ |      7 | 3,263.0 |   +9.3 | **17.199** |           106.595 |
|  33 ★ |      7 | 3,275.8 |  +12.7 | **17.112** |           106.595 |
|     40 |      6 | 3,276.1 |   +0.3 |           17.112 |           106.595 |
|     50 |      8 | 3,277.4 |   +1.3 |           17.112 |           106.595 |
|     60 |      9 | 3,277.7 |   +0.2 |           17.112 |           106.595 |
|  64 ★ |      6 | 3,281.5 |   +3.8 |           17.112 | **106.452** |
|  65 ★ |      7 | 3,288.2 |   +6.7 |           17.112 | **106.032** |
|     70 |      8 | 3,288.2 |   +0.0 |           17.112 |           106.032 |
|  71 ★ |      6 | 3,296.5 |   +8.3 | **17.060** |           106.032 |
|     80 |      7 | 3,296.6 |   +0.0 |           17.060 |           106.032 |
|  89 ★ |     10 | 3,301.4 |   +4.9 | **17.026** |           106.032 |
|     90 |     10 | 3,301.4 |   +0.0 |           17.026 |           106.032 |
|  98 ★ |      7 | 3,305.7 |   +4.2 | **17.003** |           106.032 |
|    100 |      7 | 3,305.7 |   +0.0 |           17.003 |           106.032 |
|    110 |      7 | 3,305.7 |   +0.0 |           17.003 |           106.032 |
|    120 |      8 | 3,305.7 |   +0.0 |           17.003 |           106.032 |
|    130 |      8 | 3,305.7 |   +0.0 |           17.003 |           106.032 |
| 136 ★ |      6 | 3,308.9 |   +3.2 |           17.003 | **105.836** |
|    140 |      7 | 3,309.3 |   +0.5 |           17.003 |           105.836 |
| 150 ★ |     12 | 3,309.8 |   +0.4 | **17.002** |           105.836 |

<!-- /A8:GENLOG -->

★ = 두 목적의 최솟값 중 하나가 갱신된 세대 · `ΔHV` 는 **직전 표시 행** 대비 ·
전체 150세대는 `부록8_NSGA/S3_본최적화/s3_genlog.csv`.

> **HV 기준점은 부록 6 과 같은 `(40, 250)` t 다** — 부록 6 과 숫자를 직접 비교할
> 수 있게 유지했다. 다만 코드는 이 기준점 **안쪽 점만** 남겨 HV 를 계산하므로
> (`nsga_s3_run.py:208`), 베어링 질량이 40 t 를 넘는 프론트 점은 HV 에서 빠진다.
> §8-5.3 드라이런에서 부록 8 의 가능해 베어링 질량이 최대 **42.93 t** 였으므로
> (부록 6 은 39.61 t 로 기준점 안이었다) **프론트 꼬리 일부가 잘릴 수 있다.**
>
> 잘린 점이 실제로 있으면 종료 후 기준점을 넓혀 **캐시 재생(수 분)** 으로 HV 를
> 다시 뽑아 보조열로 병기하고, 클리핑된 점 수를 명기한다. HV 는 **종료 판정에
> 쓰지 않는 사후 진단값**이라(§6-8 · R14 항목 24) 탐색 결과 자체는 기준점과
> 무관하다. 어느 경우든 `f2` 극단은 표의 **최소 총질량** 열로 온전히 읽힌다.

#### 8-6.3a `z1 ≥ 1.0 m` (전지역)

![부록 8 S3 파레토 프론트 z₁ ≥ 1.0 m (전지역)](Results/사이징최적화/figures/pareto_a8_z1_10.png)

> **프론트 점만 그렸다.** 가능해 산점도(10,722건)나 부록 6 프론트를 함께 얹으면 y축이 105 ~ 180 t 로 벌어져, 정작 이번 프론트의 폭 **1.70 t** 가 바닥에 눌려 보이지 않는다. 부록 6 과의 대조는 §8-6.4 의 표가 맡는다. 계단이 네덩이로 갈라지는 것은 `z2`(스팬)가 6.0·5.5·5.0·4.5 m 로 떨어지기 때문이다 — 같은 `z2` 안에서는 점이 거의 겹친다.

**베어링 최경량 10건** (프론트 #1 ~ #10)

<!-- A8:5A_BRG -->

**프론트 전 8건** — 베어링 오름차순(= 총질량 내림차순). 프론트가 8건뿐이라 §6-11.5 처럼 상·하위 10건으로 나누지 않고 전량을 싣는다.

|                                                                                                                                               # | D_pw [mm] | d [mm] | **t** [mm] | D [mm] | T [mm] | B [mm] | C [mm] | α [°] | **2β** [°] | D_we [mm] | L_we [mm] | 세장비 | z1 [m] | z2 [m] | Z [개] | L_eff [m] |      베어링 [t] | 샤프트 [t] | **합계** [t] | σ_max [MPa] | **UW 미스얼라인먼트** [mrad] | **축 피로 SF** |
| ----------------------------------------------------------------------------------------------------------------------------------------------: | --------: | -----: | ---------------: | -----: | -----: | -----: | -----: | ------: | -----------------: | --------: | --------: | -----: | -----: | -----: | -----: | --------: | --------------: | ---------: | -----------------: | -----------: | ---------------------------------: | -------------------: |
|                                                                                                                                               1 |     4,495 |  4,079 |  **116.5** |  4,900 |    472 |    457 |    385 |      15 |    **1.176** |     185.4 |     353.8 |  1.955 |    1.0 |    6.0 |     70 |     6.180 | **17.00** |       73.5 |   **107.53** |      2,099.3 |                              1.092 |       **5.24** |
|                                                                                                                                               2 |     4,494 |  4,082 |  **116.0** |  4,896 |    475 |    459 |    387 |      15 |    **1.145** |     180.2 |     355.8 |  2.022 |    1.0 |    6.0 |     72 |     6.181 | **17.00** |       73.3 |   **107.28** |      2,099.4 |                              1.090 |       **5.22** |
|                                                                                                                                               3 |     4,492 |  4,048 |  **118.5** |  4,925 |    490 |    474 |    400 |      20 |    **1.494** |     177.6 |     367.9 |  2.120 |    1.0 |    5.5 |     73 |     6.112 | **19.16** |       68.5 |   **106.78** |      2,099.8 |                              0.986 |       **5.23** |
|                                                                                                                                               4 |     4,499 |  4,058 |  **118.0** |  4,929 |    493 |    477 |    403 |      19 |    **1.421** |     177.8 |     370.2 |  2.131 |    1.0 |    5.5 |     73 |     6.026 | **19.16** |       68.4 |   **106.67** |      2,099.4 |                              0.999 |       **5.24** |
|                                                                                                                                               5 |     4,500 |  4,059 |  **117.5** |  4,930 |    494 |    478 |    403 |      19 |    **1.421** |     177.8 |     370.6 |  2.133 |    1.0 |    5.5 |     73 |     6.026 | **19.19** |       68.1 |   **106.47** |      2,099.2 |                              1.002 |       **5.22** |
|                                                                                                                                               6 |     4,499 |  4,007 |  **121.0** |  4,981 |    495 |    479 |    404 |      27 |    **2.123** |     190.6 |     371.8 |  1.996 |    1.0 |    5.0 |     68 |     6.271 | **21.51** |       63.4 |   **106.39** |      2,099.2 |                              0.878 |       **5.22** |
|                                                                                                                                               7 |     4,499 |  4,022 |  **120.0** |  4,965 |    504 |    488 |    412 |      27 |    **1.861** |     166.3 |     378.8 |  2.329 |    1.0 |    5.0 |     78 |     6.270 | **21.59** |       63.1 |   **106.29** |      2,099.6 |                              0.853 |       **5.22** |
|                                                                                                                                               8 |     4,499 |  3,991 |  **122.5** |  4,995 |    526 |    509 |    429 |      28 |    **2.081** |     180.2 |     395.4 |  2.242 |    1.0 |    4.5 |     72 |     5.871 | **23.89** |       58.1 |   **105.84** |      2,098.9 |                              0.819 |       **5.24** |
| *UW 미스얼라인먼트는 지배 LC `Myz_max` 기준 · 축 피로 SF 는 `DLC1.2-k-s5` 최약 빈(#11) 대표하중 기준 DIN 743 무한수명 안전율 (§8-6.2).* |           |        |                  |        |        |        |        |         |                    |           |           |        |        |        |        |           |                 |            |                    |              |                                    |                      |

<!-- /A8:5A_BRG -->

##### 후처리 결과 — 두께 규칙이 실제로 5.2 를 만든다

**축 피로 SF 는 8건 전부 5.221 ~ 5.240 이다.** 목표 5.2 를 전부 넘고, 폭은
0.019(0.4%)에 불과하다. 하한을 넘는 방향으로만 벗어난 것은 `floor` 때문이다 —
내경을 깎아 내리므로 `W` 가 목표보다 조금 커진다(§6-4.2 와 같은 안전측 정수화).

이 8건은 `z2` 가 4.5 ~ 6.0 m 로 **스팬이 33% 차이나고** 보어도 3,991 ~ 4,079 mm
로 다르지만 SF 는 사실상 같다. §7-6.7.5 가 `SF = 3.7478 · (W/10⁹)` 를 R² = 1.000000
으로 얻은 이유가 여기서 확인된다 — DIN 743 이 보는 임계 단면의 굽힘모멘트는
허브에서 들어오는 하중이 정하므로 **스팬·베어링 배치와 무관**하고, 따라서 SF 는
단면계수 `W` 하나로 결정된다. 두께 규칙을 종속변수로 넣은 것이 옳았다.

**UW 미스얼라인먼트는 0.819 ~ 1.092 mrad** 이고, 보어가 큰 설계일수록 크다
(#1 d 4,079 → 1.092 · #8 d 3,991 → 0.819). §7-6.7.6 에서 미스얼라인먼트 증가가
σ 상승을 +0.995 상관으로 설명했던 것과 같은 축이다. 여기서는 σ 가 전부 2,099
근처(제약 경계)이므로, **보어를 키워 얻은 여유를 미스얼라인먼트가 그대로
잡아먹는 관계**로 읽힌다.

> **σ 재현 오차는 0.0 MPa 다** — 후처리에서 본런 8건의 σ 를 소수 첫째 자리까지
> 그대로 재현했다. 코너 반경·두께 규칙·기하 주입이 본런과 동일하게 배선돼
> 있음을 확인한 것이다.

*`z1 ≥ 1.5` 프론트 6건(§8-6.3b)은 후처리하지 않았다 — §8-6.4.2 의 `D_pw` 상한
재설정이 정해지면 프론트가 통째로 바뀌므로, 그 뒤에 한 번에 돌린다.*

**총질량 최경량 10건** (프론트 꼬리) — 아래로 갈수록 총질량이 가벼워진다

<!-- A8:5A_TOT -->

*위 표가 프론트 전량이므로 총질량 최경량 표는 따로 두지 않는다 — 표의 마지막 행이 총질량 최경량이다.*

<!-- /A8:5A_TOT -->

*프론트 전체는 `부록8_NSGA/S3_본최적화/s3_pareto.csv`*

#### 8-6.3b `z1 ≥ 1.5 m`

![부록 8 S3 파레토 프론트 z₁ ≥ 1.5 m](Results/사이징최적화/figures/pareto_a8_z1_15.png)

> **프론트 점만 그렸다.** 가능해 산점도(10,722건)나 부록 6 프론트를 함께 얹으면 y축이 105 ~ 180 t 로 벌어져, 정작 이번 프론트의 폭 **0.80 t** 가 바닥에 눌려 보이지 않는다. 부록 6 과의 대조는 §8-6.4 의 표가 맡는다. 계단이 두덩이로 갈라지는 것은 `z2`(스팬)가 6.0·5.5 m 로 떨어지기 때문이다 — 같은 `z2` 안에서는 점이 거의 겹친다.

**베어링 최경량 10건** (프론트 #1 ~ #10)

<!-- A8:5B_BRG -->

**프론트 전 6건** — 베어링 오름차순(= 총질량 내림차순). 프론트가 6건뿐이라 §6-11.5 처럼 상·하위 10건으로 나누지 않고 전량을 싣는다.

|                                                                                                                                               # | D_pw [mm] | d [mm] | **t** [mm] | D [mm] | T [mm] | B [mm] | C [mm] | α [°] | **2β** [°] | D_we [mm] | L_we [mm] | 세장비 | z1 [m] | z2 [m] | Z [개] | L_eff [m] |      베어링 [t] | 샤프트 [t] | **합계** [t] | σ_max [MPa] | **UW 미스얼라인먼트** [mrad] | **축 피로 SF** |
| ----------------------------------------------------------------------------------------------------------------------------------------------: | --------: | -----: | ---------------: | -----: | -----: | -----: | -----: | ------: | -----------------: | --------: | --------: | -----: | -----: | -----: | -----: | --------: | --------------: | ---------: | -----------------: | -----------: | ---------------------------------: | -------------------: |
|                                                                                                                                               1 |     4,499 |  4,036 |  **119.5** |  4,951 |    492 |    476 |    401 |      20 |    **1.648** |     197.0 |     369.0 |  1.917 |    1.5 |    6.0 |     66 |     6.115 | **19.88** |       74.5 |   **114.31** |      2,096.9 |                              1.011 |       **5.24** |
|                                                                                                                                               2 |     4,497 |  4,040 |  **119.0** |  4,943 |    502 |    485 |    409 |      18 |    **1.482** |     196.1 |     376.6 |  1.964 |    1.5 |    6.0 |     66 |     5.939 | **19.97** |       74.3 |   **114.27** |      2,098.8 |                              1.042 |       **5.23** |
|                                                                                                                                               3 |     4,495 |  4,037 |  **119.0** |  4,942 |    501 |    485 |    409 |      18 |    **1.488** |     196.8 |     376.2 |  1.955 |    1.5 |    6.0 |     66 |     5.938 | **20.00** |       74.3 |   **114.27** |      2,096.7 |                              1.043 |       **5.22** |
|                                                                                                                                               4 |     4,495 |  4,055 |  **118.0** |  4,924 |    512 |    495 |    418 |      19 |    **1.333** |     166.3 |     384.5 |  2.364 |    1.5 |    6.0 |     78 |     6.025 | **20.03** |       74.0 |   **114.05** |      2,097.9 |                              0.996 |       **5.23** |
|                                                                                                                                               5 |     4,499 |  4,013 |  **121.0** |  4,974 |    511 |    494 |    417 |      26 |    **1.917** |     177.8 |     383.6 |  2.206 |    1.5 |    5.5 |     73 |     6.173 | **22.14** |       69.2 |   **113.53** |      2,099.0 |                              0.882 |       **5.24** |
|                                                                                                                                               6 |     4,496 |  4,007 |  **121.0** |  4,974 |    507 |    491 |    414 |      28 |    **2.004** |     173.2 |     380.9 |  2.249 |    1.5 |    5.5 |     75 |     6.369 | **22.19** |       69.1 |   **113.51** |      2,099.3 |                              0.856 |       **5.22** |
| *UW 미스얼라인먼트는 지배 LC `Myz_max` 기준 · 축 피로 SF 는 `DLC1.2-k-s5` 최약 빈(#11) 대표하중 기준 DIN 743 무한수명 안전율 (§8-6.2).* |           |        |                  |        |        |        |        |         |                    |           |           |        |        |        |        |           |                 |            |                    |              |                                    |                      |

<!-- /A8:5B_BRG -->

##### 후처리 결과 — `z1 ≥ 1.5` 도 같은 값이다

**축 피로 SF 5.221 ~ 5.241** · **UW 미스얼라인먼트 0.856 ~ 1.043 mrad** ·
**σ 재현 오차 0.0 MPa.** `z1 ≥ 1.0` 갈래(5.221 ~ 5.240)와 사실상 같은 범위다.

`z1` 을 1.0 → 1.5 m 로 올려 UW 를 뒤로 물려도 SF 가 움직이지 않는 것이,
§8-6.3a 에서 말한 **「DIN 743 임계 단면은 허브 하중이 정하므로 베어링 배치와
무관하다」**를 다시 보여준다. 반면 총질량은 105.84 → **113.51 t** 로 7.7 t
무거워진다 — `z1` 제한의 대가는 전부 질량 쪽으로 나온다.
...
같은 알고리즘·같은 파라미터·같은 시드에서 §8-3 의 종속변수 셋만 바꾼 결과다.
`z1 ≥ 1.0` 프론트끼리 비교한다.

| 지표                      |           부록 6 |           **부록 8** | 차이                      |
| ------------------------- | ---------------: | -------------------------: | ------------------------- |
| 평가 · MASTA             | 33,600 · 33,162 |           33,600 · 32,935 | —                        |
| 가능해 비율               |            33.9% |            **32.5%** | −1.4%p                   |
| 프론트 점수 (전량 기준)   |               64 |                **8** | **1/8**             |
| 최소**베어링** 질량 |         13.423 t |         **17.002 t** | **+26.7%**          |
| 최소**총**질량      |        133.472 t |        **105.836 t** | **−20.7%**         |
| 총질량 범위(프론트)       |  133.5 ~ 169.5 t |  **105.8 ~ 107.5 t** | 폭 36.0 →**1.7 t** |
| HV`(40, 250)`           |          2,929.5 |          **3,309.8** | +13.0%                    |
| 샤프트 두께 (프론트)      |     216 ~ 233 mm | **116.5 ~ 122.5 mm** | 약 절반                   |

**§8-5.3 이 드라이런에서 본 방향이 그대로 확인됐다** — 베어링은 무거워지고
(+26.7%) 총질량은 크게 가벼워졌다(−20.7%). HV 는 기준점 `(40, 250)` 안에서
13.0% 높다. 프론트 점 중 베어링 40 t 초과는 **0건**이라 클리핑은 없었고,
기준점을 넓혀 다시 뽑을 필요도 없었다.

#### 8-6.4.1. 프론트가 1/8 로 줄었다 — 교환이 사라졌다

프론트가 64건에서 **8건**으로 줄었는데, 원인은 점의 개수가 아니라 **총질량의
폭**이다. 부록 6 프론트는 총질량 133.5 ~ 169.5 t (폭 36.0 t)에 걸쳐 있었지만,
부록 8 프론트는 **105.8 ~ 107.5 t (폭 1.7 t)** 에 뭉쳐 있다.

베어링을 17.0 → 23.9 t 로 **6.9 t** 키우면 총질량에는 두 배인 13.8 t 가 더해지고,
그 대가로 샤프트가 73.5 → 58.1 t 로 **15.5 t** 줄어든다. 순이득은 1.7 t 뿐이다.
**두 목적 사이의 교환이 거의 상쇄된다** — 이것이 프론트가 짧아진 이유다.

이 상쇄는 두께 규칙 때문이다. 부록 6 에서 샤프트 질량은 `OD²−ID²` ∝ `OD²`
(고정 비율 0.88543) 로 **외경의 제곱**을 따라갔지만, 두께 규칙에서는 단면계수
`W` 가 고정이라 `OD⁴−ID⁴ ∝ OD` 이고 단면적은 대략 `OD^−2` 로 **작아진다**.
베어링을 키워 보어를 늘리면 벽이 얇아져 샤프트가 그만큼 가벼워지므로, 원래
목적함수가 충돌하던 지점이 사라진 것이다.

#### 8-6.4.2. `D_pw` 제약 `C6` 가 활성이다

프론트 8건 전부의 `D_pw` 가 **4,492 ~ 4,500 mm** 로 4,500 에 붙어 있다.
가능해 전체로 넓혀도 마찬가지다.

|                            |         부록 6 |       **부록 8** |
| -------------------------- | -------------: | ---------------------: |
| 가능해 중`D_pw ≥ 4,490` | **1.8%** |        **61.6%** |
| 프론트 중`D_pw ≥ 4,490` |           일부 | **8 / 8 (100%)** |
| 가능해`D_pw` 범위        |  3,719 ~ 4,500 |          3,615 ~ 4,500 |

**부록 6 에서는 이 한계가 사실상 놀고 있었지만(1.8%), 부록 8 에서는 그것이 답을
결정한다.** 두께 규칙이 큰 보어를 보상하므로 최적화가 `D_pw` 를 끝까지 밀어
올렸고, 4,500 에서 막힌 것이다.

> **4,500 은 탐색 범위가 아니라 설계제약이다 — `(C6) D_pw ≤ 4,500 mm · 제작·운송 상한**(§3 · `sizing_geom.py:79`). 임의로 잡은 격자 상한이 아니라 처음부터 명시된 공학적 한계이고, `sizing_geom.constraints` 안에서 평가 전에
> 걸러진다. 따라서 이 8건은 **「상자에 잘린 해」가 아니라 「제약이 활성화된
> 해」**로 읽어야 한다 — 최적화가 정상적으로 도달한 결론이다.
>
> 다만 **`C6` 가 지금 설계를 지배하고 있다는 사실 자체가 결과**다. 부록 6 에서는
> 이 제약이 거의 놀았으므로 그 대가를 물을 이유가 없었지만, 부록 8 에서는
> 「제작·운송 한계를 얼마나 완화하면 얼마나 가벼워지는가」가 실질적인 질문이
> 된다. §8-8 이 그 대가를 잰다.

`z2` 에서도 같은 일이 반대 방향으로 일어났다. 부록 6 프론트는 `z2` 를 3.0 m 까지
썼지만(#63·#64), 부록 8 프론트의 최소는 **4.5 m** 다. 벽이 절반으로 얇아져
스팬을 줄이면 σ 2,100 을 못 지키기 때문이다 — 짧은 스팬은 베어링 하중을 키운다.

> **260806 정정** — 이 절은 원래 「설계 상자가 구속한다」·「4,500 의 근거가
> 재검토된 적 없다」고 적혀 있었다. **틀린 서술이었다.** 근거(`C6` 제작·운송
> 상한)는 §3 에 처음부터 있었고, §8-1.7 의 미결 3 이 이미 「`C6` 를 완화하면 더
> 나은 해가 있을 수 있다」고 짚어 두었다. 문제의 성격은 「최적화가 인공적으로
> 잘렸다」가 아니라 「**제작 제약이 설계를 지배한다**」이다.

### 8-6.5. 프론트 8건 분석 — 무엇이 총질량을 줄였고, 왜 8점뿐인가

§8-6.3a 의 프론트는 `D_pw` 가 4,492 ~ 4,500 mm 로 사실상 한 점인데 `d` 는
4,079 → 3,991 mm 로 88 mm 움직인다. 이 차이가 무엇이고 무엇을 바꾸는지, 그리고
프론트가 8점뿐인 것이 무엇을 뜻하는지를 실측으로 정리한다.

#### 8-6.5.1. 총질량을 줄인 것은 `d` 가 아니라 스팬이다

`#1` 과 `#8` 의 샤프트는 74.00 → 58.43 t 로 **15.57 t** 줄었다.
`m = ρ·(π/4)(d² − ID²)·(z2 + 0.5)` 를 단면과 길이로 나누면 이렇다.

| 요인 | 변화                                            |                기여 |
| ---- | ----------------------------------------------- | ------------------: |
| 길이 | `z2` 6.0 → 4.5 m (L 6.50 → 5.00 m)          | **−17.08 t** |
| 단면 | `d` 4,079 → 3,991 · `t` 116.5 → 122.5 mm |   **+1.97 t** |
| 합   |                                                 | **−15.57 t** |

**`d` 가 작아진 것은 질량을 줄이지 않았다. 오히려 늘렸다.** 두께 규칙은 `W` 를
고정하므로 `d` 가 줄면 벽이 두꺼워져야 하고(116.5 → 122.5 mm), 단면적은
1.4503 → **1.4888 m²** 로 **커진다**. 감소분은 전부 스팬에서 나왔다.

> 이것은 §8-6.4.1 에서 본 「교환이 상쇄된다」와 같은 이야기다. 두께 규칙 아래서
> 보어를 줄여 얻는 것은 없다 — 단면적이 대략 `OD⁻²` 로 거동하기 때문이다.

#### 8-6.5.2. `d` 는 설계 레버가 아니라 결과다

`d` 는 자유변수가 아니라 종속변수다(§6-4).

$$
d = D_{pw} - D_{we}\cos\alpha - L_w\sin\alpha - 2t_i
$$

`D_pw` 가 상한에 붙어 고정된 상태에서 `d` 를 움직이는 것은 **α 와 `L_w`** 뿐이다.
프론트 8점 전부가 이 식과 1 mm 이내로 맞는다.

| # |  D_pw | α |  D_we |   L_w | `D_we·cosα` | `L_w·sinα` | `2t_i` |     **d** |
| -: | ----: | -: | ----: | ----: | --------------: | -------------: | -------: | --------------: |
| 1 | 4,495 | 15 | 185.4 | 362.4 |           179.1 |           93.8 |    143.1 | **4,079** |
| 3 | 4,492 | 20 | 177.6 | 376.5 |           166.9 |          128.8 |    148.4 | **4,048** |
| 6 | 4,499 | 27 | 190.6 | 380.4 |           169.8 |          172.7 |    149.9 | **4,007** |
| 8 | 4,499 | 28 | 180.2 | 404.0 |           159.1 |          189.7 |    159.3 | **3,991** |

프론트 8점에서의 상관이 이를 그대로 확인한다.

| 상관              |                값 | 읽는 법                                                     |
| ----------------- | ----------------: | ----------------------------------------------------------- |
| `d` ↔ α       | **−0.978** | α 가 커지면`L_w·sinα` 항이 커져 `d` 가 밀려 내려간다 |
| `d` ↔ `L_w`  | **−0.889** | 같은 항의 다른 인자                                         |
| `d` ↔ `D_we` |            +0.011 | **무관하다** — `cosα` 항의 변화가 작다            |
| `d` ↔ `z2`   |            +0.980 | 둘 다 같은 움직임의 결과다(아래)                            |

**따라서 프론트를 따라가며 실제로 일어난 일은 이것이다.** 스팬을 줄이면
베어링 하중이 커져 σ 가 한도를 넘는다. 그것을 되받으려면 **접촉각을 세우고
롤러를 길게** 해 용량을 키워야 한다(α 15 → 28° · `L_w` 362 → 404 mm). 그 결과
베어링이 무거워지고(17.0 → 23.9 t) `d` 가 따라 내려간다. **`d` 의 88 mm 는
스팬 단축의 부산물이지 설계 의도가 아니다.**

#### 8-6.5.3. 프론트가 8점뿐인 이유 — 교환 차원이 `z2` 하나다

**같은 `z2` 안에서는 두 목적이 충돌하지 않는다.** 가능해 10,710점을 `z2` 별로
나눠 `f1`(베어링) ↔ `f2`(총질량) 상관을 보면 전부 **+0.96 이상**이다.

|    `z2` [m] | 가능해 |  총질량 최소 [t] | (그때 베어링) | 프론트 점수 | `f1`↔`f2` 상관 |
| ------------: | -----: | ---------------: | ------------: | ----------: | ------------------: |
|           3.0 |      2 |           127.17 |         41.43 |           0 |              +1.000 |
|           3.5 |     11 |           107.29 |         30.05 |           0 |              +0.992 |
|           4.0 |     49 |           107.22 |         27.08 |           0 |              +0.981 |
| **4.5** |    456 | **105.84** |         23.89 |           1 |              +0.985 |
|           5.0 |  1,623 |           106.29 |         21.59 |           2 |              +0.975 |
|           5.5 |  3,817 |           106.47 |         19.19 |           3 |              +0.976 |
|           6.0 |  4,752 |           107.28 |         17.00 |           2 |              +0.960 |
|          전체 | 10,710 |               — |            — |           8 |              +0.739 |

스팬이 고정되면 베어링을 키우는 것이 총질량도 키운다 — 충돌이 없으니 그
슬라이스는 프론트에 **사실상 한 점**만 낸다. 실제로 프론트 8점은 `z2` 네
수준(4.5·5.0·5.5·6.0)에 1·2·3·2점으로 흩어져 있고, §8-6.3a 그림의 계단 네
덩이가 바로 이것이다.

**즉 프론트 점수의 상한을 정하는 것은 `z2` 의 격자다.** `z2` 는 `Choice` 로
0.5 m 간격 7수준이고, 그중 실제로 쓸 수 있는 것은 4수준뿐이다(3.0 ~ 4.0 은
가능해가 62점밖에 없고 전부 지배된다). **8점은 알고리즘이 놓친 것이 아니라
격자가 허용한 최대치에 가깝다.**

> 총질량 최소가 `z2` = 4.5 에서 나오는 것도 눈여겨볼 만하다 — 양쪽이 다 무거운
> **내부 최적**이다. 스팬을 더 줄이면 베어링이 급격히 무거워지고(4.0 에서
> 27.08 t) 늘리면 샤프트가 무거워진다. 0.5 m 격자로는 이 골짜기의 바닥을
> 4.5 라는 한 점으로밖에 못 짚는다.

#### 8-6.5.4. 수렴은 미완이다

세대 로그를 보면 **HV 가 마지막 세대까지 계속 오른다.**

| 구간       |             ΔHV | Δ최소 베어링 |        Δ최소 총질량 |
| ---------- | ---------------: | ------------: | -------------------: |
| 100 → 150 | **+4.097** |    −0.0013 t | **−0.1961 t** |
| 140 → 150 |           +0.412 |      −0.0013 |               0.0000 |
| 149 → 150 | **+0.185** |      −0.0013 |               0.0000 |

부록 6 은 「HV 가 0.5 이상 오른 마지막 세대가 97」이라는 근거로 `G` = 150 을
정당화했다(§6-11.5). **부록 8 에는 그 근거가 없다** — 150세대에서도 증분이
0.185 이고, 최솟값 갱신(★)이 136·150세대에 남아 있다. `G` = 150 은 부록 6 에서
가져온 값이고 부록 8 의 지형에서 검증된 적이 없다.

#### 8-6.5.5. 종합 — 「최적화가 엄밀했는가」

**두 가지를 나눠 봐야 한다.**

| 항목                              |             판정             | 근거                                                                                                                                |
| --------------------------------- | :---------------------------: | ----------------------------------------------------------------------------------------------------------------------------------- |
| 프론트 점이 8개뿐이다             |      **문제 아님**      | `z2` 격자가 만든 상한이다(§8-6.5.3). 알고리즘은 각 `z2` 수준에서 할 일을 했다                                                  |
| `D_pw` 가 상한에 붙었다         | **정상 · 다만 지배적** | `C6`(제작·운송 상한 · §3)이 활성이다. 인공적 절단이 아니라 공학 제약이 답을 정하는 상태다(§8-6.4.2) — 대가는 §8-8 에서 잰다 |
| HV 가 끝까지 오른다               |       **문제다**       | 종료 근거가 없다(§8-6.5.4)                                                                                                         |
| 두 목적이`z2` 안에서 상관 +0.98 |      **설계 문제**      | `f1`·`f2` 가 사실상 한 축이라 프론트가 얇다                                                                                    |

**결론 — 이 8점은 「두께 규칙 아래의 파레토 프론트」로 쓰기에 부족하다.**
방향(총질량 −20.7% · 베어링 +26.7%)과 기전(§8-6.4.1 의 교환 상쇄)은 신뢰할 수
있으나, **프론트의 해상도는 `z2` 격자가, 위치는 `C6` 와 종료조건이 정한 것**이다.
셋 중 `C6` 만이 공학적 근거를 가진 제약이고, 나머지 둘은 방법론의 선택이다.

#### 8-6.5.6. 제안 — `D_pw` 상한을 유지한 채 프론트를 늘리는 법

`C6`(`D_pw` ≤ 4,500)를 그대로 두고도 다음 셋으로 프론트를 늘릴 수 있다. 셋 다
§8-6.5.3·8-6.5.4 의 실측에서 곧바로 나온다.

**⑴ `z2` 격자를 `4.0 ~ 6.0` · `0.1 m` 로 (21수준) — 가장 큰 효과**

교환 차원이 `z2` 하나이므로 프론트 점수는 **거의 `z2` 수준 수에 비례**한다.
쓸모없는 `z2` ≤ 3.5(가능해 13점)를 버리고 쓸모 있는 구간만 5배 촘촘히 한다.
프론트가 대략 4 ~ 5배로 늘 것으로 본다. 특히 총질량 최소가 나온 `z2` = 4.5
부근의 골짜기를 0.1 m 로 짚을 수 있다.

> **대가**: 범주 조합이 `2 × 7 × 16 = 224` 에서 `2 × 21 × 16 = 672` 로 는다.
> 범주 전수 시딩(§6-8.1.2)을 유지하려면 **개체수를 672 로 올려야** 하고, 그러면
> 세대당 비용이 3배가 된다. 개체수 672 × 세대 150 이면 MASTA 약 10만회 ≈ 8.6시간이다.

**⑵ 세대수를 늘리고 정체를 기록한다**

`G` = 150 은 부록 6 에서 가져온 값이고 부록 8 에서 검증되지 않았다.
**`G` = 300 으로 늘리되 종료는 여전히 세대수 고정**으로 하고(조기 종료는
§6-11.5 가 97세대 갱신을 놓칠 뻔한 사례로 배제했다), 세대별 HV 증분을 기록해
「어디서 멈춰도 됐는가」를 사후에 판정한다.

**⑶ 시드를 늘려 프론트를 합친다**

시드 1개는 부록 6 부터 남은 한계다(미결 #9). 시드 3개를 돌려 **평가 전량을
합쳐 비지배 판정**하면, 같은 예산에서 각 시드가 다른 `z2` 수준을 잘 훑을 수
있어 프론트가 두꺼워진다. 시드 간 재현성도 함께 답이 나온다.

> **⑷ 는 권하지 않는다** — `z2` 를 `Choice` 대신 `Integer` 격자로 바꿔
> 연속화하면 가장 촘촘하지만, 범주 전수 시딩의 근거가 무너지고 설계변수 구조가
> 부록 6 과 달라져 계승 관계가 끊긴다. 프론트 해상도를 위해 방법론의 연속성을
> 버릴 만큼의 이득은 아니다.

**우선순위는 ⑴ > ⑵ > ⑶ 이다.** ⑴ 만으로 프론트 점수 문제는 대부분 해소되고,
⑵ 는 예산 문제이며, ⑶ 은 신뢰도 문제다. 다만 **§8-8 의 `C6` 완화 대가가 나온
뒤에 함께 결정하는 것이 낫다** — 둘 다 재최적화를 요구하므로 따로 돌리면 같은
시간을 두 번 쓴다.

#### 8-6.5.7. 목적함수 설정 검토 — 두 목적이 정말 상충하는가

§8-6.3 과 §8-8 이 같은 현상을 두 번 보여줬다. **`D_pw` 를 키우면 베어링도
샤프트도 함께 가벼워지므로**(§8-8.5.2) 최적화가 상한까지 밀고, 프론트는
`z2` 격자가 허용하는 몇 점으로 주저앉는다(§8-6.5.3 · §8-8.5.4). 여기서 자연스러운
의심이 나온다 — **두 목적이 상충하지 않는다면 다목적으로 풀 이유가 있는가?**

기존 평가 자료를 그대로 쓴다. 추가 해석은 0회이고, 가능해 **17,714점**
(부록 8 S3-c 10,710 + §8-8 7,004)을 네 가지 정식화로 **다시 판정**했다.

##### ⑴ 구조적 문제 — 한 목적이 다른 목적을 포함한다

현행 목적은 `f1` = 베어링 1개 질량, `f2` = 총질량이고, 정의상

$$
f_2 = 2 f_1 + m_{shaft}
$$

**`f2` 안에 `f1` 이 들어 있다.** 두 축이 독립이 아니므로 `f1` 을 줄이면 `f2` 도
자동으로 2 t 씩 따라 내려가고, 지배 판정이 그만큼 뭉개진다. 실측 상관이
**+0.739**(부록 8) · **+0.757**(§8-8)로 높게 나오는 것은 물리적 상충이 약해서가
아니라 **정의상 겹쳐 있어서**다.

정작 물리적으로 독립된 두 양은 **베어링 질량과 샤프트 질량**인데, 이 둘의
상관은 **−0.215**(부록 8) · **+0.131**(§8-8)로 사실상 무상관이다.

##### ⑵ 같은 자료를 네 가지로 다시 판정

| 정식화                               |           부록 8 S3-c |   §8-8 (`C6` 해제) |
| ------------------------------------ | --------------------: | --------------------: |
| ⒜**(베어링, 총질량)** — 현행 |   프론트**8건** |   프론트**9건** |
| ⒝**(베어링, 샤프트)**         |  프론트**27건** |  프론트**22건** |
| ⒞ 총질량 단일                       |       1점 · 105.84 t |        1점 · 76.42 t |
| ⒟ 베어링 단일                       | 1점 · 베어링 17.00 t | 1점 · 베어링 12.74 t |

**목적함수만 바꿔 다시 판정했을 뿐인데 프론트가 3배 가까이 늘어난다**(8 → 27 ·
9 → 22). 해상도 부족의 상당 부분은 탐색이 아니라 **정식화**에서 왔다.

##### ⑶ 베어링 단독 최적화로는 부족하다

`f1` 만 최소화하면 어떻게 되는지 실측으로 답이 나온다.

| 기준                         |      베어링 [t] | 샤프트 [t] |     **총질량 [t]** |
| ---------------------------- | --------------: | ---------: | -----------------------: |
| **부록 8** 총질량 최적 |           23.89 |      58.06 |         **105.84** |
| **부록 8** 베어링 최적 | **17.00** |      73.53 | 107.53 (**+1.6%**) |
| **§8-8** 총질량 최적  |           18.32 |      39.78 |          **76.42** |
| **§8-8** 베어링 최적  | **12.74** |      56.81 |  82.30 (**+7.7%**) |

**베어링을 5.6 t 깎으면 샤프트가 17.0 t 늘어난다.** 베어링 최경량 10건·50건의
평균을 봐도 같다(총질량 82.32 · 82.97 t). 즉 **두 목적은 실재로 상충하며,
베어링 단독 최적화는 총질량을 7.7% 악화시킨다.** 단일목적으로 줄이려면 그
대상은 베어링이 아니라 총질량이어야 한다.

##### ⑷ 상충을 만드는 것은 베어링 제원이 아니라 스팬이다

`z2` 를 고정하면 베어링과 샤프트의 상관이 **양(+0.55 ~ +0.68)** 으로 뒤집힌다.
...
**같은 스팬 안에서는 베어링을 키우면 샤프트도 무거워진다** — 상충이 없다.
전체에서 상관이 음으로 돌아서는 것은 `z2` 를 가로지를 때뿐이다(스팬을 줄이면
샤프트가 가벼워지고 베어링 하중이 커진다). §8-8.5.4 가 「`z2` 만이 두 목적을
갈라 놓는 변수」라고 한 것과 정확히 같은 결론이며, **상충의 원천이 설계변수
하나에 몰려 있다**는 뜻이다.

##### ⑸ 권고 — `(베어링, 샤프트)` 로 풀고, 프론트에서 **총질량으로 고른다**

정식화를 바꾸되 **총질량을 버리지는 않는다.** 최적화 단계와 선택 단계를 나눈다.

| 단계                | 내용                                                               |
| ------------------- | ------------------------------------------------------------------ |
| **① 최적화** | 목적`g1` = 베어링 1개 질량 ↓ · `g2` = 샤프트 질량 ↓         |
| **② 선택**   | 프론트 위에서**총질량 `2·g1 + g2` 가 최소인 점**을 고른다 |

이 방식이 성립하는 근거는 **가중합 정리**다. 총질량은 두 목적의 **양의
가중합**(`2·g1 + 1·g2`)이므로, 그 최소점은 **반드시 `(g1, g2)` 파레토 프론트
위에 있다.** 실측으로 확인했다.

| 확인 항목                                                                         |            부록 8 S3-c |                  §8-8 |
| --------------------------------------------------------------------------------- | ---------------------: | ---------------------: |
| 총질량 최소점이`(베어링, 샤프트)` 프론트에 있는가                               |           **예** |           **예** |
| 현행 프론트 ⊂ 새 프론트                                                          | **8 / 8 (100%)** | **9 / 9 (100%)** |
| 가중합`w·베어링 + 샤프트` 최적(`w` 0.2 ~ 10 · 50개)이 **새** 프론트에 |      **50 / 50** |      **50 / 50** |
| 같은 것이**현행** 프론트에                                                  |                42 / 50 |                41 / 50 |

읽을 것이 셋이다.

- **잃는 것이 없다.** 현행 프론트 전체가 새 프론트에 그대로 들어 있다(100%).
  정식화를 바꿔도 지금까지의 해가 사라지지 않는다.
- **얻는 것이 있다.** 새 프론트는 어떤 양의 가중치를 쓰든 그 최적을 담지만
  (50/50), 현행 프론트는 **8 ~ 9개를 놓친다**(42·41/50). 베어링과 샤프트의
  상대 중요도가 1:2 가 아닌 경우 — 제작비·조달 리드타임·운송이 다르게 걸리는
  경우 — 현행 정식화는 그 답을 아예 만들어 내지 못한다.
- **총질량은 사후에 정확히 복원된다.** `2·g1 + g2` 이므로 근사가 아니다.

##### ⑹ 그래도 총질량 검토는 따로 필요하다

「프론트에서 총질량 최소를 고르면 된다」로 끝나지 않는다. **새 프론트는 총질량
기준으로 매우 넓게 퍼져 있다.**

|                                    | 프론트 위 총질량 범위       |               폭 |
| ---------------------------------- | --------------------------- | ---------------: |
| 부록 8 — 현행`(베어링, 총질량)` | 105.84 ~ 107.53 t           |            1.7 t |
| 부록 8 — 새`(베어링, 샤프트)`   | **105.84 ~ 128.78 t** | **22.9 t** |
| §8-8 — 현행                      | 76.42 ~ 82.30 t             |            5.9 t |
| §8-8 — 새                        | 76.42 ~ 82.30 t             |            5.9 t |

부록 8 의 경우 새 프론트 27건 중에는 총질량이 **128.78 t** 인 점도 있다 — 최적
대비 **+21.7%** 다. 파레토 최적이지만 총질량 관점에서는 쓸 수 없는 해다.
**따라서 총질량은 목적함수에서 빠지는 대신 「선택 기준」으로 명시적으로
남아야 하고, 프론트를 보고할 때 총질량 열을 반드시 함께 실어야 한다.**

또 하나 남는 문제가 있다. 총질량이 목적에서 빠지면 **탐색이 총질량을 직접
개선하는 방향으로 압력을 받지 않는다.** 위 표는 「이미 평가된 점들」을 다시
판정한 것이므로 이 효과가 들어 있지 않다. 새 정식화로 실제 탐색을 돌리면
총질량 최소가 지금보다 나아질 수도, 나빠질 수도 있다 — **재판정으로는 답할 수
없는 항목**이다.
...
| 질문                        | 답                                                                                          |
| --------------------------- | ------------------------------------------------------------------------------------------- |
| 두 목적이 상충하는가        | **그렇다.** 베어링 최적은 총질량을 1.6 ~ 7.7% 악화시킨다                              |
| 상관이 높은 이유            | 물리가 아니라**정의** — `f2 = 2f1 + m_shaft` 로 겹쳐 있다                          |
| 베어링 단독으로 충분한가    | **아니다.** 단일목적으로 줄이려면 대상은 총질량이다                                   |
| 샤프트를 목적에 안 넣었는가 | **넣었다.** 총질량 안에 1:2 가중치로 고정돼 있었을 뿐이다                             |
| 해상도 부족의 원인          | `z2` 격자(§8-6.5.3) + **정식화**(⑵ — 재판정만으로 3배)                           |
| 권고                        | `(베어링, 샤프트)` 로 풀고 프론트에서 **총질량 최소를 선택** · 총질량 열 필수 표기 |

> **이번 라운드는 검토까지다.** 새 정식화로 실제 재최적화하는 것은 §8-6.5.6 ⑴
> (`z2` 격자 세분)·`C6` 결정과 묶어 한 번에 하는 것이 낫다 — 세 변경 모두
> 재최적화를 요구하므로 따로 돌리면 같은 시간을 세 번 쓴다.

##### ⑻ 프론트 재검토 — 부록 8 S3-c 를 `(베어링, 샤프트)` 로 다시 판정

⑵ 에서 「프론트가 8 → 27건이 된다」고 숫자만 적었다. 여기서는 그 27건이
**무엇인지** 편다. 평가 자료는 §8-6.3 과 동일하고 **MASTA 재해석은 0회**다 —
목적함수만 바꿔 비지배 판정을 다시 했을 뿐이다.

|                              | 현행`(베어링, 총질량)` | **재판정 `(베어링, 샤프트)`** |
| ---------------------------- | -----------------------: | ------------------------------------: |
| `z1 ≥ 1.0` 프론트         |                      8건 |        **27건 → 검토 후 25건** |
| `z1 ≥ 1.5` 프론트         |                      6건 |                        **19건** |
| `z1 ≥ 1.0` 의 `z2` 수준 |            4 (4.5 ~ 6.0) |               **6 (3.5 ~ 6.0)** |

**되살아난 것은 단스팬 설계다.** 현행 정식화는 `z2` 4.5 미만을 전부 버렸지만,
새 정식화에서는 `z2` = 3.5 · 4.0 설계가 프론트에 올라온다(`z2` = 3.0 의 2건은
미수렴으로 제외했다 — 아래 ⑤). 이들은 샤프트가
가볍고(43.66 ~ 53.05 t) 베어링이 무거운(27.08 ~ 42.56 t) 쪽 끝이며, 총질량으로
합산하는 순간 지배당해 사라졌던 해다. **베어링과 샤프트의 상대 중요도가 1:2 가
아니라면 이쪽이 답이 될 수 있다.**

###### 그림 — 총질량 등고선과 함께 본다

![베어링·샤프트 프론트 z1 1.0 이상](Results/사이징최적화/figures/pareto_bs_z1_10.png)

![베어링·샤프트 프론트 z1 1.5 이상](Results/사이징최적화/figures/pareto_bs_z1_15.png)

회색 점선은 **총질량 등고선** `2·베어링 + 샤프트 = const` 다(기울기 −2).
검은 원은 현행 정식화가 뽑았던 점, 초록 별은 **총질량 최소점**, 회색 ✕ 는
**미수렴으로 제외한 점**이다(⑤).

**등고선이 프론트에 접하는 곳이 곧 총질량 최소점**이다 — ⑸ 의 가중합 정리가
그림으로 보이는 부분이며, 「최적화는 `(베어링, 샤프트)` 로 하고 선택은 총질량으로
한다」는 절차가 왜 성립하는지가 여기서 설명된다. 기울기를 −2 가 아닌 다른 값으로
기울이면 접점이 프론트 위를 미끄러진다. **그 접점이 전부 이 27건 안에 있다는
것이 새 정식화의 값어치다.**

###### ⑻-a `z1 ≥ 1.0` — 프론트 27건

★ = 총질량 최소 · 「현행」 열의 ● 는 현행 `(베어링, 총질량)` 프론트에도 드는 점

<!-- A8:BS_A -->

|     # | D_pw [mm] | d [mm] | D [mm] | T [mm] | α [°] | 2β [°] | D_we [mm] | L_we [mm] | 세장비 | z1 [m] | z2 [m] | Z [개] | **베어링** [t] | **샤프트** [t] |       총질량 [t] | 총질량 순위 | σ_max [MPa] | **σ 여유** [MPa] | **슬라이스 표본** | 현행 |
| ----: | --------: | -----: | -----: | -----: | ------: | -------: | --------: | --------: | -----: | -----: | -----: | -----: | -------------------: | -------------------: | ---------------: | ----------: | -----------: | ----------------------: | ----------------------: | :--: |
|     1 |     4,495 |  4,079 |  4,900 |    472 |      15 |    1.176 |     185.4 |     353.8 |  1.955 |    1.0 |    6.0 |     70 |      **17.00** |      **73.53** |           107.53 |          19 |      2,099.3 |                     0.7 |                   4,752 |  ●  |
|     2 |     4,494 |  4,082 |  4,896 |    475 |      15 |    1.145 |     180.2 |     355.8 |  2.022 |    1.0 |    6.0 |     72 |      **17.00** |      **73.28** |           107.28 |          16 |      2,099.4 |                     0.6 |                   4,752 |  ●  |
|     3 |     4,492 |  4,048 |  4,925 |    490 |      20 |    1.494 |     177.6 |     367.9 |  2.120 |    1.0 |    5.5 |     73 |      **19.16** |      **68.46** |           106.78 |          12 |      2,099.8 |                     0.2 |                   3,817 |  ●  |
|     4 |     4,499 |  4,058 |  4,929 |    493 |      19 |    1.421 |     177.8 |     370.2 |  2.131 |    1.0 |    5.5 |     73 |      **19.16** |      **68.36** |           106.67 |          10 |      2,099.4 |                     0.6 |                   3,817 |  ●  |
|     5 |     4,500 |  4,059 |  4,930 |    494 |      19 |    1.421 |     177.8 |     370.6 |  2.133 |    1.0 |    5.5 |     73 |      **19.19** |      **68.09** |           106.47 |           9 |      2,099.2 |                     0.8 |                   3,817 |  ●  |
|     6 |     4,495 |  4,068 |  4,911 |    519 |      16 |    1.128 |     166.3 |     390.1 |  2.397 |    1.0 |    5.5 |     78 |      **19.61** |      **67.97** |           107.19 |          14 |      2,096.6 |                     3.4 |                   3,817 |  —  |
|     7 |     4,500 |  4,067 |  4,921 |    518 |      15 |    1.138 |     179.3 |     388.8 |  2.216 |    1.0 |    5.5 |     72 |      **19.62** |      **67.95** |           107.18 |          13 |      2,098.2 |                     1.8 |                   3,817 |  —  |
|     8 |     4,495 |  3,995 |  4,984 |    489 |      29 |    2.309 |     193.9 |     367.0 |  1.937 |    1.0 |    5.0 |     67 |      **21.51** |      **63.68** |           106.70 |          11 |      2,098.7 |                     1.3 |                   1,623 |  —  |
|     9 |     4,499 |  4,007 |  4,981 |    495 |      27 |    2.123 |     190.6 |     371.8 |  1.996 |    1.0 |    5.0 |     68 |      **21.51** |      **63.37** |           106.39 |           7 |      2,099.2 |                     0.8 |                   1,623 |  ●  |
|    10 |     4,495 |  4,018 |  4,961 |    504 |      27 |    1.863 |     166.3 |     378.8 |  2.329 |    1.0 |    5.0 |     78 |      **21.58** |      **63.30** |           106.45 |           8 |      2,099.6 |                     0.4 |                   1,623 |  —  |
|    11 |     4,499 |  4,022 |  4,965 |    504 |      27 |    1.861 |     166.3 |     378.8 |  2.329 |    1.0 |    5.0 |     78 |      **21.59** |      **63.11** |           106.29 |           6 |      2,099.6 |                     0.4 |                   1,623 |  ●  |
|    12 |     4,499 |  4,033 |  4,953 |    544 |      18 |    1.373 |     181.1 |     409.0 |  2.306 |    1.0 |    5.0 |     71 |      **22.40** |      **63.03** |           107.82 |          20 |      2,098.8 |                     1.2 |                   1,623 |  —  |
|    13 |     4,499 |  4,029 |  4,957 |    542 |      19 |    1.437 |     179.9 |     407.6 |  2.313 |    1.0 |    5.0 |     72 |      **22.60** |      **62.96** |           108.16 |          23 |      2,087.5 |                    12.5 |                   1,623 |  —  |
|    14 |     4,496 |  4,039 |  4,940 |    565 |      15 |    1.141 |     179.6 |     425.2 |  2.415 |    1.0 |    5.0 |     72 |      **22.76** |      **62.87** |           108.38 |          24 |      2,097.1 |                     2.9 |                   1,623 |  —  |
| 15 ★ |     4,499 |  3,991 |  4,995 |    526 |      28 |    2.081 |     180.2 |     395.4 |  2.242 |    1.0 |    4.5 |     72 |      **23.89** |      **58.06** | **105.84** |           1 |      2,098.9 |                     1.1 |                     456 |  ●  |
|    16 |     4,493 |  3,987 |  4,987 |    530 |      27 |    2.014 |     180.2 |     398.3 |  2.258 |    1.0 |    4.5 |     72 |      **23.95** |      **58.00** |           105.90 |           2 |      2,099.9 |                     0.1 |                     456 |  —  |
|    17 |     4,499 |  3,995 |  4,991 |    534 |      26 |    1.942 |     180.2 |     401.4 |  2.275 |    1.0 |    4.5 |     72 |      **24.07** |      **57.89** |           106.03 |           3 |      2,099.6 |                     0.4 |                     456 |  —  |
|    18 |     4,499 |  3,994 |  4,992 |    535 |      26 |    1.945 |     180.5 |     402.4 |  2.277 |    1.0 |    4.5 |     72 |      **24.18** |      **57.88** |           106.23 |           4 |      2,096.5 |                     3.5 |                     456 |  —  |
|    19 |     4,500 |  3,993 |  4,996 |    533 |      27 |    2.011 |     180.2 |     400.4 |  2.270 |    1.0 |    4.5 |     72 |      **24.20** |      **57.86** |           106.26 |           5 |      2,095.8 |                     4.2 |                     456 |  —  |
|    20 |     4,482 |  3,944 |  5,007 |    560 |      30 |    2.182 |     176.6 |     421.6 |  2.436 |    1.0 |    4.0 |     73 |      **27.08** |      **53.05** |           107.22 |          15 |      2,097.4 |                     2.6 |                      49 |  —  |
|    21 |     4,487 |  3,948 |  5,013 |    562 |      30 |    2.180 |     176.6 |     422.7 |  2.442 |    1.0 |    4.0 |     73 |      **27.26** |      **52.90** |           107.41 |          18 |      2,095.3 |                     4.7 |                      49 |  —  |
|    22 |     4,496 |  3,957 |  5,022 |    568 |      29 |    2.117 |     177.3 |     427.7 |  2.461 |    1.0 |    4.0 |     73 |      **27.63** |      **52.82** |           108.09 |          21 |      2,088.8 |                    11.2 |                      49 |  —  |
|    23 |     4,497 |  3,964 |  5,017 |    578 |      26 |    1.957 |     181.6 |     435.3 |  2.444 |    1.0 |    4.0 |     71 |      **27.70** |      **52.71** |           108.11 |          22 |      2,099.5 |                     0.5 |                      49 |  —  |
|    24 |     4,497 |  3,965 |  5,016 |    589 |      24 |    1.849 |     185.2 |     443.8 |  2.443 |    1.0 |    4.0 |     70 |      **28.15** |      **52.52** |           108.81 |          25 |      2,096.1 |                     3.9 |                      49 |  —  |
|    25 |     4,497 |  3,932 |  5,049 |    589 |      30 |    2.278 |     185.2 |     443.8 |  2.443 |    1.0 |    3.5 |     70 |      **30.05** |      **47.19** |           107.29 |          17 |      2,099.9 |                     0.1 |                      11 |  —  |

**검토 후 제외** — 아래 두 점은 비지배이나 그 슬라이스가 탐색되지 않았다(§8-6.5.7 ⑻ 읽을 것 ⑤)

|       # | D_pw [mm] | d [mm] | D [mm] | T [mm] | α [°] | 2β [°] | D_we [mm] | L_we [mm] | 세장비 | z1 [m] | z2 [m] | Z [개] | **베어링** [t] | **샤프트** [t] | 총질량 [t] | 총질량 순위 | σ_max [MPa] | **σ 여유** [MPa] | **슬라이스 표본** | 현행 |
| ------: | --------: | -----: | -----: | -----: | ------: | -------: | --------: | --------: | -----: | -----: | -----: | -----: | -------------------: | -------------------: | ---------: | ----------: | -----------: | ----------------------: | ----------------------: | :--: |
| ~~26~~ |     4,391 |  3,720 |  5,046 |    704 |      29 |    2.707 |     223.6 |     531.8 |  2.417 |    1.0 |    3.0 |     56 |      **41.43** |      **44.31** |     127.17 |          — |      2,067.1 |                    32.9 |                       2 |  —  |
| ~~27~~ |     4,438 |  3,764 |  5,096 |    711 |      29 |    2.653 |     221.3 |     537.3 |  2.467 |    1.0 |    3.0 |     57 |      **42.56** |      **43.66** |     128.78 |          — |      2,047.7 |                    52.3 |                       2 |  —  |
...
|    # | D_pw [mm] | d [mm] | D [mm] | T [mm] | α [°] | 2β [°] | D_we [mm] | L_we [mm] | 세장비 | z1 [m] | z2 [m] | Z [개] | **베어링** [t] | **샤프트** [t] |       총질량 [t] | 총질량 순위 | σ_max [MPa] | **σ 여유** [MPa] | **슬라이스 표본** | 현행 |
| ---: | --------: | -----: | -----: | -----: | ------: | -------: | --------: | --------: | -----: | -----: | -----: | -----: | -------------------: | -------------------: | ---------------: | ----------: | -----------: | ----------------------: | ----------------------: | :--: |
|    1 |     4,499 |  4,036 |  4,951 |    492 |      20 |    1.648 |     197.0 |     369.0 |  1.917 |    1.5 |    6.0 |     66 |      **19.88** |      **74.55** |           114.31 |           9 |      2,096.9 |                     3.1 |                     696 |  ●  |
|    2 |     4,497 |  4,040 |  4,943 |    502 |      18 |    1.482 |     196.1 |     376.6 |  1.964 |    1.5 |    6.0 |     66 |      **19.97** |      **74.32** |           114.27 |           8 |      2,098.8 |                     1.2 |                     696 |  ●  |
|    3 |     4,495 |  4,037 |  4,942 |    501 |      18 |    1.488 |     196.8 |     376.2 |  1.955 |    1.5 |    6.0 |     66 |      **20.00** |      **74.26** |           114.27 |           7 |      2,096.7 |                     3.3 |                     696 |  ●  |
|    4 |     4,495 |  4,055 |  4,924 |    512 |      19 |    1.333 |     166.3 |     384.5 |  2.364 |    1.5 |    6.0 |     78 |      **20.03** |      **74.00** |           114.05 |           3 |      2,097.9 |                     2.1 |                     696 |  ●  |
|    5 |     4,500 |  4,054 |  4,935 |    517 |      15 |    1.227 |     193.9 |     388.1 |  2.046 |    1.5 |    6.0 |     67 |      **20.08** |      **73.98** |           114.13 |           4 |      2,096.8 |                     3.2 |                     696 |  —  |
|    6 |     4,499 |  4,051 |  4,936 |    514 |      17 |    1.327 |     185.2 |     385.8 |  2.130 |    1.5 |    6.0 |     70 |      **20.20** |      **73.92** |           114.31 |          10 |      2,093.9 |                     6.1 |                     696 |  —  |
|    7 |     4,499 |  4,013 |  4,974 |    511 |      26 |    1.917 |     177.8 |     383.6 |  2.206 |    1.5 |    5.5 |     73 |      **22.14** |      **69.24** |           113.53 |           2 |      2,099.0 |                     1.0 |                     341 |  ●  |
| 8 ★ |     4,496 |  4,007 |  4,974 |    507 |      28 |    2.004 |     173.2 |     380.9 |  2.249 |    1.5 |    5.5 |     75 |      **22.19** |      **69.13** | **113.51** |           1 |      2,099.3 |                     0.7 |                     341 |  ●  |
|    9 |     4,498 |  4,018 |  4,966 |    529 |      22 |    1.660 |     180.4 |     397.8 |  2.253 |    1.5 |    5.5 |     72 |      **22.58** |      **69.05** |           114.22 |           6 |      2,092.0 |                     8.0 |                     341 |  —  |
|   10 |     4,492 |  4,022 |  4,950 |    546 |      19 |    1.422 |     177.6 |     410.4 |  2.359 |    1.5 |    5.5 |     73 |      **22.77** |      **68.84** |           114.39 |          12 |      2,097.7 |                     2.3 |                     341 |  —  |
|   11 |     4,484 |  3,957 |  4,999 |    534 |      28 |    2.261 |     195.8 |     401.3 |  2.094 |    1.5 |    5.0 |     66 |      **24.88** |      **64.56** |           114.32 |          11 |      2,098.8 |                     1.2 |                     140 |  —  |
|   12 |     4,500 |  3,975 |  5,013 |    538 |      27 |    2.176 |     195.6 |     404.4 |  2.111 |    1.5 |    5.0 |     66 |      **25.04** |      **64.11** |           114.20 |           5 |      2,098.4 |                     1.6 |                     140 |  —  |
|   13 |     4,498 |  3,972 |  5,012 |    556 |      24 |    2.000 |     200.9 |     418.5 |  2.126 |    1.5 |    5.0 |     64 |      **25.87** |      **64.06** |           115.80 |          15 |      2,094.7 |                     5.3 |                     140 |  —  |
|   14 |     4,498 |  3,987 |  4,996 |    582 |      22 |    1.658 |     180.2 |     438.2 |  2.479 |    1.5 |    5.0 |     72 |      **26.68** |      **63.80** |           117.16 |          16 |      2,080.8 |                    19.2 |                     140 |  —  |
|   15 |     4,494 |  3,943 |  5,033 |    569 |      29 |    2.271 |     190.6 |     428.5 |  2.293 |    1.5 |    4.5 |     68 |      **28.11** |      **58.93** |           115.14 |          13 |      2,092.4 |                     7.6 |                      34 |  —  |
|   16 |     4,496 |  3,945 |  5,034 |    578 |      27 |    2.180 |     195.8 |     435.2 |  2.267 |    1.5 |    4.5 |     66 |      **28.44** |      **58.73** |           115.61 |          14 |      2,096.3 |                     3.7 |                      34 |  —  |
|   17 |     4,494 |  3,952 |  5,023 |    615 |      22 |    1.773 |     193.0 |     463.4 |  2.446 |    1.5 |    4.5 |     67 |      **29.83** |      **58.61** |           118.27 |          17 |      2,097.1 |                     2.9 |                      34 |  —  |
|   18 |     4,481 |  3,876 |  5,072 |    670 |      22 |    2.071 |     226.4 |     505.7 |  2.272 |    1.5 |    4.0 |     57 |      **35.90** |      **54.10** |           125.90 |          18 |      2,095.1 |                     4.9 |                       1 |  —  |
|   19 |     4,392 |  3,728 |  5,040 |    697 |      28 |    2.670 |     228.1 |     526.3 |  2.345 |    1.5 |    3.5 |     55 |      **40.44** |      **50.42** |           131.30 |          19 |      2,095.9 |                     4.1 |                       1 |  —  |

<!-- /A8:BS_B -->

###### 읽을 것

**① 총질량 최소는 프론트 한가운데에 있다.** `z1 ≥ 1.0` 에서 **15 / 25**,
`z1 ≥ 1.5` 에서 **8 / 19** 번째다. 양 끝이 아니라 중간이라는 것은 두 목적이
**실제로 상충하며 절충점이 존재한다**는 뜻이다 — ⑶ 에서 「베어링 단독 최적화는
총질량을 7.7% 악화시킨다」고 한 것의 기하학적 표현이다.

**② 현행 프론트는 새 프론트의 부분집합이다.** 8건 전부(100%)가 27건 안에
● 로 표시된다. **정식화를 바꿔도 지금까지의 해는 하나도 사라지지 않는다.**

**③ 새로 들어온 19건이 어디에 있는지가 중요하다.** 대부분 `z2` 가 작은
(3.0 ~ 4.5) 영역, 즉 **샤프트를 줄이고 베어링으로 buy 하는 구간**이다. 현행
정식화는 이 구간을 총질량 한 축으로 눌러 버렸다.

**④ 총질량 폭이 넓다.** 채택 25건의 총질량이 105.84 ~ 108.81 t 이고,
제외한 2건까지 넣으면 128.78 t 까지 벌어진다. ⑹ 에서 경고한 대로 **총질량 열
없이 이 프론트를 보고하면 위험하다.**

**⑤ `z2` = 3.0 의 두 점(원 `#26`·`#27`)은 미수렴이라 제외했다.** 베어링 질량이
`#25` 의 30.05 t 에서 41.43 · 42.56 t 로 튀는 것이 눈에 걸려 들여다본 결과,
**세 신호가 모두 「그 슬라이스가 탐색되지 않았다」**고 말한다.

| 신호                  | `#1` ~ `#25` | **`#26` · `#27`**          |
| --------------------- | ---------------- | ------------------------------------- |
| 슬라이스 가능해 수    | 11 ~ 4,752건     | **2건** (2,453점 평가 중 0.08%) |
| 프론트 / 가능해       | 0.0 ~ 10.2%      | **100%** (2건이 그대로 프론트)  |
| σ 여유 (2,100 − σ) | 0.1 ~ 12.5 MPa   | **32.9 ~ 52.3 MPa**             |
| `D_pw` (상한 4,500) | 4,482 ~ 4,500    | **4,391 ~ 4,438**               |

**σ 여유가 판정의 핵심이다.** 잘 수렴한 해는 σ 제약을 끝까지 소진해 경계에
붙는다 — 25건이 전부 여유 12.5 MPa 이내다. `#26`·`#27` 은 33 ~ 52 MPa 를 남겼고,
이는 **베어링을 더 깎고도 가능했다**는 뜻이다. `D_pw` 가 상한에서 62 ~ 109 mm
떨어진 것도 같은 방향이다(§8-8.5.2 — `D_pw` 는 클수록 좋다).

두 점이 비지배인 것은 사실이다. 그러나 그것은 **그 샤프트 질량(43.66 · 44.31 t)에
도달한 다른 점이 평가되지 않았기 때문**이지 그 조합이 우수해서가 아니다.
`z2` = 3.0 슬라이스의 참 프론트는 여전히 미지다.

**⑥ 그래서 프론트 표에 진단 두 열을 상설한다.**

| 열                      | 무엇을 보는가                                             |
| ----------------------- | --------------------------------------------------------- |
| **σ 여유**       | 제약을 소진했는가 — 크면 그 점 주변이 덜 탐색됐다는 신호 |
| **슬라이스 표본** | 그`z2` 수준에서 가능해가 몇 점 나왔는가                 |

**자동으로 걸러내지는 않는다.** 두 신호가 항상 같은 방향을 가리키지는 않기
때문이다. 실제로 `z1 ≥ 1.5` 의 `#18`(`z2` 4.0)·`#19`(`z2` 3.5)는 **슬라이스
표본이 각각 1건**으로 `#26`·`#27` 보다 더 극단이지만, σ 여유는 4.9 · 4.1 MPa 로
경계에 붙어 있다 — 표본이 하나뿐이라 「그 슬라이스의 최적」이라 주장할 수는 없되,
`#26`·`#27` 처럼 명백히 덜 깎인 해도 아니다. **판단은 두 열을 보고 사람이
한다**는 것이 이 절의 운영 규칙이고, 이번 제외도 그렇게 내린 결정이다.

*프론트 전량 `부록8_NSGA/S3_본최적화/bs_pareto.csv` · 재판정 `a8_bs_front.py`*

##### ⑼ 추가 최적화 검토 — 제약을 `D_pw` 가 아니라 **외경 `D`** 로

§8-8 이 남긴 결론은 「`D_pw` 는 최적화가 정하는 값이 아니라 제약이 정하는
값」이었다(§8-8.5.3). 그렇다면 **그 제약을 옳은 물리량에 걸어야 한다.**

**제작 가능 크기를 정하는 것은 `D_pw` 가 아니라 외경 `D` 다.** 링을 깎는
선반의 스윙, 열처리로, 운송 폭 — 전부 바깥지름이 걸린다. 현행 `C6` 는
`D_pw ≤ 4,500 mm` 로 **대리변수에 걸려 있다.**

> **현행 `C6` 는 사실상 `D ≤ 5.0 m` 였다.** §8-6.3a 프론트 8건의 외경이
> 4,900 ~ **4,995 mm** 로 전부 5,000 바로 아래다. `D_pw` 를 4,500 으로 묶은 것이
> 결과적으로 외경 5.0 m 제한과 거의 같게 작동해 왔다는 뜻이며, 이는 `C6` 를
> `D` 기준으로 바꿔 다는 것이 **기존 결과를 뒤엎는 변경이 아님**을 말해 준다.

목표는 **`D` 상한 4.0 · 4.5 · 5.0 m 각각에서 총질량 최소해**를 얻는 것이다.

###### ⑼-1. 세 상한을 한 번의 실행으로 — `(총질량, D)` 2목적

제약만 바꿔 세 번 돌리는 것은 낭비다. **`D` 를 제약이 아니라 두 번째 목적으로
두면 한 번으로 끝난다.**
...
이 프론트는 곧 **「`D` 상한별로 도달 가능한 최소 총질량」 곡선**이다. 4.0 · 4.5 ·
5.0 은 그 곡선에서 **읽어내는 세 점**이고, 그 사이 값도 전부 덤으로 나온다.

**임의의 상한 `c` 에 대한 제약최적해가 반드시 이 프론트 위에 있다.**

> `c` 에서의 최적해를 `(t*, D*)` 라 하자(`D* ≤ c`). 만약 이를 지배하는
> `(t′, D′)` 가 있다면 `D′ ≤ D* ≤ c` 이므로 그 점도 상한 `c` 를 만족하고,
> `t′ ≤ t*` 이므로 `t*` 가 최소라는 가정에 어긋난다. 따라서 `(t*, D*)` 는
> 비지배 — 프론트 위에 있다. ∎
...
| 방법                                                   | 판단                                                                                                               |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| **사후 필터링** (넓게 돌리고 `D ≤ c` 로 자름) | **불가.** 아래 ⑼-3 이 보이듯 작은 `D` 영역이 거의 표본되지 않는다 — `#26`·`#27` 과 같은 실패 모드다 |
| `D` 상한을 범주변수로 추가                           | 범주 조합이 224 → 672 로 늘어 개체수도 672 로 올려야 하고 세대당 비용이 3배가 된다                                |
| `(베어링, 샤프트, D)` 3목적                          | 정보는 가장 많으나 NSGA-II 의 혼잡도 정렬이 3목적에서 약해 같은 예산에서 수렴이 느리다                             |

###### ⑼-2. `D` 상한별 `D_pw` 상한 — 결정 구조

`D` 를 목적에 넣더라도 `D_pw` 의 **탐색 범위**는 정해 줘야 한다. 관계는 정확히
해석식으로 주어진다(§6-4).

$$
D = D_{pw} + \underbrace{D_{we}\cos\alpha + L_w\sin\alpha + 2t_o}_{\text{여유}}
$$

여유는 베어링 제원이 정하므로, **여유가 최소일 때 `D_pw` 가 가장 커질 수 있다.**
격자 전 구간에서 여유를 훑었다.

|                                 |                   값 | 그때의 조합                                                   |
| ------------------------------- | -------------------: | ------------------------------------------------------------- |
| 여유**최소**              |     **216 mm** | `D_we` 110.0 · α 15° · `L_w` 175.0 (격자 최소 베어링) |
| 여유 최대                       |               676 mm | `D_we` 230.0 · α 30° · `L_w` 550.0                    |
| 여유 실측 범위(가능해 17,714점) | 314 ~ 668 (중앙 471) | —                                                            |

따라서 **`D_pw` 상한 = `D` 상한 − 216 mm** 로 두면 어떤 조합도 잘리지 않는다.

| `D` 상한 | **`D_pw` 상한** | 참고: 여유 최대일 때`D_pw` |
| ---------: | ----------------------: | ---------------------------: |
|      4,000 |         **3,784** |                        3,324 |
|      4,500 |         **4,284** |                        3,824 |
|      5,000 |         **4,784** |                        4,324 |

세 상한을 한 번에 다루므로 **`D_pw` ∈ [3,300, 4,784]** 하나면 된다. 하한 3,300 은
`D` = 4,000 을 여유 최대 조합으로도 만들 수 있게 남겨 둔 값이다(부록 6 하한과
동일).

###### ⑼-3. 기존 자료가 이미 말해 주는 것 — 그리고 못 말하는 것

가능해 17,714점(부록 8 S3-c + §8-8)을 `D` 로 잘라 보면 이렇다.

| `D` 상한 [mm] |       가능해 |  총질량 최소 [t] | 판단                                                 |
| --------------: | -----------: | ---------------: | ---------------------------------------------------- |
| **4,000** |  **0** |               — | 가능해 없음 — 불가능인지 미탐색인지**미확정** |
|           4,250 |            1 |           157.77 | 표본 1점                                             |
| **4,500** | **38** |           139.82 | **과소표본** (전체의 0.2%)                     |
| **5,000** |       11,705 | **105.84** | 현행`C6` 에 해당                                   |
|           5,500 |       13,985 |            88.10 | §8-8 영역                                           |

가능해의 `D` 최솟값은 **4,168 mm** 다.

**⑴ `D` = 4.0 m 는 가능해가 한 점도 없다.** σ ≤ 2,100 을 만족하는 설계가
그 안에 존재하지 않는 것인지, 탐색이 닿지 않은 것인지는 **이 자료로 답할 수
없다** — 두 탐색 모두 큰 `D` 쪽으로 밀려 있었기 때문이다. **답이 「불가능」으로
나온다면 그 자체가 중요한 결론**이므로 4.0 을 빼지 않고 그대로 돌린다.

**⑵ `D` = 4.5 m 의 139.82 t 는 믿을 수 없다.** 38점만으로 나온 값이고,
`#26`·`#27` 과 같은 과소표본 상태다(⑻ 읽을 것 ⑤). 이 구간을 제대로 답하려면
**탐색이 그쪽으로 강제로 뻗어야** 하고, 그것이 `D` 를 목적에 넣는 이유다.

**⑶ 사후 필터링이 왜 안 되는지가 여기서 드러난다.** 넓게 돌린 뒤 자르면
`D ≤ 4,500` 구간의 답은 38점 중 최선일 뿐이다.

###### ⑼-4. 실행 설계 (미수행)

| 항목      | 값                                               | 비고                                                        |
| --------- | ------------------------------------------------ | ----------------------------------------------------------- |
| 목적      | `f1` 총질량 ↓ · `f2` 외경 `D` ↓         | **정식화 변경**                                       |
| `D_pw`  | **[3,300, 4,784]** · 1 mm                 | ⑼-2                                                        |
| 제약      | 기하 8개 + σ ≤ 2,100 +**`D` ≤ 5,000** | `C6`(`D_pw` ≤ 4,500) 를 **`D` 기준으로 대체**  |
| 종속변수  | 두께 규칙 · 코너 반경 4.3 · 정수화             | 부록 8 그대로                                               |
| 그 밖     | 개체수 224 · 시드 1 · 범주 전수 시딩 · 연산자 | 부록 6 그대로                                               |
| 예산(안)  | **224 × 60 ≈ 75분**                      | 곡선 전 구간을 덮으려면 §8-8 의 40세대보다 여유가 필요하다 |
| HV 기준점 | 재설정 필요                                      | 목적의 물리량이 바뀌어`(40, 250)` 을 쓸 수 없다           |

**`D` ≤ 5,000 을 제약으로 두는 이유** — `D` 가 목적이지만 상한이 없으면 탐색이
더 큰 `D` 로 무한정 흘러간다(§8-8.5.3). 세 상한 중 가장 느슨한 값을 경계로 두면
곡선이 필요한 구간만 덮는다.
...
| 산출 | 내용                                                                         |
| ---- | ---------------------------------------------------------------------------- |
| 곡선 | `D` 대 최소 총질량 — 4.0 · 4.5 · 5.0 을 표시                            |
| 표   | 세 상한 각각의 총질량 최소 설계 제원(⑻ 양식 · σ 여유·슬라이스 표본 포함) |
| 판정 | `D` = 4.0 에서 가능해가 나오는가 · 4.5 → 5.0 의 질량 이득이 얼마인가     |

**이 결과가 나오면 `C6` 를 「`D_pw` ≤ 4,500」에서 「`D` ≤ (협의된 값)」으로 고쳐
쓸 근거가 선다.** 제작·운송 부서에 물어야 할 것도 `D_pw` 가 아니라 **외경**이
된다.

> **한계 두 가지를 미리 적는다.** ⑴ `D` 를 목적에 넣으면 §8-6.5.7 ⑸ 의 권고
> (`베어링`·`샤프트` 분리)와는 다른 정식화가 된다 — 두 질문이 다르므로
> 별개의 실행이며, 질량 분배는 이 실행에서 사후 보고로만 남는다. ⑵ 두께
> 규칙의 외삽 문제(§8-8.3)는 그대로다. `D` 를 조이면 보어가 작아져 벽이 오히려
> **두꺼워지므로** §8-8 과 반대 방향이고, 그 점에서는 안전측이다.

*미수행 — 실행 여부는 지시 대기*

*재판정 스크립트: `a8_obj_check.py` · 포함관계 `a8_obj_contain.py`*

## 8-7. S4-d — 프론트 14건 피로수명 검토 (50 · 70 °C)

§6-11.6 은 부록 6 프론트 40건이 **50 °C 에서 전원 합격**임을 보였고, §6-11.7 은
그중 **20건이 70 °C 에서 탈락**함을 보였다 — 탈락한 20건은 전부 베어링 최경량군
이었다. 부록 8 프론트에도 같은 검토를 건다.

### 8-7.1. 대상과 절차

**대상은 §8-6.3 의 프론트 전량 14건**이다. 부록 6 은 64·41건 중 양 끝 20건씩만
골랐지만 부록 8 은 프론트가 8·6건뿐이라 자를 것이 없다.

| 태그              | 대상                                 | 건수 |
| ----------------- | ------------------------------------ | ---: |
| `a01` ~ `a08` | §8-6.3a (`z1 ≥ 1.0`) 프론트 전량 |    8 |
| `b01` ~ `b06` | §8-6.3b (`z1 ≥ 1.5`) 프론트 전량 |    6 |

절차는 §6-11.6 과 같다 — `L_eff` 강체정역학 스크리닝으로 DLC별 `k` 선정(건당
7초) → **dt = 20 · 111 DLC** MASTA 피로(ISO/TS 16281) → 직렬 와이블 `e` = 9/8 로
`life_Sys`. 판정은 **ΣD30_UW ≤ 0.5 ∧ ΣD30_Sys ≤ 0.5**. 70 °C 는 §6-11.7 과
**같은 조건**이다 — 베어링 3항(전동체·내륜·외륜)만 70 °C 로 올리고 샤프트·하우징은
그대로 둔다. 부록 6 의 40건과 숫자를 직접 대조할 수 있어야 하기 때문이다.
**미결 #12 가 지적했던 「열팽창 효과가 섞인다」는 이 검토 중에 실측으로
부정됐다** — §8-7.4.

### 8-7.2. 파이프라인에 부록 8 규칙을 넣는다 — 필수 조치

**기존 P2 스크립트를 그대로 쓰면 안 된다.** `run_p2_fatigue.py` 는 기하를
`sg.bearing()` · `sg.shaft()` 로 **다시 계산**하므로 두께 규칙도 코너 반경도
들어가지 않는다. §7-6.7.6 에서 잡은 「샤프트 내경 `floor` 누락」과 같은 부류다.

`a8_build.py` 가 `sizing_geom` 의 세 함수만 감싸서 파이프라인 전체를 부록 8
규칙으로 돌린다.

| 감싼 함수             | 바뀌는 것                               | 왜 필요한가                                                                 |
| --------------------- | --------------------------------------- | --------------------------------------------------------------------------- |
| `sg.bearing`        | `nsga_eval.geom(…, integerize=True)` | 원래 함수는 T·B·C 를 반올림하지 않아 S3 가 실제로 넣은 제원과 어긋난다    |
| `sg.shaft`          | 내경을 두께 규칙으로                    | 벽두께가 절반이라 강성·미스얼라인먼트가 완전히 다르다                      |
| `sg.apply_to_masta` | 코너 반경 4.3 mm 주입                   | **유효 길이가 8.6 mm 줄어 `C` 가 낮아진다 — 손상에 직접 들어간다** |

세 번째가 특히 중요하다. `C` 는 상수 실측 단계에서 뽑히므로 **`probe_p2_constants`
부터** 적용돼야 하고, 그래서 세 스크립트가 모두 `a8_build` 를 부른다(phase `6`·`7`).

### 8-7.3. 결과

<!-- A8:S4SUM -->

**14건 전원 합격 — 50 °C 와 70 °C 모두.** 부록 6 이 70 °C 에서
40건 중 20건을 잃은 것과 갈린다.

| 온도   | ΣD30_UW        | ΣD30_Sys       |        최소 여유 | life_Sys     |              합격 |
| ------ | --------------- | --------------- | ---------------: | ------------ | ----------------: |
| 50 °C | 0.1039 ~ 0.1439 | —              | **3.47배** | 204 ~ 283 yr | **14 / 14** |
| 70 °C | 0.2873 ~ 0.3964 | 0.2951 ~ 0.4059 | **1.23배** | 74 ~ 102 yr  | **14 / 14** |

**손상비는 2.627 ~ 2.805배**(평균 2.752)다. 부록 6 의 2.84배와 같은 수준이므로, **온도의 효과 자체는 설계가 바뀌어도
...
> **여유가 넉넉하지는 않다.** 70 °C 최악은 `b06` 의
> ΣD30_UW **0.3964**(여유 1.26배 · life_Sys
> 74 yr)다. 부록 6 의 50 °C 최소
> 여유 1.56배보다도 얇다. 하중 스펙트럼이나 재료 가정이 바뀌면 흔들릴 수 있는
> 폭이다. (70 °C 조건에 열팽창 효과가 섞여 있다던 미결 #12 는 §8-7.4 에서
> 종결됐다 — 손상 증가는 전부 점도 효과다.)
...
| ----------------------: | -------: | ---------: | -----: | -----: | --------: | --------: | -----: | -----: | ---------------: | ---------------: | ----------------: | ---------------: | ---------------: | --------------: | --------------: | :----------------------: |
| **기준선** (v1.3) |       — |       5.60 |  3,055 |  3,600 |     110.5 |     238.0 |  2.154 |     87 | **6.9249** |               — |                — |           7.7297 |               — |             3.9 |              — | **불합격**(50 °C) |
|                 `a01` |        1 |      17.00 |  4,079 |  4,900 |     185.4 |     353.8 |  1.955 |     70 |           0.1225 | **0.3217** | **2.627배** |           0.1252 |           0.3305 |             240 |    **91** |      **합격**      |
|                 `a02` |        2 |      17.00 |  4,082 |  4,896 |     180.2 |     355.8 |  2.022 |     72 |           0.1272 | **0.3351** | **2.635배** |           0.1300 |           0.3443 |             231 |    **87** |      **합격**      |
|                 `a03` |        3 |      19.16 |  4,048 |  4,925 |     177.6 |     367.9 |  2.120 |     73 |           0.1103 | **0.3082** | **2.793배** |           0.1131 |           0.3171 |             265 |    **95** |      **합격**      |
|                 `a04` |        4 |      19.16 |  4,058 |  4,929 |     177.8 |     370.2 |  2.131 |     73 |           0.1098 | **0.3052** | **2.780배** |           0.1127 |           0.3144 |             266 |    **95** |      **합격**      |
|                 `a05` |        5 |      19.19 |  4,059 |  4,930 |     177.8 |     370.6 |  2.133 |     73 |           0.1093 | **0.3041** | **2.781배** |           0.1122 |           0.3132 |             267 |    **96** |      **합격**      |
|                 `a06` |        6 |      21.51 |  4,007 |  4,981 |     190.6 |     371.8 |  1.996 |     68 |           0.1039 | **0.2881** | **2.772배** |           0.1060 |           0.2951 |             283 |   **102** |      **합격**      |
|                 `a07` |        7 |      21.59 |  4,022 |  4,965 |     166.3 |     378.8 |  2.329 |     78 |           0.1286 | **0.3607** | **2.805배** |           0.1311 |           0.3694 |             229 |    **81** |      **합격**      |
|                 `a08` |        8 |      23.89 |  3,991 |  4,995 |     180.2 |     395.4 |  2.242 |     72 |           0.1086 | **0.3041** | **2.801배** |           0.1111 |           0.3127 |             270 |    **96** |      **합격**      |

**b — `z1 ≥ 1.5` 프론트 (§8-6.3b)**

|    태그 | 프론트 # | 베어링 [t] | d [mm] | D [mm] | D_we [mm] | L_we [mm] | 세장비 | Z [개] | ΣD30_UW 50 °C |  ΣD30_UW 70 °C |  **손상비** | ΣD30_Sys 50 °C | ΣD30_Sys 70 °C | life_Sys 50 °C | life_Sys 70 °C | **판정 70 °C** |
| ------: | -------: | ---------: | -----: | -----: | --------: | --------: | -----: | -----: | --------------: | ---------------: | ----------------: | ---------------: | ---------------: | --------------: | --------------: | :-------------------: |
| `b01` |        1 |      19.88 |  4,036 |  4,951 |     197.0 |     369.0 |  1.917 |     66 |          0.1066 | **0.2930** | **2.748배** |           0.1095 |           0.3023 |             274 |    **99** |    **합격**    |
| `b02` |        2 |      19.97 |  4,040 |  4,943 |     196.1 |     376.6 |  1.964 |     66 |          0.1067 | **0.2913** | **2.731배** |           0.1099 |           0.3014 |             273 |   **100** |    **합격**    |
| `b03` |        3 |      20.00 |  4,037 |  4,942 |     196.8 |     376.2 |  1.955 |     66 |          0.1051 | **0.2873** | **2.734배** |           0.1082 |           0.2973 |             277 |   **101** |    **합격**    |
| `b04` |        4 |      20.03 |  4,055 |  4,924 |     166.3 |     384.5 |  2.364 |     78 |          0.1338 | **0.3738** | **2.794배** |           0.1375 |           0.3860 |             218 |    **78** |    **합격**    |
| `b05` |        5 |      22.14 |  4,013 |  4,974 |     177.8 |     383.6 |  2.206 |     73 |          0.1314 | **0.3639** | **2.769배** |           0.1343 |           0.3738 |             223 |    **80** |    **합격**    |
| `b06` |        6 |      22.19 |  4,007 |  4,974 |     173.2 |     380.9 |  2.249 |     75 |          0.1439 | **0.3964** | **2.754배** |           0.1466 |           0.4059 |             205 |    **74** |    **합격**    |

*50 °C `P2_피로수명_A8/` · 70 °C `P2_피로수명_A8_70C/`*

<!-- /A8:S4TABLE -->

### 8-7.4. 샤프트·하우징 온도는 결과에 들어가지 않는다 (260806)

§6-11.7 이래 「베어링만 70 °C 로 올려 온도차가 30 °C 로 벌어졌으니 열팽창에
의한 내부 틈새 축소가 손상 증가에 섞여 있다」고 적어 왔다(미결 #12).
**틀린 서술이었다.**

모델의 하중조건은 **`include_thermal_expansion_effects = False`** 다
(`Load Case 1` · `Myz_max` 둘 다). 열팽창을 계산에 넣지 않으므로 샤프트·하우징
온도는 해석에 들어갈 **경로 자체가 없다.** 이 옵션이 꺼져 있으면
`expand_grounded_nodes_for_thermal_effects` 는 접근조차 되지 않는다.

플래그만 믿지 않고 실측했다 — `a01` · `DLC1.2-k-s5` · 30빈 · UW 손상 합.

| 조건                      | 베어링 | 샤프트 | 하우징 |    열팽창    |                        Σ1/L |
| ------------------------- | -----: | -----: | -----: | :-----------: | ---------------------------: |
| ⒜ S4-d 70 °C 가 쓴 조건 |     70 |     40 |     40 | **off** |           4.935361 × 10⁻⁸ |
| ⒝ 전부 70 °C            |     70 |     70 |     70 | **off** | **4.935361 × 10⁻⁸** |
| ⒞ 50 °C 대조            |     50 |     40 |     40 |      off      |           2.275586 × 10⁻⁸ |
| ⒟ 전부 70 °C            |     70 |     70 |     70 | **on** |           4.925320 × 10⁻⁸ |
| ⒠ ⒜ 와 같은 온도        |     70 |     40 |     40 | **on** | **1.807422 × 10⁻³** |

**⒝ / ⒜ = 1.000000000.** 유효숫자 전부가 일치한다 — 샤프트·하우징 온도는
무관하다. 미결 #12 를 종결한다. §6-11.7 의 2.84배도, §8-7.3 의 2.73배도
**전부 점도 효과**다.

> **다만 그 판단은 옵션 하나에 걸려 있었다.** ⒠ 는 같은 온도 조합에서 열팽창만
> 켠 것인데 손상이 **36,622배**로 폭발한다. 베어링만 30 °C 뜨거우면 링이
> 팽창해 내부 틈새가 사라지고 예압이 걸리기 때문이다. 반대로 전체를 균일하게
> 올리면(⒟) 켜도 0.2% 차이뿐이다.
>
> 즉 **온도차 30 °C 가 무해했던 이유는 온도차가 작아서가 아니라 열팽창을 끄고
> 해석했기 때문**이다. 열팽창을 켜는 해석으로 옮겨 갈 때는 샤프트·하우징 온도를
> 반드시 함께 정해야 한다 — 그때는 이 30 °C 가 결과를 지배한다.

*확인 스크립트: `verify_thermal.py`(스크래치) · 옵션 탐색 `probe_thermal.py`*

## 8-8. `D_pw` 상한 효과 확인 (224 × 40)

§8-6.4.2 는 프론트 8/8 이 `D_pw` 상한 4,500 mm 에 붙었음을 보였고,
§8-6.5.5 는 그것이 「답이 상자에 잘린 것」임을 지적했다. **상한을 재설정하기
전에, 상한이 실제로 답을 자르고 있었는지를 먼저 잰다.**

### 8-8.1. 무엇을 바꾸는가 — 한 줄

| 항목         | 부록 8 S3-c                | **§8-8**                      |
| ------------ | -------------------------- | ------------------------------------ |
| `D_pw`     | `[3,300, 4,500]` · 1 mm | **`[4,400, 5,500]` · 1 mm** |
| 그 밖의 전부 | —                         | **그대로**                     |

설계변수 구조·제약·목적함수·비용모델·평가기(두께 규칙 · 코너 반경 4.3 · 정수화)
·개체수 224·시드 1·범주 전수 시딩·연산자를 손대지 않는다. `D_pw` 는 `Choice` 가
아니라 스케일 `Integer` 라 **범주 조합이 224 로 그대로**이고, 그래서 개체수도
시딩도 바꿀 필요가 없다 — `z2` 를 세분하는 §8-6.5.6 ⑴ 과 결정적으로 다른 점이다.

하한을 3,300 이 아니라 **4,400** 으로 올린 것은 탐색을 아끼기 위해서다.
§8-6.4.2 에서 가능해의 61.6% 가 이미 4,490 이상이었고 4,400 미만 구간의 최선
총질량은 110.68 t 로 프론트(105.84 t)에 한참 못 미친다.

### 8-8.2. 예산과 판정

`224 × 40 = 8,960` 평가 · 0.306 s/MASTA 기준 **약 46분**. 프론트를 정밀하게
뽑는 것이 목적이 아니라 **분포가 어디로 몰리는지** 보는 것이 목적이므로 40세대로
끊는다. §8-6.5.4 에서 `f2` 갱신이 대부분 초반 30세대 안에 나온 것이 근거다.

판정은 둘 중 하나로 갈린다.

| 관찰                                      | 뜻하는 것                                            | 다음                                                                               |
| ----------------------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------------- |
| 프론트`D_pw` 가 **내부에 맺힌다** | 4,500 이 답을 자르고 있었다. 참 최적 위치가 나온다   | 그 값을 근거로 상한 재설정                                                         |
| 여전히**상한(5,500)에 붙는다**      | 두께 규칙에 상한 기전이 없다 — 「크면 클수록 좋다」 | 문제는 최적화가 아니라**모델**이다. 제작·하우징 한계를 외부에서 넣어야 한다 |

### 8-8.3. 미리 아는 한계 — 벽이 얇아진다

두께 규칙은 단면계수 `W` 를 고정하므로 보어가 커질수록 벽이 얇아진다.
격자 상단에서 실제로 어디까지 가는지 미리 계산했다(기하 위반 0건 · 두께 규칙은
전 구간 해를 갖는다).

|        `D_pw` |           `d` |           `D` |    벽두께`t` |      `d/t` |
| --------------: | --------------: | --------------: | -------------: | -----------: |
|           4,400 |           3,984 |           4,805 |          123.0 |           32 |
|           4,700 |           4,284 |           5,105 |          104.0 |           41 |
|           5,000 |           4,584 |           5,405 |           90.0 |           51 |
|           5,300 |           4,884 |           5,705 |           78.5 |           62 |
| **5,500** | **5,084** | **5,905** | **72.0** | **71** |

> **상단 구간은 두께 규칙의 외삽 영역이다.** 규칙이 보장하는 것은 DIN 743
> **전체 굽힘 피로**뿐이고, 국부 좌굴·단면 타원화·베어링 좌면 강성·제작성은
> 보장하지 않는다. 빔 요소 모델은 타원화를 계산하지도 못한다. 보어 5.1 m 에
> 벽 72 mm(`d/t` = 71)는 공학적으로 그대로 쓸 수 있는 값이 아니다.
>
> **그럼에도 두께 하한을 걸지 않았다.** 이번 실행의 목적은 「`D_pw` 상한이
> 구속이었는가」 하나를 가리는 것이고, 여기에 인위적 제약을 더하면 `D_pw` 의
> 순수 효과와 두께 하한의 효과가 섞인다. 대신 **결과 표에 벽두께와 `d/t` 를
> 함께 실어** 어느 지점부터 외삽인지 드러낸다.

### 8-8.4. 진행 (5세대 간격 실시간) — `C6` 해제 실행

> **1차 실행(260806 · `C6` 유지)은 아무것도 재지 못했다.** 변수 범위를
> 5,500 까지 열었지만 `C6` 가 `sizing_geom.constraints` 안에서 평가 전에
> 걸러내므로 `D_pw` > 4,500 은 한 점도 MASTA 에 가지 못했다(해석식 탈락
> 1,539건 · 가능해 `D_pw` 최댓값 4,500 · 4,600 이상 기하 통과율 0.0%).
> **`C6` 만 떼고 다시 돌린다** — 나머지 제약은 그대로다.

<!-- A8:DPWLOG -->

| 세대 | 프론트 |      HV | 프론트`D_pw` 최소 |  중앙 |            최대 | 가능해 | 하한 4,400 부착 | 상한 5,500 부착 | 가능해`D_pw` 중앙 | 최소 베어링 [t] | 최소 총질량 [t] |
| ---: | -----: | ------: | ------------------: | ----: | --------------: | -----: | --------------: | --------------: | ------------------: | --------------: | --------------: |
|    5 |      3 | 4,383.9 |               5,287 | 5,455 | **5,455** |  3,448 |            6.7% |  **0.5%** |               4,465 |           14.25 | **78.92** |
|   10 |      7 | 4,478.9 |               5,244 | 5,472 | **5,497** |  4,097 |            5.7% |  **1.6%** |               4,474 |           13.70 | **78.76** |
|   15 |      8 | 4,605.5 |               5,408 | 5,478 | **5,499** |  4,663 |            5.0% |  **3.3%** |               4,489 |           13.22 | **77.15** |
|   20 |      9 | 4,678.4 |               5,446 | 5,487 | **5,500** |  5,181 |            4.5% |  **5.0%** |               4,496 |           12.86 | **76.83** |
|   25 |      8 | 4,683.9 |               5,452 | 5,485 | **5,500** |  5,669 |            4.1% |  **6.5%** |               4,498 |           12.83 | **76.83** |
|   30 |      9 | 4,698.9 |               5,452 | 5,483 | **5,500** |  6,139 |            3.8% |  **8.2%** |               4,854 |           12.74 | **76.83** |
|   35 |     14 | 4,701.3 |               5,452 | 5,494 | **5,500** |  6,586 |            3.5% |  **9.8%** |               5,036 |           12.74 | **76.83** |
|   40 |      9 | 4,714.7 |               5,478 | 5,497 | **5,499** |  7,004 |            3.3% | **11.5%** |               5,179 |           12.74 | **76.42** |

*08-06 18:30 기준 · 40/40 세대 · 부착 판정은 경계에서 10 mm 이내 · 전량은 `부록8_DPW/S3_본최적화/`*

<!-- /A8:DPWLOG -->

**읽는 법** — 「상한 5,500 부착」이 세대가 갈수록 **떨어지면** 최적점이 내부에
있다는 뜻이고, **오르거나 유지되면** 규칙이 계속 큰 `D_pw` 를 밀고 있다는 뜻이다.
「프론트 `D_pw` 최대」가 5,500 에서 내려오는지가 가장 직접적인 신호다.

### 8-8.5. 결과 — `C6` 해제 224 × 40 (260806)

`224 × 40 = 8,960` 평가 · MASTA 8,864회 · **52.0분** · 0.352 s/점. 신규 평가
8,864점 중 가능해가 **46.4%**(4,116점)로, `C6` 를 유지한 1차 실행의 32.5%보다
높다 — 제약 하나가 풀리며 가능영역이 넓어진 것이다.

**프론트 총질량이 §8-6.3a 의 105.84 t 에서 76.42 t 로 −27.8% 내려갔다.**

#### 8-8.5.1. 가능해와 프론트의 `D_pw` 분포 — 둘 다 벽에 붙는다

**가능해**는 4,401 ~ 5,500 mm 에 퍼져 있으나 중앙값 **5,443** · 평균 5,354 이고
**60.3% 가 5,400 이상**에 몰려 있다. 구간별 최선값은 끝까지 단조 개선된다 —
꺾이는 지점이 없다.

| `D_pw` [mm]           |          가능해 |            비율 | 총질량 최소 [t] | 베어링 최소 [t] | 샤프트 최소 [t] | σ 중앙 |
| ----------------------- | --------------: | --------------: | --------------: | --------------: | --------------: | ------: |
| 4,400 ~ 4,499           |              18 |            0.4% |          109.10 |           17.76 |           63.97 |   2,040 |
| 4,600 ~ 4,699           |              30 |            0.7% |          101.23 |           18.15 |           45.57 |   2,035 |
| 4,800 ~ 4,899           |              73 |            1.8% |           95.46 |           16.52 |           42.10 |   2,006 |
| 5,000 ~ 5,099           |             108 |            2.6% |           88.10 |           15.39 |           35.26 |   2,000 |
| 5,200 ~ 5,299           |             323 |            7.8% |           81.84 |           13.70 |           32.89 |   2,032 |
| 5,300 ~ 5,399           |             528 |           12.8% |           80.08 |           13.24 |           31.96 |   2,024 |
| **5,400 ~ 5,500** | **2,482** | **60.3%** | **76.42** | **12.74** | **31.26** |   2,034 |

**프론트 9건은 `D_pw` 5,478 ~ 5,499**(중앙 5,497)로 사실상 상한 한 점이다.

| # | `D_pw` | `d` |          `t` |        `d/t` | α | `D_we` | `L_w` | `z1` | `z2` |          베어링 | 샤프트 |  **합계** |    σ |
| -: | -------: | ----: | -------------: | -------------: | -: | -------: | ------: | -----: | -----: | --------------: | -----: | --------------: | ----: |
| 1 |    5,478 | 5,130 |           70.5 |           72.8 | 22 |    154.3 |   266.5 |    1.0 |    6.0 | **12.74** |  56.81 |           82.30 | 2,098 |
| 2 |    5,498 | 5,152 |           70.0 |           73.6 | 21 |    154.7 |   267.5 |    1.0 |    6.0 |           12.74 |  56.66 |           82.15 | 2,095 |
| 3 |    5,499 | 5,157 | **69.5** | **74.2** | 15 |    168.2 |   275.3 |    1.0 |    6.0 |           12.82 |  56.32 |           81.97 | 2,098 |
| 4 |    5,493 | 5,153 |           70.0 |           73.6 | 19 |    137.8 |   291.8 |    1.0 |    5.5 |           13.93 |  52.31 |           80.18 | 2,097 |
| 5 |    5,497 | 5,103 |           71.5 |           71.4 | 28 |    170.8 |   281.8 |    1.0 |    5.0 |           15.29 |  48.49 |           79.07 | 2,097 |
| 6 |    5,498 | 5,125 |           70.5 |           72.7 | 29 |    136.4 |   289.2 |    1.0 |    5.0 |           15.31 |  48.03 |           78.64 | 2,098 |
| 7 |    5,481 | 5,070 |           72.5 |           69.9 | 30 |    170.3 |   294.3 |    1.0 |    4.5 |           16.70 |  44.39 |           77.79 | 2,098 |
| 8 |    5,485 | 5,087 |           71.5 |           71.1 | 29 |    152.0 |   301.0 |    1.0 |    4.5 |           16.81 |  43.94 |           77.56 | 2,095 |
| 9 |    5,497 | 5,083 |           72.0 |           70.6 | 30 |    153.7 |   313.5 |    1.0 |    4.0 |           18.32 |  39.78 | **76.42** | 2,095 |

> **⚠ 이 9건은 그대로 쓸 수 없다.** 벽두께가 **69.5 ~ 72.5 mm**, `d/t` 가
> **69.9 ~ 74.2** 다. §8-6.3a 프론트(116 ~ 123 mm · `d/t` 32 ~ 35)의 **절반 이하**
> 이고, 두께 규칙이 보장하는 것은 DIN 743 **전체 굽힘 피로뿐**이다(§8-8.3).
> 국부 좌굴·단면 타원화·베어링 좌면 강성·제작성은 어느 것도 확인되지 않았고,
> 빔 요소 모델은 타원화를 계산조차 하지 않는다. 외경도 5,905 ~ 5,996 mm 다.
>
> **따라서 −27.8% 는 「달성 가능한 감량」이 아니라 「`C6` 가 붙들고 있는 상한」의
> 크기다.** 제작·운송 완화를 협의할 때 「그 대가가 이 정도」라고 말하기 위한
> 수치이지, 채택 후보가 아니다.

**두 분포의 관계가 시사적이다.** 가능해는 4,400 부근에도 18점이 살아 있지만
프론트에는 한 점도 남지 못한다. `D_pw` 가 작은 해는 **제약을 못 지켜서가 아니라
지배당해서** 사라진다 — σ 도 2,040 으로 한도 안이다.

#### 8-8.5.2. 왜 상한으로 수렴하는가 — 물리

가능해 4,116점에서 `D_pw` 와의 상관은 이렇다.

| 상대         |              상관 | 뜻                                   |
| ------------ | ----------------: | ------------------------------------ |
| `d`(보어)  |  **+0.978** | `D_pw` 가 곧 보어다                |
| 샤프트 질량  | **−0.449** | 보어가 커지면 가벼워진다             |
| 베어링 질량  | **−0.383** | 베어링도 가벼워진다                  |
| 총질량       | **−0.614** | 둘 다                                |
| **σ** |  **+0.105** | **사실상 무관 — 벌점이 없다** |

**기전이 둘 겹쳐 있다.**

**⑴ 하중 쪽** — 전동체 하중은 대략 `M / D_pw` 로 준다. 피치원을 키우면 같은
모멘트에서 롤러 하나가 받는 힘이 줄어 σ 여유가 생기고, 그 여유를 전동체를
줄이는 데 쓴다. 프론트에서 베어링이 18.32 → **12.74 t** 로 가벼워지는 것이 이것이다.

**⑵ 샤프트 쪽** — 두께 규칙이 단면계수 `W` 를 고정하므로 `OD` 가 커지면 벽이
얇아지고 단면적이 대략 `OD⁻²` 로 준다(§8-6.4.1). 프론트에서 샤프트가
63.97 → **31.26 t** 로 절반이 된다.

**두 기전이 같은 방향이고, 그것을 막을 제약이 없다.** σ 는 `D_pw` 와 무관하고
(+0.105), 세장비·살두께 제약은 베어링 내부 비율만 본다. **`D_pw` 를 키워서
나빠지는 것이 모델 안에 하나도 없다.**

#### 8-8.5.3. 왜 상한으로 수렴하는가 — 알고리즘

NSGA-II 는 **지배 관계만** 본다. `D_pw` 증가가 두 목적을 동시에 개선하면 새 해가
기존 해를 **지배**하므로 집단이 통째로 상한 쪽으로 밀린다. 밀림을 멈추는 것은
경계뿐이다. 세대별 궤적이 그 밀림을 그대로 보여준다.

|         세대 | 프론트`D_pw` 최소 |            중앙 |            최대 | 가능해 중**상한 부착** |                HV |
| -----------: | ------------------: | --------------: | --------------: | ---------------------------: | ----------------: |
|            5 |               5,287 |           5,455 |           5,455 |                        0.46% |           4,383.9 |
|           10 |               5,244 |           5,472 |           5,497 |                        1.56% |           4,478.9 |
|           20 |               5,446 |           5,487 |           5,500 |                        4.96% |           4,678.4 |
|           30 |               5,452 |           5,483 |           5,500 |                        8.24% |           4,698.9 |
| **40** |     **5,478** | **5,497** | **5,499** |             **11.54%** | **4,714.7** |

*부착 판정은 경계에서 10 mm 이내. 가능해 통계에는 1차 실행에서 물려받은 워밍
캐시 7,341점(전부 `D_pw` ≤ 4,500)이 섞여 있어 부착률이 낮게 나온다 — 이번 실행이
새로 평가한 8,864점만 보면 60.3% 가 5,400 이상이다.*

프론트는 5세대에 이미 5,287 이상이고, 이후 40세대까지 **한 번도 아래로 내려오지
않는다.** 부착률은 단조 증가한다.

이것은 알고리즘의 결함이 아니라 **문제 정의의 성질이 그대로 드러난 것**이다.
목적함수 두 개가 모두 `D_pw` 에 대해 단조 감소하는 한, 어떤 다목적 알고리즘을
쓰더라도 답은 상한이다. 실제로 `C6` 를 4,500 에서 5,500 으로 옮기자 프론트가
그대로 5,500 으로 따라왔다 — **상한이 답을 정한다는 사실만 재확인된 것**이다.

> **결론: 이 문제에서 `D_pw` 는 최적화가 정할 수 있는 변수가 아니다.**
> 설계변수 목록에 들어 있지만 실질적으로는 **제약이 정하는 값**이다. 상한을 더
> 열어 보는 것은 같은 답을 한 번 더 얻는 일이라 의미가 없다. 필요한 것은
> **모델 밖의 근거** 둘이다.
>
> - **`C6` 의 실제 한계** — 4,500 mm 가 제작·운송의 어느 조건에서 나온 값인지,
>   얼마까지 완화 가능한지. 이것이 정해지면 `D_pw` 는 자동으로 결정된다.
> - **두께·형상 제약** — `d/t` 상한이든 절대 두께 하한이든, 두께 규칙이
>   보장하지 못하는 좌굴·타원화·좌면 강성을 대신할 제약이 있어야 한다. 없으면
>   `D_pw` 를 얼마로 열든 벽이 그만큼 얇아진다. 이번 프론트의 `d/t` 70 ~ 74 가
>   그 증거다.

#### 8-8.5.4. `z1`·`z2` 가 고정되면 왜 프론트가 안 생기는가

§8-6.5.3 은 「같은 `z2` 안에서 두 목적의 상관이 +0.96 이상」이라고만 적었다.
이번 자료가 **그 이유**를 보여준다.

| 고정 범위                               |     표본 | `f1`↔`f2` 상관 | 베어링 질량 폭 | **샤프트 질량 폭** |
| --------------------------------------- | -------: | ------------------: | -------------: | -----------------------: |
| `z1`·`z2`                          | 33 ~ 963 |     +0.966 ~ +0.992 |      22 ~ 35 t |                 8 ~ 23 t |
| **`z1`·`z2`·`D_pw`(±5)** | 22 ~ 110 |    **+1.000** |      11 ~ 23 t |  **1.26 ~ 2.79 t** |

**총질량은 정의상 `f2 = 2·f1 + m_shaft` 다.** 두 목적이 갈라지려면 `m_shaft` 가
`f1` 과 **독립적으로** 움직여야 한다. 그런데

- `m_shaft` 는 `d` 와 `z2` 로 결정된다(두께 규칙이 `W` 를 고정하므로 `d` 하나로
  단면이 정해진다)
- `d` 는 `D_pw` 가 지배한다(상관 **+0.978**)

따라서 **`D_pw` 와 `z2` 를 고정하면 `m_shaft` 가 사실상 상수**가 되고,
`f2 = 2·f1 + const` 라는 **완전 선형 관계**가 된다 — 실측 상관 **+1.000**,
샤프트 질량 폭 **1.26 ~ 2.79 t**(베어링 폭의 1/10). 이 슬라이스 안에서는
비지배집합이 **원리적으로 한 점**이다.

**베어링 제원을 미세 조정해도 소용없는 이유가 이것이다.** `D_we`·`L_w`·α 는
`d = D_pw − D_we·cosα − L_w·sinα − 2t_i` 를 통해 `d` 에 영향을 주지만, 그 폭이
`D_pw` 가 만드는 폭에 비해 미미하다(§8-6.5.2 에서 88 mm 대 1,100 mm). 즉
**베어링 제원은 `f1` 만 움직이고 `f2` 는 그 두 배로 따라 움직인다.**

> **그래서 프론트 점수는 `z2` 수준 수를 넘지 못한다.** 이번 프론트 9건이
> `z2` = 6.0(3점) · 5.5(1) · 5.0(2) · 4.5(2) · 4.0(1) 으로 **5수준에 흩어진**
> 것이 그 증거다. §8-6.5.3 에서 본 구조가 `D_pw` 영역을 1,000 mm 옮겨도 그대로
> 재현됐다.
>
> 이것은 §8-6.5.6 ⑴(`z2` 격자 세분)이 왜 유일하게 효과적인 처방인지도 말해
> 준다. **`z2` 만이 두 목적을 실제로 갈라 놓는 변수**이기 때문이다.

*산출: `부록8_DPW/S3_C6해제/` · 분석 `a8_dpw_anal.py`*
...
| :------------: | ---------------------------- | ---------------------------------------------: |
| **S3-a** | 스크립트 + 배선 시험         |                               ✅**완료** |
| **S3-b** | 드라이런 224 × 5 (실 MASTA) |          ✅**완료** · 5.9분 · 5/5 통과 |
|      S3-c      | 본 최적화 224 × 150         | MASTA 33,330회 × 0.320 s ≈**2.96시간** |

## 8-10. 스크립트

| 스크립트              | 내용                                                                                 |
| --------------------- | ------------------------------------------------------------------------------------ |
| `a8_eval.py`        | 평가기 —`nsga_eval.Evaluator` 상속 · 두께 규칙 · 코너 반경 · `L_we`/2β 기록 |
| `a8_s3_run.py`      | 부록 6`nsga_s3_run` 을 평가기·산출 폴더만 바꿔 재사용                             |
| `a8_s3_wiretest.py` | 배선 시험 (대용 평가기 · MASTA 0회)                                                 |

기반 평가기 `nsga_eval.Evaluator` 에는 확장 지점 넷(`FIELDS` · `shaft_of()` ·
`tweak()` · `finish()`)만 새로 뚫었고 기본 동작은 항등이라, **부록 1 ~ 7 의 결과는
그대로 재현된다.** 산출은 `부록8_NSGA/` 아래에 부록 6 과 같은 이름으로 쌓인다.

# 부록 9. 추가 최적화 — 외경 `D` 기준 사이징

> 절 번호 `9-x` 는 부록 9 안에서만 쓴다. 본문 §9(미확정 사항)와는 별개다.

## 9-1. 무엇을 바꾸는가

부록 8 이 남긴 진단에 각각 대응한다.

| 진단                                                              | 출처                  | 부록 9 의 처방                                          |
| ----------------------------------------------------------------- | --------------------- | ------------------------------------------------------- |
| 목적`f2`(총질량)가 `f1`(베어링)을 포함해 지배 판정이 뭉개진다 | §8-6.5.7 ⑵          | **목적을 `(외경 D ↓, 베어링 1개 질량 ↓)` 로** |
| 제약이 대리변수`D_pw` 에 걸려 있다                              | §8-6.5.7 ⑼          | **`C6` 를 `D ≤ 5,000 mm` 로 대체**           |
| `D_pw` 상한이 답을 정한다                                       | §8-8.5.3             | `D` 를 목적에 넣어 **상한별 곡선**으로          |
| `z1` 은 프론트에서 늘 하한을 택한다                             | §8-6.3a · §8-8.5.1 | **`z1` = 1.0 m 고정**                           |
| α 가`Choice` 라 연속적 이동이 어렵다                           | —                    | **α 를 `Integer` 로**                          |

**핵심은 목적함수다.** `(D, 베어링)` 은 **제작 가능 크기**와 **부품 크기**를
맞세운다. 프론트가 곧 **「외경 `D` 를 얼마로 제한하면 베어링을 얼마나 가볍게
만들 수 있는가」 곡선**이 되고, 4.0 · 4.5 · 5.0 m 는 그 곡선에서 읽는 세 점이다.

그 밖은 부록 8 을 그대로 계승한다 — 두께 규칙 · 코너 반경 4.3 mm · 정수화 ·
세장비 밴드 재매개화 · 시드 1 · 연산자.
...
**두 목적은 실재로 상충한다.** 피치원을 키우면 롤러 하중이 `M/D_pw` 로 줄어
베어링을 작게 할 수 있지만(§8-8.5.2), `D` 는 `D_pw` 를 따라 커진다(상관 +0.98).
`D` 를 조이면 그만큼 베어링을 키워야 σ ≤ 2,100 을 지킨다.

**총질량·샤프트 질량은 목적이 아니다.** 프론트 표에 열로 실어 **선택 단계**에서
본다(§8-6.5.7 ⑹ 과 같은 운영). 총질량은 `2·f2 + m_shaft` 로 정확히 복원된다.

> **`z2` 를 고정하는 근거가 여기서 나온다.** `z2` 는 `D` 에 전혀 들어가지 않고
> 베어링 질량과는 **−0.403** 으로 음의 상관이다 — 스팬이 길수록 베어링 하중이
> 줄어 `f2` 만 좋아지고 `f1` 은 그대로다. 따라서 **상한에 박힌다.**
> 기존 가능해 17,714점에서 `(D, 베어링)` 프론트를 뽑으면 `z2` 가 **33 / 33
> 전부 6.0** 이었다(`D` ≤ 5,000 로 한정해도 20 / 20).
>
> 대가는 샤프트다 — `z2` = 6.0 이면 샤프트가 최대로 무거워진다. 다만
> `D` ≤ 5,000 에서 총질량 손실은 **+1.45 t (1.4%)** 로 작다(107.28 대 105.84).

## 9-3. 설계변수

| 변수     | 자료형                | 범위                               |   수준 | 부록 8 대비                         |
| -------- | --------------------- | ---------------------------------- | -----: | ----------------------------------- |
| `z1`   | **고정**        | **1.0 m**                    |      1 | `Choice [1.0, 1.5]` → 고정       |
| `z2`   | **고정**        | **6.0 m**                    |      1 | `Choice`(7) → 고정 (§9-2 ▸)    |
| α       | **`Integer`** | 15 ~ 30° · 1°                   |     16 | `Choice` → **`Integer`** |
| `D_pw` | `Integer`           | **3,300 ~ 4,800 mm** · 1 mm |  1,501 | 상한 4,500 →**4,800**        |
| `D_we` | `Integer`           | 110.0 ~ 230.0 mm · 0.1 mm         |  1,201 | 그대로                              |
| `u`    | `Integer`           | 0 ~ 10⁴                           | 10,001 | 그대로                              |

`L_w`(롤러 전장)는 `u` 로 **밴드 재매개화**된다 — `L_w = lo + round(u(hi−lo)/10⁴)`,
`[lo, hi]` 는 `[1.5·D_we, 2.5·D_we]` 를 0.1 mm 격자에 맞춘 값이다(§6-11.3).

**범주가 사라졌다.** `z1`·`z2` 가 고정되고 α 가 `Integer` 로 옮겨 가면서 `Choice`
변수가 없다 — 순수 `Integer` 4변수 문제가 된다. 부록 6 이래의 **범주 전수 시딩
제약이 없어지므로 개체수를 자유롭게 정할 수 있다**(§9-6).

**`D_pw` 상한을 4,800 으로 올린 이유** — `C6` 가 `D` 기준으로 바뀌면서 `D_pw`
4,500 은 물리 제약이 아니라 탐색 범위가 됐다. 여유(`D − D_pw`)의 이론 최솟값이
216 mm 이므로 `D` = 5,000 에 닿으려면 `D_pw` 가 **4,784** 까지 열려야 한다
(§8-6.5.7 ⑼-2). 4,800 은 그 값을 덮는다. 넘는 조합은 `C6` 가 해석식으로 걸러내
MASTA 를 부르지 않는다.

## 9-4. 종속변수

설계변수에서 해석식 또는 MASTA 로 자동 결정되는 값이다. **부록 8 에서 바뀐 것은
없다.**

| 종속변수                                    | 산출                                                                     | 근거                                              |
| ------------------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------- |
| 조립폭`T` · 내륜폭 `B` · 외륜폭 `C` | `1.30226` · `1.26025` · `1.06281` × `L_w` · 1 mm 반올림      | §6-4 (v1.3 실측 역산)                            |
| 보어`d`                                   | `D_pw − D_we·cosα − L_w·sinα − 2t_i` · 1 mm                    | `t_i = 0.15652·B`                              |
| **외경 `D`**                        | `D_pw + D_we·cosα + L_w·sinα + 2t_o` · 1 mm                       | `t_o = 0.17215·C` — **이제 목적함수다** |
| 롤러 수`Z`                                | MASTA 자동                                                               | —                                                |
| **샤프트 내경**                       | **`floor((d⁴ − 32·W·d/π)^¼)`** · `W` = 1.393×10⁹ mm³ | §7-6.7.5 · 목표 SF 5.2                          |
| 샤프트 길이                                 | `z2 + 0.5 m` = 6.5 m (고정)                                            | `SHAFT_TAIL`                                    |
| **롤러 코너 반경**                    | **좌·우 각 4.3 mm 주입**                                          | §8-3 ②                                          |
| **유효 롤러 길이 `L_we`**           | MASTA 산출 =`L_w − 8.6 mm`                                            | §8-3 ②                                          |
| 테이퍼각 2β · cone angle                  | MASTA 산출값을 읽어 기록                                                 | §8-3 ③                                          |
| 베어링 질량 · 샤프트 질량                  | MASTA 실측                                                               | §6-6                                             |
| σ_max                                      | MASTA — 지배 LC`Myz_max` 22,673 kNm                                   | §8-1.2 · §8-4.2                                |

## 9-5. 제약

| #             | 제약                                                     | 판정            |        표본 60,000 활성 | 부록 8 대비                                                    |
| ------------- | -------------------------------------------------------- | --------------- | ----------------------: | -------------------------------------------------------------- |
| **C2**  | 세장비`1.5 ≤ L_w / D_we ≤ 2.5`                       | 해석식          |              2 (0.003%) | 그대로 — 밴드 재매개화가 구조적으로 보장                      |
| ~~C4~~       | ~~스팬 `z2 − z1 ≥ 1.5 m`~~                          | —              |                      — | **삭제** (자동 만족 · 5.0 m)                            |
| ~~C5~~       | ~~`z1 ≥ 0.3 m`~~                                     | —              |                      — | **삭제** (자동 만족 · `z1` = 1.0)                     |
| **C6**  | ~~`D_pw ≤ 4,500 mm`~~ → **`D ≤ 5,000 mm`** | 해석식          | **8,321 (13.9%)** | **변경**                                                 |
| C7            | 축방향 간섭`z2 − z1 ≥ T + 0.1 m`                     | 해석식          |                       0 | **자동 만족** (5.00 ≥ 0.816)                            |
| C8            | `z1 − T/2 ≥ 0`                                       | 해석식          |                       0 | **자동 만족** (1.0 ≥ 0.358)                             |
| C9            | `T/2 ≤ SHAFT_TAIL`                                    | 해석식          |                       0 | **자동 만족** (0.358 ≤ 0.5)                             |
| **C12** | 궤도륜 살두께`≥ 0.20 · D_we`                         | 해석식          |                       0 | 자동 만족 — 세장비 하한 1.5 가`t/D_we ≥ 0.274` 보장(§4-4) |
| **σ**  | `σ_max ≤ 2,100 MPa`                                  | **MASTA** |                      — | 그대로                                                         |

**`C4`·`C5` 는 삭제한다.** `z1` = 1.0 · `z2` = 6.0 에서 정의상 항상 만족한다.

**`C7`·`C8`·`C9`·`C12` 는 남기되 「자동 만족」으로 표기한다.** 표본 60,000점에서
한 번도 활성화되지 않았지만 이는 **`z1`·`z2` 를 고정한 결과**다. 되살리거나
`L_w` 상한을 올리면 다시 필요하므로 코드에서 빼지 않는다 — 판정 비용은 0 이다.

> **실질적으로 살아 있는 해석식 제약은 `C6` 하나다.** 해석식 탈락률 13.9% 가
> 전부 `C6` 이고 `C2` 는 0.003%(밴드 재매개화의 정수 반올림 잔차)다. 부록 8 의
> 탈락 0건과 대비되는데, `D_pw` 상한을 4,800 으로 열면서 `D` 5,000 을 넘는
> 조합이 탐색 공간에 들어왔기 때문이다.

**`C6` 를 왜 바꾸는가** — 제작 가능 크기를 정하는 것은 피치원이 아니라
**바깥지름**이다(선반 스윙 · 열처리로 · 운송 폭). `D_pw` 기준은 대리변수였다.

> **바꿔도 기존 결과와 어긋나지 않는다.** §8-6.3a 프론트 8건의 외경이
> 4,900 ~ **4,995 mm** 로 전부 5,000 바로 아래다 — **현행 `C6`(`D_pw` ≤ 4,500)는
> 사실상 `D ≤ 5.0 m` 로 작동해 왔다.** 기준을 옳은 물리량으로 옮겨 다는 것이지
> 한계를 완화하는 것이 아니다.

피로(ΣD30 ≤ 0.5)는 루프에서 뺀다 — 부록 8 S4-d 에서 프론트 14건이 50 · 70 °C
양쪽 모두 합격했다(§8-7.3). 프론트 확정 후 검증으로 돌린다.

## 9-6. 적용 파라미터 (as-run 예정)

§6-11.5 의 as-run 표에서 **재현에 꼭 필요한 것만** 추렸다. 적지 않은 항목은
§6-11.5 와 같다.

| 구분     | 항목                     | 적용값                                                                         | 근거                                  |
| -------- | ------------------------ | ------------------------------------------------------------------------------ | ------------------------------------- |
| 알고리즘 | 라이브러리               | `pymoo 0.6.2`                                                                | §6-2                                 |
| 알고리즘 | 방법                     | **NSGA-II**                                                              | 2목적 정석                            |
| 알고리즘 | **개체수**         | **100**                                                                  | 아래 ▸                               |
| 알고리즘 | **세대수**         | **150 (고정)**                                                           | 조기 종료 미채택 (R14 항목 24)        |
| 알고리즘 | 총 평가                  | **15,000**                                                               | `100 × 150`                        |
| 알고리즘 | 시드                     | 1                                                                              | 단일 시드 — 한계는 미결#9            |
| 알고리즘 | 초기집단                 | **무작위** (범주 없음)                                                   | §9-3 ▸                              |
| 알고리즘 | 교차·변이 (`Integer`) | SBX`eta=15`·`prob=0.9`·`prob_var=0.5` / PM `eta=20`·개체당 `0.9`  | pymoo 기본값                          |
| 알고리즘 | 생존 · 중복제거 · 종료 | `RankAndCrowdingSurvival` · **끔** · 세대수 도달                     | §6-11.5 동일                         |
| 알고리즘 | HV 기준점                | **`(5.5 m, 45 t)`**                                                    | 목적이 바뀌어`(40, 250)` 을 못 쓴다 |
| 문제     | 목적                     | **`f1` 외경 `D` ↓ · `f2` 베어링 1개 질량 ↓**                    | §9-2                                 |
| 문제     | 설계변수                 | α·`D_pw`·`D_we`·`u` **전부 `Integer`** (`z1`·`z2` 고정) | §9-3                                 |
| 문제     | 제약                     | 해석식 6개(**실질 `C6` 하나**) + σ ≤ 2,100                           | §9-5                                 |
| 문제     | 종속변수                 | 두께 규칙 · 코너 반경 4.3 · 정수화 켬                                        | §9-4                                 |
| 실행     | 지배 LC                  | `Myz_max` 22,673 kNm · 듀티사이클 래핑                                      | §8-1.2                               |
| 실행     | 모델                     | v1.4 · 50 °C ·`DIN_LUNDBERG` · 무프리로드                                | §3                                   |
| 실행     | 사전 게이트              | v1.3 기준선 8/8 (σ 3,407.9 · 샤프트 58,384.5 kg)                             | §8-5.1                               |
| 실행     | **예산 추정**      | MASTA 약**12,900회** × 0.32 s ≈ **1.15시간**                     | 아래 ▸                               |

**개체수 100 의 근거** — 범주가 사라져 「전수 시딩 = 개체수」 제약이 없다.
남은 것은 `Integer` 4변수 · 2목적 문제이고, 프론트가 `D` 축을 따라 연속으로
퍼지므로 개체수는 **원하는 프론트 해상도**가 정한다. 100개면 `D` 4.2 ~ 5.0 m
구간을 8 mm 간격으로 덮는 셈이라 충분하다. §8-6.5.4 에서 「개체수보다 세대가
구속」임이 확인됐으므로 세대 150 을 유지하고 개체를 줄였다.

|        개체수 |          세대 |           총평가 |       MASTA 추정 |        시간 추정 |
| ------------: | ------------: | ---------------: | ---------------: | ---------------: |
| **100** | **150** | **15,000** | **12,915** | **1.15 h** |
|           130 |           150 |           19,500 |           16,790 |           1.49 h |
|           224 |           150 |           33,600 |           28,930 |           2.57 h |

*0.32 s/MASTA · 해석식 통과 86.1% 기준. 실측은 부록 8 S3-c 0.306 · §8-8 0.352 s/점.*

## 9-7. S3-a — 배선 시험 (260807)

MASTA 를 결정론적 대용식으로 갈아끼우고 `100 × 4` 를 돌려 배선만 확인했다
(`a9_s3_wiretest.py` · MASTA 0회).

|  # | 점검                          | 결과                                                                              |
| -: | ----------------------------- | --------------------------------------------------------------------------------- |
| ① | `z1`·`z2` 고정           | **`z1` = 1.0 · `z2` = 6.0**                                            |
| ② | 목적이`(D, 베어링)` 인가    | `f1` **4.080 ~ 4.898 m** · `f2` **7.12 ~ 18.66 t** · 프론트 9점 |
| ③ | α 가`Integer` 인가         | 15 ~ 30 정수 · 서로 다른 값**16종**                                        |
| ④ | `D_pw` 4,800 · `C6` 작동 | `D_pw` 최대 **4,634** (>4,500) · `D` 최대 **4,999** (≤5,000)    |
| ⑤ | `C4`·`C5` 제거           | 활성 제약`C2`·`C6`·`C12` — **`C4`·`C5` 없음**                 |
| ⑥ | 밴드·두께규칙·산출 열       | `L_w` 격자 이탈 **0** · 세장비 1.5059 ~ 2.4881 · 질량·진단 열 포함     |

**판정 통과.** 해석식 탈락 7.8%(전부 `C6`)로 §9-5 의 추정(13.9%)보다 낮은데,
탐색이 이미 `D` 를 줄이는 방향으로 움직인 결과다.

> **대용 평가기를 한 번 고쳤다.** 처음 식에는 `D_pw` 항이 없어 작은 `D` 와
> 가벼운 베어링이 동시에 가능해졌고 **프론트가 1점으로 무너졌다.** 실제 기전
> (피치원이 커지면 롤러 하중이 `M/D_pw` 로 준다 · §8-8.5.2)을 넣어
> `σ ∝ (4.5/D_pw)^1.6` 항을 더하자 프론트가 9점으로 퍼졌다. **대용식이라도
> 목적 간 상충의 방향은 맞아야 배선 시험이 의미를 갖는다.**

## 9-8. S3-b — 드라이런

`100 × 5 = 500` 평가 · 실 MASTA **421회** · **2.1분** · **0.300 s/점**.

|  # | 점검                | 결과                                                                      |
| -: | ------------------- | ------------------------------------------------------------------------- |
| ① | 초기집단 α 분포    | **16종 15 ~ 30** · `(z1, z2)` 1조합 고정                         |
| ② | `L_w` 0.1 mm 격자 | 420점 중 이탈**0**                                                  |
| ③ | 세장비 밴드         | 1.5062 ~ 2.4950 ·**경계집중 0.0%**                                 |
| ④ | `C6` 외경 상한    | `D` 3,619 ~ **4,999 mm** (≤ 5,000)                               |
| ⑤ | 프론트              | **15건** · `D` 4,201 ~ 4,951 · 베어링 **17.31 ~ 28.40 t** |
| ⑥ | 체크포인트          | 생성                                                                      |

**판정 통과.** 사전 게이트도 v1.3 기준선 8/8 일치했다.

### 9-8.1. 게이트를 하나 고쳤다

첫 시도는 **「시딩 커버리지 16 / 1」** 로 실패했다(exit 3). 부록 6 판의 검사가
`(z1, z2, α)` 조합 수를 `N_CAT` 과 비교하는데, 부록 9 는 **범주가 없어** 그
비교가 성립하지 않는다 — α 는 `Integer` 로 16종이 나오고 `N_CAT` 은 1 이다.

그 자리에 **「초기집단 α 분포」**(α 가 15 ~ 30 을 폭넓게 덮는가)와 **「`C6` 외경
상한」**(평가된 `D` 가 5,000 을 넘지 않는가)을 넣었다. 나머지 셋(격자 · 밴드 ·
체크포인트)은 그대로다. **검사를 느슨하게 한 것이 아니라, 없어진 범주 대신
그 자리를 지키던 성질을 다른 지표로 옮긴 것**이다.

### 9-8.2. 읽을 것

**⑴ 프론트가 5세대 만에 15건으로 퍼졌다.** 부록 8 S3-c 는 150세대를 돌고도
8건이었다(§8-6.3a). 목적을 `(D, 베어링)` 으로 바꾸자 **`D` 가 연속변수라 프론트가
연속적으로 채워진다** — §8-6.5.7 ⑵ 가 예측한 대로다.

**⑵ 두 목적이 실제로 상충한다.** `D` 4,201 → 4,951 mm 에서 베어링이
28.40 → **17.31 t** 로 준다. 외경을 750 mm 조이면 베어링이 **11.1 t 무거워진다** —
이것이 곧 「제작 한계의 가격표」이고, 부록 9 가 답하려는 질문 그 자체다.

**⑶ 캐시가 완전히 재현된다.** 게이트를 고친 뒤 다시 돌렸을 때 **MASTA 0회 ·
캐시적중 453** 으로 세대별 HV 가 소수점까지 동일하게 재생됐다(28.808 → 32.461).
§6-11.4 의 재현성이 부록 9 에서도 성립한다.

**⑷ 해석식 탈락 9.6%** (48 / 500) — 전부 `C6` 다. §9-5 의 무작위 표본 추정
13.9% 보다 낮은데, 탐색이 이미 `D` 를 줄이는 쪽으로 움직인 결과다.

### 9-8.3. 본런 예산

**MASTA 약 12,630회 × 0.300 s ≈ 1.05시간** (드라이런 실측 기반). §9-6 의
추정 1.15시간과 9% 차이다.

## 9-9. S3-c — 본 최적화

### 9-9.1. 실행 요약 (10세대 실시간)

`100 × 150 = 15,000` 평가 · MASTA **12,975회**(86.5%) · 캐시적중 1,786 ·
해석식 탈락 240(1.6%) · 밴드경계 1 · **62.1분 · 0.287 s/점**. §9-8.3 의 추정
1.05시간과 1.4% 차이다. 가능해 **49.5%**(6,423 / 12,975).

<!-- A9:GENLOG -->

| 세대 | 프론트 |     HV |   ΔHV | **최소 `D`** [mm] | **최소 베어링** [t] | 최소 총질량 [t] |
| ---: | -----: | -----: | -----: | ------------------------: | ------------------------: | --------------: |
|   10 |     20 | 32.989 |     — |           **4,201** |           **17.31** |          107.29 |
|   20 |     44 | 34.363 | +1.374 |           **4,153** |           **17.17** |          107.72 |
|   30 |     48 | 34.943 | +0.580 |           **4,130** |           **17.13** |          107.74 |
|   40 |     84 | 35.784 | +0.841 |           **4,088** |           **17.13** |          107.74 |
|   50 |     85 | 36.403 | +0.619 |           **4,059** |           **17.09** |          107.90 |
|   60 |    100 | 36.438 | +0.035 |           **4,059** |           **17.09** |          107.90 |
|   70 |    100 | 36.523 | +0.085 |           **4,057** |           **17.09** |          107.90 |
|   80 |     97 | 36.614 | +0.091 |           **4,054** |           **17.09** |          107.85 |
|   90 |    100 | 36.667 | +0.053 |           **4,054** |           **17.09** |          107.83 |
|  100 |    100 | 36.686 | +0.019 |           **4,054** |           **17.09** |          107.83 |
|  110 |    100 | 36.693 | +0.007 |           **4,054** |           **17.08** |          107.78 |
|  120 |    100 | 36.898 | +0.205 |           **4,054** |           **16.72** |          104.83 |
|  130 |    100 | 36.945 | +0.048 |           **4,053** |           **16.69** |          104.96 |
|  140 |    100 | 36.973 | +0.028 |           **4,052** |           **16.69** |          104.96 |
|  150 |     98 | 37.012 | +0.039 |           **4,051** |           **16.69** |          104.96 |

*08-07 14:34 기준 · 150/150 세대 · HV 기준점 (5.5 m, 45 t) · 총질량은 목적이 아니라 참고값이다*

<!-- /A9:GENLOG -->

**읽는 법** — `최소 D` 가 내려가면 프론트가 작은 외경 쪽으로 뻗는 것이고,
`최소 베어링` 이 내려가면 큰 외경 쪽 끝이 개선되는 것이다. 총질량은 목적이
아니므로 **참고값**이다 — `z2` 가 6.0 으로 고정돼 샤프트가 최대이기 때문에
부록 8 의 105.84 t 보다 나쁠 것으로 본다(§9-2 ▸).

### 9-9.2. 결과

<!-- A9:RES -->

![부록 9 파레토 프론트](Results/사이징최적화/figures/pareto_a9_D.png)

회색 점선이 `D` 상한 4.0 · 4.5 · 5.0 m, 초록 별이 각 상한에서의 **최소 베어링 질량**이다.

#### 9-9.2.1. `D` 상한별 최소 베어링

*맨 아래 행은 상한이 아니라 **프론트가 도달한 최소 외경**이다 — 그 이하로는 가능해가 없다.*

| `D` 상한 [mm] |      가능해 |    그때`D` [mm] | **베어링** [t] | 샤프트 [t] | 총질량 [t] | `D_pw` [mm] | d [mm] | **t** [mm] | **d/t** | α [°] | 2β [°] | `D_we` [mm] | `L_we` [mm] | 세장비 |  Z | z1 [m] | z2 [m] | σ_max [MPa] | **σ 여유** [MPa] |
| --------------: | ----------: | ----------------: | -------------------: | ---------: | ---------: | ------------: | -----: | ---------------: | ------------: | ------: | -------: | ------------: | ------------: | -----: | -: | -----: | -----: | -----------: | ----------------------: |
| **4,000** | **0** | **해 없음** |                   — |         — |         — |            — |     — |               — |            — |      — |       — |            — |            — |     — | — |     — |     — |           — |                      — |
| **4,500** |       2,285 |             4,500 |      **20.52** |       85.8 |     126.83 |         4,055 |  3,598 |  **156.5** |          23.0 |      16 |    1.308 |         174.9 |         422.6 |  2.465 | 67 |    1.0 |    6.0 |      2,099.6 |                     0.4 |
| **5,000** |       6,423 |             4,975 |      **16.69** |       71.6 |     104.96 |         4,581 |  4,177 |  **110.5** |          37.8 |      16 |    1.143 |         171.7 |         347.6 |  2.075 | 77 |    1.0 |    6.0 |      2,094.8 |                     5.2 |
| *(도달 하한)* |           1 |   **4,051** |      **27.59** |      117.2 |     172.37 |         3,491 |  2,916 |  **279.0** |          10.5 |      16 |    1.901 |         223.1 |         529.4 |  2.412 | 45 |    1.0 |    6.0 |      2,098.8 |                     1.2 |

#### 9-9.2.2. 프론트 대표점 16건

프론트 **53건**을 `D` **50 mm 간격**으로 솎았다 — 양 끝과 `D` 상한 3점은 간격과 무관하게 넣었다. `#` 는 전량 기준 순번이다. ★ = `D` 상한 3점 · `t` 는 두께 규칙이 만든 샤프트 벽두께 · σ 여유 = 2,100 − σ_max

|     # | **베어링** [t] | 샤프트 [t] | 총질량 [t] |      `D` [mm] | `D_pw` [mm] | d [mm] | **t** [mm] | **d/t** | α [°] | 2β [°] | `D_we` [mm] | `L_we` [mm] | 세장비 |  Z | z1 [m] | z2 [m] | σ_max [MPa] | **σ 여유** [MPa] |
| ----: | -------------------: | ---------: | ---------: | --------------: | ------------: | -----: | ---------------: | ------------: | ------: | -------: | ------------: | ------------: | -----: | -: | -----: | -----: | -----------: | ----------------------: |
|     1 |      **27.59** |      117.2 |     172.37 | **4,051** |         3,491 |  2,916 |  **279.0** |          10.5 |      16 |    1.901 |         223.1 |         529.4 |  2.412 | 45 |    1.0 |    6.0 |      2,098.8 |                     1.2 |
|     5 |      **26.45** |      111.9 |     164.79 | **4,104** |         3,557 |  2,995 |  **256.5** |          11.7 |      16 |    1.859 |         222.0 |         511.0 |  2.341 | 46 |    1.0 |    6.0 |      2,099.5 |                     0.5 |
|     7 |      **25.16** |      107.7 |     158.02 | **4,157** |         3,620 |  3,068 |  **239.0** |          12.8 |      16 |    1.871 |         227.4 |         488.6 |  2.187 | 46 |    1.0 |    6.0 |      2,098.4 |                     1.6 |
|    10 |      **24.34** |      102.5 |     151.23 | **4,217** |         3,698 |  3,165 |  **218.5** |          14.5 |      15 |    1.683 |         222.0 |         479.6 |  2.199 | 48 |    1.0 |    6.0 |      2,096.8 |                     3.2 |
|    13 |      **23.41** |       97.9 |     144.73 | **4,292** |         3,786 |  3,266 |  **200.5** |          16.3 |      16 |    1.686 |         213.1 |         460.9 |  2.203 | 51 |    1.0 |    6.0 |      2,099.9 |                     0.1 |
|    18 |      **22.23** |       92.0 |     136.42 | **4,398** |         3,912 |  3,413 |  **178.5** |          19.1 |      15 |    1.520 |         211.0 |         443.3 |  2.142 | 53 |    1.0 |    6.0 |      2,096.2 |                     3.8 |
|    23 |      **21.11** |       88.1 |     130.30 | **4,450** |         3,994 |  3,526 |  **164.5** |          21.4 |      16 |    1.366 |         180.3 |         431.6 |  2.442 | 64 |    1.0 |    6.0 |      2,099.4 |                     0.6 |
| 25 ★ |      **20.52** |       85.8 |     126.83 | **4,500** |         4,055 |  3,598 |  **156.5** |          23.0 |      16 |    1.308 |         174.9 |         422.6 |  2.465 | 67 |    1.0 |    6.0 |      2,099.6 |                     0.4 |
|    29 |      **19.63** |       83.0 |     122.23 | **4,573** |         4,138 |  3,691 |  **147.0** |          25.1 |      15 |    1.246 |         181.2 |         408.2 |  2.300 | 66 |    1.0 |    6.0 |      2,099.3 |                     0.7 |
|    31 |      **19.18** |       81.2 |     119.57 | **4,631** |         4,200 |  3,757 |  **141.0** |          26.6 |      16 |    1.288 |         178.3 |         395.9 |  2.269 | 68 |    1.0 |    6.0 |      2,099.6 |                     0.4 |
|    33 |      **18.98** |       79.7 |     117.67 | **4,681** |         4,254 |  3,816 |  **136.0** |          28.1 |      16 |    1.262 |         176.8 |         391.7 |  2.264 | 69 |    1.0 |    6.0 |      2,099.8 |                     0.2 |
|    36 |      **18.22** |       77.5 |     113.93 | **4,753** |         4,332 |  3,900 |  **129.0** |          30.2 |      15 |    1.223 |         186.1 |         377.5 |  2.075 | 67 |    1.0 |    6.0 |      2,098.7 |                     1.3 |
|    42 |      **17.80** |       75.5 |     111.05 | **4,805** |         4,403 |  3,990 |  **122.5** |          32.6 |      16 |    1.129 |         163.0 |         373.9 |  2.347 | 78 |    1.0 |    6.0 |      2,098.6 |                     1.4 |
|    46 |      **17.19** |       73.9 |     108.24 | **4,871** |         4,473 |  4,064 |  **117.5** |          34.6 |      16 |    1.143 |         167.7 |         360.9 |  2.203 | 77 |    1.0 |    6.0 |      2,099.3 |                     0.7 |
|    51 |      **16.99** |       72.3 |     106.30 | **4,937** |         4,539 |  4,131 |  **113.0** |          36.6 |      16 |    1.153 |         171.8 |         353.5 |  2.108 | 76 |    1.0 |    6.0 |      2,097.1 |                     2.9 |
| 53 ★ |      **16.69** |       71.6 |     104.96 | **4,975** |         4,581 |  4,177 |  **110.5** |          37.8 |      16 |    1.143 |         171.7 |         347.6 |  2.075 | 77 |    1.0 |    6.0 |      2,094.8 |                     5.2 |

#### 9-9.2.3. 읽을 것

**① `D` = 4.0 m 는 불가능하다.** 12,975점을 평가해 `D ≤ 4,000` 인 가능해가
**0건**이고, 프론트의 최소 `D` 는 **4,051 mm** 다. 51 mm 차이로 못 미치는데,
150세대 내내 이 값이 4,201 → 4,051 로 내려오며 **더 내려가지 못하고 멈췄다**
(§9-9.1). σ ≤ 2,100 을 지키면서 외경 4.0 m 안에 들어가는 조합은 이 설계변수
범위에 없다고 보아야 한다.
...
| ---------: | ----------------: | -------------------------: | -------: |
|      4.0 m | **해 없음** |                         — |       — |
|      4.5 m | **20.52 t** | +3.83 t (**+23.0%**) | 126.83 t |
|      5.0 m | **16.69 t** |                         — | 104.96 t |

**외경을 5.0 → 4.5 m 로 500 mm 조이면 베어링이 3.83 t(23.0%) 무거워지고
총질량은 21.87 t 늘어난다.** 제작·운송 완화를 협의할 때 쓸 수치가 이것이다.

**③ 대리변수를 바꾼 것만으로 이득이 났다.** 부록 8 은 `C6`(`D_pw` ≤ 4,500)
아래서 총질량 최소 **105.84 t** 였다(§8-6.3a). 부록 9 는 같은 물리적 한계
(`D` ≤ 5,000 ≈ 그때의 외경 4,995)에서 **104.96 t** 를 얻었다 — **0.88 t 가볍다.**
`z2` 를 6.0 으로 고정해 샤프트가 최대인데도 그렇다. `D_pw` 로 묶던 제약이
**옳은 물리량이 아니어서 과도하게 조이고 있었다**는 뜻이다.

**④ 프론트가 53건으로 촘촘하다.** 부록 8 S3-c 는 같은 150세대에 8건이었다.
`D` 가 연속변수라 프론트가 `D` 4,051 ~ 4,975 mm 를 **평균 18 mm 간격**으로
덮는다. §8-6.5.7 ⑵ 가 「해상도 부족은 탐색이 아니라 정식화에서 왔다」고 한 것이
확인됐다.

**⑤ 수렴은 120세대에서 한 번 튀었다.** HV 증분이 60세대 이후 0.02 ~ 0.09 로
잦아들었다가 **120세대에 +0.205** 로 뛰며 최소 베어링이 17.08 → **16.72 t** 로
갱신됐다. §6-11.5 가 97세대 갱신을 근거로 조기 종료를 배제한 것과 같은 사례다 —
**정체처럼 보여도 끊지 않은 것이 맞았다.**

**⑥ 벽두께는 안전측으로 움직인다.** 두께 규칙이 `W` 를 고정하므로 보어가
작아지면 벽이 두꺼워진다. 프론트 양 끝이 이렇다.

|              | `D` [mm] | 보어`d` [mm] | 벽두께`t` [mm] | `d/t` |
| ------------ | ---------: | -------------: | ---------------: | ------: |
| 작은 외경 끝 |      4,051 |          2,916 |  **279.0** |    10.5 |
| 큰 외경 끝   |      4,975 |          4,177 |  **110.5** |    37.8 |

§8-8 에서 문제가 됐던 `d/t` 70 ~ 74 의 외삽 영역(벽 70 mm)과 정반대다 — 부록 9
프론트는 `d/t` **10.5 ~ 37.8**
로, §8-6.3a 프론트(32 ~ 35)보다도 두껍다. **`D` 를 제약으로 걸면 두께 규칙의
외삽 문제가 자동으로 사라진다.**

> **총질량은 목적이 아니었음을 기억해야 한다.** 위 표의 총질량은 선택 단계를
> 위한 참고값이다(§9-2). `z2` = 6.0 고정 때문에 샤프트가 최대이므로, 총질량을
> 목적으로 삼으면 더 나은 조합이 있을 수 있다.

*프론트 **전량 53건**은 `부록9_NSGA/S3_본최적화/a9_pareto.csv` · 정리 `a9_result.py`*
*프론트 **전량 53건**은 `부록9_NSGA/S3_본최적화/a9_pareto.csv` · 정리 `a9_result.py`*

<!-- /A9:RES -->

## 9-10. S4-d — `D` 상한 3건 피로수명 검토 (50 · 70 °C)

§9-9.2.1 의 세 설계에 §8-7 과 **같은 절차**로 피로를 건다 — `L_eff` 강체정역학
스크리닝으로 DLC별 `k` → **dt = 20 · 111 DLC** MASTA(ISO/TS 16281) → 직렬
와이블 `e` = 9/8. 판정은 **ΣD30_UW ≤ 0.5 ∧ ΣD30_Sys ≤ 0.5**. 70 °C 는
§6-11.7 과 같은 조건(베어링 3항만 승온)이며, 샤프트·하우징 온도는 열팽창이
꺼져 있어 결과에 들어가지 않는다(§8-7.4).

모델 구성은 `a8_build` 가 부록 8 규칙(두께 규칙 · 코너 반경 4.3 mm · 정수화)을
주입한다(§8-7.2). 실행은 `run_a9_s4.py` — phase `8`(50 °C) · `9`(70 °C).

**세 설계의 베어링 질량이 16.69 ~ 27.59 t 로 크게 다르다.** §6-11.6 의 「여유는
질량이 결정한다」(상관 −0.954)를 적용하면 **`c50`(16.69 t)이 가장 위험**하다.
다만 `z2` 가 6.0 m 로 고정돼 스팬이 길어 베어링 하중이 낮은 점은 유리하게
작용한다 — 부록 8 S4-d 의 14건은 `z2` 가 4.0 ~ 6.0 로 섞여 있었다.

### 9-10.1. 결과 (설계 1건마다 실시간)

<!-- A9:S4 -->

판정 기준: **ΣD30_UW ≤ 0.5 ∧ ΣD30_Sys ≤ 0.5** (30년 손상)

|                     태그 | `D` [mm] |      베어링 [t] | d [mm] | t [mm] | D_we [mm] | L_we [mm] |  Z | ΣD30_UW 50 °C |  ΣD30_UW 70 °C |  **손상비** | ΣD30_Sys 70 °C | life_Sys 50 °C | life_Sys 70 °C | **판정 70 °C** |
| -----------------------: | ---------: | --------------: | -----: | -----: | --------: | --------: | -: | --------------: | ---------------: | ----------------: | ---------------: | --------------: | --------------: | :-------------------: |
|     `cmin` (도달 하한) |      4,051 | **27.59** |  2,916 |  279.0 |     223.1 |     529.4 | 45 |          0.0473 | **0.1618** | **3.419배** |           0.1658 |             622 |   **181** |    **합격**    |
| `c45` (`D` ≤ 4,500) |      4,500 | **20.52** |  3,598 |  156.5 |     174.9 |     422.6 | 67 |          0.0976 | **0.2914** | **2.985배** |           0.2991 |             301 |   **100** |    **합격**    |
| `c50` (`D` ≤ 5,000) |      4,975 | **16.69** |  4,177 |  110.5 |     171.7 |     347.6 | 77 |          0.1347 | **0.3560** | **2.644배** |           0.3654 |             218 |    **82** |    **합격**    |

*50 °C 3/3 · 70 °C 3/3 완료 · 설계 1건이 끝날 때마다 갱신된다 · `t` 는 두께 규칙이 만든 샤프트 벽두께*

<!-- /A9:S4 -->

### 9-10.2. 읽을 것

**3건 모두 50 · 70 °C 양쪽에서 합격**했다. 70 °C 최악은 `c50` 의
ΣD30_UW **0.3560**(여유 **1.40배** · life_Sys 82 yr)이다.

|          |      베어링 [t] | ΣD30_UW 50 °C |  ΣD30_UW 70 °C |     여유(70 °C) |          손상비 |
| -------- | --------------: | --------------: | ---------------: | ---------------: | --------------: |
| `cmin` |           27.59 |          0.0473 |           0.1618 | **3.09배** | **3.419** |
| `c45`  |           20.52 |          0.0976 |           0.2914 |           1.72배 |           2.985 |
| `c50`  | **16.69** |          0.1347 | **0.3560** | **1.40배** |           2.644 |

**① 예측대로 `c50` 이 가장 위험하다.** §6-11.6 의 「여유는 질량이 결정한다」
(ΣD30 ↔ 베어링 질량 상관 −0.954)가 세 점에서 그대로 나타난다 — 베어링이
27.59 → 16.69 t 로 가벼워지면 70 °C 손상이 0.162 → 0.356 으로 **2.2배** 커진다.
**`D` 상한을 푸는 대가는 질량만이 아니라 피로 여유이기도 하다.**

**② 손상비가 설계마다 다르다 — 2.644 ~ 3.419.** 부록 8 S4-d 14건은
2.627 ~ 2.805 로 좁았는데(§8-7.3), 여기서는 더 벌어진다. **`D_pw` 가 작을수록
손상비가 크다**(`cmin` 3,491 mm → 3.419 · `c50` 4,581 mm → 2.644). 점도
파라미터 `κ ∝ ν · D_pw^0.5` 이므로 피치원이 작으면 κ 가 낮아 `a_ISO` 곡선의
가파른 구간에 놓이고, 같은 점도 감소가 더 큰 손상 증가로 번역된다.

> 이는 §8-7.3 이 「온도의 효과는 설계가 바뀌어도 거의 일정하다」고 한 것을
> **좁혀야 한다는 뜻**이다. 그 결론은 `D_pw` 가 4,492 ~ 4,500 으로 사실상 한
> 점이던 부록 8 프론트에서 나온 것이었다. `D_pw` 가 3,491 ~ 4,581 로 벌어지면
> 손상비도 함께 벌어진다.

**③ 여유가 §8-7.3 보다 넉넉하다.** 부록 8 S4-d 의 70 °C 최악은 0.3964
(여유 1.26배)였는데 여기서는 0.3560(1.40배)이다. `z2` 를 6.0 m 로 고정해 스팬이
최대이고 베어링 하중이 낮기 때문이다 — 부록 8 은 `z2` 4.0 ~ 6.0 이 섞여 있었다.
**`z2` 고정이 총질량에는 불리하지만 피로에는 유리하게 작용했다.**

**④ 피로는 여전히 `D` 상한 선택을 구속하지 않는다.** 세 상한 어디에서도
탈락이 없으므로, `D` 를 얼마로 잡을지는 **제작·운송과 질량의 문제**이지 피로의
문제가 아니다. 다만 `c50` 의 1.40배는 부록 8 의 1.26배와 마찬가지로 넉넉하다고
할 수 없는 폭이다 — 하중 스펙트럼이나 재료 가정이 바뀌면 흔들린다.

## 9-11. 미결

|  # | 항목                          | 내용                                                                                                                             |
| -: | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| ① | `z2` 고정의 대가            | 샤프트가 최대(6.5 m 축)로 무거워진다. 총질량 관점의 손실은`D` ≤ 5,000 에서 +1.45 t 로 작지만, `D` 를 더 조이면 커질 수 있다 |
| ② | 「`z2` = 6.0 이 항상 최선」 | 기존 자료 33/33 의**관측**이지 증명이 아니다. `D` 4.0 ~ 4.5 m 의 조인 영역에서도 성립하는지는 확인되지 않았다            |
| ③ | 시드 1개                      | 부록 6 이래의 한계(미결#9) 그대로                                                                                                |
| ④ | 두께 규칙 외삽                | `D` 를 조이면 보어가 작아져 벽이 **두꺼워지므로** §8-8.3 과 반대 방향 — 이 실행에서는 안전측이다                       |

# 부록 10. 추가 최적화 — 3목적 (외경 · 베어링 · 총질량)

> 절 번호 `10-x` 는 부록 10 안에서만 쓴다.

## 10-1. 왜 3목적인가

부록 9 는 `(외경 D, 베어링 1개 질량)` 2목적으로 **「`D` 상한별 최소 베어링」
곡선**을 얻었다(§9-9.2). 그 대가로 `z2` 를 6.0 m 에 고정했고, 그래서 **샤프트가
언제나 최대**였다 — 총질량은 목적이 아니라 부산물이었다(§9-2 ▸).

**총질량을 움직일 수 있는 변수는 `z2` 하나다.** 부록 8 이 이를 두 번 보였다.

| 근거          | 내용                                                                                                                                         | 출처                   |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| 슬라이스 상관 | `(z1, z2, D_pw)` 를 고정하면 `f1`↔`f2` 상관이 **+1.000**, 샤프트 질량 폭이 **1.26 ~ 2.79 t**(베어링 폭의 1/10)로 붕괴한다 | §8-8.5.4              |
| 프론트 구조   | 프론트 점수가`z2` 수준 수를 넘지 못한다 — 8건이 `z2` 4수준, 9건이 5수준에 흩어졌다                                                      | §8-6.5.3 · §8-8.5.4 |
| 고정의 대가   | `z2` 를 6.0 으로 고정한 부록 9 는 샤프트가 71.6 ~ 117.2 t 로 언제나 최대다                                                                 | §9-9.2.1              |

**그래서 `z2` 를 되살리고 총질량을 목적에 넣는다.** `z2` 를 늘리면 베어링은
가벼워지고 샤프트는 무거워지므로, 총질량이 목적이 되는 순간 `z2` 는 상한에
박히지 않고 **내부에서 균형점을 찾는다.**
...
| ------ | ----------------------------------------- | ---- | -------------- |
| `f1` | **외경 `D`** ↓                   | m    | 제작 가능 크기 |
| `f2` | **베어링 1개 질량** ↓              | t    | 부품 크기      |
| `f3` | **총질량** = `2·f2 + m_shaft` ↓ | t    | 시스템 전체    |

> **알려진 한계 — `f3` 가 `f2` 를 포함한다.** §8-6.5.7 ⑵ 는 `(베어링, 총질량)`
> 쌍이 **정의상 겹쳐 있어**(`f3 = 2f2 + m_shaft`) 지배 판정이 뭉개진다는 것을
> 실측으로 보였다 — 같은 자료를 `(베어링, 샤프트)` 로 다시 판정하면 프론트가
> **8 → 27건**으로 늘었다. 상관도 +0.739 ~ +0.757 로 높다.
>
> 물리적으로 독립된 셋은 `(D, 베어링, 샤프트)` 이고 총질량은 `2f2 + f3` 로
> 정확히 복원되며, 가중합 정리에 따라 총질량 최소점도 그 프론트에 반드시
> 포함된다(§8-6.5.7 ⑸ · 실측 50/50). **그럼에도 총질량을 직접 목적으로 두기로
> 결정했다** — 해석이 직관적이고 선택 단계가 필요 없기 때문이다. 프론트가
> 겹침 때문에 얇아질 수 있다는 점은 결과에서 확인한다(§10-7 ①).

**HV 기준점은 `(5.5 m, 45 t, 250 t)`** 로 새로 잡는다 — 목적이 셋이 되어
부록 9 의 `(5.5, 45)` 를 쓸 수 없다.

## 10-3. 설계변수

| 변수             | 자료형               | 범위                           |         수준 | 부록 9 대비                |
| ---------------- | -------------------- | ------------------------------ | -----------: | -------------------------- |
| `z1`           | 고정                 | 1.0 m                          |            1 | 그대로                     |
| **`z2`** | **`Choice`** | **3.5 ~ 6.0 m · 0.1 m** | **26** | **6.0 고정 → 복원** |
| α               | `Integer`          | 15 ~ 30° · 1°               |           16 | 그대로                     |
| `D_pw`         | `Integer`          | 3,300 ~ 4,800 mm · 1 mm       |        1,501 | 그대로                     |
| `D_we`         | `Integer`          | 110.0 ~ 230.0 mm · 0.1 mm     |        1,201 | 그대로                     |
| `u`            | `Integer`          | 0 ~ 10⁴                       |       10,001 | 그대로                     |

`L_w`(롤러 전장)는 `u` 로 밴드 재매개화된다 — `L_w = lo + round(u(hi−lo)/10⁴)`,
`[lo, hi]` = `[1.5·D_we, 2.5·D_we]` 를 0.1 mm 격자에 맞춘 값(§6-11.3).

**범주가 26 으로 되살아난다.** `z2` 가 `Choice` 로 돌아오면서 범주 전수 시딩
제약이 다시 생긴다 — 개체수는 26의 배수로 잡는다(§10-6).
...
| 종속변수                               | 산출                                                                                                                                                                    |
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `T`·`B`·`C`                    | `1.30226`·`1.26025`·`1.06281` × `L_w` · 1 mm 반올림                                                                                                         |
| 보어`d`                              | `D_pw − D_we·cosα − L_w·sinα − 2t_i` · 1 mm (`t_i` = 0.15652·B)                                                                                            |
| 외경`D`                              | `D_pw + D_we·cosα + L_w·sinα + 2t_o` · 1 mm (`t_o` = 0.17215·C) — **목적**                                                                             |
| 롤러 수`Z`                           | MASTA 자동                                                                                                                                                              |
| **샤프트 내경**                  | **`floor((d⁴ − 32·W·d/π)^¼)`** · `W` = 1.393×10⁹ mm³ (§7-6.7.5)                                                                                    |
| 샤프트 길이                            | `z2 + 0.5 m` — **`z2` 가 변수로 돌아와 이제 설계마다 다르다**                                                                                                |
| 롤러 코너 반경                         | 좌·우 각**4.3 mm** 주입 → `L_we` = `L_w` − 8.6 mm                                                                                                          |
| **롤러 축방향 오프셋** `e_off` | **`o_off` − (`D_we`/2)·sin(`α`−`β`)** — 롤러 **접촉선 중점**을 외륜 궤도 중앙에 둔다(§10-11.3.6). **적용 보류** · 현재 20.742 mm 고정 |
| 2β · cone angle                      | MASTA 산출값 기록                                                                                                                                                       |
| 질량 · σ_max                         | MASTA 실측 (지배 LC`Myz_max` 22,673 kNm)                                                                                                                              |
...
| #                | 제약                               | 판정            | 비고                                        |
| ---------------- | ---------------------------------- | --------------- | ------------------------------------------- |
| **C2**     | 세장비`1.5 ≤ L_w / D_we ≤ 2.5` | 해석식          | 밴드 재매개화가 구조적 보장                 |
| ~~C4~~·~~C5~~ | —                                 | —              | 삭제 (자동 만족)                            |
| **C6**     | **`D ≤ 5,000 mm`**        | 해석식          | 제작·운송 상한 · 실질적으로 유일하게 활성 |
| C7·C8·C9·C12  | 축방향 간섭 · 살두께 등           | 해석식          | **자동 만족** — 코드에는 남긴다      |
| **σ**     | `σ_max ≤ 2,100 MPa`            | **MASTA** | 그대로                                      |

> **`C7` 은 `z2` 복원으로 다시 볼 필요가 생겼다.** 부록 9 는 `z2` = 6.0 고정
> 이라 `z2 − z1` = 5.0 m 로 여유가 컸지만, `z2` = 3.5 면 2.5 m 다. `T` 최대가
> 716 mm 이므로 `T + 0.1` = 0.816 m 로 여전히 만족하나, **배선 시험에서
> 실측으로 확인한다.**

피로(ΣD30 ≤ 0.5)는 루프에서 뺀다 — 부록 8 S4-d 14건과 부록 9 S4-d 3건이
50 · 70 °C 양쪽 모두 합격했다(§8-7.3 · §9-10). 프론트 확정 후 검증으로 돌린다.

## 10-6. 적용 파라미터 (as-run 예정)

§6-11.5 양식에서 재현에 필요한 것만 추렸다. 적지 않은 항목은 §6-11.5 와 같다.

| 구분     | 항목                     | 적용값                                                                                    | 근거                           |
| -------- | ------------------------ | ----------------------------------------------------------------------------------------- | ------------------------------ |
| 알고리즘 | 라이브러리 · 방법       | `pymoo 0.6.2` · **NSGA-II**                                                      | §6-2                          |
| 알고리즘 | **개체수**         | **182** (= 7 × 26)                                                                 | 아래 ▸                        |
| 알고리즘 | **세대수**         | **150 (고정)**                                                                      | 조기 종료 미채택 (R14 항목 24) |
| 알고리즘 | 총 평가                  | **27,300**                                                                          | `182 × 150`                 |
| 알고리즘 | 시드                     | 1                                                                                         | 미결#9                         |
| 알고리즘 | 초기집단                 | **범주 26 전수 시딩** + 수치 무작위                                                 | §6-8.1.2                      |
| 알고리즘 | 연산자                   | SBX`eta=15` / PM `eta=20` (`Integer`) · UX / `ChoiceRandomMutation` (`Choice`) | pymoo 기본값                   |
| 알고리즘 | 생존 · 중복제거 · 종료 | `RankAndCrowdingSurvival` · **끔** · 세대수 도달                                | §6-11.5 동일                  |
| 알고리즘 | **HV 기준점**      | **`(5.5 m, 45 t, 250 t)`**                                                        | 목적 3개로 변경                |
| 문제     | **목적**           | **`f1` `D` ↓ · `f2` 베어링 ↓ · `f3` 총질량 ↓**                         | §10-2                         |
| 문제     | 설계변수                 | `z2` `Choice`(26) · α·`D_pw`·`D_we`·`u` `Integer` · `z1` 고정         | §10-3                         |
| 문제     | 제약                     | 해석식 6개(실질`C6` 하나) + σ ≤ 2,100                                                 | §10-5                         |
| 문제     | 종속변수                 | 두께 규칙 · 코너 반경 4.3 · 정수화 켬                                                   | §10-4                         |
| 실행     | 지배 LC · 모델          | `Myz_max` 22,673 kNm · v1.4 · 50 °C · `DIN_LUNDBERG`                              | §8-1.2 · §3                 |
| 실행     | 사전 게이트              | v1.3 기준선 8/8 (σ 3,407.9 · 샤프트 58,384.5 kg)                                        | §8-5.1                        |
| 실행     | **예산 추정**      | MASTA 약**23,600회** × 0.30 s ≈ **2.0시간**                                 | 아래 ▸                        |

**개체수 182 의 근거** — 범주가 26 으로 돌아와 전수 시딩을 유지하려면 26의
배수여야 한다. `z2` 수준당 **7개체**이고, **3목적 프론트는 곡면이라 2목적보다
훨씬 많은 점이 필요하다** — 부록 9(100개체 · 2목적)보다 82% 키웠다.
부록 6(224 × 150 · 2.6시간)과 비슷한 규모다.

|        개체수 | `z2` 수준당 |           총평가 |       MASTA 추정 |       시간 추정 |
| ------------: | ------------: | ---------------: | ---------------: | --------------: |
|           156 |             6 |           23,400 |           20,240 |           1.7 h |
| **182** |   **7** | **27,300** | **23,610** | **2.0 h** |
|           208 |             8 |           31,200 |           26,990 |           2.3 h |

*0.30 s/MASTA · 해석식 통과 86.5% 기준(부록 9 실측).*

> **NSGA-II 를 유지한다.** 3목적에서 혼잡도 정렬이 약해지는 것은 알려진
> 한계이고 NSGA-III 가 그 목적으로 설계됐지만, §6-2 이래의 방법론 계승과
> 부록 6·8·9 와의 직접 비교를 우선했다. **개체수를 키워 보완**하고, 프론트
> 분포가 한쪽으로 쏠리는지는 결과에서 확인한다(§10-7 ②).

## 10-8. S3-a — 배선 시험 (260807)

MASTA 를 결정론적 대용식으로 갈아끼우고 `182 × 4` 를 돌려 배선만 확인했다
(`a10_s3_wiretest.py` · MASTA 0회).

|  # | 점검                    | 결과                                                                                             |
| -: | ----------------------- | ------------------------------------------------------------------------------------------------ |
| ① | 목적이 3개인가          | **`D` 4.093 ~ 4.925 m · 베어링 8.31 ~ 18.63 t · 총질량 86.1 ~ 159.2 t** · 프론트 16점 |
| ② | `z2` 전수 시딩        | **26 / 26수준** 3.5 ~ 6.0                                                                  |
| ③ | α ·`D_pw` · `C6` | α 15 ~ 30(16종) ·`D_pw` 최대 4,691 · `D` 최대 **5,000**                             |
| ④ | **`C7` 여유**   | **최소 1.691 m** (`z2−z1` − (`T`+0.1)) — **미결 ③ 해소**                     |
| ⑤ | 밴드·두께규칙·산출 열 | `L_w` 이탈 **0** · 세장비 1.5054 ~ 2.4881 · 열 정상                                    |

**판정 통과.** 해석식 탈락 7.3%(전부 `C6`).

### 10-8.1. 대용식에 `z2` 항을 넣었다

부록 9 의 1차 배선시험은 σ 에 `D_pw` 항이 없어 작은 `D` 와 가벼운 베어링이
동시에 가능해졌고 **프론트가 1점으로 무너졌다**(§9-7 ▸). 이번에는 `z2` 가 새
교환 차원이므로 같은 함정이 `z2` 에서 생긴다 — 스팬이 짧으면 샤프트가 가벼워
총질량이 좋아지는데, σ 에 벌점이 없으면 `z2` 가 하한에 전부 몰린다.

그래서 대용식에 **두 항**을 넣었다.

| 항                           | 기전                                          | 근거      |
| ---------------------------- | --------------------------------------------- | --------- |
| `(4.5 / D_pw)^1.6`         | 피치원이 커지면 롤러 하중이`M/D_pw` 로 준다 | §8-8.5.2 |
| **`(6.0 / z2)^0.9`** | 스팬이 짧으면 베어링 하중이 커진다            | §8-6.5.2 |

크기는 가짜지만 **상충의 방향은 실측 기전과 같다.** 대용식이라도 그렇지 않으면
배선 시험이 「3목적이 작동하는가」를 답하지 못한다.

## 10-9. S3-b — 드라이런 (260807)

`182 × 5 = 910` 평가 · 실 MASTA **820회** · **4.1분 · 0.303 s/점** · 사전 게이트
v1.3 기준선 **8/8 일치**.
...
| ① | `z2` 전수 시딩      | **26 / 26수준** 3.5 ~ 6.0           |
| ② | 초기집단 α           | 16종 15 ~ 30                              |
| ③ | `L_w` 0.1 mm 격자   | 819점 중 이탈**0**                  |
| ④ | 세장비 밴드           | 1.5061 ~ 2.4969 ·**경계집중 0.0%** |
| ⑤ | `C6` 외경 상한      | `D` 3,619 ~ **4,999** (≤ 5,000)  |
| ⑥ | **`C7` 여유** | **최소 1.694 m**                    |
| ⑦ | 프론트                | **26건** · `D` 4,295 ~ 4,985     |
| ⑧ | 체크포인트            | 생성                                      |

**판정 통과.** 본런 예산 **MASTA 24,600회 × 0.303 s ≈ 2.07시간**(§10-6 추정
2.0시간과 3% 차이).

### 10-9.1. 읽을 것

**① 3목적이 작동한다.** 5세대 만에 프론트가 **26건**이고 `z2` 가 **12수준**
(4.8 ~ 6.0)에 걸쳐 있다. 부록 9(2목적 · `z2` 고정)에서는 프론트가 `D` 축
한 줄이었는데, 총질량이 목적에 들어오자 **`z2` 가 살아나 프론트가 퍼졌다** —
§10-1 의 전제가 실측으로 확인됐다.

**② 아직 긴 스팬 쪽에 몰려 있다.** 프론트의 `z2` 가 4.8 ~ 6.0 이고 하한
3.5 ~ 4.7 은 비어 있다. 5세대 시점이라 판정하기 이르고(§10-6 ▸ 부록 8·9 모두
초반에 흔들렸다) **판정 항목에서 뺐다** — 표에 기록만 하고 추세를 본다.

**③ `C7` 은 확실히 죽어 있다.** `z2` 하한 3.5 에서도 여유가 **1.69 m** 다
(`T` 최대 716 mm 기준 필요치 0.816 m). §10-7 ③ 을 종결한다.

**④ 최소 총질량이 5세대 내내 105.589 t 로 멈춰 있다.** 부록 8 S3-c 의
105.84 t(150세대)와 거의 같은 값이 5세대에 나왔다. 3목적이라 총질량 축에도
직접 압력이 걸리기 때문으로 보이나, 본런에서 더 내려가는지 확인해야 한다.

## 10-11. S3-c — 본 최적화

### 10-11.1. 실행 요약 (10세대 실시간)

<!-- A10:GENLOG -->

| 세대 | 프론트 |      HV | **ΔHV** | **ΔHV / HV** | **최소 `D`** [mm] | **최소 베어링** [t] | **최소 총질량** [t] |
| ---: | -----: | ------: | -------------: | ------------------: | ------------------------: | ------------------------: | ------------------------: |
|   10 |     48 | 4,074.2 |             — |                  — |           **4,276** |           **17.48** |          **105.59** |
|   20 |     59 | 4,413.2 |         +339.0 |   **+8.320%** |           **4,153** |           **17.48** |          **105.59** |
|   30 |     87 | 4,571.6 |         +158.4 |   **+3.590%** |           **4,104** |           **17.26** |          **105.36** |
|   40 |    112 | 4,639.4 |          +67.9 |   **+1.484%** |           **4,104** |           **16.87** |          **105.36** |
|   50 |    122 | 4,664.2 |          +24.8 |   **+0.535%** |           **4,104** |           **16.87** |          **105.35** |
|   60 |    168 | 4,697.7 |          +33.5 |   **+0.718%** |           **4,093** |           **16.87** |          **105.30** |
|   70 |    182 | 4,724.6 |          +26.8 |   **+0.571%** |           **4,086** |           **16.85** |          **105.28** |
|   80 |    182 | 4,751.6 |          +27.0 |   **+0.572%** |           **4,080** |           **16.85** |          **104.88** |
|   90 |    182 | 4,763.2 |          +11.5 |   **+0.243%** |           **4,080** |           **16.85** |          **104.88** |
|  100 |    182 | 4,802.4 |          +39.3 |   **+0.825%** |           **4,057** |           **16.85** |          **104.88** |
|  110 |    182 | 4,816.4 |          +13.9 |   **+0.290%** |           **4,057** |           **16.85** |          **104.21** |
|  120 |    182 | 4,838.4 |          +22.0 |   **+0.457%** |           **4,057** |           **16.65** |          **104.21** |
|  130 |    182 | 4,869.9 |          +31.6 |   **+0.652%** |           **4,042** |           **16.59** |          **104.13** |
|  140 |    182 | 4,873.3 |           +3.3 |   **+0.068%** |           **4,042** |           **16.59** |          **104.13** |
|  150 |    182 | 4,903.4 |          +30.1 |   **+0.618%** |           **4,039** |           **16.40** |          **103.69** |

*10세대 간격 · `ΔHV` 는 **직전 표시 행** 대비 · HV 기준점 (5.5 m, 45 t, 250 t) · 셋 다 목적이다*

<!-- /A10:GENLOG -->

**읽는 법** — 부록 9(§9-9.1)와 달리 **세 값이 모두 목적**이다. `최소 D` 가
내려가면 프론트가 작은 외경 쪽으로 뻗고, `최소 베어링` 은 큰 외경 쪽 끝이,
`최소 총질량` 은 `z2` 를 쓰는 구간이 개선되는 신호다.

### 10-11.2. 결과

<!-- A10:RES -->

![부록 10 3목적 파레토 프론트](Results/사이징최적화/figures/pareto_a10.png)

왼쪽은 `(D, 베어링)`·색이 총질량, 오른쪽은 `(D, 총질량)`·색이 베어링이다. 회색 점선이 `D` 상한 4.0 · 4.5 · 5.0 m, 빨간 별이 각 상한에서의 **그 패널 목적의 최적**이다.

#### 10-11.2.1. `D` 상한별 두 최적

**목적이 셋이라 상한마다 최적이 둘이다** — 최소 베어링 설계와 최소 총질량 설계가 갈린다. 둘이 얼마나 다른지(특히 `z2`·샤프트)가 3목적의 핵심 산출이다.

*맨 아래 행은 상한이 아니라 **프론트가 도달한 최소 외경**이다 — 그 이하로는 가능해가 없다. 그 `D` 에는 프론트 점이 하나뿐이라 두 기준이 갈리지 않는다.*

| `D` 상한 [mm] |                  기준 |    그때`D` [mm] | **베어링** [t] | 샤프트 [t] | **총질량** [t] | `D_pw` [mm] | d [mm] | t [mm] |  d/t | **A** [m²] | **L** [m] | α [°] | `D_we` [mm] | `L_we` [mm] | 세장비 |  Z | **z2** [m] | σ_max [MPa] | σ 여유 [MPa] |
| --------------: | --------------------: | ----------------: | -------------------: | ---------: | -------------------: | ------------: | -----: | -----: | ---: | ----------------: | --------------: | ------: | ------------: | ------------: | -----: | -: | ---------------: | -----------: | ------------: |
| **4,000** |                    — | **해 없음** |                   — |         — |                   — |            — |     — |     — |   — |                — |              — |      — |            — |            — |     — | — |               — |           — |            — |
| **4,500** | **최소 베어링** |             4,498 |      **20.94** |       87.1 |     **128.94** |         4,033 |  3,556 |  161.0 | 22.1 |              1.72 |             6.5 |      15 |         202.4 |         422.9 |  2.132 | 57 |    **6.0** |      2,098.3 |           1.7 |
| **4,500** | **최소 총질량** |             4,498 |      **21.43** |       86.0 |     **128.83** |         4,028 |  3,546 |  162.0 | 21.9 |              1.72 |             6.4 |      15 |         204.2 |         427.8 |  2.137 | 57 |    **5.9** |      2,092.9 |           7.1 |
| **5,000** | **최소 베어링** |             4,983 |      **16.44** |       71.0 |     **103.88** |         4,596 |  4,199 |  109.0 | 38.5 |              1.40 |             6.5 |      15 |         170.0 |         347.6 |  2.095 | 78 |    **6.0** |      2,099.8 |           0.2 |
| **5,000** | **최소 총질량** |             4,983 |      **16.44** |       71.0 |     **103.88** |         4,596 |  4,199 |  109.0 | 38.5 |              1.40 |             6.5 |      15 |         170.0 |         347.6 |  2.095 | 78 |    **6.0** |      2,099.8 |           0.2 |
| *(도달 하한)* |                    — |   **4,039** |      **27.54** |      117.9 |     **173.00** |         3,481 |  2,907 |  282.0 | 10.3 |              2.33 |             6.5 |      15 |         227.6 |         533.5 |  2.382 | 44 |    **6.0** |      2,096.9 |           3.1 |

#### 10-11.2.2. 프론트 대표점 33건

프론트 **210건**을 `D` **50 mm 구간마다 총질량 최소 · 베어링 최소 두 점씩** 솎았다. `D` 상한 6점과 양 끝은 구간과 무관하게 넣었다(★). `#` 는 전량 기준 순번이다.

**「선정」 열이 그 행이 뽑힌 기준**이다 — `둘 다` 는 그 구간에서 총질량 최소와 베어링 최소가 **같은 설계**라는 뜻이고, 그런 구간은 행이 하나뿐이다(19구간 중 **8구간**). `★` 는 상한·양 끝 강제 포함이다.

**`A` 는 샤프트 중공 단면적**이다.

$$
A = \frac{\pi}{4}\left(d^{2} - ID^{2}\right), \qquad ID = \left\lfloor \left(d^{4} - \frac{32\,W\,d}{\pi}\right)^{1/4} \right\rfloor, \qquad W = 1.393\times10^{9}\ \mathrm{mm^{3}}
$$

`ID` 는 두께 규칙이 정하는 샤프트 내경이다(§7-6.7.5 · 목표 DIN 743 안전율 5.2 · `floor` 는 안전측). 벽두께로 쓰면 `A = π·t·(d − t)` 로도 같다.

`샤프트 [t] ≈ A × L × 7.85` 로 검산되므로, 샤프트 질량이 **단면과 길이 중 어느 쪽에서 왔는지**를 표에서 바로 분해할 수 있다(§8-6.5.1 과 같은 분해).

**「축 SF」는 DIN 743:2012-12 무한수명 피로 안전율**이다. 하중은 **극한 LC 가 아니라** `DLC1.2-k-s5` 의 **최약 빈 #11** 대표하중(`k` = 0.5 · 6.342 rpm · `Mx` 42.5 / `My` 42.8 / `Mz` 48.3 kNm)이다 — 두께 규칙 `W` = 1.393×10⁹ mm³ 를 유도한 바로 그 하중이며(§7-6.7.5), 극한 LC `Myz_max` 는 실운전 대비 과대하고 두께 규칙의 근거 하중도 아니라 쓰지 않는다(§8-6.2). `#1`·`#103`·`#210` 세 대표점만 계산했다.

**목표 5.2 가 그대로 재현된다** — 5.223 ~ 5.232 로 편차 0.2 % 다. 두께 규칙이 단면계수를 고정하므로 보어가 2,907 → 4,199 mm 로 변해도 굽힘응력이 같아지기 때문이며, **스팬(`z1`·`z2`)이 달라지는 프론트 설계들에서도 규칙이 의도대로 작동함**을 보여준다. 같은 하중의 영구변형 방지 안전율은 15.38 ~ 15.52 로 훨씬 크다.

**「정규화」는 이 표 33행 안에서의 min-max 값**이다 — `(최대 − 값) / (최대 − 최소)` 로, **최소가 1.00 · 최대가 0.00** 이다(작을수록 좋으므로 클수록 우수). 기준집합이 이 표이므로 대표점을 다르게 솎으면 값도 달라진다 — 프론트 전량 기준 값은 `a10_pareto.csv` 에서 직접 계산하면 된다.

|   #   | **선정** | `D` 구간 [mm] | 구간 점수 |      `D` [mm] | **베어링** [t] | 정규화 | 샤프트 [t] | **총질량** [t] | 정규화 | `D_pw` [mm] | d [mm] | t [mm] |  d/t | **A** [m²] | **L** [m] | α [°] | `D_we` [mm] | `L_we` [mm] | 세장비 |  Z | **z2** [m] | σ_max [MPa] | σ 여유 [MPa] | **축 SF** |
| :----: | :-------------: | --------------: | --------: | --------------: | -------------------: | -----: | ---------: | -------------------: | -----: | ------------: | -----: | -----: | ---: | ----------------: | --------------: | ------: | ------------: | ------------: | -----: | -: | ---------------: | -----------: | ------------: | --------------: |
|  1 ★  |       —       |   4,000 ~ 4,049 |        — | **4,039** |      **27.54** |   0.00 |      117.9 |     **173.00** |   0.00 |         3,481 |  2,907 |  282.0 | 10.3 |              2.33 |             6.5 |      15 |         227.6 |         533.5 |  2.382 | 44 |    **6.0** |      2,096.9 |           3.1 | **5.224** |
|   7   | **둘 다** |   4,039 ~ 4,088 |         7 | **4,086** |      **26.65** |   0.08 |      112.2 |     **165.53** |   0.11 |         3,545 |  2,989 |  258.0 | 11.6 |              2.21 |             6.5 |      15 |         217.4 |         521.4 |  2.438 | 47 |    **6.0** |      2,098.4 |           1.6 |              — |
|   21   | **둘 다** |   4,089 ~ 4,138 |        14 | **4,128** |      **25.69** |   0.17 |      109.3 |     **160.65** |   0.18 |         3,591 |  3,040 |  245.5 | 12.4 |              2.16 |             6.5 |      15 |         224.6 |         503.2 |  2.279 | 46 |    **6.0** |      2,099.0 |           1.0 |              — |
|   30   |    베어링↓    |   4,139 ~ 4,188 |        11 | **4,185** |      **25.09** |   0.22 |      104.8 |     **155.00** |   0.26 |         3,660 |  3,120 |  227.5 | 13.7 |              2.07 |             6.5 |      15 |         220.0 |         491.8 |  2.275 | 48 |    **6.0** |      2,093.0 |           7.0 |              — |
|   32   |    총질량↓    |   4,139 ~ 4,188 |        11 | **4,188** |      **25.44** |   0.19 |      103.3 |     **154.22** |   0.27 |         3,660 |  3,118 |  228.0 | 13.7 |              2.07 |             6.4 |      15 |         220.0 |         496.2 |  2.295 | 48 |    **5.9** |      2,096.6 |           3.4 |              — |
|   37   |    베어링↓    |   4,189 ~ 4,238 |         7 | **4,232** |      **23.95** |   0.32 |      100.0 |     **147.92** |   0.36 |         3,733 |  3,220 |  208.5 | 15.4 |              1.97 |             6.5 |      15 |         199.8 |         480.6 |  2.448 | 54 |    **6.0** |      2,098.7 |           1.3 |              — |
|   39   |    총질량↓    |   4,189 ~ 4,238 |         7 | **4,235** |      **24.00** |   0.32 |       99.7 |     **147.66** |   0.37 |         3,736 |  3,223 |  207.5 | 15.5 |              1.97 |             6.5 |      15 |         199.8 |         481.2 |  2.451 | 54 |    **6.0** |      2,098.2 |           1.8 |              — |
|   48   | **둘 다** |   4,239 ~ 4,288 |         9 | **4,285** |      **23.23** |   0.39 |       96.7 |     **143.12** |   0.43 |         3,795 |  3,292 |  196.0 | 16.8 |              1.91 |             6.5 |      15 |         198.7 |         468.3 |  2.400 | 55 |    **6.0** |      2,099.7 |           0.3 |              — |
|   52   |    베어링↓    |   4,289 ~ 4,338 |         9 | **4,301** |      **23.11** |   0.40 |       96.1 |     **142.27** |   0.44 |         3,812 |  3,310 |  193.5 | 17.1 |              1.89 |             6.5 |      15 |         199.8 |         465.0 |  2.370 | 55 |    **6.0** |      2,097.0 |           3.0 |              — |
|   57   |    총질량↓    |   4,289 ~ 4,338 |         9 | **4,316** |      **23.35** |   0.38 |       93.9 |     **140.60** |   0.47 |         3,828 |  3,326 |  191.0 | 17.4 |              1.88 |             6.4 |      15 |         197.3 |         467.5 |  2.413 | 56 |    **5.9** |      2,099.6 |           0.4 |              — |
|   66   | **둘 다** |   4,339 ~ 4,388 |         9 | **4,367** |      **22.01** |   0.50 |       92.1 |     **136.09** |   0.53 |         3,894 |  3,408 |  179.0 | 19.0 |              1.82 |             6.5 |      15 |         194.0 |         447.9 |  2.353 | 58 |    **6.0** |      2,099.2 |           0.8 |              — |
|   76   |    베어링↓    |   4,389 ~ 4,438 |        16 | **4,415** |      **21.53** |   0.54 |       90.9 |     **133.95** |   0.56 |         3,936 |  3,445 |  174.5 | 19.7 |              1.79 |             6.5 |      15 |         210.2 |         432.8 |  2.100 | 54 |    **6.0** |      2,099.4 |           0.6 |              — |
|   82   |    총질량↓    |   4,389 ~ 4,438 |        16 | **4,438** |      **21.69** |   0.53 |       90.0 |     **133.38** |   0.57 |         3,958 |  3,466 |  171.5 | 20.2 |              1.78 |             6.5 |      15 |         210.5 |         433.3 |  2.099 | 54 |    **6.0** |      2,096.2 |           3.8 |              — |
|   96   |    베어링↓    |   4,439 ~ 4,488 |        18 | **4,470** |      **20.96** |   0.59 |       88.2 |     **130.09** |   0.62 |         4,001 |  3,520 |  165.0 | 21.3 |              1.74 |             6.5 |      15 |         205.8 |         423.2 |  2.098 | 56 |    **6.0** |      2,099.6 |           0.4 |              — |
|  100  |    총질량↓    |   4,439 ~ 4,488 |        18 | **4,488** |      **21.50** |   0.54 |       86.6 |     **129.59** |   0.63 |         4,016 |  3,531 |  164.0 | 21.5 |              1.73 |             6.4 |      15 |         205.8 |         429.2 |  2.127 | 56 |    **5.9** |      2,099.0 |           1.0 |              — |
| 102 ★ |       —       |   4,450 ~ 4,499 |        — | **4,498** |      **20.94** |   0.59 |       87.1 |     **128.94** |   0.64 |         4,033 |  3,556 |  161.0 | 22.1 |              1.72 |             6.5 |      15 |         202.4 |         422.9 |  2.132 | 57 |    **6.0** |      2,098.3 |           1.7 |              — |
| 103 ★ |       —       |   4,450 ~ 4,499 |        — | **4,498** |      **21.43** |   0.55 |       86.0 |     **128.83** |   0.64 |         4,028 |  3,546 |  162.0 | 21.9 |              1.72 |             6.4 |      15 |         204.2 |         427.8 |  2.137 | 57 |    **5.9** |      2,092.9 |           7.1 | **5.223** |
|  110  | **둘 다** |   4,489 ~ 4,538 |        10 | **4,538** |      **20.25** |   0.66 |       85.5 |     **125.96** |   0.68 |         4,078 |  3,606 |  155.5 | 23.2 |              1.69 |             6.5 |      15 |         206.0 |         409.8 |  2.031 | 57 |    **6.0** |      2,098.9 |           1.1 |              — |
|  120  |    베어링↓    |   4,539 ~ 4,588 |        13 | **4,579** |      **19.93** |   0.68 |       83.5 |     **123.33** |   0.72 |         4,134 |  3,677 |  148.5 | 24.8 |              1.65 |             6.5 |      15 |         191.1 |         408.7 |  2.184 | 62 |    **6.0** |      2,099.2 |           0.8 |              — |
|  122  |    총질량↓    |   4,539 ~ 4,588 |        13 | **4,581** |      **20.31** |   0.65 |       82.0 |     **122.62** |   0.73 |         4,137 |  3,681 |  148.0 | 24.9 |              1.64 |             6.4 |      15 |         185.9 |         415.0 |  2.279 | 64 |    **5.9** |      2,099.3 |           0.7 |              — |
|  132  | **둘 다** |   4,589 ~ 4,638 |         9 | **4,637** |      **19.10** |   0.76 |       81.0 |     **119.19** |   0.78 |         4,204 |  3,760 |  140.5 | 26.8 |              1.60 |             6.5 |      15 |         186.9 |         395.1 |  2.160 | 65 |    **6.0** |      2,099.3 |           0.7 |              — |
|  144  |    베어링↓    |   4,639 ~ 4,688 |        15 | **4,674** |      **19.06** |   0.76 |       80.0 |     **118.10** |   0.79 |         4,244 |  3,802 |  137.0 | 27.8 |              1.58 |             6.5 |      15 |         185.8 |         392.8 |  2.160 | 66 |    **6.0** |      2,093.1 |           6.9 |              — |
|  147  |    총질량↓    |   4,639 ~ 4,688 |        15 | **4,685** |      **19.44** |   0.73 |       78.8 |     **117.67** |   0.80 |         4,250 |  3,804 |  137.0 | 27.8 |              1.58 |             6.4 |      15 |         188.0 |         396.5 |  2.155 | 65 |    **5.9** |      2,098.0 |           2.0 |              — |
|  152  |    베어링↓    |   4,689 ~ 4,738 |         7 | **4,727** |      **18.41** |   0.82 |       78.0 |     **114.80** |   0.84 |         4,310 |  3,882 |  130.5 | 29.7 |              1.54 |             6.5 |      15 |         177.1 |         384.9 |  2.222 | 70 |    **6.0** |      2,099.0 |           1.0 |              — |
|  153  |    총질량↓    |   4,689 ~ 4,738 |         7 | **4,733** |      **18.90** |   0.78 |       76.7 |     **114.45** |   0.85 |         4,317 |  3,890 |  130.0 | 29.9 |              1.54 |             6.4 |      15 |         170.6 |         392.9 |  2.353 | 73 |    **5.9** |      2,097.7 |           2.3 |              — |
|  165  |    총질량↓    |   4,739 ~ 4,788 |        12 | **4,783** |      **18.44** |   0.82 |       75.0 |     **111.90** |   0.88 |         4,373 |  3,952 |  125.0 | 31.6 |              1.50 |             6.4 |      15 |         170.5 |         384.5 |  2.306 | 74 |    **5.9** |      2,097.5 |           2.5 |              — |
|  166  |    베어링↓    |   4,739 ~ 4,788 |        12 | **4,786** |      **18.04** |   0.86 |       75.9 |     **111.94** |   0.88 |         4,381 |  3,965 |  124.0 | 32.0 |              1.50 |             6.5 |      15 |         168.2 |         380.1 |  2.311 | 75 |    **6.0** |      2,097.9 |           2.1 |              — |
|  176  |    베어링↓    |   4,789 ~ 4,838 |        11 | **4,821** |      **17.63** |   0.89 |       75.2 |     **110.48** |   0.90 |         4,412 |  3,993 |  122.0 | 32.7 |              1.48 |             6.5 |      15 |         179.2 |         368.5 |  2.104 | 71 |    **6.0** |      2,099.1 |           0.9 |              — |
|  177  |    총질량↓    |   4,789 ~ 4,838 |        11 | **4,825** |      **18.18** |   0.84 |       73.9 |     **110.29** |   0.91 |         4,419 |  4,002 |  121.5 | 32.9 |              1.48 |             6.4 |      15 |         170.0 |         378.9 |  2.279 | 75 |    **5.9** |      2,096.9 |           3.1 |              — |
|  188  | **둘 다** |   4,839 ~ 4,888 |        11 | **4,880** |      **17.24** |   0.93 |       73.4 |     **107.87** |   0.94 |         4,489 |  4,088 |  116.0 | 35.2 |              1.45 |             6.5 |      15 |         161.7 |         366.6 |  2.320 | 80 |    **6.0** |      2,099.3 |           0.7 |              — |
|  195  |    베어링↓    |   4,889 ~ 4,938 |        13 | **4,909** |      **17.16** |   0.93 |       73.0 |     **107.36** |   0.95 |         4,511 |  4,103 |  115.0 | 35.7 |              1.44 |             6.5 |      15 |         173.6 |         359.7 |  2.122 | 75 |    **6.0** |      2,094.6 |           5.4 |              — |
|  200  |    총질량↓    |   4,889 ~ 4,938 |        13 | **4,931** |      **17.93** |   0.87 |       70.6 |     **106.42** |   0.96 |         4,524 |  4,106 |  114.5 | 35.9 |              1.44 |             6.3 |      15 |         178.9 |         366.9 |  2.099 | 73 |    **5.8** |      2,094.4 |           5.6 |              — |
| 210 ★ | **둘 다** |   4,939 ~ 4,988 |         9 | **4,983** |      **16.44** |   1.00 |       71.0 |     **103.88** |   1.00 |         4,596 |  4,199 |  109.0 | 38.5 |              1.40 |             6.5 |      15 |         170.0 |         347.6 |  2.095 | 78 |    **6.0** |      2,099.8 |           0.2 | **5.232** |

#### 10-11.2.3. 읽을 것

`182 × 150 = 27,300` 예산 · MASTA **24,979회** · 해석식 탈락 1,111(4.1%) ·
**119.3분 · 0.287 s/점**. 가능해 **31.2%**(7,789 / 24,979) — 부록 9(49.5%)보다
낮은데, `z2` 가 변수로 돌아와 짧은 스팬이 σ 를 못 지키는 조합이 늘었기 때문이다.

**① 프론트가 210건이다.** 부록 9(2목적 · 53건)의 4배다. 3목적 곡면이 실제로
채워졌고, `D` 4,039 ~ 4,983 mm 를 촘촘히 덮는다.

**② 세 부록의 최적이 순서대로 좋아진다.**

|                   | 제약·목적                            | 최소 베어링 [t] |  최소 총질량 [t] |
| ----------------- | ------------------------------------- | --------------: | ---------------: |
| 부록 8            | `D_pw ≤ 4,500` · (베어링, 총질량) |              — |           105.84 |
| 부록 9            | `D ≤ 5,000` · (D, 베어링)         |           16.69 |           104.96 |
| **부록 10** | `D ≤ 5,000` · (D, 베어링, 총질량) | **16.44** | **103.88** |

부록 9 대비 베어링 **−0.25 t** · 총질량 **−1.08 t** 로, **총질량을 목적에 넣은
효과가 실제로 나왔다.**

**③ `D` ≤ 5,000 에서는 두 최적이 같은 설계다.** 최소 베어링(16.44 t)과 최소
총질량(103.88 t)이 한 점이다 — 외경 여유가 충분하면 베어링을 줄이는 것이
총질량에도 최선이다. 반면 **`D` ≤ 4,500 에서는 갈린다** — 최소 베어링은
20.94 t(총 128.94), 최소 총질량은 128.83 t(베어링 21.43). 외경을 조이면
**베어링을 0.49 t 더 쓰고 축을 100 mm 줄여 총질량 0.11 t 를 아끼는** 절충이
생긴다. **3목적이 실제로 절충을 만든 유일한 지점**이다.

**④ `z2` 는 여전히 상단에 몰린다 — 5.6 ~ 6.0 (5수준).** 26수준을 열었는데
프론트는 그중 다섯만 쓴다. 총질량을 목적에 넣어도 **긴 스팬이 이긴다** —
스팬을 줄이면 베어링 하중이 커져 σ 경계에서 베어링을 키워야 하고, 그 증가분이
2배로 총질량에 실려 샤프트 절감을 넘어선다.

> 부록 8 의 총질량 최적이 `z2` = 4.5 였던 것과 대비된다(§8-6.3a). 그때는
> `D_pw` ≤ 4,500 이라 베어링을 키우는 비용이 컸지만, `D` ≤ 5,000 으로 외경
> 여유가 생기자 **긴 스팬 + 큰 피치원 + 작은 베어링** 조합이 이긴다.
> 그 결과 **샤프트 길이 `L` 은 6.1 ~ 6.5 m 로 6% 폭**인 반면 **단면적 `A` 는
> 1.40 ~ 2.33 m² 로 67% 폭**이다 — 샤프트 질량 변화는 사실상 **단면적이
> 지배**하고, 그 단면적은 두께 규칙이 보어에서 자동으로 정한다.

**⑤ 수렴이 미완이다.** ΔHV 가 **150세대까지 0.05% 이상**으로 계속 올랐고,
마지막 10세대 누적이 **+0.618%** 다. 95·125·143세대에 늦은 튐도 있었다.
§6-11.5 가 조기 종료를 배제한 근거와 같은 양상이며, **150세대는 이 문제에
부족하다**(§10-13 ①).

**⑥ 프론트가 65세대부터 개체수 전체(182)로 포화됐다.** 모든 개체가 비지배라
순위 선택이 멈추고 혼잡도 거리만으로 선택이 이뤄진다 — §10-7 ② 로 적어 둔
「3목적 NSGA-II 의 혼잡도 정렬 약화」가 실측으로 나타났다. 개체수를 키우면
완화되지만 예산이 비례해 늘어난다.

*프론트 **전량 210건**은 `부록10_NSGA/S3_본최적화/a10_pareto.csv` · 정리 `a10_result.py`*

<!-- /A10:RES -->

### 10-11.3. 형상 분석 — 외륜이 롤러를 덮지 못한다

MASTA 에서 §10-11.2.2 의 `#1` 을 열어 보면 **외륜이 롤러를 좌측에서 충분히
덮지 못한다.** v1.3 기준선은 좌우로 넉넉히 덮는데, 부록 10 의 큰 설계에서
형상이 무너진다.

<!-- A10:SHAPE -->

*형상 비교 그림(v1.3 대 `#1`) — MASTA 캡처를 `figures/` 에 넣으면 링크한다.*

<!-- /A10:SHAPE -->

#### 10-11.3.1. 원인 — `element_offset` 만 비례하지 않았다

§6-4.1 은 종속변수를 **v1.3 실측 비율로 확대·축소**한다고 선언했다. 실제로
`T`·`B`·`C` 는 물론 MASTA 가 내부에서 잡는 **외륜 오프셋까지 `L_w` 에 정확히
비례**한다. **롤러의 축방향 오프셋(`element_offset`) 하나만 예외**다 — v1.4
모델 파일의 값 **20.742 mm 에 고정**된 채 한 번도 갱신되지 않았다.

|                             | `L_w` [mm] |             `T` |             `C` |         **`element_offset`** |                          외륜 오프셋 |
| --------------------------- | -----------: | ----------------: | ----------------: | -----------------------------------: | -----------------------------------: |
| v1.3 기준선                 |        238.0 |               310 |               253 |                      **20.74** |                                33.50 |
| 부록 10`#1`               |        542.1 |               706 |               576 |                      **20.74** |                                76.50 |
| 부록 10`#210`             |        356.2 |               464 |               379 |                      **20.74** |                                50.00 |
| **`L_w` 대비 비율** |           — | **1.30226** | **1.06281** | **0.0871 → 0.0383 → 0.0582** | **0.1407 · 0.1411 · 0.1404** |

**외륜 오프셋은 세 설계에서 0.1404 ~ 0.1411 로 일정**한데(정수화 반올림 잔차),
`element_offset` 은 0.0871 → 0.0383 으로 **절반 이하로 줄어든다.** 베어링이
커질수록 롤러가 상대적으로 한쪽에 치우치고, 그것이 그림에 나타난 것이다.

**폭이 모자란 것이 아니다.** 외륜폭에서 롤러의 축방향 투영을 뺀 여유는 세
설계 모두 비슷하다.

|          | `C` | 롤러 투영`L_w·cosα` |  여유 |   비율 |
| -------- | ----: | ----------------------: | ----: | -----: |
| v1.3     | 253.0 |                   225.1 | +27.9 | +12.4% |
| `#1`   | 576.0 |                   523.6 | +52.4 | +10.0% |
| `#210` | 379.0 |                   344.1 | +34.9 | +10.2% |

**위치 문제이지 폭 문제가 아니다.**

#### 10-11.3.2. MASTA 파라미터 실측 — 무엇을 건드리면 무엇이 따라오는가

`#1` 상태에서 세 값을 각각 흔들어 부작용을 확인했다.

| 조작                             |                `T` |                `C` | `a`(유효 하중중심) | `L_we` | 판정                                                            |
| -------------------------------- | -------------------: | -------------------: | -------------------: | -------: | --------------------------------------------------------------- |
| `element_offset` **+20** |                  706 |                  576 | **812 → 792** |    533.5 | **롤러만 이동** · `a` 가 **1:1 반대**로 움직인다 |
| `outer_ring_offset` +20        | **706 → 726** |                  576 |           812 → 832 |    533.5 | **`T` 가 함께 늘어** 독립 조작이 안 된다                |
| `C` ×1.10                     |                  706 | **576 → 634** | **812 (불변)** |    533.5 | 컵이**좌우 대칭**으로 커진다 · `a` 무영향              |

**`element_offset` 은 설정 가능하고 롤러만 움직인다.** 다만 `a` 가 정확히
같은 크기만큼 반대로 이동한다 — `a` 는 **롤러 위치의 결과**이지 독립 입력이
아니라는 뜻이다.
...
| 안           | 방법                                                         | 장점                                                                                                                                       | 대가                                                                                                                                                     |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **⑴** | `element_offset` 을 **`L_w` 비례 종속변수**로 추가 | §6-4.1 원칙과 일치 · 형상이 v1.3 의 확대판이 된다 ·`T`·`B`·`C`·`d`·`D` 불변이라 **`C6`·살두께 제약에 영향 없음** | `a` 가 바뀌어 **`L_eff` 가 −0.3 ~ −0.9% 이동** → σ·피로 재계산 필요                                                                       |
| ⑵           | `C` 를 키워 덮는다                                         | `a`·`L_eff` 불변 → **기존 결과 보존**                                                                                          | `t_o = 0.17215·C` 이므로 **`D` 가 커져 `C6`(`D ≤ 5,000`)에 직접 부딪힌다** · 외륜 질량 증가 · 위치 오차를 덮기만 할 뿐 고치지는 않는다 |
| ⑶           | `a` 를 직접 지정                                           | 목표 형상을 바로 지정                                                                                                                      | `outer_ring_offset` 과 `T` 가 따라 바뀌어 제어가 뒤엉킨다(실측 확인)                                                                                 |

**⑴ 이 원칙에 맞다.** 문제는 **어떤 비례식을 쓸 것인가**이고, 그것이 아직
확정되지 않았다(§10-11.3.4).

#### 10-11.3.4. 규칙은 조사 중이다 — 두 가설

`L_w` 비례가 맞다 해도 **비례상수를 v1.3 에서 역산한 0.08713 으로 두는 것이
옳은지**는 확인되지 않았다. 실제 테이퍼 롤러 베어링에서 롤러의 축방향 위치는
자유롭게 고르는 값이 아닐 수 있다.

| 가설                   | 내용                                                                                           | 함의                                                                  |
| ---------------------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| **A. apex 조건** | 내륜 궤도·외륜 궤도·롤러의 세 원뿔**꼭짓점이 축 위 한 점에서 만나야** 순수 구름이 된다 | 위치가**각도와 지름에서 유도**된다 — 비례상수를 쓸 자유가 없다 |
| **B. 궤도 중앙** | 롤러 접촉선의 중점이**외륜 궤도면 중앙**에 오도록 잡는다                                 | 설계 관행 · 리브·케이지 사정에 따라 치우칠 수 있다                  |

**가설 B 를 채택했고 수식이 닫혔다(§10-11.3.6).** 아래는 조사 당시의 기록이다. 두 가설이 같은 답을 주는지, 다르면 무엇이 우선인지가 조사의 핵심이었다. **α 가
설계마다 15 ~ 30° 로 달라지므로**(부록 10 프론트는 15°에 몰려 있다) 가설 A 가
맞다면 단순 `L_w` 비례는 근사에 그친다.

> **SKF 등 실제 TRB 카탈로그로 규칙을 확인 중이다.** 카탈로그는 형번마다
> **유효 하중중심 `a` 를 공표**하므로, `a/T`·`C/T` 같은 비율이 크기에 따라
> 일정한지 보면 규칙을 역산할 수 있다. 도출된 규칙으로 v1.3 의
> `a` = 713.33 mm 가 재현되는지가 검증 기준이다.

#### 10-11.3.5. 현재 상태와 영향

**`element_offset` 을 종속변수 목록에 등재하되 값은 바꾸지 않았다** —
현행 20.742 mm 고정을 그대로 둔다. 따라서 **부록 8·9·10 의 기존 결과는 모두
그대로 유효**하고, 규칙이 확정되면 한 번에 반영해 재실행한다.

바꿀 경우의 영향은 해석식으로 미리 추정해 두었다(`L_eff = (z2−z1) + 2(a − T/2)`).

|          | 현행`a` | `0.08713·L_w` 적용 시 `a` |                 `L_eff` |              변화 |
| -------- | --------: | -----------------------------: | ------------------------: | ----------------: |
| `#1`   |    812.00 |                         785.51 | 5.918 →**5.865 m** | **−0.90%** |
| `#210` |    835.32 |                         825.02 | 6.207 →**6.186 m** |           −0.33% |

`L_eff` 는 **스크리닝 상수이자 하중분배의 기준**이므로 σ 와 피로가 함께
움직인다. 규칙 확정 후 대표 설계 몇 건으로 **σ 변화를 먼저 실측**하고, 그
크기를 보고 재최적화 범위(부록 10 만 · 또는 부록 8·9 까지)를 정한다.

> **이 추정은 폐기됐다.** `0.08713·L_w` 비례 가설로 계산한 값이며, 확정된 규칙(§10-11.3.6)은 `L_eff` 를 그보다 2 ~ 3배 큰 **−0.94 ~ −1.88 %** 움직인다.

*규칙 확정 완료(§10-11.3.6) — 다만 현행 값은 유지한다.*

#### 10-11.3.6. 규칙 확정 — 가설 B (궤도 중앙)

<!-- A10:EOFF -->

**가설 B 를 채택한다** — 롤러 접촉선의 중점이 외륜 궤도 모선의 중점에 오도록 잡는다. 그 수식을 MASTA 실측으로 닫았다.

##### ⑴ MASTA 의 정의와 좌표계

MASTA 툴팁이 `element_offset` 을 **「롤러 중심과 그 롤러가 속한 row 중심 사이의 축방향 거리」** 로 정의한다. 같은 프레임에서 세 물체의 축방향 위치를 `mass_properties_of_*_from_geometry` 로 읽을 수 있다.

| 물체           | MASTA 속성                                      |                       v1.4 원본`z` [mm] |
| -------------- | ----------------------------------------------- | ----------------------------------------: |
| **롤러** | `mass_properties_of_elements_from_geometry`   | **+20.7416** (= `element_offset`) |
| 내륜           | `mass_properties_of_inner_ring_from_geometry` |                                  −63.709 |
| 외륜           | `mass_properties_of_outer_ring_from_geometry` |                                   +74.577 |

`element_offset` 을 0 → 20.742 → 50 → 76.5 으로 흔들면 롤러 중심 `z` 가 **정확히 같은 값**으로 따라온다. **롤러 중심 `z` = `element_offset`** 이 확인됐다.

외륜의 위치는 `outer_ring_offset` 이 준다. 그 값이 무엇인지 네 설계에서 역산했더니 **mm 단위로 닫히는 항등식**이 나왔다.

|             | `T` | `B` | `C` | `T − (B+C)/2` | MASTA`outer_ring_offset` |
| ----------- | ----: | ----: | ----: | ---------------: | -------------------------: |
| v1.4 원본   |   420 |   400 |   350 |             45.0 |             **45.0** |
| v1.3 기준선 |   310 |   300 |   253 |             33.5 |             **33.5** |
| `#1`      |   706 |   683 |   576 |             76.5 |             **76.5** |
| `#103`    |   568 |   550 |   464 |             61.0 |             **61.0** |
| `#210`    |   464 |   449 |   379 |             50.0 |             **50.0** |

즉 `outer_ring_offset` 은 **외륜 중심의 row 중심 대비 축방향 위치**이고, 우리가 넣는 `T`·`B`·`C` 에서 MASTA 가 유도한다. 두 오프셋이 같은 원점을 쓰므로 **편심량이 곧 두 값의 차**다.

$$
\varepsilon = \underbrace{o_{\text{off}}}_{\text{외륜 중심}}-\underbrace{e_{\text{off}}}_{\text{롤러 중심}}
$$

##### ⑵ 수식

조건은 **롤러 접촉선의 중점이 외륜 궤도 모선의 중점에 오는 것**이다. 그런데 `element_offset` 이 가리키는 것은 **롤러 중심**이지 접촉선이 아니다 — 둘은 같은 점이 아니므로 보정이 붙는다.

$$
\boxed{\;e_{\text{off}} = o_{\text{off}} - \frac{D_{we}}{2}\sin\!\left(\alpha-\beta\right)\;}
$$

| 기호      | 의미                                  | MASTA                                                                             |
| --------- | ------------------------------------- | --------------------------------------------------------------------------------- |
| `o_off` | 외륜 중심의 row 중심 대비 축방향 위치 | `outer_ring_offset` ( = `T` − (`B`+`C`)/2 · ⑴ 에서 5설계 mm 일치 확인) |
| `D_we`  | 롤러 평균 지름                        | `element_diameter`                                                              |
| `α`    | 궤도 경사각                           | `cup_angle` ( = `contact_angle`)                                              |
| `β`    | **롤러 반각**                   | `element_taper_angle` / 2 ( = (`cup_angle` − `cone_angle`)/2 )             |

보정항 `(D_we/2)·sin(α−β)` 는 **롤러 중심에서 접촉선까지의 축방향 성분**이다. 롤러 축의 경사가 `cone_angle` + `β` = **`α` − `β`** 이고 접촉선은 그 축에서 반지름 `D_we`/2 만큼 수직으로 떨어져 있으므로, 그 수직 변위의 축방향 성분이 곧 보정항이 된다.

| 설계     | `T` | `B` | `C` | `o_off` | `D_we` |    `α` |  `2β` | `α−β` (롤러축) |  보정 | **`e_off`** |
| -------- | ----: | ----: | ----: | --------: | -------: | --------: | -------: | ------------------: | ----: | ------------------: |
| `#1`   |   706 |   683 |   576 |     76.50 |    227.6 | 15.0000° | 1.8236° |           14.0882° | 27.70 |     **48.80** |
| `#103` |   568 |   550 |   464 |     61.00 |    204.2 | 15.0000° | 1.4332° |           14.2834° | 25.19 |     **35.81** |
| `#210` |   464 |   449 |   379 |     50.00 |    170.0 | 15.0000° | 1.0591° |           14.4704° | 21.24 |     **28.76** |

> `element_taper_angle` 이 **`2β`** 라는 것은 §4 의 도면 기호 대응표(`2β` = `cup_angle` − `cone_angle`)와 v1.3 검산(19.000 − 17.800 = 1.200°)으로 이미 확정돼 있다. 여기서도 `α` − `β` 가 `cone_angle` + `β` 와 일치함을 확인했다.

보정항이 `D_we` 에 비례하므로 **큰 롤러일수록 보정이 크다**(27.7 → 21.2 mm). 그 결과 신규 `e_off` 는 현행 20.74 와 외륜 중심 `o_off` 의 **중간**에 놓인다.

##### ⑶ 적용값

| 설계               | 현행`e_off` | **신규 `e_off`** | `Δ` |        `a` 현행 → 신규 |      `L_eff` 현행 → 신규 |               변화 |
| ------------------ | ------------: | -----------------------: | -----: | ------------------------: | --------------------------: | -----------------: |
| **`#1`**   |         20.74 |          **48.80** | +28.06 | 812.00 →**783.94** | 5,918.0 →**5,861.9** | **−0.95 %** |
| **`#103`** |         20.74 |          **35.81** | +15.07 | 813.23 →**798.16** | 5,958.5 →**5,928.3** | **−0.51 %** |
| **`#210`** |         20.74 |          **28.76** |  +8.02 | 835.32 →**827.30** | 6,206.6 →**6,190.6** | **−0.26 %** |

`a` 는 `e_off` 와 **정확히 1:1 반대**로 움직인다(§10-11.3.2 실측). `L_eff` = (`z2`−`z1`) + 2(`a` − `T`/2) 이므로 `L_eff` 는 `2Δa` 만큼 줄어든다.

> §10-11.3.5 는 `0.08713·L_w` 비례 가설로 `L_eff` 변화를 **−0.90 % / −0.33 %** 로 추정했었다. 확정된 규칙의 **−0.26 ~ −0.95 %** 와 크기가 비슷하지만, 그 추정은 근거가 다른 별개의 가설이었으므로 폐기한다.

##### ⑷ 영향 — σ

지배 LC `Myz_max` 에서 쟀다.

| 설계     | σ 현행 |           σ 신규 | **Δσ** | σ ≤ 2,100 |
| -------- | ------: | ----------------: | -------------: | :---------: |
| `#1`   | 2,096.9 | **2,105.8** |           +8.9 |  ❌ (+5.8)  |
| `#103` | 2,092.9 | **2,097.7** |           +4.8 |     ✅     |
| `#210` | 2,099.8 | **2,102.3** |           +2.5 |  ❌ (+2.3)  |

스팬이 짧아지면(`L_eff` −0.26 ~ −0.95 %) 베어링 하중이 커져 σ 가 오른다. **`#103` 은 규칙만으로 제약을 만족**하고, `#1`·`#210` 은 각각 **5.8 · 2.3 MPa** 넘는다 — 종전에 검토했던 「롤러 중심을 외륜 중심에 맞추는」 안(`e_off` = `o_off`)이 세 설계 모두 9 ~ 18 MPa 초과였던 것에 비하면 **초과폭이 1/3 이하**다. 보정항이 `e_off` 를 덜 밀기 때문이다.

##### ⑸ 비대칭 프로파일과 함께 걸면 세 설계 모두 만족한다

§10-12.6 의 비대칭 Fujiwara 프로파일을 같이 넣고 다시 쟀다.

| 설계     |    현행 | 신규`e_off` 만 | **신규 + 프로파일** | σ ≤ 2,100 |
| -------- | ------: | ---------------: | ------------------------: | :---------: |
| `#1`   | 2,096.9 |       2,105.8 ❌ |         **2,088.1** |     ✅     |
| `#103` | 2,092.9 |       2,097.7 ✅ |         **2,064.7** |     ✅     |
| `#210` | 2,099.8 |       2,102.3 ❌ |         **2,094.2** |     ✅     |

**세 설계 모두 만족한다.** 프로파일은 옛 `e_off` 기준으로 최적화된 것인데도 여유가 남는다(현행 대비 −8.8 / −28.2 / −5.6 MPa). 새 형상에서 다시 맞추면 더 나아질 여지가 있다.

##### ⑹ MASTA 파일

형상을 GUI 에서 직접 확인할 수 있도록 현행판과 신규판을 짝으로 저장했다 — `부록10_NSGA/S3_본최적화/MASTA/`.

| 설계     | 현행 (`e_off` = 20.74) | **신규**                                  | `a` 현행 → 신규 |
| -------- | ------------------------ | ----------------------------------------------- | -----------------: |
| `#1`   | `A10_front001.masta`   | **`A10_front001_offset.masta`** (48.80) |   812.00 → 783.94 |
| `#103` | `A10_front103.masta`   | **`A10_front103_offset.masta`** (35.81) |   813.23 → 798.16 |
| `#210` | `A10_front210.masta`   | **`A10_front210_offset.masta`** (28.76) |   835.32 → 827.30 |

롤러 프로파일은 **현행 DIN Lundberg 그대로**이고 지배 LC `Myz_max`(토크 22,673 kNm · speed 0)가 들어간 상태다. `#130` 은 `D` ≤ 4,500 최적이 아니어서 파일을 지우고 `#103` 으로 교체했다.

> **`_offset` 파일 중 `#1`·`#210` 은 σ 제약을 위반한 상태다**(⑷). 형상 확인용이며, 제약을 만족시키려면 비대칭 프로파일을 함께 넣어야 한다(⑸).

##### ⑺ 판정

| 항목                      | 결론                                                                                                                          |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| **규칙**            | **확정** — `e_off` = `o_off` − (`D_we`/2)·sin(`α`−`β`)                                                  |
| **근거**            | 접촉선 중점을 궤도 모선 중점에 맞추는 별도 유도 + MASTA 정의(툴팁)·질량중심 프레임 실측(⑴)·`o_off` 항등식(5설계 mm 일치) |
| **단독 적용**       | `#103` 만 가능. `#1`·`#210` 은 각각 5.8 · 2.3 MPa 초과                                                                |
| **프로파일과 병행** | **세 설계 모두 만족**(2,064.7 ~ 2,088.1)                                                                                |
| **적용 시점**       | 두 변경을**한 묶음으로** 반영하고 부록 10 을 재실행하는 것이 맞다                                                       |

**현행 값(20.742 mm)은 그대로 유지한다.** 규칙은 확정됐지만 `#1`·`#210` 이 단독으로는 제약을 넘고, 이를 덮는 프로파일 변경은 가공성·특허 검토가 끝나지 않았다(§10-12.6.7). 두 조건이 풀리면 **함께** 반영한다.

*실행 — `Results/사이징최적화/a10_eoff_v2.py`(규칙·영향 · 규칙 함수 `rule_v2` 가 단일 소스) · `a10_save_masta.py ... offset`(파일 저장). 결과 `부록10_NSGA/S3_본최적화/element_offset/eoff_v2.json`.*

<!-- /A10:EOFF -->

## 10-12. 롤러 프로파일 최적화 검토

<!-- A10:PROFILE -->

### 10-12.1. 문제 제기 — 부록 1 의 판단은 최대응력만 본 것이다

부록 1 에서 롤러 프로파일을 **DIN Lundberg 로 충분**하다고 확인했다. 그 판단의 근거는 **최대 접촉응력 하나**였다. 그런데 부록 10 의 설계안들을 지배 하중 `Myz_max` 에서 다시 보면, 모멘트에 의한 정렬오차 때문에 **내륜 응력분포가 한쪽으로 크게 치우치고 접촉이 롤러 끝단까지 도달**한다 — 모서리 접촉(edge contact) 위험이다.

크기 감각을 먼저 잡아 둔다. 부록 8 프론트에서 UW 미스얼라인먼트는 **0.82 ~ 1.09 mrad** 였다(§8-6.3a). `#1` 의 반길이 267 mm 에 걸면 끝단에서 **약 220 ~ 290 μm** 의 간극 변화다. 반면 DIN Lundberg 의 크라운 낙차는 **수십 μm** 수준이다 — **정렬오차가 크라운보다 한 자릿수 크다.** 프로파일로 이것을 흡수하려면 크라운을 그만큼 깊게 파야 하고, 그러면 유효 접촉길이가 줄어 **σ_max 가 올라간다**. 그리고 **σ ≤ 2,100 MPa 는 최적화의 제약**이다(§3 C1). 이것이 이 절이 다루는 트레이드오프의 전부다.

### 10-12.2. 무엇을 어떻게 쟀는가

세 지표를 MASTA API 에서 직접 읽는다. 유도값이 아니라 실측값이다.

| 지표                | MASTA 속성                                     | 의미                                                        |
| ------------------- | ---------------------------------------------- | ----------------------------------------------------------- |
| **σ_max**    | `maximum_normal_stress_inner`                | 최적화 제약 (≤ 2,100 MPa)                                  |
| **끝단 응력** | `maximum_normal_edge_stress_inner` (행 단위) | 롤러 끝단에서의 응력.**0 이면 끝단이 완전히 떠 있다** |
| **접촉 손실** | `L_we − max(contact_length_inner)`          | 하중을 받지 못하는 롤러 길이                                |

대상은 §10-11.2.2 의 **`#1`(최소 `D`) · `#103`(중간) · `#210`(최소 총질량)** 세 설계, 지배 LC `Myz_max` 1건이다. 코너 반경 4.3 mm 는 그대로 넣었다.

MASTA 가 여는 프로파일 중 조절 손잡이가 있는 셋을 스윙했다.

| 프로파일                      | 손잡이                                              | 스윙 범위                                |
| ----------------------------- | --------------------------------------------------- | ---------------------------------------- |
| **DIN Lundberg** (현행) | `axial_offset`                                    | −60 ~ +20 mm · 9수준                   |
| **Johns-Gohar**         | `design_load` → `end_drop` 유도                | 실측 최대 롤러하중의 ×1 ~ ×16 · 5수준 |
| **Lundberg**            | `load` (`use_bearing_dynamic_capacity` = False) | ×1 ~ ×8 · 4수준                       |

총 **54회** 해석 · 0.3분. Johns-Gohar 의 `design_load` 는 모델 기본값이 **모든 설계에서 30 MN 으로 고정된 자리표시자**였으므로, 설계별 실측 최대 롤러하중(2.37 ~ 4.36 MN)에 배수를 곱해 넣었다.

### 10-12.3. 결과

![부록 10 롤러 프로파일 비교](Results/사이징최적화/figures/profile_a10.png)

*채운 표식은 σ_max ≤ 2,100 을 만족, 빈 표식은 위반. 세 설계 모두에서 **현행 DIN Lundberg(`axial_offset` = 0)가 가능영역의 좌하단 구석**을 차지한다 — 끝단응력과 접촉손실이 동시에 가장 낮은 점이다.*

**`#1`** — `L_we` = 533.5 mm · 롤러 기울기 0.56 mrad

| 프로파일               | 설정                                | σ_max [MPa] | 끝단 σ [MPa] | 접촉 [mm] | 손실 [%] |      판정      |
| ---------------------- | ----------------------------------- | -----------: | ------------: | --------: | -------: | :------------: |
| DIN Lundberg           | `a_off` = -20 mm                  |      2,098.4 |       2,019.1 |     469.4 |     12.0 |       ○       |
| DIN Lundberg           | `a_off` = -10 mm                  |      2,091.7 |       1,702.8 |     478.5 |     10.3 |       ○       |
| **DIN Lundberg** | `a_off` = 0 mm                    |      2,096.9 |         991.0 |     486.5 |      8.8 | **현행** |
| DIN Lundberg           | `a_off` = 10 mm                   |      2,100.3 |         997.0 |     486.6 |      8.8 |       ×       |
| DIN Lundberg           | `a_off` = 20 mm                   |      2,103.8 |       1,003.0 |     486.6 |      8.8 |       ×       |
| Johns-Gohar            | `P_d` = 4.36 MN · 낙차 68 μm    |      2,471.2 |       2,471.2 |     520.2 |      2.5 |       ×       |
| Johns-Gohar            | `P_d` = 8.72 MN · 낙차 135 μm   |      2,247.5 |       2,247.5 |     512.8 |      3.9 |       ×       |
| Johns-Gohar            | `P_d` = 17.45 MN · 낙차 271 μm  |      2,079.8 |       1,697.9 |     482.4 |      9.6 |       ○       |
| Johns-Gohar            | `P_d` = 34.89 MN · 낙차 541 μm  |      2,133.6 |           0.0 |     446.6 |     16.3 |       ×       |
| Johns-Gohar            | `P_d` = 69.78 MN · 낙차 1083 μm |      2,282.6 |           0.0 |     402.2 |     24.6 |       ×       |
| Lundberg               | `load` = 4.36 MN                  |      2,391.3 |       2,391.3 |     520.2 |      2.5 |       ×       |
| Lundberg               | `load` = 8.72 MN                  |      2,143.3 |       2,071.2 |     507.5 |      4.9 |       ×       |
| Lundberg               | `load` = 17.45 MN                 |      2,092.9 |       1,165.3 |     491.5 |      7.9 |       ○       |
| Lundberg               | `load` = 34.89 MN                 |      2,146.7 |           0.0 |     458.7 |     14.0 |       ×       |

**`#103`** — `L_we` = 427.8 mm · 롤러 기울기 0.54 mrad

| 프로파일               | 설정                                | σ_max [MPa] | 끝단 σ [MPa] | 접촉 [mm] | 손실 [%] |      판정      |
| ---------------------- | ----------------------------------- | -----------: | ------------: | --------: | -------: | :------------: |
| DIN Lundberg           | `a_off` = -20 mm                  |      2,099.0 |       2,075.7 |     379.2 |     11.4 |       ○       |
| DIN Lundberg           | `a_off` = -10 mm                  |      2,088.5 |       1,751.7 |     388.4 |      9.2 |       ○       |
| **DIN Lundberg** | `a_off` = 0 mm                    |      2,092.9 |         939.3 |     396.5 |      7.3 | **현행** |
| DIN Lundberg           | `a_off` = 10 mm                   |      2,096.3 |         945.9 |     396.6 |      7.3 |       ○       |
| DIN Lundberg           | `a_off` = 20 mm                   |      2,099.7 |         951.6 |     396.3 |      7.4 |       ○       |
| Johns-Gohar            | `P_d` = 3.35 MN · 낙차 65 μm    |      2,409.0 |       2,409.0 |     427.8 |      0.0 |       ×       |
| Johns-Gohar            | `P_d` = 6.70 MN · 낙차 130 μm   |      2,164.9 |       2,164.9 |     415.8 |      2.8 |       ×       |
| Johns-Gohar            | `P_d` = 13.40 MN · 낙차 259 μm  |      2,072.7 |       1,545.8 |     402.7 |      5.9 |       ○       |
| Johns-Gohar            | `P_d` = 26.80 MN · 낙차 518 μm  |      2,157.0 |           0.0 |     371.2 |     13.2 |       ×       |
| Johns-Gohar            | `P_d` = 53.59 MN · 낙차 1037 μm |      2,330.8 |           0.0 |     332.0 |     22.4 |       ×       |
| Lundberg               | `load` = 3.35 MN                  |      2,324.3 |       2,324.3 |     417.1 |      2.5 |       ×       |
| Lundberg               | `load` = 6.70 MN                  |      2,101.1 |       1,971.4 |     417.1 |      2.5 |       ×       |
| Lundberg               | `load` = 13.40 MN                 |      2,088.0 |       1,020.5 |     398.8 |      6.8 |       ○       |
| Lundberg               | `load` = 26.80 MN                 |      2,174.3 |           0.0 |     372.3 |     13.0 |       ×       |

**`#210`** — `L_we` = 347.6 mm · 롤러 기울기 0.52 mrad

| 프로파일               | 설정                               | σ_max [MPa] | 끝단 σ [MPa] | 접촉 [mm] | 손실 [%] |      판정      |
| ---------------------- | ---------------------------------- | -----------: | ------------: | --------: | -------: | :------------: |
| DIN Lundberg           | `a_off` = -20 mm                 |      2,170.9 |       2,170.9 |     307.1 |     11.7 |       ×       |
| DIN Lundberg           | `a_off` = -10 mm                 |      2,097.4 |       1,852.0 |     316.2 |      9.0 |       ○       |
| **DIN Lundberg** | `a_off` = 0 mm                   |      2,099.8 |         941.7 |     324.7 |      6.6 | **현행** |
| DIN Lundberg           | `a_off` = 10 mm                  |      2,103.1 |         948.1 |     324.7 |      6.6 |       ×       |
| DIN Lundberg           | `a_off` = 20 mm                  |      2,107.5 |         936.3 |     318.4 |      8.4 |       ×       |
| Johns-Gohar            | `P_d` = 2.37 MN · 낙차 56 μm   |      2,392.1 |       2,392.1 |     347.6 |     -0.0 |       ×       |
| Johns-Gohar            | `P_d` = 4.74 MN · 낙차 113 μm  |      2,137.0 |       2,137.0 |     338.9 |      2.5 |       ×       |
| Johns-Gohar            | `P_d` = 9.49 MN · 낙차 226 μm  |      2,081.5 |       1,476.7 |     329.0 |      5.4 |       ○       |
| Johns-Gohar            | `P_d` = 18.98 MN · 낙차 452 μm |      2,179.6 |           0.0 |     302.6 |     12.9 |       ×       |
| Johns-Gohar            | `P_d` = 37.95 MN · 낙차 904 μm |      2,362.8 |           0.0 |     270.0 |     22.3 |       ×       |
| Lundberg               | `load` = 2.37 MN                 |      2,304.4 |       2,304.4 |     347.6 |     -0.0 |       ×       |
| Lundberg               | `load` = 4.74 MN                 |      2,095.7 |       1,933.3 |     338.9 |      2.5 |       ○       |
| Lundberg               | `load` = 9.49 MN                 |      2,098.1 |         962.9 |     325.4 |      6.4 |       ○       |
| Lundberg               | `load` = 18.98 MN                |      2,196.2 |           0.0 |     296.1 |     14.8 |       ×       |

`○` = σ 제약 만족 · `×` = 위반.

### 10-12.4. 읽기

**① 끝단까지 접촉하지만, 끝단이 최대응력점은 아니다.** 현행에서 끝단 응력은 **939 ~ 991 MPa** 로 σ_max(2,093 ~ 2,100)의 **45 ~ 47 %** 다. 그림에서 본 「끝단까지 닿는다」는 관찰은 맞지만, 닿는 지점의 응력은 최대치의 절반이다. **응력 최대점이 끝단으로 옮겨가는 진짜 모서리 접촉**(끝단 σ = σ_max)은 `axial_offset ≤ −20`, Johns-Gohar ×2 이하, Lundberg ×1 에서 비로소 나타나며, 그 경우는 **모두 σ 제약을 위반**한다.

**② `axial_offset` 은 도움이 안 된다.** 음의 방향은 끝단응력을 **991 → 1,703 → 2,019 MPa** 로 곧장 악화시킨다(`#1`). 양의 방향은 끝단응력이 991 → 997 → 1,003 으로 사실상 정체하는데 σ 는 2,096.9 → 2,100.3 → 2,103.8 로 올라 **+10 mm 에서 이미 제약을 넘는다**. `#210` 은 +20 mm 에서 접촉손실까지 6.6 → 8.4 % 로 나빠진다. **현행 0 이 국소 최적**이다.

**③ Johns-Gohar 는 현행을 이기지 못한다.** 기본 낙차(×1, 56 ~ 68 μm)에서는 σ 가 **2,392 ~ 2,471** 로 제약을 크게 위반하고 끝단이 곧 최대점이다. 낙차를 키우면 σ 가 내려와 **×4(226 ~ 271 μm)에서 2,073 ~ 2,082 로 최소**가 되는데 — 이때 끝단응력은 **1,477 ~ 1,698 MPa** 로 현행보다 **1.6 ~ 1.7배 높다**. 더 키우면(×8) 끝단은 **0 으로 완전히 떨어지지만** σ 가 2,134 ~ 2,180 으로 다시 제약을 넘고 접촉손실이 **12.9 ~ 16.3 %** 로 커진다.

**④ Lundberg(`load` 직접 지정)도 마찬가지다.** ×4 에서 σ 는 2,088 ~ 2,098 로 현행과 대등하고 접촉손실은 오히려 근소하게 낫지만(`#1` 7.9 % 대 8.8 %), 끝단응력이 **963 ~ 1,165 MPa** 로 현행보다 **높거나 같다**. 세 지표를 동시에 개선하는 설정은 나오지 않았다.

**⑤ 그래서 결론은 대칭적이다.** σ 제약을 지키는 범위에서 끝단 접촉을 **없앨 수는 없다**. 끝단을 완전히 띄우려면(끝단 σ = 0) 낙차가 **450 ~ 540 μm** 필요한데, 그 낙차는 유효 접촉길이를 13 ~ 16 % 깎아 σ 를 제약 밖으로 밀어낸다. **현 정렬오차 수준에서 σ ≤ 2,100 과 끝단 접촉 제거는 양립하지 않는다.**

### 10-12.5. 판정과 다음 수

**현행 DIN Lundberg(`axial_offset` = 0)를 유지한다.** 검토한 세 프로파일 계열 · 18조건 중 σ ≤ 2,100 을 지키면서 끝단응력과 접촉손실을 동시에 낮추는 설정은 **없었다**. 부록 1 의 선택은 — 근거가 최대응력 하나였음에도 — 결과적으로 옳았고, 이제 **끝단응력과 접촉손실까지 포함한 근거로 재확인**되었다.

다만 부록 1 의 판단 근거는 이 절로 **교체**한다. 「최대응력이 낮으니 충분하다」가 아니라 **「세 지표의 파레토 구석점이므로 최선이다」**가 확정된 근거다.

남은 방향은 프로파일이 아니다.

| 방향                             | 내용                                                                                                                                                                                               | 상태                                                                           |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| **정렬오차 저감**          | 근본 원인은 0.82 ~ 1.09 mrad 의 미스얼라인먼트다. 샤프트 강성(`d`·`t`)·스팬(`z2`)·하우징 강성으로 줄여야 한다. 그런데 `d`·`z2` 는 최적화 설계변수이고 **질량과 정면 충돌**한다 | 미결(§10-13 ⑦)                                                               |
| **σ 상한 재검토**         | 2,100 MPa 는 정적 안전 기준이다. 이를 완화하면 깊은 크라운(JG ×8 · LB ×8)으로**끝단 접촉을 완전히 제거**할 수 있다 — σ 2,134 ~ 2,196 을 허용할 수 있는지가 관건                         | 미결(§10-13 ⑦)                                                               |
| **USERSPECIFIED 프로파일** | MASTA 는 좌표 직접 입력을 연다. Fujiwara 계열의**비대칭 로그 프로파일**(치우친 하중 쪽만 깊게)이면 대칭 크라운의 트레이드오프를 우회할 여지가 있다                                           | **수행 완료 — 성공(§10-12.6). §10-12.4 ⑤ 의 결론은 이로써 뒤집혔다** |
| **끝단 응력 제약화**       | 현재 최적화는 σ_max 만 제약한다. 끝단응력을 제약(예: ≤ 0.5·σ_max)으로 추가하면 프론트가 어떻게 움직이는지 확인 가능하다 — 현행 45 ~ 47 % 이므로**지금은 여유가 거의 없다**              | 미검토                                                                         |

**비대칭 프로파일이 유일하게 남은 실질 카드**다. 정렬오차가 항상 한쪽 방향인지(모멘트 부호가 DLC 에 따라 뒤집히는지)를 먼저 확인해야 쓸 수 있으므로, 그 확인을 선행 과제로 둔다.

*실행 — `Results/사이징최적화/a10_profile.py` · 결과 `부록10_NSGA/S3_본최적화/profile_study.csv`(54행) · 그림 `a10_profile_fig.py`.*

<!-- /A10:PROFILE -->

### 10-12.6. Fujiwara 프로파일 적용 — 수행과 결과

<!-- A10:FUJIWARA -->

§10-12.5 는 **MASTA 가 여는 표준 프로파일 안에서는 더 갈 곳이 없다**로 끝났다. 남은 카드는 좌표를 직접 넣는 **비대칭 로그 프로파일**이었고, 그 근거를 `풍력터빈_메인베어링_롤러프로파일_보고서.html` 의 딥리서치 결과에서 확보했다. 이 절은 그것을 실제로 적용한 기록이다.

#### 10-12.6.1. 목표의 정량화

MASTA 의 `element.results_at_roller_offsets` 는 롤러 길이방향 응력분포를 **44점**으로 준다. §10-12 의 문제 제기가 근거로 삼은 그림 그 자체다. 여기서 네 지표를 정의한다.

| 지표         | 정의                                                  | 목표                                                 |
| ------------ | ----------------------------------------------------- | ---------------------------------------------------- |
| `σ_max`   | 분포의 최대값                                         | **현행 이하** (⇒ 제약 `≤ 2,100` 자동 만족) |
| `edge_L`   | **최좌 접촉점**의 응력                          | **현행보다 감소**                              |
| `margin_L` | 롤러 좌단(`−L_we/2`) → 최좌 접촉점 **거리** | **현행보다 증가**                              |
| `y*`       | `σ_max` 가 발생하는 축방향 위치                    | **롤러 중앙(0) 쪽으로 이동**                   |

`margin_L` 이 지시받은 「롤러 접촉 가능 최좌측 지점에서 끝단 응력이 발생하는 위치까지의 거리」다. 대상은 **`#1`**(최소 `D`) · **`#103`**(`D ≤ 4,500` 최소 총질량) · **`#210`**(최소 총질량), 지배 LC `Myz_max` 1건이며 §10-12 프로파일 검토와 같은 세 설계다.

| 설계               | `L_we` [mm] | `Q_max` [MN] | 기울기 [mrad] | `σ_max` [MPa] | `y*` [mm] | `y*` / 반길이 | `edge_L` [MPa] | **`margin_L`** [mm] | `margin_R` [mm] |
| ------------------ | ------------: | -------------: | ------------: | ---------------: | ----------: | --------------: | ---------------: | --------------------------: | ----------------: |
| **`#1`**   |         533.5 |           4.36 |         0.566 |          2,096.9 |    −175.64 |          65.8 % |            988.4 |              **1.55** |             45.77 |
| **`#103`** |         427.8 |           3.35 |         0.546 |          2,092.9 |    −129.06 |          60.3 % |            939.3 |              **1.41** |             30.11 |
| **`#210`** |         347.6 |           2.37 |         0.529 |          2,099.8 |    −104.39 |          60.1 % |            941.7 |              **1.10** |             22.05 |

세 설계가 **거의 같은 병을 앓는다**. `y*` 는 반길이의 **60 ~ 66 %** 지점에 있고, 좌단 여유는 **1.1 ~ 1.6 mm** — 사실상 0이다. 반대편 여유는 22 ~ 46 mm 로 20배 이상 남아돈다. **좌우가 극단적으로 비대칭인데 프로파일은 대칭이다** — 이 불일치가 문제의 구조다. 대칭 크라운을 깊게 파면 남아도는 우측도 같이 깎여 접촉길이만 잃는다(§10-12.4 ⑤ 가 관측한 것이 바로 이것이다).

#### 10-12.6.2. 실행 가능성 — 두 문이 열리고 한 문이 막혔다

**① 임의 좌표 입력 — 열린다.** `USERSPECIFIED` 프로파일이 `points[i].position` / `.deviation` 을 모두 쓰기 허용한다. `number_of_points` 는 20 → 61 로 확장했고 좌표 범위는 `±L_we/2` 와 일치했다. **대칭 강제가 없으므로 비대칭 프로파일을 그대로 넣을 수 있다.**

**함정 하나** — `data_to_use` 의 선택지는 `ACTUAL_DATA` · `FITTED_STANDARD_PROFILE` · `SMOOTHED` 셋이고 **기본값이 `FITTED_STANDARD_PROFILE`** 이다. 그대로 두면 우리가 넣은 좌표를 표준 프로파일로 되피팅해 버린다. `ACTUAL_DATA` 가 필수다.

**② 부호 규약 — 확정했다.** `#1` 에서 세 가지를 넣어 확인했다.

| 입력                                                | `σ_max` [MPa] | `edge_L` [MPa] |
| --------------------------------------------------- | ---------------: | ---------------: |
| 낙차 0 (평탄)                                       |          2,666.5 |          2,664.3 |
| `deviation` **양수** (`z_m` = ISO 기본값) |          2,412.9 |          2,410.7 |
| `deviation` 음수                                  |          2,798.6 |          2,798.6 |

**양의 `deviation` = 낙차(재료 제거)**. 동시에 **ISO 기본 진폭(`0.00035·D_we`)으로는 턱없이 부족**하다는 것도 드러났다 — 2,412.9 MPa 는 현행 DIN(2,096.9)보다 훨씬 나쁘다. §10-12.1 의 자릿수 논증(정렬오차 ~290 μm 대 크라운 수십 μm)과 정확히 일치한다.

**③ 표준 프로파일을 좌표로 읽어오기 — 막혔다.** 당초 교정 계획은 현행 DIN Lundberg 를 좌표로 뽑아 `USERSPECIFIED` 로 되넣고 결과가 일치하는지 보는 것이었다. 그런데 `FITTED_STANDARD_PROFILE` / `ProfileToFit.DIN_LUNDBERG` 는 **반대 방향**이었다 — 사용자 좌표에 표준 프로파일을 *피팅*하는 기능이지, 표준 프로파일을 좌표로 전개해 주는 기능이 아니다(낙차가 전부 0으로 읽힌다). **DIN 형상은 API 로 읽을 수 없다.** 교정 경로를 바꿔야 했다(§10-12.6.4 S0).

#### 10-12.6.3. 적용 수식 — Fujiwara–Kawase (2006) 수정 로그

보고서가 **현 산업 표준**으로 지목한 3-파라미터 수정 로그식을 쓴다. **특허 표기**(원점 = 롤러 중심, `a` = `L_we`/2)를 채택한다.

$$
z(y) = K_1 A \ln\!\left[\frac{1}{1-\left\{1-e^{-z_m/(K_1A)}\right\}\left(\dfrac{|y|-a}{K_2 a}+1\right)^{2}}\right], \qquad A=\frac{2Q}{\pi L_{we} E'}
$$

정의역은 `|y| > a(1−K₂)` 이고 안쪽은 직선부(낙차 0)다. `|y| = a(1−K₂)` 에서 `z = 0`, `|y| = a` 에서 `z = z_m` 을 만족한다 — 검산해 확인했다.

> **표기 주의** — 보고서가 「등가 대칭표기」로 병기한 `z = AK₁ln[1/(1−(1−e^(−z_m/(AK₁)))(2y/(K₂L_we))²)]` 는 **원점이 롤러 중심이 아니라 크라운 시작점**일 때만 성립한다. 롤러 중심 원점으로 읽으면 롤러 끝단에서 로그 인수가 음수가 되어 정의되지 않는다. 특허 표기만 쓴다.

| 기호    | 의미                                                  | 탐색 범위            | 비고                                                          |
| ------- | ----------------------------------------------------- | -------------------- | ------------------------------------------------------------- |
| `K₁` | 하중계수 — 실효적으로`A → K₁A`. 클수록 완만      | **1.0 ~ 12.1** | 문헌 전형범위는 1.0 ~ 2.8 이나**확장했다** — 아래 참조 |
| `K₂` | 크라운 길이 / 유효 반길이.`K₂` = 1 이면 full crown | 0.35 ~ 1.0           | 가공성 권장은`K₂` ≤ 0.5 (§10-12.6.5 ④)                  |
| `z_m` | 유효길이 끝단 낙차                                    | 26.8 ~ 900 μm       | ISO 기본값은 하한 근처에 불과함을 확인                        |
| `Q`   | 요소 최대 롤러하중                                    | 실측 2.37 ~ 4.36 MN  | 설계별 고정 상수                                              |
| `E′` | 등가 탄성계수                                         | 226.4 GPa            | 강-강 ·`E` = 206 GPa · `ν` = 0.3                       |

> **`K₁` 범위를 넓힌 이유** — `A` 를 **실제 롤러하중** `Q_max` 로 정의했기 때문이다. Fujiwara 의 `A` 는 *설계하중*(정격 `C` 의 25 ~ 35 %) 기준이라 훨씬 크다. 실효 진폭은 `K₁A` 하나이므로 이 차이는 전부 `K₁` 로 흡수된다 — 1차 격자(`K₁` ≤ 3.0)에서 **가능해가 한 건도 나오지 않아** 실측으로 확인하고 `K₁` 을 8.0 까지, 국소 정밀화에서 12.1 까지 열었다. **`K₁` 값 자체를 문헌과 직접 비교하면 안 된다.**

`z_m` 결정의 3경로 중 보고서는 **풍력에는 경로 C(직접 최적화)가 필수**라고 명시한다. 경로 A(회귀식)는 **롤러 직경 ≤ 24 mm 에서 도출**되어 우리 롤러(110 ~ 140 mm)에는 외삽이고, 경로 B(ISO)는 위에서 부족함이 확인됐다. **경로 C 로 갔다.**
...
| -------------- | -------------------------------------------- | -------------: | ------------------------ |
| **S0**   | 교정 게이트                                  |           18회 | **통과**           |
| **S1**   | 대칭 격자`K₁`×`K₂`×`z_m` = 6×5×6 |     180 / 설계 | 가능해 11 ~ 27           |
| **S2**   | 비대칭 확장 (좌 깊게 · 우 얕게)             |      60 / 설계 | 가능해 37 ~ 45           |
| **S3**   | 국소 정밀화 (좌표하강)                       | 20 ~ 28 / 설계 | 최적 확정                |
| **S4**   | 정렬오차 방향 확인 (극한 LC 16건)            |             48 | **전건 동일 방향** |
| **보강** | 가공성`K₂ ≤ 0.5` 전용 탐색               |      96 / 설계 | **가능해 0**       |

총 **823 + 288 = 1,111회** 해석 · 약 75분. 설계당 가능해는 68 · 101 · 109 건이다.

**S0 — 교정 게이트를 바꿨다.** DIN 형상을 읽을 수 없으므로(§10-12.6.2 ③) **Johns-Gohar 가 Fujiwara 의 `K₁` = `K₂` = 1 특수해**라는 점을 이용했다. MASTA 내장 JG 와 같은 `z_m`(= `end_drop`)을 우리 수식에 넣어 결과가 일치하는지 본다. **수식 · 좌표주입 · 부호규약을 한꺼번에 검증**하는 경로이며, DIN 재현보다 오히려 강한 검증이다.

| 설계     |   `end_drop` [μm] |                JG`σ_max` |              우리`σ_max` |          **Δσ** [MPa] |
| -------- | -------------------: | --------------------------: | --------------------------: | ----------------------------: |
| `#1`   | 67.7 / 270.6 / 541.3 | 2,471.2 / 2,079.8 / 2,133.6 | 2,463.3 / 2,080.5 / 2,137.0 | **−7.9 / +0.7 / +3.4** |
| `#130` | 54.6 / 218.6 / 437.1 | 2,449.5 / 2,080.1 / 2,149.7 | 2,441.5 / 2,081.1 / 2,152.5 | **−8.0 / +1.0 / +2.8** |
| `#210` | 56.5 / 225.9 / 451.8 | 2,392.1 / 2,081.5 / 2,179.6 | 2,383.4 / 2,083.1 / 2,182.9 | **−8.7 / +1.6 / +3.3** |

낙차 55 ~ 541 μm 의 9개 대조점에서 **Δσ 가 −8.7 ~ +3.4 MPa** — 통과다. (게이트는 `#130` 으로 돌린 1차 실행 기록이며, 검증 대상은 수식이지 설계가 아니므로 그대로 둔다.)

**좌표 주입 방식도 바꿨다.** 위치를 직접 쓰면 이전 설계의 좌표와 순서가 엉켜 해석이 깨진다(`#1` 다음 `#103` 에서 재현). `number_of_points` 를 흔들어 **현 롤러 길이에 맞게 재생성**시킨 뒤, MASTA 가 준 위치에서 수식을 평가해 낙차만 넣는 방식으로 고쳤다.

**선택 기준** — σ 제약(현행 이하)을 만족하는 해 중 개선도 합을 최대화한다.

$$
\text{score}=\frac{\Delta\,edge_L}{edge_{L,0}}+\frac{\Delta\,margin_L}{(L_{we}/2)-margin_{L,0}}+\frac{\Delta\,|y^{*}|}{|y^{*}_{0}|}
$$

세 항이 모두 「기준 대비 개선 비율」이라 0 이면 기준과 같고 음수면 나빠진 것이다. **파레토가 아니라 단일 척도**인 이유는 국소 정밀화를 구동해야 했기 때문이며, 판정은 네 조건 각각으로 따로 한다(§10-12.6.6).

#### 10-12.6.5. 결과

![부록 10 Fujiwara 프로파일](Results/사이징최적화/figures/fujiwara_a10.png)

*위 = 낙차 형상, 아래 = 그 결과인 길이방향 응력분포. 같은 축이다. 현행 DIN 은 형상을 API 로 읽을 수 없어 아래 칸에만 나온다. 비대칭(붉은 실선)은 **좌측만 깊게 파고 우측은 얕게** 남긴 형상이며, 그 결과 좌단 응력이 무너지고 최대응력점(●)이 중앙 쪽으로 끌려온다.*

**① 대칭 Fujiwara 는 실패했다.**

| 설계     | 최적`K₁`/`K₂`/`z_m` | `σ_max` |                   `edge_L` |            `y*` / 반길이 |             score |
| -------- | --------------------------- | ---------: | ---------------------------: | -------------------------: | ----------------: |
| `#1`   | 4.0 / 1.0 / 400 μm         | 2,094.8 ✓ |              983.7 (−0.5 %) | 65.8 →**70.9 %** ✗ | **−0.078** |
| `#103` | 4.0 / 1.0 / 400 μm         | 2,090.3 ✓ |               742.4 (−21 %) |          60.3 → 60.5 % ✗ |  **+0.202** |
| `#210` | 4.0 / 1.0 / 400 μm         | 2,096.8 ✓ | **1,220.3 (+30 %)** ✗ |             60.1 → 59.6 % | **−0.282** |

세 설계 모두 σ 는 지키지만 **끝단응력과 위치를 함께 개선하지 못한다**. `#210` 은 끝단응력이 오히려 **30 % 악화**된다. §10-12.4 가 표준 프로파일에서 관측한 트레이드오프가 **수식을 바꿔도 대칭인 한 그대로**임을 확인한 것이다. 셋 다 `K₂` = 1.0(full crown)이 최적인 것도 보고서의 「해석상 최적은 `K₂` = 1」과 일치한다.

**② 비대칭이 문제를 푼다.**

| 설계     | 좌`K₁`/`K₂`/`z_m` | 우`K₁`/`K₂`/`z_m` |                 `σ_max` |                         `edge_L` |         `y*` / 반길이 |
| -------- | ------------------------- | ------------------------- | -------------------------: | ---------------------------------: | ----------------------: |
| `#1`   | 8.0 / 0.85 / 400 μm      | 4.8 / 0.70 / 120 μm      | **2,079.9** (−17.0) |  **65.3** (**−93 %**) | 65.8 →**53.8 %** |
| `#103` | 12.1 / 0.78 / 325 μm     | 3.46 / 0.59 / 26.8 μm    | **2,060.5** (−32.4) | **324.7** (**−65 %**) | 60.3 →**43.6 %** |
| `#210` | 4.4 / 0.96 / 325 μm      | 5.04 / 0.81 / 261 μm     |  **2,091.8** (−8.0) | **101.4** (**−89 %**) | 60.1 →**55.6 %** |

**끝단응력이 65 ~ 89 % 떨어지고 최대응력점이 중앙 쪽으로 옮겨졌으며, σ 는 오히려 내려갔다.** 좌우 낙차비(`z_mR`/`z_mL`)는 `#1` 0.30 · `#103` 0.08 · `#210` 0.80 으로, **좌우 여유 불균형이 클수록 더 비대칭한 해가 이긴다** — `#103` 은 우측을 사실상 크라운하지 않는다.

**③ `margin_L` 은 1 MPa 기준으로는 미달이다 — 그러나 임계를 올리면 뒤집힌다.**

접촉이 여전히 롤러 끝까지 「닿기는」 한다. 다만 그 응력이 65 ~ 325 MPa 로 떨어졌다. 그래서 **응력이 `σ_max` 의 10 / 25 / 50 % 를 처음 넘는 지점**까지의 거리를 다시 쟀다 — 「끝단이 얼마나 가볍게 닿는가」의 정량화다.

| 설계     | 구분             | `margin_L` (1 MPa) |  **10 %** |  **25 %** | **50 %** |
| -------- | ---------------- | -------------------: | --------------: | --------------: | -------------: |
| `#1`   | 기준             |                 1.55 |            1.55 |            1.55 |          14.34 |
|          | 대칭             |                 0.00 |            0.00 |            0.00 |          12.93 |
|          | **비대칭** |                 0.00 | **13.68** | **13.68** |          13.68 |
| `#103` | 기준             |                 1.41 |            1.41 |            1.41 |          11.84 |
|          | 대칭             |                 0.47 |            0.47 |            0.47 |          10.96 |
|          | **비대칭** |                 0.00 |            0.00 | **10.97** |          10.97 |
| `#210` | 기준             |                 1.10 |            1.10 |            1.10 |           9.64 |
|          | 대칭             |                 2.15 |            2.15 |            2.15 |           2.15 |
|          | **비대칭** |                 0.00 |  **8.58** |  **8.58** |           8.58 |

**25 % 임계에서 여유가 `#1` 1.55 → 13.68 mm (8.8배) · `#103` 1.41 → 10.97 (7.8배) · `#210` 1.10 → 8.58 (7.8배)** 로 늘어난다. 비대칭 세 건 모두 10 %·25 %·50 % 임계가 **같은 값**인데, 이는 그 지점에서 응력이 0 에서 `σ_max` 의 절반 이상으로 **한 번에 솟는다**는 뜻이다 — 즉 실질 접촉이 그 지점에서 시작한다. 반대로 기준(DIN)은 10 %·25 % 가 1.1 ~ 1.6 mm 로 **끝단부터 이미 유의미한 응력을 받는다**.

**④ 가공성 제약 `K₂ ≤ 0.5` 에서는 해가 없다.** `K₂`·`K₁`·`z_m`·낙차비를 `K₂` ∈ {0.35, 0.5} 로 고정해 설계당 96조합을 따로 돌렸다.

| 설계     | 조합 |      가능해 | 도달 최저`σ_max` |       현행 대비 |
| -------- | ---: | ----------: | ------------------: | --------------: |
| `#1`   |   96 | **0** |             2,122.8 | **+25.9** |
| `#103` |   96 | **0** |             2,109.3 | **+16.4** |
| `#210` |   96 | **0** |             2,117.7 | **+17.9** |

특허가 권장하는 **직선부 ≥ 전장 50 %** 를 지키면 크라운 구간이 짧아져 낙차가 급해지고, σ 가 현행보다 **16 ~ 26 MPa 높아져 제약을 넘는다.** **가공성과 성능이 정면으로 충돌한다** — 채택하려면 `K₂` ≈ 0.6 ~ 1.0 (직선부 0 ~ 40 %)의 가공이 가능한지가 전제조건이다.

**⑤ 정렬오차 방향은 전건 동일하다 — 비대칭의 전제조건이 성립한다.**

극한 LC 16건 × 3설계 = **48건 전부에서 `y*` 가 음수(좌측)** 다. 우측 치우침은 한 건도 없다. 치우침의 크기는 `Myz_max` 에서 가장 크고(`#1` −175.6 mm), 가벼운 LC 에서는 −5 ~ −30 mm 로 작아진다. **방향이 뒤집히는 케이스가 없으므로 좌측만 깊게 파는 비대칭 프로파일이 역효과를 낼 LC 가 없다.**

#### 10-12.6.6. 판정

|   # | 조건                        |     `#1`     |    `#103`    |    `#210`    |
| --: | --------------------------- | :-------------: | :-------------: | :-------------: |
|   1 | `σ_max` ≤ 현행          |    ✅ −17.0    |    ✅ −32.4    |    ✅ −8.0    |
|   2 | `edge_L` < 현행           |    ✅ −93 %    |    ✅ −65 %    |    ✅ −89 %    |
|   3 | `margin_L` > 현행 (1 MPa) |       ❌       |       ❌       |       ❌       |
| 3′ | 유효 여유(25 %) > 현행      |    ✅ 8.8배    |    ✅ 7.8배    |    ✅ 7.8배    |
|   4 | `y*` 절대값 < 현행        | ✅ 65.8→53.8 % | ✅ 60.3→43.6 % | ✅ 60.1→55.6 % |

**비대칭 Fujiwara 프로파일이 목표를 달성한다.** 조건 3 은 1 MPa 라는 임계가 지나치게 예민해 미달로 나오지만, **끝단이 닿는다는 사실보다 얼마나 세게 닿는가가 모서리 접촉의 실질**이고 그것은 65 ~ 325 MPa 로 떨어졌다. 조건 3′ 로 대체해 판정한다 — 다만 **원래 지표로는 미달임을 숨기지 않는다.**

**§10-12.5 의 결론을 뒤집는다.** 그 절은 「σ ≤ 2,100 과 끝단 접촉 제거는 양립하지 않는다」로 닫았는데, 그것은 **대칭 프로파일 안에서만 참**이었다. 비대칭을 열면 셋 다 동시에 개선된다. §10-12.5 의 「비대칭이 유일하게 남은 실질 카드」라는 진단이 맞았고, 그 카드가 실제로 통했다.

#### 10-12.6.7. 채택 전 남은 것

| 항목                        | 내용                                                                                                                                                                         | 판단                                                  |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| **가공성**            | `K₂ ≤ 0.5` 에서 가능해 0. 최적해는 `K₂` 0.59 ~ 0.96 (직선부 4 ~ 41 %)                                                                                                 | **전제조건** — 제작처 확인 필요                |
| **특허**              | Fujiwara 식·`z_m` 포락선은 NTN US8398312B2 / EP2228556B1 청구 범위                                                                                                        | 본 검토는 타당성 확인. 적용 시**회피검토 별도** |
| **궤도륜 분담**       | 실제 낙차는**롤러 + 궤도륜 크라우닝의 합**(보고서 §3.2). 본 검토는 롤러만 다뤘다                                                                                      | 미결                                                  |
| **피로수명**          | σ·끝단응력만 봤다. ISO/TS 16281 수명이 어떻게 바뀌는지 미확인                                                                                                              | 미결                                                  |
| **최적화 반영**       | 프로파일이 σ 를 8 ~ 32 MPa 낮추므로, 제약 경계에서**더 작은 베어링이 가능해진다** — 프론트가 개선될 여지                                                             | 미결                                                  |
| **`z_m` 절대 크기** | 325 ~ 400 μm 는 ISO 기본값의**7 ~ 8배**다. 정렬오차가 그만큼 크기 때문이나, 경하중에서 과크라우닝으로 수명이 떨어질 수 있다(보고서: `R` = 100·`D_w` 에서 −86 %) | **경하중 LC 검증 필요**                         |

*실행 — `a10_profile_lib.py`(수식·조립·지표) · `a10_fujiwara.py`(S0~S4) · `a10_fuji_k2.py`(가공성) · `a10_fuji_final.py`(유효 여유·분포) · `a10_fuji_fig.py`(그림). 결과 `부록10_NSGA/S3_본최적화/fujiwara/` — `fujiwara_all.csv`(823행) · `k2cap.csv`(288행) · `s4_direction.csv`(48행) · `final.json` · `s0_gate.json`.*

<!-- /A10:FUJIWARA -->

### 10-12.7. 비대칭 DIN Lundberg 프로파일 적용 검토 (계획)

<!-- A10:ASYMDIN -->

§10-12.6 은 Fujiwara 수정 로그를 좌우 독립 파라미터로 열어 성공했다. 그런데 **현행 DIN Lundberg 자체를 좌우 비대칭으로 쓰는 길**이 더 단순하다 — 형상이 **계수 하나**로 결정되기 때문이다. 이 절은 그 검토 계획이다.

착상은 이렇다. **우측 계수를 작게 하면 그쪽이 더 큰 응력을 지지**한다(크라운이 얕아 접촉이 길어진다). 좌측은 하중이 쏠리는 쪽이므로 계수를 키워 끝단을 띄운다. 여기에 **오프셋**을 더해 좌우 응력분포를 고르게 맞춘다. `USERSPECIFIED` + `ACTUAL_DATA` 로 주입한다.

#### 10-12.7.1. MASTA DIN Lundberg 의 정확한 정의 — 201점 전수 확정

비대칭으로 만들려면 **원본을 정확히 알아야** 한다. `#1`·`#103`·`#210` 의 `outer_race_and_roller_profiles`(201점)에서 `roller_deviation` 을 통째로 읽어 확정했다.

**형상은 순수 로그식이다.**

$$
z(y)=A\ln\!\left[\frac{1}{1-\left(2y/L_{we}\right)^{2}}\right]
$$

중앙과 끝단을 뺀 **198점 전수**에서 `A` 를 역산하면 상대 표준편차가 **7.2 × 10⁻¹² %** 다 — 근사가 아니라 이 식 자체다.

| 설계     | `L_we` | `D_we` |         `A` [μm] | `A` 상대편차 | `z_end` [μm] | `z_end`/`D_we` | **`K` = `z_end`/`A`** |
| -------- | -------: | -------: | ------------------: | -------------: | --------------: | -----------------: | --------------------------------: |
| `#1`   |    533.5 |    227.6 | **102.42000** |     7.2e−12 % |        503.0881 |         0.00221040 |                **4.912010** |
| `#103` |    427.8 |    204.2 |  **91.89000** |     7.3e−12 % |        451.3646 |         0.00221040 |                **4.912010** |
| `#210` |    347.6 |    170.0 |  **76.50000** |     7.2e−12 % |        375.7688 |         0.00221040 |                **4.912010** |

**⑴ 계수는 롤러 직경에만 비례한다.**

$$
A = 4.5\times10^{-4}\,D_{we}
$$

세 설계에서 `A`/`D_we` 가 **4.5 × 10⁻⁴ 로 완전히 동일**하다(102.42/227.6 = 91.89/204.2 = 76.50/170.0). `L_we`·`D_pw`·하중과 무관하다 — **부록 2 §2-6 이 실측한 「`end_drop`/`D_we` = 0.00221 이 전 조합에서 정확히 일정」과 같은 사실**이며, 여기서 그 내부 구조가 드러났다.

$$
\underbrace{0.00221040}_{\text{부록 2}} = \underbrace{4.5\times10^{-4}}_{A/D_{we}}\times \underbrace{4.912010}_{K}
$$

**⑵ 끝단은 고정 배율로 절단된다.** `|y|` = `L_we`/2 에서 로그는 발산하는데(ISO 16281 원식의 특이점) MASTA 는 유한한 값을 준다. 그 값이 **정확히 `K`·`A`** 이고 `K` = 4.912010 이 **세 설계에서 폭 0** 이다. 등가적으로 `|y|`/(`L_we`/2) = **0.99631437** 에서 평가한 값이다.

**⑶ ISO 16281 기본형과 같은 계열이다.** 딥리서치 보고서는 ISO 16281 기준 진폭을 `P_r` = 3.5 × 10⁻⁴·`D_w` 로 적는다. MASTA DIN 은 **4.5 × 10⁻⁴** 로 그 **1.286배**다. 「로그 프로파일 진폭 ≈ 3.5e−4 × 롤러직경」이라는 rule of thumb 과 같은 자리의 수다.

> **§10-12.6.2 ③ 을 정정한다.** 거기서 「DIN 형상은 API 로 읽을 수 없다」고 적었는데 **틀렸다.** 막힌 것은 `USERSPECIFIED.points` +`FITTED_STANDARD_PROFILE` 경로뿐이었고, `outer_race_and_roller_profiles` 로는 처음부터 열려 있었다. 다만 그때 대신 쓴 **Johns-Gohar 대조 게이트는 여전히 유효**하다 — 오히려 우리 수식까지 함께 검증하는 경로였다.

> **§10-12.6.2 ② 도 표현을 바로잡는다.** 「ISO 기본 진폭(0.00035·`D_we`)으로는 턱없이 부족」이라 적었는데, 그 수는 **진폭 `A`** 이지 **끝단 낙차 `z_m`** 이 아니다. Fujiwara 식의 `z_m` 자리에 진폭을 넣었으니 작게 나온 것이 당연하다 — 등가 끝단 낙차는 약 `K`배, 즉 **0.0017·`D_we`** 수준이다. 관측된 수치(σ 2,412.9)는 그대로 유효하나 「ISO 값이 부족하다」는 해석은 성립하지 않는다.

#### 10-12.7.2. 지표 재정의 — §10-12.6 의 지표에는 결함이 있다

§10-12.6 이 쓴 `edge_L` 은 **응력이 발생하는 위치 중 가장 바깥의 값**이었다. 그것은 **물리적 끝단의 응력이 아니다** — 접촉이 끝단에서 멀어지면 그 안쪽 어딘가의 값을 읽게 되므로, 값이 낮아도 「끝단이 안전하다」는 뜻이 되지 않는다. `margin_L` 도 절대 길이(mm)라 설계 크기가 다르면 비교가 안 된다.

**중요한 것은 두 가지다** — ⑴ 물리적 양 끝단에서 응력이 발생하는가, ⑵ 발생한다면 최대값 대비 얼마나 큰가.

| 지표                                                             | 새 정의                                                    | 목표                                         |
| ---------------------------------------------------------------- | ---------------------------------------------------------- | -------------------------------------------- |
| `σ_max`                                                       | 분포의 최대값 [MPa]                                        | **현행 이하** (제약)                   |
| **`end_L`**                                              | 최외곽 하중 지점의 응력 [MPa] —**절대값**           | **낮게**                               |
| **`end_L%`**                                             | `end_L` / `σ_max` × 100                              | **낮게**                               |
| **`margin_L%`**                                          | (최외곽 하중 지점 − (−`L_we`/2)) / (`L_we`/2) × 100 | **크게** (0 이면 물리적 끝단이 물린다) |
| **`margin_L` (라미나)**                                  | 위 거리 / (`L_we`/38)                                    | **크게** — 정수 감각                  |
| `end_R` · `end_R%` · `margin_R%` · `margin_R`(라미나) | 우측 동일                                                  | 좌우 균일                                    |
| `y*`                                                           | `σ_max` 위치 / (`L_we`/2) × 100                      | **0 쪽으로**                           |

라미나 단위를 함께 쓰는 이유는 MASTA 의 이산화 때문이다. **MASTA 는 접촉구간을 38등분**한다 — `#1` 486.18/38 = 12.794 · `#103` 10.428 · `#210` 8.538 mm 로 확인했다. 여유가 1 라미나 미만이면 해석이 그것을 분해하지 못하므로, 백분율과 함께 **몇 라미나인가**를 보아야 신뢰할 수 있다.

현행(DIN 대칭)을 새 지표로 다시 적으면 이렇다.

| 설계     | `σ_max` | `y*` | **`end_L`** [MPa] | **`end_L%`** | **`margin_L%`** | 라미나 | **`end_R`** [MPa] | `end_R%` | `margin_R%` | 라미나 |
| -------- | ---------: | -----: | ------------------------: | -------------------: | ----------------------: | -----: | ------------------------: | ---------: | ------------: | -----: |
| `#1`   |    2,096.9 | 65.8 % |           **988.4** |     **47.1 %** |        **0.58 %** |   0.11 |           **162.9** |      7.8 % |       17.16 % |   3.26 |
| `#103` |    2,092.9 | 60.3 % |           **939.3** |     **44.9 %** |        **0.66 %** |   0.13 |           **418.5** |     20.0 % |       14.08 % |   2.68 |
| `#210` |    2,099.8 | 60.1 % |           **941.7** |     **44.9 %** |        **0.63 %** |   0.12 |           **521.2** |     24.8 % |       12.69 % |   2.42 |

**좌측 여유가 0.1 라미나 수준**이다 — 사실상 물리적 끝단이 물려 있고, 그 지점의 응력이 최대값의 **45 ~ 47 %** 다. 이것이 실제 문제의 크기다.

#### 10-12.7.3. 설계변수와 수식

형상이 `A` 하나로 결정되므로 **좌우에 배율을 곱하는 것**이 가장 단순한 비대칭화다. 여기에 **오프셋**과, Fujiwara 의 `K₂` 에 대응하는 **원점 직선 구간**을 더한다.

$$
z(y)=\begin{cases}0 & |y-\delta|\le f\\[6pt] k_s\,A_{\mathrm{DIN}}\ln\!\left[\dfrac{1}{1-\left(\dfrac{|y-\delta|-f}{a_s-f}\right)^{2}}\right] & |y-\delta|>f\end{cases}\qquad k_s=\begin{cases}k_L & y<\delta\\ k_R & y\ge\delta\end{cases}
$$

`a_s` 는 해당 측의 유효 반길이다 — 크라운 원점이 `δ` 로 옮겨지므로 **좌측 `a_L` = `L_we`/2 + `δ` · 우측 `a_R` = `L_we`/2 − `δ`** 가 된다(`δ` > 0 이면 좌측이 길어진다).

| 변수              | 의미                                                           | 범위(안)   | 현행 대응 |
| ----------------- | -------------------------------------------------------------- | ---------- | --------- |
| `k_L`           | 좌(하중 치우침 쪽) 계수 배율 —**크게** 하면 끝단이 뜬다 | 0.5 ~ 4.0  | 1         |
| `k_R`           | 우 계수 배율 —**작게** 하면 접촉이 길어져 더 지지한다   | 0.2 ~ 1.5  | 1         |
| `δ`            | 프로파일 원점의 축방향 오프셋 [mm]                             | −40 ~ +40 | 0         |
| **`f/a`** | **원점 직선 구간의 반길이 비**                           | 0 ~ 0.6    | 0         |

`A_DIN` = 4.5 × 10⁻⁴·`D_we` 는 설계마다 자동으로 정해지므로 **배율로 두면 설계 간 비교가 그대로 성립**한다. **`k_L` = `k_R` = 1 · `δ` = 0 · `f` = 0 이 현행 DIN 과 정확히 같아야 한다** — 이것이 교정 게이트다.

직선 구간을 **`f/a` 비율**로 두는 이유는 Fujiwara 와 같은 잣대로 읽히기 때문이다 — `f/a` = 1 − `K₂` 이므로 `f/a` = 0.5 가 특허 권장 「직선부 ≥ 전장의 50 %」(`K₂` ≤ 0.5)에 정확히 대응한다. 우선 **좌우 공통 단일 `f`** 로 효과를 보고, 이득이 있으면 그때 좌우로 분리한다.

**직선 구간은 자유도를 늘리면서 위험 하나를 없앤다.** `k_L` ≠ `k_R` 이면 `y` = `δ` 에서 낙차는 0 으로 이어지지만 **기울기가 꺾인다** — 접선 불연속은 2차 압력피크를 낳을 수 있다. 그런데 로그항의 기울기는 크라운 시작점에서 정확히 0 이므로

$$
\frac{d}{dt}\ln\frac{1}{1-t^{2}}=\frac{2t}{1-t^{2}}\ \xrightarrow{\;t\to0\;}\ 0
$$

`f` > 0 이면 좌우 크라운이 `±f` 에서 **각각 기울기 0 으로 시작**해 직선부와 매끄럽게 이어진다. **`f` = 0 일 때만 꺾임이 생긴다.**

끝단 절단은 **MASTA 와 같은 규칙**을 쓴다 — `z_end` = `k`·`A_DIN`·`K` (`K` = 4.912010). 그래야 `k` = 1 에서 완전히 일치하고, 이후의 차이가 **오직 `k_L`·`k_R`·`δ`·`f` 로만** 설명된다. 직선 구간이 생기면 같은 낙차를 더 짧은 구간에서 내려야 하므로 **크라운이 가팔라진다** — 그것이 `f` 의 물리적 효과이며, `K` 를 함께 바꾸면 두 효과가 섞여 분리되지 않는다.

> 오프셋 `δ` 를 넣으면 좌우의 유효 반길이가 달라진다. 로그 인수가 음수가 되지 않도록 `|y−δ|` > `a_s` 인 구간은 끝단값으로 고정한다. 이 처리가 §10-12.6 의 `axial_offset` 스윙(§10-12.3)과 다른 점이며, 그때는 MASTA 내부 처리라 우리가 볼 수 없었다.

#### 10-12.7.4. 작업 단계

각 단계의 이득이 따로 기록되도록 **한 변수씩 열어 간다.**

| 단계         | 여는 변수        | 내용                                                                                           |        규모 |
| ------------ | ---------------- | ---------------------------------------------------------------------------------------------- | ----------: |
| **T0** | —               | 교정 게이트 —`k`=1·`δ`=0·`f`=0 이 현행 DIN 을 재현하는가 (σ 와 분포 44점 전부 대조) |         6회 |
| **T1** | `k_R`          | 「우측을 얕게 하면 더 지지한다」는 착상의 직접 확인                                            |      8 × 3 |
| **T2** | `k_L`·`k_R` | 좌우 배율 격자 (`δ`=0 · `f`=0)                                                           | 6 × 6 × 3 |
| **T3** | `δ`           | 유망해에 오프셋 추가                                                                           | 3 × 9 × 3 |
| **T4** | **`f`**  | 유망해에 직선 구간 추가 ·`f/a` 0 ~ 0.6                                                      | 3 × 7 × 3 |
| **T5** | 전체             | 국소 정밀화 (좌표하강)                                                                         |    ~40 × 3 |
| **T6** | —               | §10-12.6 의 Fujiwara 최적과 나란히 대조                                                       |         6회 |

총 **약 400회** · 15 ~ 20분이다. `Myz_max` 지배 LC 기준이며 대상은 `#1`·`#103`·`#210` 이다. 정렬오차 방향이 극한 LC 48건 전부 좌측임은 §10-12.6.5 ⑤ 에서 이미 확인됐으므로 방향 확인은 다시 하지 않는다.

**가공성 제약판(`f/a` ≥ 0.5)은 별도로 돌리지 않는다.** 대신 최적해의 `f/a` 를 그대로 보고해 특허 권장(≥ 0.5)과의 거리를 읽을 수 있게 한다 — Fujiwara 에서는 그 제약 하에 가능해가 0 이었다(§10-12.6.5 ④).
...
| # | 조건                                            | `#1` 기준                         |
| -: | ----------------------------------------------- | ----------------------------------- |
| 1 | `σ_max` ≤ 현행                              | ≤ 2,096.9 MPa                      |
| 2 | `end_L` · `end_L%` < 현행                  | < 988.4 MPa · < 47.1 %             |
| 3 | `margin_L%` > 현행 · **1 라미나 이상** | > 0.58 % 이고 ≥ 1 라미나           |
| 4 | `y*` 절대값 < 현행                            | < 65.8 %                            |
| 5 | 좌우 균일 —`end_L` 와 `end_R` 의 차 축소   | 현행 988.4 대 162.9 (47.1 대 7.8 %) |

조건 3 에 **라미나 하한**을 넣은 것이 §10-12.6 과의 차이다. 백분율만 보면 해석이 분해하지 못하는 크기도 「개선」으로 잡힌다.

| 위험                         | 내용                                                                                   | 대응                                                                                                                      |
| ---------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **교정 실패**          | `k`=1·`δ`=0·`f`=0 이 현행 DIN 과 어긋나면 이후 비교가 무너진다                | T0 게이트 — 분포 44점 전부 대조                                                                                          |
| **원점 불연속**        | `f` = 0 이면서 `k_L` ≠ `k_R` 이면 `y` = `δ` 에서 **기울기가 꺾인다** | **`f` > 0 이면 구조적으로 해소**된다(§10-12.7.3). T2·T3 구간(`f`=0)에서는 분포에서 2차 피크를 직접 확인한다   |
| **가공성**             | 좌우 비대칭 + 직선부 부족                                                              | 최적해의`f/a` 를 보고해 특허 권장(≥ 0.5)과의 거리를 남긴다. **파라미터가 4개**라 Fujiwara(6개)보다 도면화가 쉽다 |
| **Fujiwara 대비 열위** | 자유도가 적어 §10-12.6 만 못할 수 있다                                                | T6 에서 나란히 비교해**둘 중 무엇을 채택할지**를 판정한다                                                           |
| **`A` 의 근거**      | 4.5e−4·`D_we` 는 MASTA 내부 규칙이며 DIN 5412 원문 대조는 하지 않았다              | 비율 자체가 아니라**배율**을 설계변수로 두므로 결론은 영향받지 않는다                                               |

*사전조사 완료 — `Results/사이징최적화/a10_din_probe.py` · 결과 `부록10_NSGA/S3_본최적화/din/din_definition.json`. 실행 지시 대기.*

#### 10-12.7.6. 수행 결과 (실시간)

<!-- A10:ASYMRUN -->

*실행 중 — 단계가 끝날 때마다 갱신된다. 경과 24.5분.*  **완료 — 총 324회.**

**진행**

| 단계         | 여는 변수         | 시행 | 가능해 |   경과 | 비고           |
| ------------ | ----------------- | ---: | -----: | -----: | -------------- |
| **T0** | —                |    3 |      1 |  0.1분 | 교정 통과      |
| **T1** | `k_R`           |   24 |     15 |  1.8분 | 우측 계수 단독 |
| **T2** | `k_L`·`k_R`  |  108 |     31 | 10.7분 | 좌우 배율 격자 |
| **T3** | `δ`            |   81 |     67 |  4.8분 | 오프셋 추가    |
| **T4** | **`f/a`** |   63 |     26 |  3.4분 | 직선 구간 추가 |
| **T5** | 전체              |   49 |     39 |  3.2분 | 국소 정밀화    |
| **T6** | —                |    3 |      3 |  0.4분 | Fujiwara 대조  |

**현재 최적해** — 각 설계의 현행(DIN 대칭) 대비

| 설계               | 구분           | `k_L` | `k_R` | `δ` | `f/a` |        `σ_max` |         `end_L` |       `end_L%` |     `margin_L%` |         라미나 | `end_R` | `end_R%` | `margin_R%` | 라미나 |          `y*%` |  score |
| ------------------ | -------------- | ------: | ------: | -----: | ------: | ----------------: | ----------------: | ---------------: | ----------------: | -------------: | --------: | ---------: | ------------: | -----: | ---------------: | -----: |
| **`#1`**   | 현행           |       1 |       1 |      0 |       0 |           2,096.9 |             988.4 |           47.1 % |            0.58 % |           0.11 |     162.9 |      7.8 % |       17.16 % |   3.26 |           65.8 % |     — |
|                    | **최적** |       3 |    0.25 |     38 |       0 | **2,085.3** |   **886.9** | **42.5 %** | **17.67 %** | **3.36** |     830.7 |     39.8 % |        0.00 % |   0.00 | **19.9 %** | 1.9035 |
| **`#103`** | 현행           |       1 |       1 |      0 |       0 |           2,092.9 |             939.3 |           44.9 % |            0.66 % |           0.12 |     418.5 |     20.0 % |       14.08 % |   2.67 |           60.3 % |     — |
|                    | **최적** |     2.5 |     0.2 |    -10 |       0 | **2,091.0** | **1,059.0** | **50.6 %** | **13.82 %** | **2.63** |   1,077.7 |     51.5 % |        0.00 % |   0.00 | **32.3 %** | 1.4342 |
| **`#210`** | 현행           |       1 |       1 |      0 |       0 |           2,099.8 |             941.7 |           44.9 % |            0.63 % |           0.12 |     521.2 |     24.8 % |       12.69 % |   2.41 |           60.1 % |     — |
|                    | **최적** |       2 |     0.2 |      0 |       0 | **2,065.8** | **1,173.6** | **56.8 %** | **11.03 %** | **2.10** |   1,177.1 |     57.0 % |        0.00 % |   0.00 | **34.3 %** | 1.2795 |

*score = 끝단응력 감소율 + 좌단여유 증가율 + `y*` 중앙화율 + 좌우 끝단응력 격차 축소율 (σ ≤ 현행 만족 시에만 정의).*

<!-- /A10:ASYMRUN -->

#### 10-12.7.7. 재탐색 — 양측 여유 제약

1차는 좌측 여유를 벌면서 **우측을 물리게** 했다(`margin_R` 17.16 → 0.00 %). 목적함수가 좌측 항 위주였기 때문이다. 여기서는 **양측 여유를 제약으로 올리고**, 직선 구간을 **좌우 독립**으로 연다.

|                     | 내용                                                                       |
| ------------------- | -------------------------------------------------------------------------- |
| **설계변수**  | `k_L` · `k_R` · `δ` · `f_L` · `f_R` (5개)                   |
| **제약**      | `σ_max` ≤ 현행 · `margin_L` ≥ 1 라미나 · `margin_R` ≥ 1 라미나 |
| **목적**      | `score` = `s₁` + `s₃` (동일 가중치)                                |
| **참고 기록** | `s₂` 최소여유(제약으로 올라가 목적에서 제외) · `s₄` 진입응력        |

목적의 두 항은 이렇게 정의한다.

$$
s_1 = 1-\frac{y^{*}\%}{y^{*}\%_{0}}\qquad\text{최대응력이 \textbf{어디에} 있는가}
$$

$$
s_3 = 1-\frac{\left\lvert \sigma_{end,L}-\sigma_{end,R}\right\rvert}{\sigma_{max}}\qquad\text{양단이 \textbf{얼마나 고른가}}
$$

`s₁` 은 현행과 같으면 0, 최대응력이 롤러 정중앙에 오면 1 이다. `s₃` 는 양단 진입응력이 같으면 1, 벌어질수록 낮아진다. 따라서 `score` 의 상한은 **2** 다.

`s₂` 를 제약으로 올려 목적에서 뺀 것이 중복 제거의 핵심이다. 남은 두 항은 성격이 겹치지 않는다.

<!-- A10:ASYMRUN2 -->

*실행 중 — 단계가 끝날 때마다 갱신된다. 경과 52.2분.*  **완료 — 총 559회.**

| 단계         | 여는 변수                | 시행 | 가능해 |   경과 | 비고                         |
| ------------ | ------------------------ | ---: | -----: | -----: | ---------------------------- |
| **U0** | —                       |  324 |     15 |  0.0분 | 1차 자료 재판정 · 만족 15건 |
| **U1** | `k_L`·`k_R`·`δ` |  375 |     98 | 35.2분 | 양측 제약 격자               |
| **U2** | `f_L`·`f_R`         |  135 |     64 | 12.6분 | 좌우 독립 직선 구간          |
| **U3** | 전체                     |   58 |     34 |  4.3분 | 국소 정밀화                  |

| 설계               | 구분           | `k_L` | `k_R` | `δ` | `f_L` | `f_R` |        `σ_max` | `margin_L` 라미나 | `margin_R` 라미나 | `end_L` | `end_R` |        `y*%` | `s₁` | `s₂` | `s₃` | `s₄` |  **score** |
| ------------------ | -------------- | ------: | ------: | -----: | ------: | ------: | ----------------: | ------------------: | ------------------: | --------: | --------: | -------------: | ------: | ------: | ------: | ------: | ---------------: |
| **`#1`**   | 현행           |       1 |       1 |      0 |       0 |       0 |           2,096.9 |                0.11 |                3.26 |     988.4 |     162.9 |           65.8 |      — |      — |      — |      — |               — |
|                    | **최적** |     2.9 |     0.4 |     40 |       0 |    0.04 | **2,095.8** |      **3.21** |      **1.02** |     915.5 |   1,044.2 | **20.7** |   0.686 |   0.339 |   0.939 |   0.502 | **1.6248** |
| **`#103`** | 현행           |       1 |       1 |      0 |       0 |       0 |           2,092.9 |                0.12 |                2.67 |     939.3 |     418.5 |           60.3 |      — |      — |      — |      — |               — |
|                    | **최적** |     2.3 |   0.575 |     37 |       0 |       0 | **2,088.6** |      **2.53** |      **1.20** |   1,030.8 |   1,031.1 | **23.3** |   0.614 |   0.399 |   1.000 |   0.506 | **1.6134** |
| **`#210`** | 현행           |       1 |       1 |      0 |       0 |       0 |           2,099.8 |                0.12 |                2.41 |     941.7 |     521.2 |           60.1 |      — |      — |      — |      — |               — |
|                    | **최적** |     2.4 |     0.6 |     40 |       0 |       0 | **2,099.1** |      **2.70** |      **1.11** |     967.7 |   1,170.5 | **17.7** |   0.705 |   0.370 |   0.903 |   0.442 | **1.6085** |
...
##### ⑴ `δ` 상한 확장 — 2차 최적은 최적이 아니었다

2차에서 세 설계 모두 `δ` 가 탐색 상한 40 mm 에 붙었다. 상한에 붙은 해를 「최적」이라 적을 수 없어 **80 mm 까지 열고 `k_L`·`k_R` 도 함께 흔들었다**(134회 · 12분).

| 설계     | 2차`score` | **확장 최적**                                | `δ` |        `k_R` |    `δ`/반길이 | Δ`score` |
| -------- | -----------: | -------------------------------------------------- | -----: | -------------: | ---------------: | ----------: |
| `#1`   |       1.6248 | `k` 2.9 / **0.5** · `δ` **70**   |     70 |     0.4 → 0.5 | **26.2 %** |      +0.047 |
| `#103` |       1.6134 | `k` 2.3 / **0.675** · `δ` **60** |     60 | 0.575 → 0.675 | **28.1 %** |      +0.055 |
| `#210` |       1.6085 | `k` 2.4 / **0.7** · `δ` **60**   |     60 |     0.6 → 0.7 | **34.5 %** |      +0.047 |

**`δ` 와 `k_R` 은 함께 움직여야 하는 짝이다.** 원점을 오른쪽으로 밀면 우측 유효 반길이 `a_R` = `L_we`/2 − `δ` 가 짧아지므로, 같은 `k_R` 로는 우측 크라운이 상대적으로 가팔라져 여유가 무너진다. `k_R` 을 함께 올려 얕게 만들어야 여유가 유지된다. 2차 격자는 이 둘을 독립적으로 훑어 **결합 효과를 놓쳤다.**

> `k_R` 을 고정한 채 `δ` 만 키우면 `#1` 기준 60 mm 에서 우측 여유가 **0.08 라미나**로 붕괴한다. 「`δ` = 40 이 최적」으로 보였던 것은 그 단면만 본 결과였다.

**기하 제약이 이제 실제로 걸린다.** `δ`/반길이가 26 ~ 35 % 다. 프로파일 원점이 롤러 중앙에서 반길이의 4분의 1 이상 밀려난다는 뜻이며, `#210` 은 **34.5 %** 로 검토선 30 % 를 넘는다. 가공·검사 관점의 상한이 있는지가 채택 전 확인 사항이다.

##### ⑵ 결과

**프로파일 설계변수** — 세 해법은 파라미터 집합이 서로 다르므로(`k`·`δ`·`f` 대 `K₁`·`K₂`·`z_m`) **물리량으로 환산해** 같은 축에 놓았다.

| 설계               | 프로파일             | 좌 끝단낙차 [μm] | 우 끝단낙차 [μm] | 원점이동`δ` [mm] |  직선부 좌 / 우 | 원 파라미터                                                   |
| ------------------ | -------------------- | ----------------: | ----------------: | ------------------: | --------------: | ------------------------------------------------------------- |
| **`#1`**   | 현행 DIN             |             503.1 |             503.1 |                   0 |           0 / 0 | `k` = 1 (대칭)                                              |
|                    | Fujiwara             |             400.0 |             120.0 |                   0 | 15.0 % / 30.0 % | `K₁` 8.0/4.8 · `K₂` 0.85/0.70 · `z_m` 400/120       |
|                    | **비대칭 DIN** | **1,459.0** |             251.5 |        **70** |       0 / 4.0 % | `k` 2.9/0.5 · `f` 0/0.04                                 |
| **`#103`** | 현행 DIN             |             451.4 |             451.4 |                   0 |           0 / 0 | `k` = 1 (대칭)                                              |
|                    | Fujiwara             |             325.0 |              26.8 |                   0 | 22.5 % / 41.2 % | `K₁` 12.10/3.46 · `K₂` 0.775/0.588 · `z_m` 325/26.8 |
|                    | **비대칭 DIN** | **1,038.1** |             304.7 |        **60** |           0 / 0 | `k` 2.3/0.675                                               |
| **`#210`** | 현행 DIN             |             375.8 |             375.8 |                   0 |           0 / 0 | `k` = 1 (대칭)                                              |
|                    | Fujiwara             |             325.0 |             261.2 |                   0 |  3.7 % / 18.8 % | `K₁` 4.4/5.04 · `K₂` 0.963/0.812 · `z_m` 325/261.2  |
|                    | **비대칭 DIN** |   **901.8** |             263.0 |        **60** |           0 / 0 | `k` 2.4/0.7                                                 |

*환산 — 비대칭 DIN 은 `z_end` = `k`·`A`·`K`, 현행 DIN 은 `A`·`K` (`A` = 4.5×10⁻⁴·`D_we` = 102.42 / 91.89 / 76.50 μm · `K` = 4.912010 · §10-12.7.1). Fujiwara 는 `z_m` 이 곧 끝단낙차이고 직선부는 `1 − K₂` 다. 비대칭 DIN 의 `δ` 는 두 해법 중 유일하게 **프로파일 원점 자체를 옮긴다** — Fujiwara 에는 대응 변수가 없다.*

**두 해법이 정반대로 간다는 것이 여기서 먼저 보인다.** 비대칭 DIN 은 좌측 낙차를 현행의 **2.1 ~ 2.9배**로 깊게 파고 우측은 오히려 얕게(현행의 0.50 ~ 0.70배) 남긴 뒤 **원점을 60 ~ 70 mm 옮긴다.** Fujiwara 는 좌우 모두 현행보다 얕게(0.06 ~ 0.80배) 두되 **직선부를 4 ~ 41 % 둔다.** 비대칭 DIN 은 직선부를 쓰지 않는다.

![세 프로파일 비교](Results/사이징최적화/figures/asymdin_a10.png)

*위 = 낙차 형상, 아래 = 그 결과인 길이방향 응력분포. §10-12.6 의 그림과 달리 **현행 DIN 형상도 그렸다** — 읽을 수 있음을 §10-12.7.1 에서 확인했다. 비대칭 DIN(붉은 실선)은 좌측 낙차가 `#1` 기준 1,459 μm 로 현행(503 μm)의 2.9배다.*

| 설계               | 구분                |        `σ_max` |   `margin_L` |   `margin_R` |                `end_L` |        `end_R` |        `y*%` |
| ------------------ | ------------------- | ----------------: | -------------: | -------------: | -----------------------: | ---------------: | -------------: |
| **`#1`**   | 현행                |           2,096.9 |           0.11 |           3.26 |           988.4 (47.1 %) |    162.9 (7.8 %) |           65.8 |
|                    | **확장 최적** | **2,085.2** | **3.32** | **1.09** |           864.4 (41.5 %) | 1,053.2 (50.5 %) | **15.6** |
|                    | Fujiwara            |           2,079.1 | **0.00** |           0.97 | **309.7 (14.9 %)** |   249.3 (12.0 %) |           53.9 |
| **`#103`** | 현행                |           2,092.9 |           0.12 |           2.67 |           939.3 (44.9 %) |   418.5 (20.0 %) |           60.3 |
|                    | **확장 최적** | **2,077.7** | **2.60** | **1.26** |           987.6 (47.5 %) | 1,045.7 (50.3 %) | **18.3** |
|                    | Fujiwara            |           2,060.2 | **0.00** | **0.00** |           358.8 (17.4 %) | 1,244.9 (60.4 %) |           43.6 |
| **`#210`** | 현행                |           2,099.8 |           0.12 |           2.41 |           941.7 (44.9 %) |   521.2 (24.8 %) |           60.1 |
|                    | **확장 최적** | **2,089.0** | **2.79** | **1.17** |           922.0 (44.1 %) | 1,199.9 (57.4 %) | **12.7** |
|                    | Fujiwara            |           2,088.7 | **0.00** |           2.35 |           714.3 (34.2 %) |   500.2 (23.9 %) |           55.6 |

*`margin` 은 라미나 단위 · `end` 괄호 안은 `σ_max` 대비.*

**Fujiwara 를 빼고 현행 대 비대칭 DIN 만 놓으면** 두 곡선의 관계가 더 분명하다.

![현행 DIN 대 비대칭 DIN](Results/사이징최적화/figures/asymdin_vs_din.png)

*형상(위)에서 비대칭 DIN(붉은색)은 **좌측을 현행보다 훨씬 깊게** 파고 우측은 오히려 **얕게** 남긴다. 그 결과 응력분포(아래)에서 좌단이 안쪽으로 물러나고 최대응력점(●)이 중앙 쪽으로 끌려오며, 대신 우측은 끝까지 하중을 받는다 — **응력을 좌에서 우로 옮긴 것**이 한 그림에 보인다.*

##### ⑶ 이득과 대가

> **이 절의 판정은 「응력 잣대」 안에서의 것이다.** 같은 설계를 수명으로 재면 `ΣD30` 이 **1.56 ~ 1.64배**로 늘고, **`#210` 은 기준(`ΣD30` ≤ 0.5)을 넘어 적용할 수 없다**(§10-12.7.9 ⑶).
...
| ---------------------------------------- | ------------------- | ---------------------------------------- |
| `σ_max` ≤ 현행                       | —                  | **−10 ~ −15 MPa**                |
| 물리적 끝단 하중 없음 (양측 ≥ 1 라미나) | ❌ 좌측 0.11 ~ 0.12 | ✅**좌 2.6 ~ 3.3 · 우 1.1 ~ 1.3** |
| `y*` 중앙화                            | 60 ~ 66 %           | ✅**12.7 ~ 18.3 %**                |

`y*` 가 반길이의 **8분의 1 ~ 5분의 1** 지점까지 들어왔다. 현행은 3분의 2 지점이었다. 최대응력이 롤러 중앙 부근에서 발생한다는 것은 **모멘트에 의한 쏠림을 프로파일이 상쇄했다**는 뜻이다.

**대가 — 우측 진입응력이 오른다.**

`end_R` 이 163 ~ 521 → **1,045 ~ 1,200 MPa** 로 올랐다. 이것을 단순히 「악화」로 읽으면 오독이다 — **현행의 `end_L` 이 이미 939 ~ 988 MPa** 였으므로, 결과는 **양단이 현행의 나쁜 쪽 수준으로 균등해진 것**에 가깝다. `σ_max` 대비로는 41 ~ 57 % 이고 좌우 격차가 39 %p → **6 ~ 13 %p** 로 좁혀졌다.

다만 `#210` 은 `end_R` 이 1,199.9 로 **현행 최악값(941.7)보다 27 % 높다.** 그 설계에서는 「균등해졌다」는 해석이 그대로 통하지 않는다.

**직선 구간은 끝내 쓰이지 않았다.** 좌우 독립으로 열어도 `#1` 의 `f_R` = 0.04 하나뿐이고 나머지는 0 이다. 크라운 구간이 짧아져 가팔라지는 대가가 이득보다 크다 — Fujiwara 의 `K₂` 에서 얻은 결론(§10-12.6.5 ④)과 같은 방향이다.

##### ⑷ 두 해법의 대비 — 판정 기준이 없다

|              | Fujiwara 비대칭 (§10-12.6)             | 비대칭 DIN (§10-12.7)                                     |
| ------------ | --------------------------------------- | ---------------------------------------------------------- |
| 성격         | **끝단이 닿되 가볍게**            | **끝단을 띄우되 안쪽이 세게**                        |
| `margin`   | 0.00 ~ 2.35 라미나 (좌측 전부 0)        | 좌 2.6 ~ 3.3 · 우 1.1 ~ 1.3                               |
| `end` 최대 | 250 ~ 1,245 MPa                         | 864 ~ 1,200 MPa                                            |
| `y*`       | 43.6 ~ 55.6 %                           | **12.7 ~ 18.3 %**                                    |
| 파라미터     | 6개 (`K₁`·`K₂`·`z_m` × 좌우) | **5개** (`k_L`·`k_R`·`δ`·`f_L`·`f_R`) |
| 가공         | 로그 프로파일 좌우 이종                 | 로그 프로파일 좌우 이종 +**원점 이동 26 ~ 35 %**     |

**어느 쪽이 안전한지는 이 자료만으로 갈리지 않는다.** 「접촉이 물리적 끝단에 닿되 응력이 낮은 것」과 「끝단은 떠 있으나 접촉 시작점 응력이 높은 것」 중 무엇이 위험한가는 **모서리 접촉의 판정 기준**에 달려 있는데, 그 기준을 아직 세우지 않았다. 둘을 나란히 남기고 기준 수립을 미결로 올린다(§10-13 ⑧).

한 가지는 분명하다 — **`y*` 중앙화만 놓고 보면 비대칭 DIN 이 압도적**이다(12.7 ~ 18.3 % 대 43.6 ~ 55.6 %). 최대응력 위치를 옮기는 것이 목적이면 이쪽이다.

##### ⑸ `element_offset` 규칙과 함께 걸면 — 세 설계 모두 제약을 만족한다

§10-11.3.6 의 오프셋 규칙(`e_off` = `o_off` − (`D_we`/2)·sin(`α`−`β`))을 단독으로 적용하면 `#1`·`#210` 이 σ 제약을 넘었다. 여기에 비대칭 DIN 프로파일을 얹어 실측했다. **프로파일은 UW 에만** 건다.

**UW** — 지배 베어링. 비대칭 DIN 프로파일이 걸린다.

| 설계               | 조건                   |               `σ` | `end_L` | `end_R` |   `margin_L` |   `margin_R` |        `y*%` |
| ------------------ | ---------------------- | -------------------: | --------: | --------: | -------------: | -------------: | -------------: |
| **`#1`**   | 오프셋만               |           2,105.8 ❌ |   1,003.5 |     163.7 |           0.10 |           3.25 |           65.9 |
|                    | **+ 비대칭 DIN** | **2,092.9** ✅ |     873.8 |   1,057.2 | **3.29** | **1.09** | **15.7** |
| **`#103`** | 오프셋만               |           2,097.7 ✅ |     948.3 |     419.4 |           0.12 |           2.67 |           60.3 |
|                    | **+ 비대칭 DIN** | **2,081.9** ✅ |     993.2 |   1,047.7 | **2.59** | **1.26** | **18.4** |
| **`#210`** | 오프셋만               |           2,102.3 ❌ |     946.3 |     521.7 |           0.12 |           2.41 |           60.1 |
|                    | **+ 비대칭 DIN** | **2,091.1** ✅ |     924.8 |   1,201.0 | **2.78** | **1.17** | **12.7** |

**DW** — 프로파일을 걸지 않았다(현행 DIN 대칭 유지). 그런데도 값이 움직인다.

| 설계               | 조건         |                     `σ` | `end_L` | `end_R` |   `margin_L` | `margin_R` | `y*%` |
| ------------------ | ------------ | -------------------------: | --------: | --------: | -------------: | -----------: | ------: |
| **`#1`**   | 오프셋만     |                    1,679.1 |     782.8 |     868.5 |           2.00 |         1.84 |     7.7 |
|                    | + 비대칭 DIN | **1,793.9** (+114.8) |     847.3 |     235.7 | **0.00** |         6.12 |    20.5 |
| **`#103`** | 오프셋만     |                    1,720.8 |     736.9 |   1,002.0 |           2.09 |         1.63 |    18.3 |
|                    | + 비대칭 DIN |  **1,809.1** (+88.3) |     244.5 |     424.0 | **0.21** |         4.58 |    16.1 |
| **`#210`** | 오프셋만     |                    1,740.5 |     722.0 |   1,066.2 |           2.10 |         1.54 |    18.6 |
|                    | + 비대칭 DIN |  **1,831.6** (+91.1) |     288.3 |     410.3 | **0.20** |         4.67 |    20.9 |

*`margin` 은 라미나 단위 · `σ`·`end` 는 MPa · `y*%` 는 반길이 대비.*
**① 프로파일이 오프셋의 손실을 덮고도 남는다.** 오프셋 단독의 위반폭(+5.8 · +2.3 MPa)이 사라지고 세 설계 모두 만족한다. §10-11.3.6 ⑹ 이 Fujiwara 로 확인했던 상쇄가 **비대칭 DIN 으로도 성립**한다.

**② 프로파일이 새 오프셋에서도 그대로 듣는다.** 이 파라미터는 **현행 오프셋(20.742 mm) 기준으로 찾은 것**이라 재최적화가 필요할지 우려했으나, `margin` 3.29/1.09 · `y*` 15.7 % 로 §10-12.7.8 ⑵ 의 값(3.32/1.09 · 15.6 %)과 거의 같다. **재최적화는 불필요**했다.

**③ DW 를 처음으로 확인했다 — 지배 베어링은 UW 가 맞다.** DW 의 σ 는 1,679 ~ 1,832 MPa 로 UW 보다 **250 ~ 410 MPa 낮다.** 지금까지 UW 만 보고 σ 를 판정한 것이 타당했다.

**④ 그러나 UW 프로파일이 DW 를 건드린다.** DW 에는 프로파일을 걸지 않았는데도 두 상태의 값이 다르다 — UW 의 하중분배가 바뀌면 축을 통해 DW 로 전달되는 하중도 바뀌기 때문이다. 방향은 **모두 나쁜 쪽**이다.

| DW 지표      |           오프셋만 |                 + 비대칭 DIN | 변화                      |
| ------------ | -----------------: | ---------------------------: | ------------------------- |
| `σ`       |      1,679 ~ 1,741 |      **1,794 ~ 1,832** | **+88 ~ +115 MPa**  |
| `margin_L` | 2.00 ~ 2.10 라미나 | **0.00 ~ 0.21 라미나** | **여유가 사라진다** |
| `margin_R` |        1.54 ~ 1.84 |                  4.58 ~ 6.12 | 늘어난다                  |

**UW 를 고치면 DW 의 좌단이 물린다.** `#1` 은 DW 좌단 여유가 2.00 → **0.00 라미나**로 무너진다. UW 에서 없앤 문제가 DW 에 나타난 셈이며, §10-12.7.6 의 「좌측을 벌면 우측이 물린다」와 같은 구조가 **베어링 사이에서 반복**된다.

다만 DW 의 σ 는 여전히 UW 보다 260 ~ 300 MPa 낮고 제약(2,100)에서 270 MPa 이상 떨어져 있다. **당장의 제약 위반은 아니지만 방치할 수 없는 부작용**이며, DW 프로파일 검토가 필요하다는 근거가 된다(§10-13 미결).

**MASTA 파일** — `부록10_NSGA/S3_본최적화/MASTA/`

| 설계     | 파일                               | 담긴 것                                             |
| -------- | ---------------------------------- | --------------------------------------------------- |
| `#1`   | `A10_front001_offset_adin.masta` | `e_off` 48.80 + `k` 2.9/0.5 · `δ` 70 (UW)   |
| `#103` | `A10_front103_offset_adin.masta` | `e_off` 35.81 + `k` 2.3/0.675 · `δ` 60 (UW) |
| `#210` | `A10_front210_offset_adin.masta` | `e_off` 28.76 + `k` 2.4/0.7 · `δ` 60 (UW)   |

DW 는 세 파일 모두 **현행 DIN 대칭**이다. 프로파일은 `USERSPECIFIED` 201점 좌표로 들어가 있다.

##### ⑹ 남은 것

| 항목                        | 내용                                                                                                  |
| --------------------------- | ----------------------------------------------------------------------------------------------------- |
| **판정 기준**         | 모서리 접촉 위험을 「끝단 응력」으로 볼지 「끝단 접촉 여부」로 볼지. 이것이 서면 두 해법이 갈린다     |
| **수명**              | σ 와 분포만 봤다. ISO/TS 16281 수명이 어떻게 바뀌는지 미확인 —**바로 착수한다**               |
| **가공성**            | `δ`/반길이 26 ~ 35 % 의 원점 이동이 도면·검사에서 허용되는가. `#210` 은 34.5 %                  |
| **`end_R` 상승**    | `#210` 은 현행 최악값보다 27 % 높다. 허용 여부 판단 필요                                            |
| **DW 좌단 여유**      | DW 는 현행 DIN 이라 좌단 여유가 0.00 ~ 0.21 라미나다(⑸ ③). σ 가 낮아 위험도는 낮으나 확인은 남았다 |
| **경하중 과크라우닝** | 좌측 낙차가 현행의 2.9배(1,459 μm)다. 경하중 LC 에서 과크라우닝 수명 저하 가능성                     |

*실행 — `a10_asymdin3.py`(`δ` 확장 134회) · `a10_final_fig.py`(3해법 수집·그림). 결과 `부록10_NSGA/S3_본최적화/asymdin/` — `asymdin3_delta.csv` · `best3.json` · `final_fig.json`.*

<!-- /A10:ASYMJUDGE -->

#### 10-12.7.9. 피로수명 검토 (실시간)

§10-12.7.8 까지는 **응력과 분포**만 봤다. 두 비대칭 해법이 갈리지 않는 것도(⑷), UW 를 고치면 DW 가 물리는 것도(⑸ ④) **응력 잣대 안에서의 관찰**이다. 여기서는 **수명**이라는 단일 잣대로 옮긴다.

|                | 조건                                                                                                                                           |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| **형상** | `element_offset` = `o_off` − (`D_we`/2)·sin(`α`−`β`) (§10-11.3.6) + **UW 에만** 비대칭 DIN (확장 최적 · §10-12.7.8 ⑴) |
| **온도** | 베어링**70 °C** · 샤프트 40 · 하우징 40 (§6-11.7 과 동일)                                                                            |
| **하중** | 111 DLC · 빈 대표하중`dt` = 20 · `k` 는 ν(70) 재선정값                                                                                  |
| **지표** | `ΣD30` (UW · DW) → 수명 [년] · 직렬 와이블 `e` = 9/8                                                                                   |

부록 1 의 `run_fatigue` 는 111 DLC 를 통째로 돌고 끝에 반환하므로, 실시간 갱신을 위해 **20 DLC 마다 콜백**하는 형태로 다시 썼다.

**우선 `#1` 하나만 돌린다** — `USERSPECIFIED` 좌표식은 내장식보다 크게 느려(앞선 실행에서 12배) 소요를 먼저 재고 확대 여부를 정한다.

<!-- A10:LIFE79 -->

*실행 중 — 설계 완료마다 · 20 DLC 마다 갱신. 경과 45.6분.*

**진행 · 수명**

| 설계     | 조건                |     DLC | ΣD30 UW | ΣD30 DW | 수명 UW | 수명 DW | **수명 Sys** |
| -------- | ------------------- | ------: | -------: | -------: | ------: | ------: | -----------------: |
| `#1`   | 오프셋만            | 111/111 |   0.1623 |   0.0070 |   184.8 | 4,306.9 |    **180.2** |
| `#1`   | 오프셋 + 비대칭 DIN | 111/111 |   0.2525 |   0.0125 |   118.8 | 2,393.1 |    **115.3** |
| `#103` | 오프셋만            | 111/111 |   0.2269 |   0.0108 |   132.2 | 2,773.3 |    **128.5** |
| `#103` | 오프셋 + 비대칭 DIN | 111/111 |   0.3425 |   0.0188 |    87.6 | 1,593.9 |     **84.7** |
| `#210` | 오프셋만            | 111/111 |   0.3778 |   0.0171 |    79.4 | 1,755.5 |     **77.3** |
| `#210` | 오프셋 + 비대칭 DIN | 111/111 |   0.6190 |   0.0325 |    48.5 |   923.5 |     **46.9** |

**`#1` 손상 상위 15 DLC** — UW 손상 기준 정렬

|  # | DLC             | 기준 UW | + 비대칭 UW | 변화       | 기준 DW | + 비대칭 DW | 누적 UW 비율 |
| -: | --------------- | ------: | ----------: | ---------- | ------: | ----------: | -----------: |
|  1 | `DLC1.2-a-s5` | 0.00697 |     0.01057 | ▲ 0.00360 | 0.00023 |     0.00039 |        4.2 % |
|  2 | `DLC1.2-a-s4` | 0.00688 |     0.01045 | ▲ 0.00357 | 0.00022 |     0.00037 |        8.3 % |
|  3 | `DLC1.2-a-s6` | 0.00681 |     0.01035 | ▲ 0.00354 | 0.00021 |     0.00036 |       12.4 % |
|  4 | `DLC1.2-a-s3` | 0.00679 |     0.01032 | ▲ 0.00352 | 0.00021 |     0.00036 |       16.5 % |
|  5 | `DLC1.2-a-s2` | 0.00676 |     0.01027 | ▲ 0.00351 | 0.00021 |     0.00036 |       20.6 % |
|  6 | `DLC1.2-a-s1` | 0.00674 |     0.01025 | ▲ 0.00351 | 0.00020 |     0.00035 |       24.6 % |
|  7 | `DLC1.2-b-s5` | 0.00633 |     0.00970 | ▲ 0.00337 | 0.00017 |     0.00031 |       28.5 % |
|  8 | `DLC1.2-b-s1` | 0.00631 |     0.00966 | ▲ 0.00336 | 0.00016 |     0.00028 |       32.3 % |
|  9 | `DLC1.2-b-s2` | 0.00619 |     0.00949 | ▲ 0.00330 | 0.00015 |     0.00027 |       36.1 % |
| 10 | `DLC1.2-b-s4` | 0.00616 |     0.00944 | ▲ 0.00329 | 0.00015 |     0.00027 |       39.8 % |
| 11 | `DLC1.2-b-s3` | 0.00613 |     0.00940 | ▲ 0.00327 | 0.00015 |     0.00027 |       43.5 % |
| 12 | `DLC1.2-b-s6` | 0.00610 |     0.00936 | ▲ 0.00326 | 0.00015 |     0.00027 |       47.2 % |
| 13 | `DLC1.2-d-s1` | 0.00450 |     0.00711 | ▲ 0.00261 | 0.00027 |     0.00051 |       50.0 % |
| 14 | `DLC1.2-d-s5` | 0.00447 |     0.00706 | ▲ 0.00260 | 0.00028 |     0.00052 |       52.8 % |
| 15 | `DLC1.2-d-s2` | 0.00435 |     0.00690 | ▲ 0.00255 | 0.00022 |     0.00041 |       55.6 % |

**`#103` 손상 상위 15 DLC** — UW 손상 기준 정렬

|  # | DLC             | 기준 UW | + 비대칭 UW | 변화       | 기준 DW | + 비대칭 DW | 누적 UW 비율 |
| -: | --------------- | ------: | ----------: | ---------- | ------: | ----------: | -----------: |
|  1 | `DLC1.2-a-s5` | 0.00946 |     0.01389 | ▲ 0.00443 | 0.00032 |     0.00054 |        4.1 % |
|  2 | `DLC1.2-a-s4` | 0.00933 |     0.01373 | ▲ 0.00440 | 0.00030 |     0.00051 |        8.1 % |
|  3 | `DLC1.2-a-s6` | 0.00923 |     0.01358 | ▲ 0.00436 | 0.00029 |     0.00049 |       12.0 % |
|  4 | `DLC1.2-a-s3` | 0.00921 |     0.01355 | ▲ 0.00434 | 0.00029 |     0.00050 |       16.0 % |
|  5 | `DLC1.2-a-s2` | 0.00916 |     0.01348 | ▲ 0.00433 | 0.00029 |     0.00048 |       19.9 % |
|  6 | `DLC1.2-a-s1` | 0.00914 |     0.01346 | ▲ 0.00432 | 0.00028 |     0.00048 |       23.9 % |
|  7 | `DLC1.2-b-s5` | 0.00839 |     0.01246 | ▲ 0.00407 | 0.00027 |     0.00047 |       27.5 % |
|  8 | `DLC1.2-b-s1` | 0.00830 |     0.01233 | ▲ 0.00403 | 0.00025 |     0.00043 |       31.1 % |
|  9 | `DLC1.2-b-s2` | 0.00819 |     0.01218 | ▲ 0.00399 | 0.00024 |     0.00041 |       34.6 % |
| 10 | `DLC1.2-b-s4` | 0.00811 |     0.01206 | ▲ 0.00396 | 0.00024 |     0.00041 |       38.2 % |
| 11 | `DLC1.2-b-s3` | 0.00810 |     0.01205 | ▲ 0.00395 | 0.00024 |     0.00042 |       41.7 % |
| 12 | `DLC1.2-b-s6` | 0.00803 |     0.01196 | ▲ 0.00393 | 0.00024 |     0.00041 |       45.2 % |
| 13 | `DLC1.2-d-s1` | 0.00669 |     0.01023 | ▲ 0.00354 | 0.00044 |     0.00077 |       48.2 % |
| 14 | `DLC1.2-d-s5` | 0.00664 |     0.01016 | ▲ 0.00352 | 0.00045 |     0.00079 |       51.1 % |
| 15 | `DLC1.2-d-s2` | 0.00645 |     0.00990 | ▲ 0.00345 | 0.00035 |     0.00062 |       54.0 % |

**`#210` 손상 상위 15 DLC** — UW 손상 기준 정렬

|  # | DLC             | 기준 UW | + 비대칭 UW | 변화       | 기준 DW | + 비대칭 DW | 누적 UW 비율 |
| -: | --------------- | ------: | ----------: | ---------- | ------: | ----------: | -----------: |
|  1 | `DLC1.2-a-s5` | 0.01534 |     0.02428 | ▲ 0.00894 | 0.00048 |     0.00087 |        3.9 % |
|  2 | `DLC1.2-a-s4` | 0.01513 |     0.02401 | ▲ 0.00888 | 0.00044 |     0.00082 |        7.8 % |
|  3 | `DLC1.2-a-s6` | 0.01496 |     0.02376 | ▲ 0.00879 | 0.00043 |     0.00079 |       11.6 % |
|  4 | `DLC1.2-a-s3` | 0.01493 |     0.02370 | ▲ 0.00877 | 0.00043 |     0.00080 |       15.5 % |
|  5 | `DLC1.2-a-s2` | 0.01485 |     0.02358 | ▲ 0.00874 | 0.00042 |     0.00078 |       19.3 % |
|  6 | `DLC1.2-a-s1` | 0.01482 |     0.02355 | ▲ 0.00873 | 0.00042 |     0.00077 |       23.1 % |
|  7 | `DLC1.2-b-s5` | 0.01339 |     0.02150 | ▲ 0.00811 | 0.00043 |     0.00080 |       26.6 % |
|  8 | `DLC1.2-b-s1` | 0.01318 |     0.02119 | ▲ 0.00800 | 0.00039 |     0.00074 |       30.0 % |
|  9 | `DLC1.2-b-s2` | 0.01307 |     0.02102 | ▲ 0.00795 | 0.00037 |     0.00070 |       33.4 % |
| 10 | `DLC1.2-b-s3` | 0.01292 |     0.02079 | ▲ 0.00787 | 0.00037 |     0.00071 |       36.7 % |
| 11 | `DLC1.2-b-s4` | 0.01289 |     0.02075 | ▲ 0.00786 | 0.00037 |     0.00070 |       40.1 % |
| 12 | `DLC1.2-b-s6` | 0.01279 |     0.02060 | ▲ 0.00781 | 0.00037 |     0.00070 |       43.4 % |
| 13 | `DLC1.2-d-s1` | 0.01162 |     0.01936 | ▲ 0.00773 | 0.00071 |     0.00135 |       46.5 % |
| 14 | `DLC1.2-d-s5` | 0.01153 |     0.01921 | ▲ 0.00768 | 0.00073 |     0.00139 |       49.6 % |
| 15 | `DLC1.2-d-s2` | 0.01121 |     0.01875 | ▲ 0.00754 | 0.00056 |     0.00109 |       52.7 % |

<!-- /A10:LIFE79 -->

##### ⑴ 결과

기준은 **오프셋만**(양쪽 DIN)이다. 이렇게 두면 비교가 **프로파일 효과만 분리**한다.

| 설계               | 조건         |         ΣD30 UW | ΣD30 DW | **ΣD30 Sys** | 수명 UW | 수명 DW | **수명 Sys** |        기준 대비 |
| ------------------ | ------------ | ---------------: | -------: | ---------------: | ------: | ------: | -----------------: | ---------------: |
| **`#1`**   | 오프셋만     |           0.1623 |   0.0070 | 0.1665 |   184.8 | 4,306.9 |  **180.2년** |               — |
|                    | + 비대칭 DIN |           0.2525 |   0.0125 | **0.2601** |   118.8 | 2,393.1 |  **115.3년** | **−36 %** |
| **`#103`** | 오프셋만     |           0.2269 |   0.0108 | 0.2335 |   132.2 | 2,773.3 |  **128.5년** |               — |
|                    | + 비대칭 DIN |           0.3425 |   0.0188 | **0.3541** |    87.6 | 1,593.9 |   **84.7년** | **−34 %** |
| **`#210`** | 오프셋만     |           0.3778 |   0.0171 | 0.3881 |    79.4 | 1,755.5 |   **77.3년** |               — |
|                    | + 비대칭 DIN | **0.6190** |   0.0325 | **0.6390** |    48.5 |   923.5 |   **46.9년** | **−39 %** |

*`ΣD30_Sys` 는 UW·DW 를 직렬 와이블(`e` = 9/8)로 합친 값이며 **수명 Sys = 30 / ΣD30_Sys** 로 검산된다.*

*`#1` 의 **현행**(오프셋 전 · 양쪽 DIN)은 별도로 쟀다 — ΣD30 UW 0.1582 · 수명 **185.0년**. `#103`·`#210` 은 현행을 재지 않았다.*
...
| 단계                         | 수명 Sys |         직전 대비 |
| ---------------------------- | -------: | ----------------: |
| 현행 (오프셋 전 · 양쪽 DIN) |  185.0년 |                — |
| **오프셋만**           |  180.2년 | **−2.6 %** |
| **+ 비대칭 DIN**       |  115.3년 |  **−36 %** |

**`element_offset` 규칙의 몫은 −2.6 % 에 불과하다.** `L_eff` 가 −0.95 % 짧아진 것치고는 작다. 나머지 **−36 %** 가 비대칭 프로파일이며, 세 설계에서 −34 ~ −39 % 로 일관된다.

원인은 형상에서 바로 읽힌다 — 좌측 낙차가 현행의 **2.9배**(`#1` 1,459 μm)라 **유효 접촉길이가 줄고** 단위 길이당 하중이 오른다. §10-12.6.7 에 「`z_m` 이 ISO 기본값의 7 ~ 8배라 경하중에서 과크라우닝으로 수명이 떨어질 수 있다」고 적어 둔 우려가 **실측으로 확인**된 것이다.

##### ⑶ 판정 — 설계에 따라 갈린다

판정 기준은 **`ΣD30` ≤ 0.5**(30년 설계수명 · 안전여유 2배)다. 베어링 쌍 전체의 건전성을 보는 것이므로 **`Sys` 기준**으로 판정한다 — UW 기준으로 재도 판정은 같다(50.5 / 68.5 / 123.8 %).

| 설계     | ΣD30 Sys (오프셋만) | ΣD30 Sys (+ 비대칭) |          기준 0.5 대비 |          판정          |
| -------- | ------------------: | ------------------: | ---------------------: | :---------------------: |
| `#1`   |              0.1665 |              0.2601 |            52.0 % 사용 |     ✅ 여유 1.92배     |
| `#103` |              0.2335 |              0.3541 |            70.8 % 사용 |     ✅ 여유 1.41배     |
| `#210` |              0.3881 |    **0.6390** | **127.8 % 사용** | ❌**위반 +0.139** |

**`#210` 은 기준을 넘는다 — 적용 불가다.** 셋 중 가장 작은 베어링(`L_we` 347.6 · `D_we` 170.0)이라 오프셋만으로도 이미 0.3881(77.6 %)로 여유가 얇았고, 그 위에 프로파일이 얹혔다.

`#1`·`#103` 은 기준을 지킨다. 즉 **응력 관점의 이득(§10-12.7.8)을 수명 여유로 지불하는데, 그 지불 능력이 설계마다 다르다** — 큰 베어링일수록 여유가 있고 작은 베어링은 감당하지 못한다.

> §10-12.7.8 ⑹ 의 「남은 것」에 적은 **경하중 과크라우닝** 항목이 여기서 실제 제약으로 나타났다. 비대칭 DIN 을 채택하려면 **설계별로 수명 여유를 확인**해야 하며, `#210` 급에는 더 얕은 프로파일(작은 `k_L`)이 필요하다 — 그러면 응력 이득이 줄어드는 재절충이 생긴다.

##### ⑷ 속도 — 좌표식은 구조적으로 느리다

같은 111 DLC 를 현행 DIN 은 **4.3분**, 좌표식(UW 만 `USERSPECIFIED` 201점)은 **64.1분**에 돌았다 — **15배**다. 실측으로 가르지는 않았고, 구조에서 오는 차이로 본다.

| 항목      | DIN Lundberg                                            | USERSPECIFIED                                                                      |
| --------- | ------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| 낙차 계산 | `z` = `A`·ln[1/(1−(2y/L)²)] — **닫힌 식** | 201점 표에서**보간**                                                         |
| 호출 빈도 | 라미나마다 즉시                                         | 라미나마다 보간 ·`ACTUAL_DATA` 는 평활·피팅을 거치지 않아 매번 원자료를 훑는다 |
| 대칭성    | 좌우 대칭이라 절반만 계산할 여지                        | **비대칭이라 전 구간 계산**                                                  |
| 캐시      | 설계가 고정되면 재사용 가능                             | 프로파일이 설계변수라 조건마다 재구성                                              |

규모를 어림하면, 접촉 요소 20여 개 × 라미나 38 × 하중케이스(111 DLC × 빈 30) ≈ **250만 회**의 보간이 조건 하나에 들어간다. 닫힌 식 한 줄과 표 탐색+보간의 차이가 그만큼 누적된다.

**대책과 대가** — 어느 것도 공짜가 아니다.

| 대책                            | 기대 효과                   | 대가                                                                                                                                   |
| ------------------------------- | --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| 점수 축소 (201 → 61)           | 보간 비용 ~1/3              | **끝단 충실도 손실** — 61점은 `end_L` 이 +73 MPa 어긋났다(§10-12.7.4 T0). 수명은 끝단 의존이 작을 수 있으나 **미검증** |
| `SMOOTHED` / `FITTED` 사용  | 내부 표현이 매끄러워질 여지 | **형상이 바뀐다** — 우리가 넣은 좌표가 아닌 것을 푸는 셈                                                                        |
| 배치 확대 (`NBATCH` 20 → 40) | 듀티사이클 오버헤드 감소    | 보간 비용은 그대로 · 메모리 부담                                                                                                      |
| DLC 축약                        | 선형 감소                   | 손상 상위 15건이 누적의 대부분이나**꼬리를 버리는 근사**                                                                         |

**당장은 예산으로 받아들인다.** 3설계 × 2조건 전수는 **3시간 이상**으로 잡아야 한다. 점수 축소는 「수명이 끝단에 얼마나 의존하는가」를 먼저 재야 쓸 수 있다 — 그 실측은 하지 않았다.

**실측 — `data_to_use` 를 바꿔 보았다.** `#1`(오프셋 + 비대칭 DIN · 70 °C)에서 세 설정을 걸고 **① 같은 것을 푸는가 ② 빨라지는가**를 함께 쟀다. 빨라도 다른 것을 풀면 쓸 수 없기 때문이다.

| 설정                        | 좌 끝단낙차 | 우 끝단낙차 | 비대칭도 | UW`σ` |   UW margin | UW`y*%` | **DLC 5건 소요** |
| --------------------------- | ----------: | ----------: | -------: | -------: | ----------: | --------: | ---------------------: |
| `ACTUAL_DATA` (현행)      |     1,459.0 |       251.5 |   82.8 % |  2,092.9 | 3.29 / 1.09 |      15.7 |      **266.6초** |
| **`SMOOTHED`**      |     1,459.0 |       251.5 |   82.8 % |  2,092.9 | 3.29 / 1.09 |      15.7 |       **31.0초** |
| `FITTED_STANDARD_PROFILE` |          — |          — |       — |       — |          — |        — |    **사용 불가** |

**① `SMOOTHED` 는 형상을 바꾸지 않는다.** 실제 적용된 201점을 `outer_race_and_roller_profiles` 로 읽어 대조하니 **최대 차이 0.000000 μm** 다 — 좌표도 낙차도 전부 같다. 비대칭도 82.8 % 가 그대로 유지된다.

**② 지표도 사실상 같다.**

| 지표                          | `ACTUAL_DATA` | `SMOOTHED` |                     차이 |
| ----------------------------- | --------------: | -----------: | -----------------------: |
| UW`σ`                      |         2,092.9 |      2,092.9 |                      0.0 |
| UW`end_L`                   |           873.8 |        873.8 |                      0.0 |
| UW`end_R`                   |         1,057.2 |      1,055.4 | **−1.8** (0.17 %) |
| UW`margin_L` / `margin_R` |     3.29 / 1.09 |  3.29 / 1.09 |                        0 |
| UW`y*%`                     |            15.7 |         15.7 |                        0 |
| DW`σ`                      |         1,793.9 |      1,793.8 |                    −0.1 |
| DW`end_L`                   |           847.3 |        855.1 |  **+7.8** (0.92 %) |

**③ 8.6배 빠르다.** DLC 5건에 266.6초 → **31.0초**. 111 DLC 로 환산하면 64.1분 → **약 7.4분**이다. 3설계 × 2조건 전수가 3시간에서 **25분 수준**으로 내려간다.

**④ `FITTED_STANDARD_PROFILE` 은 쓸 수 없다.** 설정 직후 프로파일을 읽으려 하면 MASTA 내부에서 예외가 난다(`ReportablePropertyDefinition` 계열). 애초에 `ProfileToFit` 의 선택지가 `AUTO`·`DIN_LUNDBERG`·`QUADRATIC` 로 **모두 좌우 대칭**이라, 성공했더라도 비대칭성을 잃었을 것이다.

##### ⑸ 개선안 — 피로에만 `SMOOTHED`, 응력은 `ACTUAL_DATA` 유지

빠르다고 전부 바꾸지는 않는다. **두 해석은 비용 구조가 다르다.**

| 용도                      | 규모                        | 설정                      | 이유                                                                                                                                                 |
| ------------------------- | --------------------------- | ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| **응력·형상 판정** | LC 1건 · 2 ~ 3초           | **`ACTUAL_DATA`** | 속도 이득이 무의미하다. 판정에 쓰는 값은**우리가 넣은 원자료 그대로** 두는 편이 안전하다 — `end_R`·DW `end_L` 에 0.2 ~ 0.9 % 차이가 있다 |
| **피로 대량해석**   | 111 DLC × 빈 30 · 수십 분 | **`SMOOTHED`**    | 같은 값을**7.2배** 빠르게 준다. 이 규모에서는 결정적이다                                                                                       |

**대조로 확인했다.** `#1`(오프셋 + 비대칭 DIN · 70 °C)의 111 DLC 를 두 설정으로 각각 완주했다.

| 설정                   |         ΣD30 UW |         ΣD30 DW | 수명 UW | 수명 DW | **수명 Sys** |            소요 |
| ---------------------- | ---------------: | ---------------: | ------: | ------: | -----------------: | --------------: |
| `ACTUAL_DATA`        |           0.2525 |           0.0125 |   118.8 | 2,392.6 |  **115.3년** |          64.1분 |
| **`SMOOTHED`** | **0.2525** | **0.0125** |   118.8 | 2,393.1 |  **115.3년** | **8.9분** |

`ΣD30` 이 **소수 4자리까지 같고** 수명도 같다. 유일한 차이는 DW 수명 2,392.6 → 2,393.1년(**0.02 %**)인데, 손상의 2.5 % 만 담당하는 쪽이라 결과에 영향이 없다.

> 앞서 DLC 5건 표본으로 잰 배수는 **8.6배**였는데 111건 완주에서는 **7.2배**였다. 표본이 배수를 과대추정했다 — 소규모 측정으로 속도를 약속하지 않는다.

**적용** — `a10_life79.py` 가 프로파일 주입 직후 `SMOOTHED` 로 바꾸도록 했다(`set_smoothed`). 응력계 스크립트(`a10_asymdin*` · `a10_save_combo` · `a10_datause`)는 **건드리지 않았다.**

**남은 불확실성**

| 항목                 | 내용                                                                                                                                                                                                        |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SMOOTHED` 의 정체 | MASTA 가 내부적으로 무엇을 하는지 모른다. 우리 자료에서**형상이 201점 전부 동일**(최대차 0.000000 μm)한 것이 일반적 성질인지 우연인지 알 수 없다 — **다른 형상에 쓸 때는 매번 대조**해야 한다 |
| 기존 결과            | §10-12.6 · §10-12.7 은`ACTUAL_DATA` 로 얻었다. 형상이 같고 응력은 계속 `ACTUAL_DATA` 를 쓰므로 **재실행 불필요**                                                                               |
| `FITTED`           | 사용 불가(⑷ ④). 되살릴 계획 없음                                                                                                                                                                          |

*실행 — `a10_datause.py`(설정 비교) · `a10_life79.py`(두 설정 완주 대조) · 결과 `부록10_NSGA/S3_본최적화/datause/`.*
*실행 — `a10_life79.py`(20 DLC 마다 콜백·문서 갱신) · 결과 `부록10_NSGA/S3_본최적화/life79/life79_perdlc.csv`.*

## 10-13. 미결

|            # | 항목                                   | 내용                                                                                                                                                                                                                                                                                                                                                    |
| -----------: | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|           ① | **수렴 미완**                    | ΔHV 가 150세대까지 0.05% 이상으로 올랐다(§10-11.2.3 ⑤). 세대 연장이 필요하다 —`eval_cache` 덕에 **앞 150세대는 MASTA 0회로 재생**되므로 추가 비용은 151세대 이후분뿐이다                                                                                                                                                                    |
|           ② | 목적 겹침                              | `f3 = 2f2 + m_shaft` 로 `f2` 를 포함한다(§10-2 ▸). `(D, 베어링, 샤프트)` 로 재판정해 프론트가 얼마나 달라지는지 확인이 남았다                                                                                                                                                                                                                   |
|           ③ | 3목적 · NSGA-II                       | 프론트가 65세대부터 개체수 전체로 포화됐다(§10-11.2.3 ⑥)                                                                                                                                                                                                                                                                                              |
|           ④ | `z2` 세분의 실효                     | 26수준을 열었으나 프론트는 5수준(5.6 ~ 6.0)만 썼다. 구간을 좁히고 더 촘촘히 하는 편이 나을 수 있다                                                                                                                                                                                                                                                      |
|           ⑤ | 시드 1개                               | 부록 6 이래의 한계(미결#9) 그대로                                                                                                                                                                                                                                                                                                                       |
| **⑥** | **`element_offset` 적용 시점** | 규칙은**확정됐다** — `e_off` = `o_off` − (`D_we`/2)·sin(`α`−`β`) (§10-11.3.6). 단독 적용 시 `#103` 은 만족하나 `#1`·`#210` 이 각각 5.8 · 2.3 MPa 초과하고, **비대칭 프로파일(§10-12.6)과 함께 걸면 세 설계 모두 만족**한다(2,064.7 ~ 2,088.1). 두 변경을 한 묶음으로 반영하고 부록 10 을 재실행할 시점이 미정이다 |
| **⑦** | **비대칭 프로파일 채택 여부**    | §10-12.6 에서 비대칭 Fujiwara 가 σ·끝단응력·응력위치를**동시에 개선**함을 확인했다(끝단응력 −65 ~ −93 %). 다만 **가공성(`K₂ ≤ 0.5` 에서 가능해 0)** · 특허 회피 · 궤도륜 크라우닝 분담 · 피로수명 영향이 미확인이다(§10-12.6.7)                                                                                               |
| **⑧** | **모서리 접촉 판정 기준**        | 「끝단이 닿되 가볍게」(Fujiwara)와 「끝단을 띄우되 안쪽이 세게」(비대칭 DIN) 중 무엇이 안전한지 가릴 기준이 없다(§10-12.7.8 ⑷). 기준이 서야 두 해법의 채택이 갈린다                                                                                                                                                                                   |

*S3-a·b·c 완료 (260807) · 프로파일 검토 완료 (260808) — 세대 연장 여부는 지시 대기.*
