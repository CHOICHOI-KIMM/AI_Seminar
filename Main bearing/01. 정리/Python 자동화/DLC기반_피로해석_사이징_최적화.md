...
> 작성 260727~ · **현재 판 R13 (260803)** · 선행 문서: `DLC기반_피로해석_DLC별해석.md`(부록 1~5), `DLC기반_피로해석_프로세스_v1.md`
> 전체 개정 경위는 문서 최하단 **[개정 이력](#개정-이력)** 참조 — 번복·무효화 25건을 R0~R13 로 정리했다.
...
| 부록 6 S1~S5 — NSGA-II 수행 | ⬜ 미착수 | 평가기 → 재현시험 → 본최적화 → P2 검증 → 엑셀 |
| P3 — 최종 확정 | ⬜ 미착수 | |
...
| **개체수** | **224** | **범주 조합 수와 일치** — 조합당 1개씩 시딩해 커버리지 100% (§6-8.1.4 ⒝) |
| **초기화** | **범주 전수 시딩 + 수치 무작위** | 무작위 224 는 조합의 63% 만 덮는다 |
| **수리** | **`L_we ← clip(L_we, 1.5·D_we, 2.5·D_we)`** | 세장비가 해석식 탈락의 60% — 페널티 대신 수리 (§6-8.1.4 ⒜) |
| **세대수** | **150 (상한)** | 총 **33,600 평가 ≈ 2.8시간** · 조합당 평균 150 평가 |
| **종료** | 세대수 도달 **또는 하이퍼볼륨 정체** | `RobustTermination` — 먼저 닿는 쪽 |
...
| 중복 제거 | `eliminate_duplicates=True` | 전 변수가 이산이라 중복이 실제로 발생 |
...
|---|---|---|
| 알고리즘 | **NSGA-II** | 2목적에는 NSGA-II 가 정석(NSGA-III 는 3목적 이상) |
| 개체수 | **200** | 범주 조합이 224개 — 100 이면 세대당 절반도 못 덮는다 |
| 세대수 | **150** | 총 **30,000 평가 ≈ 2.5시간** · 조합당 평균 134 평가 |
| 교차 | SBX (`eta=15`, `prob=0.9`) → 정수 반올림 | `Integer` 3변수 |
| 변이 | 다항식 (`eta=20`, `prob=1/n`) → 정수 반올림 | `Integer` 3변수 |
| 범주변수 | `MixedVariableMating` (무작위 리셋) | `z1`·`z2`·α |
| 생존 | `RankAndCrowdingSurvival` | NSGA-II 정의 |
| 제약 | 제약 지배 (Deb) | 해석식 제약 선평가 |
| 종료 | 세대수 도달 + 하이퍼볼륨 정체 | S2 에서 확정 |
...
```
L_we ← clip(L_we, 1.5 · D_we, 2.5 · D_we)
```
...
| 개선안 | 채택 | 반영 |
|---|:-:|---|
| ⒜ 세장비 수리 | **채택** | 해석식 통과율 33% → 90%대 · 초기 가능해 5.9 → 약 18개 |
| ⒝ 범주 전수 시딩 | **채택** | `N` 200 → **224**, 커버리지 59% → **100%** |
| ⒞ 웜스타트 | **미채택** | 격자 근방 고착 위험 · S2 검증성 보존 |
| ⒟ 하이퍼볼륨 정체 | **채택** | `G`=150 은 상한으로만 작동 |
...
| S1 | 평가기 — NSGA 세대(100점) ↔ P1 배치 연결 | `nsga_eval.py` | 30 s/세대 |
| S2 | **재현 시험** — 현 격자로 제한해 전수 파레토(10·8건) 재현 확인 | 검증 로그 | ~1 h |
| S3 | 본 최적화 — 224×150 · 33,600 평가 | 프론트 CSV·그림 | **2.8 h** |
...
| 3 | 하이퍼볼륨 정체의 구체 임계값(`n_last`·`tol`) | S2 |
| 4 | UW/DW 제원 분리(§4-2 2차 탐색, 변수 10개) 적용 여부 | S3 이후 |
| 5 | 0.1 mm 분해능이 프론트를 과소분해하는지 — S2 재현시험에서 확인 | S2 |
| 6 | 세장비 수리가 경계에 개체를 몰아 프론트를 왜곡하는지 (§6-8.1.5) | S2 |
