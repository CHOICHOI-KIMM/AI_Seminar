...
### 7-6.5 베어링 손상 상위 10 DLC 의 샤프트 안전율

§7-6.2 는 손상 1위 DLC 한 건만 봤다. **상위 10건**으로 넓혀 운전 하중에서의
샤프트 여유가 어느 범위에 있는지 본다. 대상은 `a01`, 변환은 같은 경로
(`dt=20` · μ+kσ · `screen_k.csv` 의 `k`), 값은 듀티사이클 집계(=최악 빈)다.

<!-- A7:TOP10 -->
| # | DLC | 베어링 ΣD30_UW | 비중 | `k` | 빈 | **무한수명 피로** | 영구변형 | 극한 LC 대비 |
|--:|---|--:|--:|--:|--:|--:|--:|--:|
| 1 | `DLC1.2-d-s1` | 0.01110 | 3.7% | 0.19 | 30 | **13.171** | 40.294 | 3.10배 |
| 2 | `DLC1.2-d-s5` | 0.01094 | 3.6% | 0.2 | 30 | **13.454** | 40.314 | 3.16배 |
| 3 | `DLC1.2-d-s2` | 0.01066 | 3.5% | 0.19 | 30 | **13.535** | 41.020 | 3.18배 |
| 4 | `DLC1.2-d-s3` | 0.01033 | 3.4% | 0.18 | 30 | **12.627** | 38.245 | 2.97배 |
| 5 | `DLC1.2-d-s4` | 0.01019 | 3.4% | 0.18 | 30 | **12.998** | 38.923 | 3.06배 |
| 6 | `DLC1.2-d-s6` | 0.01010 | 3.3% | 0.19 | 30 | **13.681** | 40.259 | 3.22배 |
| 7 | `DLC1.2-a-s5` | 0.00936 | 3.1% | 0.18 | 30 | **20.385** | 55.019 | 4.79배 |
| 8 | `DLC1.2-a-s4` | 0.00920 | 3.0% | 0.16 | 30 | **22.054** | 60.126 | 5.19배 |
| 9 | `DLC1.2-a-s6` | 0.00909 | 3.0% | 0.17 | 30 | **21.936** | 59.504 | 5.16배 |
| 10 | `DLC1.2-a-s3` | 0.00907 | 3.0% | 0.16 | 30 | **20.970** | 56.184 | 4.93배 |
<!-- /A7:TOP10 -->

**상위 10건 전부가 `DLC1.2`(정상 발전)다.** 손상 상위 6건이 `d` 시리즈,
7~10위가 `a` 시리즈이며 두 그룹이 뚜렷이 갈린다.

| 그룹 | 피로 안전율 | 극한 LC 대비 |
|---|---|---|
| `DLC1.2-d-s1`~`s6` (손상 1~6위) | **12.63 ~ 13.68** | 2.97 ~ 3.22배 |
| `DLC1.2-a-s3`~`s6` (손상 7~10위) | 20.39 ~ 22.05 | 4.79 ~ 5.19배 |

**손상이 큰 하중이 샤프트에도 가혹하다** — 베어링 손상 순위와 샤프트 안전율
순위가 같은 방향이다(`d` 시리즈가 손상은 크고 안전율은 낮다). 다만 **베어링
손상은 3.7% 대 2.6% 로 1.4배 차이인데 샤프트 안전율은 13 대 21 로 1.6배**라
민감도가 다르다 — 베어링은 접촉응력(하중의 3~10승), 샤프트는 굽힘응력(하중의
1승)이라 지수가 다르기 때문이다.

**샤프트 여유는 운전 하중에서 12.6배 이상**이다. 극한 LC 기준 4.25배도 이미
충분했지만, 실제 운전 하중으로 보면 **세 배 더 여유롭다.**

> **한계** — 상위 10건은 전부 `DLC1.2` 라 **하중이 큰 다른 계열**(`DLC2.4.x`
> 정지·`DLC6.4` 대기)은 표본에 없다. 손상 기여는 작아도 순간 하중은 클 수 있어
> 최악 안전율은 이 표 밖에 있을 가능성이 남는다 — **§7-6.6 에서 전수로 확인한다.**

### 7-6.6 111 DLC 전수

§7-6.5 의 한계를 없앤다. `a01` 의 **111 DLC · 2,646빈 전부**를 같은 경로로
돌려 계열별 분포와 진짜 최악 케이스를 찾는다.
...
**① §7-6.5 의 우려는 기우였다.** 하중이 큰 다른 계열(`DLC2.4.x` 정지 ·
`DLC6.4` 대기)이 더 가혹할 것을 걱정했으나, 실제로는 **가장 여유로운 쪽**이었다
(`DLC6.4` 17.03 ~ 26.25 · `DLC4.1` 15.33 ~ 23.19). **최악은 여전히 `DLC1.2`**
(정상 발전)이며 그 안에서 9.57 까지 내려간다.

**② 최악은 `DLC1.2-k-s5` 의 9.566 이다** — §7-6.5 의 상위 10건 최저(12.63)보다
**24% 낮다.** 즉 **손상 상위 10건만 봤다면 최악을 놓쳤을 것**이다. 이 DLC 의
베어링 손상 기여는 상위 10위 밖이라 §7-6.5 표에 없었다.
...
`run_appendix7_dutycycle.py` — 극한 16 LC 와 듀티사이클을 한 번에 낸다.
산출 `부록7_샤프트/dutycycle.csv`(17행). 소요 약 1분.
