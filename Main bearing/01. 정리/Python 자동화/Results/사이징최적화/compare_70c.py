...
# v1.3 기준선 — 표 최상단에 비교 기준으로 놓는다. **50 °C 값만 있다**(P2
# Phase 1 · §8-3). 70 °C 로는 돌리지 않았다 — 50 °C 에서 이미 한도의 13.8배라
# 판정이 바뀔 여지가 없어 해석을 아꼈다. 그 자리는 `—` 로 둔다.
BASE = dict(tag="**기준선** (v1.3)", rank="—", mass=5.60, bore=3055, D=3600,
            D_we=110.51, L_we=238.048, sl=2.154, Z=87,
            d50=6.9249, s50=7.7297, life50=3.9)      # §8-6.1 실측
...
def main():
    ...
    HEAD = ("| 태그 | 프론트 # | 베어링 [t] | d [mm] | D [mm] | D_we [mm] | "
            ...
            "**손상비** | ΣD30_Sys 50 °C | ΣD30_Sys 70 °C | "
    ...
    BROW = (f"| {BASE['tag']} | {BASE['rank']} | {BASE['mass']:.2f} | "
            ...
            f"**{BASE['d50']:.4f}** | — | — | {BASE['s50']:.4f} | — | "
            f"{BASE['life50']:.1f} | — | **불합격**(50 °C) |")
    DIV = "| **— 총질량 최경량 —** |" + " |" * 16
    ...
    def line(k):
        ...
        return (f"| `{k}` | {t['rank']} | {float(t['mass_brg_kg'])/1e3:.2f} | "
                ...
                f"{float(x['D30_Sys']):.4f} | {float(y['D30_Sys']):.4f} | "
