...
> **확보 현황(2026-07-05)**: **29편 확보** — Tier A 16 + Tier B 6 + Tier C 7 (**구현 관련 Tier A/B/C 전부 확보**). **남은 미보유는 Tier D(배경·정의) 12편뿐**. (10)(35)는 Tier D로 이동. 편별 보유는 부록 1 참조.
...
- **✅ G-M2-1 해소(2026-07-14, 원문 추출)**: 상보파 진폭감소 데이터는 **(15) Venner 1997**(`03. 정리/1997. (Venner) Amplitude Reduction of Waviness in Transient EHL Line Contacts.md`, 우리 repo 보유)에 존재 → 추출:
  - **Table 1** 순수 rolling `A_d/A_i` (M=100, L=11; A_i/H_c 무관=선형): λ/b= **4.0→0.030 · 2.0→0.073 · 1.0→0.183 · 0.5→0.394 · 0.25→0.660 · 0.125→0.839**.
  - **마스터커브 식(5)**: `A_d/A_i = 1/(1 + 0.17∇ + 0.03∇²)`, `∇ = (λ/b)·M^{3/4}/L^{1/2}` (= λ/b/√(ᾱλ̄) = λ/b/H_c^{3/4}). A_d/A_i≤0.5서 정확, >0.5선 과소예측. **정확도 2~5%**.
  - GW1994 등가형 식(6): `A_d/A_i = 1/(1 + C*∇̄)`. 조건 파라미터: α=2.2e-8·z=0.67·η0=4.0e-2·P0=1.96e8·H_c=1.417e-2·정확도체크 스팟(λ/b=0.5,A_i/H_c=0.1→0.394).
  - **용도**: 상보파 절대진폭(g) **정량 검증 오라클** — 모델의 순수 rolling `A_d/A_i` vs ∇ 를 식(5)+Table1과 대조(2~5% 허용) → g=0.5(GW half-pumping) 검증/캘리브레이션. **기존 "미제공 gap" 해소**(더 이상 conditional 아님).
  - **(31) Venner 2000**(`03. 정리/2000. (Venner) Multigrid techniques.md`)도 데이터 보유 — **원형/점접촉·비등방** 마스터커브 **식(29)**: `A_d/A_i = 1/(1 + 0.15·f̄(r)·∇₂ + 0.015·[f̄(r)·∇₂]²)`, `f̄(r)=e^{1−1/r}(r>1), else 1`, `r=λx/λy`(방향인자), `∇₂=(λ/a)·M^{1/2}/L^{1/2}`, λ=min(λx,λy). Fig6/7 비등방 A_d/A_i vs ∇₂, 예제 M=1007.6·L=12.05. **워크플로 VC-M2-Master가 이미 (31) 식(29)를 사용** 중.
  - **(15) vs (31) 구분**: (15)=**선접촉·과도(순수 rolling)** eq(5) 0.17/0.03·∇∝M^{3/4} → 상보파 지배 regime의 **가장 직접 오라클**; (31)=**점접촉·비등방** 식(29) 0.15/0.015·∇₂∝M^{1/2}·f̄(r) → 2D 거칠기 기하 정합. 원문(31) 명시: "진폭감소는 선/점접촉 **공통 단일 메커니즘**"(∇₂=√(2π³/3)(λ/a)(ᾱp̄_H)^{3/2}L^{−2}로 선접촉과 상수배 동일) → **두 곡선 교차검증 가능**. 계수·∇ 정의가 다른 이유 = 기하(선 vs 점)·조건 차이(같은 물리).
...
| (10) | Lainé & Olver 2007, STLE | 마모 일반형·피로-마모 경쟁 (배경) |
| (35) | Fouvry et al. 2000, ASTM STP 1367 | 프레팅 Dang Van 성공례 (배경) |

**요약**: 구현에 반드시 확보할 원문 = **Tier A 16편 + Tier B (33)(37) 우선** = 약 18편. 이 중 (21)이 절대 최우선, (33)이 Dang Van gap 해소 핵심. **보유 29편(2026-07-05)** — **구현 관련 Tier A/B/C 전 29편 확보 완료**. 남은 미보유는 **Tier D 배경·정의 12편뿐**((10)(35)는 Tier D로 이동). (18) Wöhler 1870·(20) Palmgren 1924는 **Tier D로 격하**(각각 A·B 상수(19)·Miner 규칙만 알면 되어 원전 불요).
...
> **배치**: §4의 **Tier(A~D) 분류·순서와 동일**하게 정렬. **보유 집계(2026-07-05)**: 41편 중 **29편 보유** — Tier A 16 + Tier B 6 + Tier C 7 (**구현 관련 Tier A/B/C 전부 확보**) / 나머지 12편은 **Tier D 배경·정의**. **링크 집계**: 직접 DOI 35편 / URL만 2편 / Scholar만 4편. DOI·링크는 자동 조사 결과이므로 **접속 시 스팟체크 권장**. (분류·서지 상세는 §4 참조)
...
> **MD변환** 열: 로컬 MinerU 파이프라인으로 **Markdown+이미지 통합본** 생성 완료(✅). 현재 **Tier A 16/16 · Tier B 6/6 · Tier C 7/7 전부 변환**(총 29편, (28)은 Part1·2 모두). 산출물: `03. 정리/<논문명>.md` + 이미지 폴더. (표 2단헤더·수식·정크제거·경로캡 자동 적용)

### Tier A — 구현 핵심 (16편) — 보유 16/16 · MD변환 16/16

| Ref | 서지(약칭) | 보유 | MD변환 | Google Scholar | 직접 링크 |
|---|---|---|---|---|---|
...
| (28) | Hooke et al. 2007, Proc. IMechE Part C 221 | ✅ (P1·P2) | ✅(P1·P2) | [🔍](https://scholar.google.com/scholar?q=Rapid+Calculation+of+the+Pressures+and+Clearances+in+Rough+Rolling+Sliding+EHL+Contacts+Part+1+Low-Amplitude+Sinusoidal+Roughness) | [DOI](https://doi.org/10.1243/0954406JMES519) |
...
### Tier B — 구현 지원 (6편) — 보유 6/6 · MD변환 6/6
...
### Tier C — 검증·비교·현상이해 (7편) — 보유 7/7 · MD변환 7/7
...
### Tier D — 배경·정의 (12편) — 보유 0/12 · MD변환 0/12
