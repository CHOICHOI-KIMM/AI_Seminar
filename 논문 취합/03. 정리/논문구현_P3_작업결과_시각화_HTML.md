...
## S⑫ — 응력 6성분 토글 (연구자 지시, 2026-07-24) — **통과**

**배경**: 뷰어가 σ_vM 만 표시했으나 M3(`StressTensor6`)는 **6성분 전부 산출**(sxx·syy·szz·sxy·syz·sxz). 데이터는 존재, 셸에서 미노출이었을 뿐(S③ 크기대책).
**연구자 3문 사인오프**: 성분=**0중심 발산 컬러맵**(파랑−/흰0/빨강+)·vM=viridis 유지 / 세로버튼 7개 그래프 우측 / **D맵은 토글 대상 아님**(M4가 6성분→D 스칼라 집약, 성분 개념 없음).

**구현**:
- 셸: `ChainResp.stress_xz: BTreeMap<String,[z][x]>` — 6성분 y₀ 슬라이스(M3 산출물 그대로 노출, ~한 성분당 nz×nx). JSON ~1MB 이내.
- plot.js `heatmap` 에 `diverging` 모드: 대칭범위(±max|v|)+RdBu(파랑↔흰↔빨강), 0=중앙 고정. viridis/발산 컬러램프 통합(`rampRGB`).
- 뷰어: σ_vM 캔버스 우측 세로버튼(σ_vM+6성분), **받아둔 데이터에서 선택→재계산 없이 즉시 전환**. 활성버튼 파랑, 재실행 시 선택 유지.

**검증**: 셸 13 green · 서버판 실시간 autorunPlotted · **자립판 file:// σzz 자동클릭 스크린샷**: 발산맵 대칭 컬러바(−2.318~+2.318 GPa)·σzz 버튼 활성·D맵 불변(∞ 띠 유지) 확인.
**★포착(부호 규약)**: 초안 테스트가 "szz<0(압축)" 단정 → FAIL. **실측**: 이 모델은 압력을 **σzz>0** 로 표현(인장+ 규약이나 압력하중 부호는 반대), 또 부호혼재는 **격자의존**(nx=64 sxx 혼재, nx=32 전부 양수 — 리플 해상도차). → 유닛에서 성분 부호 단정 제거(격자의존이라 취약), vM≥0(비음수 규약)만 검증. **교훈: 검증 안 한 물리 부호를 테스트에 단정 말 것 — 실측 후 규약 확인.**
**하네스 재확인**: `--virtual-time-budget` 이 **서버판 module worker 를 얼림**(S④ 재발) → 서버판=실시간 판정, 스크린샷=자립판(initSync 면역)으로 이원화.

## S⑬ — σxy·σyz=0 안내 배너 (연구자 지시, 2026-07-24) — **통과**

**배경**: 연구자 관찰 — σxy·σyz 히트맵이 전부 0(흰색). **결함 아님**: 현재 입력이 y-불변 사인 거칠기(β=0)이고, Tripp 식[10]/[16]에서 이 두 전단은 **β·sin(βy) 를 인수**로 가져 β=0 이면 수학적으로 정확히 0(코드 주석 실증). 발산맵 흰색 = 0을 정직하게 표시.
**구현**: 지어내지 않고 **"0인 이유"만** 안내. `drawComp` 에서 **실측**(`max|값|==0`) + 성분(sxy/syz) 조건 만족 시 히트맵 위 배너: β=0 근거·물리(y방향 구배 필요)·활성 조건(y-의존 2D 거칠기)·원논문 Fig6 대비. β 가정이 아닌 데이터 실측 기반이라, 향후 2D 입력으로 값이 생기면 자동으로 배너 사라짐.
**검증**: web 재빌드 · 자립판 σyz 자동클릭 스크린샷 — 배너 표출·타 성분 전환 시 소멸 확인.
