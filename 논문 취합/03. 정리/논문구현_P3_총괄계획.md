...
### 8.C.3 M4 — 다축피로 (Dang Van/Milano) · 중리스크

- **알고리즘(P2-3 §3.4 파이프라인)**: 각 물질점 (x,y,z) 에서 시간(=x-스냅샷)스텝별 응력텐서 σ(t) → (a) 편차 S(t) 수집 → (b) **5D 편차공간 최소외접초구 중심 z\***(MCE) → (c) ŝ(t)=S(t)−z\* → (d) `τ̂=½(ŝ_I−ŝ_III)`(대칭화 편차 Tresca)·`p̂=⅓trσ` → (e) **식[9] `D=maxₜ{τ̂/(τ_f−α_dv·p̂)}`**, `τ_f≈σ_f/√3`, `σ_f=A·lnN+B`(Wöhler 식[10], **A=−43.0 MPa·B=1220 MPa** 확정, N_ref=1e6 가정+민감도) → 식[11] 수명 N → **Palmgren-Miner 누적 d, d>1→피트**(체적+상부 제거). α_dv=(τ_f−σ_f/2)/(σ_f/3)≈0.232(von Mises 가정).
...
- **RQ/리스크**: **G-M4-1(최대난점)** — Dang Van 응력이력 τ̂/p̂ 계산법이 **원논문 부재**("will not be discussed here"). 단 **ref(33) Desimone 보유** + MCE+대칭화 Tresca Q3 사인오프로 해소경로 확정. **MCE(5D 최소외접초구)가 비자명 수치 부문제**(Welzl/볼록최적화) — 구현 난이도 핵심. Milano 닫힌해가 독립 앵커. N_ref=1e6·무잔류응력 가정+민감도. **리스크 중**(근거는 있으나 MCE 구현+시간이력 해석 주의).
