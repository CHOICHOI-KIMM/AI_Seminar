...
> **확정(2026-07-09)**: 개발 접근 = **별도 Rust crate `micropitting-model` → CRB-main `solver/`로 이식**; **참조-재구현 + 경계 어댑터**, 코드 SSOT = **(B) 표준/CRB 정렬**(규약 조정표 §1.4). **위치** = `Main bearing/01. 정리/논문구현_P3/`, **결과 로그** = `논문구현_P3_작업결과.md`. **CRB 재사용은 5개 한정**(§1.3). **M6 파일럿 보류**(계획 검토 후).
...
1. **개발 위치**: ✅ 별도 Rust crate `micropitting-model` — **`Main bearing/01. 정리/논문구현_P3/`** 폴더 내 생성. 검증 후 CRB-main `solver/`로 이식(통합 리스크는 P4로 이연).
...
### 7.3 다음 단계
1. ✅ (연구자) **계획·잔여큐 확정** → 2. **Phase 0**: `Main bearing/01. 정리/논문구현_P3/`에 crate `micropitting-model` 스캐폴드 + **D0 규약조정표** + 공유 `types.rs`/유틸(FFT·Hertz·Dowson-Toyoda) + Python 오라클 venv + `논문구현_P3_작업결과.md` → 3. **Phase 1**: M1·M2·M6 병렬 무인(모듈+해석해 테스트) → 4. **Phase 2**: 통합·`cargo test`·M1+M2→M6 스모크 → 부분윤활 서브시스템 G3 → 5. **M3·M4·M5 확장** → 전 모델 G3 → **P4(CRB-main 이식)**.
