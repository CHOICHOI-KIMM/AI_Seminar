...
**「외부 하중 방위」와는 다른 물리량**이라 임의 대체하지 않았다.

**→ 해소 (S4-3, 커밋 `HEAD`)**: 사용자 결정에 따라 **Solve 시점 입력을 스냅샷**한다.
`store` 에 `resultInput: BbInput | null` 을 신설하고 `SET_RESULT` 가 `bbInput` 을 함께 고정,
`CLEAR_RESULTS` 가 비운다. 뷰는 `bbInput`(편집 중)이 아니라 **`resultInput`** 을 읽는다.
**S5 와 향후 뷰에도 같은 문제가 생기지 않는다** — 결과와 짝이 맞는 입력이 store 에 항상 있다.
배너 문구도 경고 → 사실 서술로 바꿨다.
...
### ⭐ 최종 런타임 헬스체크 (④) — BB 커맨드 3종 전부 통과

```
[healthcheck] bb_compute_contact (q_n=0, 하중무관 전처리)
              chi_inner=8.829994003311707  chi_outer=5.9643343396234405
              a*_i=3.6990927410646433 b*_i=0.41892358473599084 δ*_i=0.6148720822472493
              a*_e=2.864503343714383  b*_e=0.48027209418565797
              e_star_mpa=113736.26373626373  c_p=329349.2570196955
[healthcheck] bb_compute_contact (q_n=Q_max=1178.368, 최대하중 볼)
              delta_mm=0.023392837  a_i=1.5639292815 b_i=0.1771155542 p_i=2031.180898
                                    a_e=1.3051080827 b_e=0.2188187329 p_e=1970.114370
              sigma_hu_mpa=1500  alerts=1
[healthcheck] ContactResponse 형상·항등 검증 PASS · 종료 — ALL PASS
```

**웹뷰 오류 0건.** `bb_compute_geometry` · `bb_solve_bearing` · `bb_compute_contact` **3종을 헬스체크가 전부 덮는다.**

**손계산 대조 — Level B 의 정의가 화면 데이터와 일치**

| 항등 | 화면 | 손계산 | |
|---|---|---|:--:|
| `χ_inner = a_i / b_i` | 8,829994003 | 1,5639292815 / 0,1771155542 = **8,82999** | ✅ |
| `χ_outer = a_e / b_e` | 5,964334340 | 1,3051080827 / 0,2188187329 = **5,96433** | ✅ |
| `p_i = 3Q/(2π a_i b_i)` | 2031,180898 | 3·1178,368/(2π·1,5639293·0,1771156) = **2031,18** | ✅ |
| `E* = 1/[2(1−ν²)/E]` | 113 736,2637 | 207 000/(2·0,91) = **113 736,26** | ✅ |

> ⭐ **`alerts=1`** — `p_max = 2031 MPa > σ_Hu = 1500` 이므로 `CONTACT_STRESS_OVER_FATIGUE_LIMIT`
> 경고가 **실제로 발생**했다. §3.6.4.2 의 `AlertPanel` 항목이 실증됐고,
> Stress Contour 의 σ_Hu 등고선과 **같은 상수**를 쓰므로 화면에서 서로 연결된다.
