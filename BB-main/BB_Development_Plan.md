...
| 🟡 **BB 축단면** (`α₀` ↔ `α_j`) — **Phase 4 미구현, 별건**(§3.6.4.8) | 하중 후 접촉각이 벌어지는 방향 · `A = r_i + r_e − D_w` · `δ_x` | **D-1** · **D-2d**(틸트축 정합) | `α_j` 선이 `α₀` 와 겹치면 하중이 안 걸린 것 · 틸트 방향이 모멘트와 반대 |
...
> ### 🟡 7개 항목 중 1개가 Phase 4 범위 밖이다 (2026-08-24 확정)
>
> **「BB 축단면」은 Phase 4 에서 만들지 않았다.** §3.6.4.3 처분표의 신규 BB 뷰는 5개이고
> 축단면이 없다(Section 탭은 TRB 잔존·회색). 두 절이 어긋나 있었으므로 여기서 정리한다.
>
> | | 상태 |
> |---|---|
> | **부분 커버됨** | `α₀` ↔ `α_j` 비교는 `BbLoadDistView` 의 `AlphaCurve` 가 직교좌표로 한다 → **D-1 은 화면에서 확인된다.** `A = r_i + r_e − D_w` 는 Geometry 탭에 숫자로 있다 |
> | **미커버** | 「접촉각이 **벌어지는 방향**」·「**틸트 방향이 모멘트와 맞는가(D-2d)**」의 **그림에 의한** 판정 |
>
> → **Phase 4 는 「접촉/하중 검증에 필요한 최소 집합」으로 닫고, 축단면은 별건으로 둔다** (§3.6.4.8).

##### 3.6.4.8 후속 — BB 축단면 뷰 (Phase 4 범위 밖)

만들 값어치는 크다(§3.6.3.3 의 판정). 다만 **접촉/하중 검증에 필수는 아니어서** Phase 4 에서 뺐다.

**만들 때의 내용** (§3.6.3.3 에서 이미 설계됨):

- 볼은 **원**(반경 `D_w/2`, 중심 `r = D_pw/2`), 궤도는 홈반경 `r_i`·`r_e` 의 **원호**
- 곡률중심 `O_i`·`O_e` 를 찍고 그 사이 거리 **`A = r_i + r_e − D_w`** 를 도시
  → **단면도가 곧 Theory §2 (A.3) 의 교과서 그림**이 된다
- 무하중 `α₀` 선과 하중 후 `α_j` 선을 **겹쳐** 그려 접촉각 변화를 시각화
- 축을 **X = 회전축**으로 잡고 `δ_x` 를 화살표로
- 기존 `SectionView2D` 의 **치수선/`Annotations` 헬퍼와 `viewBox` 셋업만 복사**하고
  형상 계산부(사다리꼴 롤러·리브·γ 경사축) 약 200줄은 통째로 버린다

**착수 조건**: 육안 확인에서 「그림이 더 필요하다」는 판단이 서면. 그때 `bb/BbAxialSectionView.tsx` 를
6번째 뷰로 추가하고 `CanvasArea` 에 `sectionView` prop 을 **prop 주입 패턴**(§3.6.5.6)으로 붙인다.

##### 3.6.4.9 데이터 흐름 — command → store → view
...
##### 3.6.5.3 단계별 검증 절차 — 5단 사다리

> **각 단계(S1~S5)를 마칠 때마다 아래를 순서대로 밟는다.** 위 칸이 깨지면 아래로 내려가지 않는다.
> 값싼 검사를 먼저 두는 이유는 **비싼 검사(사람 눈)를 아끼기 위해서**다.
...
1. `npm run tauri dev` 를 **백그라운드**로 띄우고 stdout/stderr 를 파일로 받는다.
2. 로그에서 다음을 확인한다 — Rust 컴파일 성공 · Vite `ready` · **에러 브리지 출력 0건**.
3. **자동 스모크**(선택, `VITE_BB_HEALTHCHECK=1` 일 때만): 앱 기동 직후 기본 프리셋으로
   `bb_solve_bearing` 을 1회 호출하고 요약(`converged` · `loaded_count` · `Q_max`)을 로그로 찍는다.
   → **「화면이 떴다 + 솔버 왕복이 됐다 + 렌더 오류가 없다」를 로그만으로 판정**할 수 있다.
   평상시 dev 에는 영향이 없도록 환경변수로 잠근다.
4. 확인 후 **반드시 프로세스를 종료한다.** 장기 구동·포트 점유는 사용자 몫이다 (CLAUDE.md 필수 준수 1).

**에러 브리지** — 이것이 없으면 ④ 가 성립하지 않는다

`npm run tauri dev` 터미널에는 **Rust 패닉과 Vite 빌드 오류만** 나온다.
**웹뷰 안의 JS 런타임 오류(`undefined.l10` 같은 것)는 보이지 않는다.**
이번 Phase 에서 가장 위험한 `ResultsCard` 류 크래시가 정확히 그 종류다.

→ `@tauri-apps/plugin-log`(JS) 를 추가하고 `src/bb/errorBridge.ts` 에서
`window.onerror` · `unhandledrejection` · `console.error` 를 Rust 로그로 넘긴다.
Rust 쪽 `tauri-plugin-log` 는 **이미 의존성에 있다**(`lib.rs` 에서 초기화 중).
의존성 추가는 **JS 패키지 1개뿐**이다.

**오류를 발견했을 때**

| 유형 | 조치 |
|---|---|
| **명백한 것** — 오타 · import 누락 · 필드명 불일치 · 타입 불일치 등 **정답이 하나인 것** | **자동 수정**. 단 단계 마무리에 **자동수정 목록을 전량 보고**한다 |
| **판단이 필요한 것** — 표시할 물리량 선택 · 레이아웃 · **솔버가 틀렸을 가능성** · 설계 판단이 뒤따르는 것 | **멈추고 안내 + 질문.** 자의적으로 정하지 않는다 |

> 「솔버가 틀렸을 가능성」을 자동 수정 금지에 넣은 이유 — **화면을 고쳐 숫자를 맞추면 검증이 무의미해진다.**
> 화면과 검증 결과가 다르면 **어느 쪽이 틀렸는지부터 가려야** 한다 (§3.6.4.2).

##### 3.6.5.4 기본 프리셋 2종

입력이 없으면 아무 기능도 쓸 수 없다. 성격이 다른 두 개를 둔다.

| 프리셋 | 기하 출처 | 용도 |
|---|---|---|
| **① `Harris-Mindel 1973 (ACBB)`** | **Harris & Mindel, *Wear* 23(3) 311–337 (1973), Fig. 15** — 전량 인쇄된 실기 ACBB | **B · B-3 대조용.** 문헌 근거가 있는 유일한 기하 |
| **② `ACBB Verification Fixture`** | 문헌 근거 **없음** (`Z`·`D_w` 가정값) | **C · D 대조용.** §3.6.4.2 검증 매핑의 대부분이 이 기하 기준이다 |

**① 의 값** (in·lb·psi → mm·N·MPa 환산, `tests/contact_level_b3.rs` 와 동일 출처)
...
| `f_i` = `f_e` | 0,5200 | `r_i` = `r_e` = **10,7315 mm** |
| `α₀` (free contact angle) | 27,0000° | **0,471239 rad** |
...
> ⚠️ **두 가지를 명시해야 정직하다.**
>
> 1. **볼의 `E` 는 Fig. 15 에 인쇄되어 있지 않다** (HOUSING·OUTER RING·INNER RING·SHAFT 만).
>    프리셋에서는 링과 같은 값을 쓰되 **가정임을 주석에 남긴다.** B-3b 의 `a`·`b` 계통편차
>    −0,68 % 의 유력한 원인이기도 하다 (§3.6.3.1 대체판정 참조).
> 2. **원 예제는 24 000 rpm 고속이라 우리 정적 모델 범위 밖이다** (B-3c 가 자료로 확인 —
>    원심력/접촉하중 = 1,05). **기하만 문헌값을 쓰고 하중·속도는 정적 범위로 따로 잡는다.**
>    권장: 축하중 3 000 lbf = **13 345 N**(원 예제와 동일) · `n = 1 000 rpm`
>    (`n·D_pw` = 1,58e5 < 1e6 이라 `HIGH_SPEED` 경고 없음).

**② 의 처리** — 현 `presets.rs` 의 `7210 (ACBB Default)` 를 **개명**한다.

주석에 「`Z` 와 `D_w` 는 제조사별 값이라 **가정값**(실 카탈로그 미확인 — **T-6**)」이라 적혀 있는데,
그 값(`D_w` 11,5 · `D_pw` 70 · `α₀` 40° · `Z` 16)이 **Level C·D 검증 픽스처와 정확히 같다.**
즉 실제로는 7210 이 아니라 **우리 검증 기준 기하**인데 형번 이름을 달고 있다.
...
### Phase 4 — 프론트엔드 (접촉/하중 검증 뷰) 〔구 Phase 6〕 ✅ **완료 2026-08-24**
...
> 🔍 **각 단계는 §3.6.5.3 의 5단 사다리로 검증한다** — 빌드 → 린트 → `cargo test` → **런타임 헬스체크(Claude)** → 육안(사용자).
> 기본 프리셋 2종은 §3.6.5.4.
...
**신 Phase 4 완료 (2026-08-24).** S0~S5 · BB 브랜치 커밋 67건 · `cargo test` **147** · clippy 0 ·
`npm run build`/`lint` 통과 · `src/bb/` **5개 뷰**가 실제 솔버 출력으로 동작 · **삭제 0건**(최소 변경).
런타임 헬스체크가 **BB 커맨드 3종을 전부 덮으며** `ALL PASS` · 웹뷰 오류 0.

다음은 **신 Phase 5 — 정격하중 및 수명**. ISO 표 전사 분량이 커서 착수 전 별도 확인이 필요하다.
후속 소품으로 **BB 축단면 뷰**(§3.6.4.8)가 남아 있다.
