...
| **D-9b** | **모멘트** 팔 (`M_y`·`M_z`) | **`D_pw/2`** (식 A.8) | 키네마틱 팔과 다른 것은 Jones 규약이며 오류가 아니다. Harris *Adv* (1.20)(1.21) 동일 (Theory §4.5) |
...
축   성분  X_j = A sin α₀ + δ_x − R_i (sin γ_z · cos φ_j − sin γ_y · sin φ_j)

δ_j = max(0, √(R_j² + X_j²) − A)          (A.2 확장)
α_j = atan2(X_j, R_j)                      (A.5 확장)
```

> **`sin` 을 쓰는 이유**: ISO (A.2) 가 `R_i sin ψ` 로 쓴다. Harris *Advanced* (1.13) 은 `ℜ_i θ̄` 로
> 선형화했으나 본 SW 는 ISO 준용이다 (Theory §4.5).
>
> **모멘트 팔은 `D_pw/2`** — ISO (A.8) 및 Harris *Advanced* (1.20)(1.21) 과 동일.
> 키네마틱 팔(`R_i`)과 다른 것은 오류가 아니라 **Jones 정식화의 확립된 규약**이다 (Theory §4.5).
> `M_y` 의 팔도 `D_pw/2` — Harris (1.21) 이 명시적으로 Y 축 모멘트에 `d_m/2` 를 쓴다.
