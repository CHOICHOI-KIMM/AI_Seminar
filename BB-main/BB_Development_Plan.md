...
| ID | 항목 | 결정 | 근거·비고 |
|---|---|---|---|
| **D-1** | 평형 DOF | **코드는 5-DOF 로 구성.** 단 P3 에서 **먼저 3-DOF 구속 모드로 검증**한 뒤 5-DOF 를 해방하여 검증 | ISO 16281 A.1 이 확장 허용. Harris Table 7.4 대조는 3-DOF 상태여야 성립 |
...
struct BearingDisplacement {   // 5-DOF
    dx: f64, dy: f64, dz: f64,     // 축·반경2축 [mm]
    gy: f64, gz: f64,              // 미스얼라인먼트 2축 [rad]
}
```

> **3-DOF ↔ 5-DOF 호환 규약**: ISO 정식화의 `δ_r` ≡ `dy`, `δ_a` ≡ `dx`, `ψ` ≡ `gz` 로 매핑하고 `dz = gy = 0` 으로 구속하면 ISO 식 (A.2)(A.5)(A.6)(A.7)(A.8) 과 **항등적으로 일치**해야 한다. 이 항등성이 P3 검증의 핵심이다.
...
5. `mod.rs` 정리, 프론트엔드 `@ts-nocheck` stub 로 빌드 그린 확보
6. TypeScript 타입 미러 갱신
...
**DoD**: Level A 전 항목 통과, 삭제 모듈이 어디에서도 참조되지 않음
...
**작업**
1. `bearing.rs` 재작성 — 5-DOF 구조로 작성하되 **DOF 구속 마스크** 지원
2. `δ_j` (A.2) 의 5-DOF 일반화:
   - 반경 성분: `A cos α₀ + dy·cos ϕ_j + dz·sin ϕ_j`
   - 축 성분: `A sin α₀ + dx − R_i(gz·cos ϕ_j − gy·sin ϕ_j)`
3. `α_j` (A.5) — `atan2` 로 사분면 보존
4. 잔차 5개: `F_x, F_y, F_z, M_y, M_z`
5. 비접촉 클리핑(`δ_j < 0 → 0`) 처리 — active set 또는 스무딩 (T-3)
6. 축방향 예압: `F_a0` → 초기 `dx` 로 변환하는 사전 해석 (D-2)
7. `n·D_pw > 1e6` 경고 (D-3)

**Level C 검증 — 해석해 (5-DOF 코드, 대칭 하중)**

| 항목 | 조건 | 판정 |
|---|---|---|
| 순수 축하중 | `F_r = 0, M = 0` → 모든 볼 동일 `δ_j`, `α_j` | 대칭성 완전 일치 |
| 축하중 해석해 | `F_a = Z c_P δ^1.5 sin α_j`, `α_j` 단일 미지수 | 잔차 < 1e-10 |
| 예압 무하중 | `F_ext = 0` 인 예압 상태 → 모든 볼 균등 하중 | 균등 |
| 회전 불변성 | 하중 방향을 `ϕ` 만큼 회전 → `Q_j` 분포도 동일 회전 | 일치 |

**Level D-1 검증 — Harris 대조 (3-DOF 구속 모드)** ★ 유일한 외부 검증

- 구속: `dz = 0`, `gy = 0` → ISO 3-DOF 와 항등
...
| 항목 | 방법 | 판정 |
|---|---|---|
| 축퇴 항등성 | `dz = gy = 0` 구속 해를 5-DOF 로 자유 계산 → 같은 해에 수렴 | rel. err < 1e-8 |
| 반경하중 방향 불변 | `F_y` 만 / `F_z` 만 / 45° 합성 → 크기 동일 시 `Q_max` 동일 | rel. err < 1e-8 |
| 2축 모멘트 | `M_y`, `M_z` 를 바꿔가며 하중분포 비대칭 방향 확인 | 물리적 정합 |
| 수렴 강건성 | 하중·클리어런스 격자 스윕에서 수렴 실패율 | 0 % 목표 |

**DoD**: Level C, D-1, D-2 전 항목 통과
...
| C | P3 | 평형 해석해 | 대칭성·해석해 |
| **D-1** | P3 | 통합 (3-DOF) | **Harris Table 7.4** |
| D-2 | P3 | 5-DOF 일반화 | 축퇴 항등성 |
...
| 리스크 | 영향 | 대응 |
|---|---|---|
| 비접촉 클리핑의 비평활성 (T-3) | Newton 수렴 실패 | Level C 에서 조기 노출. active set / 스무딩 / line search |
...
| T-3 클리핑 비평활 | 열림 — 구현 과제 | P3 |
| T-6 ISO 수치예제 부재 | 열림 — 카탈로그로 대체 | P4 |
| T-7 고속 정식화 부재 | 보류 (D-3 = 경고만) | — |
