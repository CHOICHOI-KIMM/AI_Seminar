...
5. **케이지 위상 스윕** 기능 (§3.4.3) — `φ₀` 를 `[0, 2π/Z)` 로 `n_phase` 분할, 최악값·발생위상 출력
6. 비접촉 클리핑(`δ_j < 0 → 0`) 처리 — active set 또는 스무딩 (T-3)
7. 축방향 예압: `F_a0` → 초기 `δ_x` 로 변환하는 사전 해석 (D-2)
8. `n·D_pw > 1e6` 경고 (D-3)
...
- 구속: `δ_z = 0`, `γ_y = 0` → ISO 3-DOF 와 항등 (DofMask)
- **`φ` 기준 주의 (D-8)**: Harris Table 7.4 는 `Q_max` 를 최대하중 볼 기준으로 정의한다. 고정 원점(`φ_1 = 0`)에서는 볼이 정확히 하중 방향에 없을 수 있으므로, **위상 스윕으로 얻은 `Q_max` 최악값**을 대조에 쓴다
...
| 항목 | 방법 | 판정 |
|---|---|---|
| **축퇴 항등성** | ISO 3-DOF 구속 해를 5-DOF 로 자유 계산 → 같은 해에 수렴 (`δ_z → 0`, `γ_y → 0`) | rel. err < 1e-8 |
